...
#define PG_PRINT(X, ...)                                                       \
	printf("<%s @ 0x%lx>" X, pg_name(page_size), start_addr(),             \
	       ##__VA_ARGS__)
#else
#define PG_PRINT(X, ...) /* X */
#endif
...
namespace paging
{
	...
	static constexpr uintptr_t any_addr =
...
{
	template <> struct enable_bitmask_ops<x86::paging::Flags> {
		using type = std::underlying_type<x86::paging::Flags>::type;
		static constexpr bool enable = true;
	};

	template <> struct enable_bitmask_ops<decltype(4_KiB)> {
...
namespace paging
{
     ...
     **/
	template <uintptr_t Psz, typename Sub = void, typename Arc = x86_64,
	...
	class Page_table {
		...
		static constexpr uintptr_t range_size =
			page_size * Arch::table_size;
		...
		Page_table() = default;
		Page_table(uintptr_t vstart) : linear_addr_start_{ vstart }
		{
			...
			static_assert(alignof(Page_table) ==
				      Arch::min_pagesize);
			Expects((((uintptr_t)this) &
				 ((uintptr_t)(4_KiB - 1))) == 0);
		}
		Page_table(uintptr_t lin, Pflag flags) : Page_table(lin)
		{
		...
		{
			if
				constexpr(not std::is_void_v<Subdir>)
				{
					for (auto &ent : tbl_) {
						if (is_page_dir(ent)) {
							Subdir *ptr =
		...
		{
			return ((entry & Pflag::huge & allowed_flags) or
				page_size == min_pagesize) and
			       !(entry & Pflag::pdir) and
			       is_page_aligned(addr_of(entry));
		}
		...
		{
			return (entry & Pflag::pdir) and !is_page(entry) and
			       addr_of(entry) != 0;
		}
		...
		{
			return addr >= start_addr() and
			       addr < (start_addr() + page_size * tbl_.size());
		}
		...
		int indexof(uintptr_t addr)
		{
			...
			Ensures(i >= 0 and
				static_cast<size_t>(i) < tbl_.size());
			return i;
		}
			...
			for (auto &ent : tbl_) {
				...
				if (is_page_dir(ent)) {
					...
					if (print) {
						printf("%.*s-+<%s> 0x%zx\n",
						       print_lvl * 2, pad,
						       util::Byte_r(page_size)
							       .to_string()
							       .c_str(),
						       (void *)sub
							       ->start_addr());
					}
					sum += sub->summary(print,
							    print_lvl + 1);
				}
			}
			if (print) {
	 ...
	 **/
		Subdir *create_page_dir(uintptr_t lin, uintptr_t phys,
					Pflag flags = Pflag::none)
		{
			...
			*entry_ = reinterpret_cast<uintptr_t>(sub) |
				  (flags & ~Pflag::huge) | Pflag::present |
				  Pflag::pdir;
		...
		{
			for (auto &ent : tbl_) {
				if (addr_of(ent) != 0 and
				    this->has_flag(ent, Pflag::present))
					return false;
			}
			return true;
		}
	 ...
	 **/
		...
		Pflag set_flags(uintptr_t *entry, Pflag flags)
		{
			Expects(entry >= tbl_.begin() && entry < tbl_.end());
			auto new_entry =
				addr_of(*entry) | (flags & allowed_flags);
			...
			if (util::has_flag(flags, Pflag::present) and
			    !is_page(new_entry) and !is_page_dir(new_entry)) {
				PG_PRINT(
					"<set_flags> Can't set flags on non-aligned entry ");
				return Pflag::none;
			}
	 ...
	 **/
		Map map(Map req)
		{
			...
			if (offs < 0) {
				PG_PRINT(
					"<map> Got invalid offset 0x%i for req. %s \n",
		...
		Map map_entry_r(uintptr_t *ent, Map req)
		{
			...
			if (!is_page_dir(*ent) and req.size >= page_size and
			    (req.page_sizes & page_size) and
			    is_page_aligned(req.lin) and
			    is_page_aligned(req.phys)) {
			...
			bool is_unmap =
				req.phys == 0 and req.flags == Pflag::none;
			if (is_unmap) {
				Expects(bits::is_aligned(Arch::min_pagesize,
							 req.size));
			...
			if (!is_page_dir(*ent)) {
				...
				create_page_dir(aligned_addr, current_addr,
						current_flags);
			}
			...
			if (res) {
				...
				Ensures((req.page_sizes & page_size) == 0 or
					(pdir->is_page_dir(req.lin) and
					 (req.page_sizes & sub_psize)) or
					(sub_psize & req.page_sizes) == 0);
			}
	 ...
	 **/
		Map map_r(Map req)
		{
			...
			if (req.phys != any_addr) {
				Expects(bits::is_aligned<min_pagesize>(
					req.phys));
				Expects(bits::is_aligned(req.min_psize(),
							 req.phys));
				Expects(req.phys < Arch::max_memory);
			}

			Expects((req.page_sizes &
				 os::mem::supported_page_sizes()) != 0);

			Map res{};

			for (auto i = tbl_.begin() + indexof(req.lin);
			     i != tbl_.end(); i++) {
				auto *ent = entry(req.lin + res.size);

				Map sub{ req.lin + res.size,
					 req.phys == any_addr ?
						 any_addr :
						 req.phys + res.size,
					 req.flags, req.size - res.size,
					 req.page_sizes };
			...
			Ensures(res.size <=
				util::bits::roundto<4_KiB>(req.size));
	    ...
	    private:
		alignas(Arch::min_pagesize)
			std::array<uintptr_t, Arch::table_size> tbl_{};
	...
	//
	using Pml1 = Page_table<4_KiB, void, x86_64,
				Flags::all & ~(Flags::huge | Flags::pdir)>;
	using Pml2 = Page_table<Pml1::range_size, Pml1>;
	using Pml3 = Page_table<Pml2::range_size, Pml2>;
	using Pml4 = Page_table<Pml3::range_size, Pml3, x86_64,
	...
	template <> inline bool Pml1::is_page_dir(uintptr_t) noexcept
	{
		return false;
	}

	template <> inline uintptr_t *Pml1::entry(uintptr_t addr)
	{
		auto index = indexof(addr);
	...
	template <> inline uintptr_t *Pml1::entry_r(uintptr_t addr)
	{
		return entry(addr);
	}

	template <> inline Flags Pml1::set_flags_r(uintptr_t addr, Pflag flags)
	{
		return set_flags(addr, flags);
	}

	template <> inline void *Pml1::page_dir(uintptr_t *)
	{
		return nullptr;
	}

	template <> inline uintptr_t Pml1::active_page_size(uintptr_t addr)
	{
		auto *entry_ = entry(addr);
		if (entry_ == nullptr) {
			return 0;
	...
	template <> inline x86::paging::Summary Pml1::summary(bool, int)
	{
		Summary sum;
		for (auto ent : tbl_) {
	...
	template <> inline Map Pml1::map_r(Map req)
	{
		...
		if ((req.page_sizes & page_size) == 0) {
			PG_PRINT(
				"<map> pml1 asked for different page sizes: 0x%lx\n",
				req.page_sizes);
			return Map();
		}
	...
	template <> inline void Pml1::traverse(delegate<void(void *, size_t)>)
	{
	}
