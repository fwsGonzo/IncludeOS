...
class Virtio {
	...
	class Token {
		...
		// "Direction" of tokens
		using span =
			std::pair<uint8_t *, size_t>; // gsl::span<uint8_t>;
...
#warning "update_avail_idx() not implemented for selected arch"
		...
		Queue(const std::string &name, uint16_t size, uint16_t q_index,
		      uint16_t iobase);

		/** Get the queue descriptor. To be written to the Virtio device. */
		virtq_desc *queue_desc() const
		{
			return _queue.desc;
		}
	...
	*/
		...
		uint16_t _free_head =
			0; // First available descriptor (_queue.desc[_free_head])
		uint16_t _num_added =
			0; // Entries to be added to _queue.avail->idx
		uint16_t _desc_in_flight =
			0; // Entries in _queue_desc currently in use
		uint16_t _last_used_idx =
			0; // Last known value of _queue.used->idx
    ...
    */
	static inline bool version_supported(uint16_t i)
	{
		return i <= 0;
	}
