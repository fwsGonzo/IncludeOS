...
struct VFS_entry {
	...
	VFS_entry(
		...
		const std::string &
			desc) // Can't have default desc due to clash with next ctor
		: type_{ typeid(T) }, obj_is_const{ std::is_const<T>::value },
		  obj_{ const_cast<typename std::remove_cv<T>::type *>(&obj) },
		  name_{ name }, desc_{ desc }, is_fd_compatible{
			  std::is_base_of<FD_compatible, T>::value
		  }
	{
	}

	VFS_entry(std::string name, std::string desc)
		: type_{ typeid(nullptr) }, obj_{ nullptr }, name_{ name },
		  desc_{ desc }, is_fd_compatible{ false }
	{
	}
	...
	template <typename T> T &obj() const
	{
		...
		if (UNLIKELY(typeid(T) != type_ and
			     not(std::is_same<FD_compatible, T>::value and
				 this->is_fd_compatible))) {
			throw Err_bad_cast(name_ + " is not of type " +
					   ::type_name(typeid(T)));
		}

		if (UNLIKELY(obj_is_const and not std::is_const<T>::value))
			throw Err_bad_cast(name_ +
					   " must be retrieved as const " +
					   type_name());
	...
	Obs_ptr walk(Path &path, bool partial = false)
	{
		...
		while (not path.empty()) {
			...
			if (not next_node) {
				if (partial and
				    current_node->type() == typeid(Dirent))
					return current_node;
	...
	typename std::enable_if<Throw>::type throw_if(std::string msg)
	{
		throw Exception(msg);
	}

	template <bool Throw, typename Exception>
	typename std::enable_if<not Throw>::type throw_if(std::string)
	{
	}
	...
	void mount(Path path, T &obj, std::string desc)
	{
		...
		if (not path.empty()) {
			...
			if (not parent) {
				...
				throw_if<not create, Err_mountpoint_invalid>(
					path.back() + " doesn't exist");
			}
		} else {
			parent = this;
		}
		...
		if (parent->get_child(token))
			throw Err_mountpoint_invalid(
				std::string("Mount point ") + token +
				" occupied");
	...
	Obs_ptr insert_parent(const std::string &token)
	{
		children_.emplace_back(
			std::make_unique<VFS_entry>(token, "Directory"));
		return children_.back().get();
	}

	template <typename T>
	VFS_entry &insert(const std::string &token, T &obj,
			  const std::string &desc)
	{
		children_.emplace_back(
			std::make_unique<VFS_entry>(obj, token, desc));
...
struct VFS {
	...
	{
		INFO("VFS", "Mounting %s on %s", type_name(typeid(obj)).c_str(),
	...
	static inline void mount(Path local, Disk_id disk, Path remote,
				 std::string desc, on_mount_delg callback)
	{
		INFO("VFS", "Creating mountpoint for %s::%s on %s",
		     (std::string("blk") + std::to_string(disk)).c_str(),
		     remote.to_string().c_str(), local.to_string().c_str());

		VFS::insert_dirent(
			disk, remote,
			insert_dirent_delg::make_packed(
				[local, callback, desc](error_t err,
							auto &dirent_ref) {
					VFS::mount<true>(local, dirent_ref,
							 desc);
					callback(err);
				}));
	}

	template <typename P = Path> static VFS_entry &get_entry(P path)
	{
		Path p{ path };
		auto item = VFS::mutable_root().walk(p);

		if (not item)
			throw Err_not_found(std::string("Path ") +
					    p.to_string() + " does not exist");

		return *item;
	}

	template <typename T, typename P = Path> static T &get(P path)
	{
		return get_entry(path).template obj<T>();
	}

	template <typename P = Path> static void stat(P path, on_stat_func fn)
	{
		Path p{ path };
	...
	template <typename P = Path> static Dirent stat_sync(P path)
	{
		Path p{ path };
		auto item = VFS::mutable_root().walk(p, true);
	...
	static void insert_dirent(Disk_key disk_id, Path path,
				  insert_dirent_delg fn)
	{
		// Get the disk, and file system from the disk
		auto disk_it = disk_map().find(disk_id);

		if (disk_it == disk_map().end())
			throw Err_not_found(std::string("Disk ") +
		...
		if (not disk->fs_ready())
			throw Disk::Err_not_mounted(
				std::string("Disk ") + std::to_string(disk_id) +
				" does not have an initialized file system");
		auto &fs = disk_it->second->fs();

		// Get the dirent from the file system at path
		fs.stat(path, fs::on_stat_func::make_packed([fn, disk_id,
							     path](auto err,
								   auto dir) {
				if (err)
					throw Err_not_found(
				...
				// Preserve the dirent
				auto res_it = (dirent_map().emplace(
					Dirent_mountpoint{ disk_id,
							   path.to_string() },
					dir));
...
/** fs::get **/
template <typename T, typename P = Path> inline T &get(P pathstr)
{
	return VFS::get<T>(pathstr);
}

/** fs::stat_sync **/
template <typename P = Path> inline Dirent stat_sync(P path)
{
	Path p{ path };
...
/** fs::stat async **/
template <typename P = Path> inline void stat(P path, on_stat_func func)
{
	Path p{ path };
	VFS::stat(p, func);
}
