...
struct File_system {
	...
	virtual void read(const Dirent &, uint64_t pos, uint64_t n,
			  on_read_func) const = 0;

	/** Read - sync */
	virtual Buffer read(const Dirent &, uint64_t pos, uint64_t n) const = 0;

	/** Return information about a file or directory - async */
	virtual void stat(Path_ptr, on_stat_func fn,
			  const Dirent *const = nullptr) const = 0;

	/** Stat async - for various types of path initializations **/
	template <typename P = Path>
	inline void stat(P pathstr, on_stat_func fn,
			 const Dirent *const = nullptr) const;
	...
	virtual void init(uint64_t lba, uint64_t size,
			  on_init_func on_init) = 0;
