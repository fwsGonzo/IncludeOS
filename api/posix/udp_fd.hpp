...
class UDP_FD : public SockFD {
	...
	explicit UDP_FD(const int id)
		: SockFD(id), buffer_(), sock(nullptr), non_blocking_(false),
		  broadcast_(0), rcvbuf_(16 * 1024)
	{
	...
	ssize_t sendto(const void *, size_t, int, const struct sockaddr *,
		       socklen_t) override;

	ssize_t recv(void *, size_t, int fl) override;
	ssize_t recvfrom(void *__restrict__, size_t, int,
			 struct sockaddr *__restrict__,
			 socklen_t *__restrict__) override;
	...
	int getsockopt(int, int, void *__restrict__,
		       socklen_t *__restrict__) override;
	int setsockopt(int, int, const void *, socklen_t) override;

	struct Message {
		Message(const in_addr_t addr, const in_port_t port,
			net::tcp::buffer_t buf)
			: buffer(std::move(buf))
		{
	...
	void recv_to_buffer(net::udp::addr_t, net::udp::port_t, const char *,
			    size_t);
	void set_default_recv();
	int read_from_buffer(void *, size_t, int, struct sockaddr *,
			     socklen_t *);
