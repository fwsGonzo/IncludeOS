...
class TCP_FD : public SockFD {
	...
	explicit TCP_FD(int id) : SockFD(id)
	{
	}
	...
	long accept(struct sockaddr *__restrict__,
	...
	ssize_t sendto(const void *, size_t, int fl, const struct sockaddr *,
		       socklen_t) override;
	ssize_t recv(void *, size_t, int fl) override;
	ssize_t recvfrom(void *, size_t, int fl, struct sockaddr *,
			 socklen_t *) override;
...
struct TCP_FD_Listen {
	TCP_FD_Listen(net::tcp::Listener &l) : listener(l)
	{
	}
