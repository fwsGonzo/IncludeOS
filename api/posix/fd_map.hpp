...
class FD_not_found : public FD_map_error {
    public:
	FD_not_found(int id)
		: FD_map_error(std::string{ "ID " } + std::to_string(id) +
			       " not found")
	{
	}
};
...
class FD_map {
	...
	template <typename T, typename... Args> static T &_open(Args &&... args)
	{
...
template <typename T, typename... Args> T &FD_map::open(Args &&... args)
{
