 ...
 */
template <typename Protocol> class Vif : public Link_layer<Protocol> {
    public:
	using Linklayer = Link_layer<Protocol>;

	Vif(hw::Nic &link, const int id)
		: Linklayer(Protocol{ { this, &Vif<Protocol>::transmit },
				      link.mac(),
				      id }),
		  link_{ link }, id_{ id }, phys_down_{
			  link_.create_physical_downstream()
		  }
	{
	}
