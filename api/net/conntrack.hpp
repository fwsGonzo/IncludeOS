...
class Conntrack {
     ...
     */
	using Packet_tracker =
		delegate<Entry *(Conntrack &, Quadruple, const PacketIP4 &)>;
	using Packet_tracker6 =
		delegate<Entry *(Conntrack &, Quadruple, const PacketIP6 &)>;
	...
	struct Quintuple {
		...
		bool operator<(const Quintuple &other) const noexcept
		{
			return proto < other.proto or
			       (proto == other.proto and quad < other.quad);
		}
	};
	...
	struct Quintuple_hasher {
		std::size_t operator()(const Quintuple &key) const noexcept
		{
			...
			const auto h2 = std::hash<uint8_t>{}(
				static_cast<uint8_t>(key.proto));
			return h1 ^ h2;
		}
	};
	...
	enum class Flag : uint8_t { UNREPLIED = 1 << 0, ASSURED = 1 << 1 };
	...
	struct Entry {
		...
		Entry(Quadruple quad, Protocol p)
			: first{ std::move(quad) }, second{ first.dst,
							    first.src },
		...
		{
			return first.src == second.dst and
     ...
     */
	Entry *update_entry(const Protocol proto, const Quadruple &oldq,
			    const Quadruple &newq);
    ...
    private:
	using Entry_table =
		std::unordered_map<Quintuple, std::shared_ptr<Entry>,
				   Quintuple_hasher>;
	Entry_table entries;
	Timer flush_timer;

	inline void update_timeout(Entry &ent,
				   const Timeout_settings &timeouts);
...
{
	Expects(pkt.ip_protocol() == Protocol::TCP or
		pkt.ip_protocol() == Protocol::UDP);

	const auto *ports =
		reinterpret_cast<const uint16_t *>(pkt.ip_data().data());
...
{
	Expects(pkt.ip_protocol() == Protocol::ICMPv4 or
		pkt.ip_protocol() == Protocol::ICMPv6);
	...
	auto id = reinterpret_cast<const partial_header *>(pkt.ip_data().data())
			  ->id;

	return { { pkt.ip_src(), id }, { pkt.ip_dst(), id } };
}

inline void Conntrack::update_timeout(Entry &ent,
				      const Timeout_settings &timeouts)
{
