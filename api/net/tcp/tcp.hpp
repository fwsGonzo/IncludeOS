...
class TCP {
	...
	using Connections =
		std::unordered_map<tcp::Connection::Tuple, tcp::Connection_ptr>;
     ...
     */
	tcp::Listener &listen(const tcp::port_t port,
     ...
     * @return     A TCP Listener
     */
	tcp::Listener &listen(const Socket &socket,
			      ConnectCallback cb = nullptr);
     ...
     */
	void connect(tcp::Address source, Socket remote,
	...
	{
		Expects(factor <= 14 && "WScale factor cannot exceed 14");
		wscale_ = factor;
	}
	...
	/**
     * @brief      Called by Inet (triggered by IP) when a Path MTU value has grown stale and the
     * value is reset (increased) to check if the PMTU for the path could have increased This is NOT
     * a change in the Path MTU in response to receiving an ICMP Too Big message and no
     * retransmission of packets should take place
     *
	...
	/** Maximum segment lifetime (this affects TIME-WAIT period - a connection will be closed after
     * 2*MSL) */
     ...
     */
	tcp::Connection_ptr create_connection(Socket local, Socket remote,
					      ConnectCallback cb = nullptr);
	...
	/**
     * @brief      Force the TCP to process the it's queue with the current amount of available
     * packets.
     */
	void kick();
