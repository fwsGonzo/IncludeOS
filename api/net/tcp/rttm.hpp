...
namespace tcp
{
	/** Round Trip Time Measurer, based on Jacobson/ Karels Algorithm found in RFC 793 (rev RFC
     * 6298) */
	// TODO: Appendix G.  RTO Calculation Modification
	// https://tools.ietf.org/html/rfc7323#appendix-G
	struct RTTM {
		using milliseconds = std::chrono::milliseconds;
		using seconds =
			std::chrono::duration<float>; // seconds as float
	 ...
	 */
		RTTM()
			: SRTT{ 1.0 }, RTTVAR{ 1.0 }, RTO{ 1.0 }, time{ 0 },
			  samples{ 0 }
		{
		}
	 ...
	 */
		// TODO: It would be preferable to remove the RTO member and rather use this function than
		// the getter. Then the RTO would only have to be calculated when a new rtx timer is
		// started, instead of with every single measurement. Two thoughts (see
		// Connection::rtx_timeout):
		// 1. A multiplier member could be added to be used for "backing off" the timer (return
		// compute_rto() * multiplier).
		// 2. Dunno how to set RTO = 3s when timeout on ACK for SYN.
		seconds compute_rto() const
		{
			return seconds{ std::max(
				SRTT.count() +
					std::max(CLOCK_G, K * RTTVAR.count()),
				1.0f) };
		}
