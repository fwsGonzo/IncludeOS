...
namespace tcp
{
    ...
    */
	class Packet : public PacketIP4 {
		...
		{
			tcp_header().offset_flags.offset_reserved =
				(offset & 0xF) << 4;
		}
	 ...
	 */
		...
		inline void add_tcp_option(Args &&... args)
		{
			...
			// update offset
			auto newoffset =
				offset() + round_up(opt.length + Padding, 4);
	 ...
	 */
		...
		void clear_options()
		{
			...
			if (UNLIKELY(tcp_data_length() > 0)) {
				throw std::runtime_error(
					"Can't clear options on TCP packet with data");
			}
			set_offset(5);
			set_length(); // update
		}
		...
		std::string to_string() const
		{
			...
			int len = snprintf(
				...
				destination().to_string().c_str(), seq(), ack(),
				tcp_header_length(), tcp_options_length(),
				tcp_data_length(), win(),
				tcp_header().offset_flags.flags);
		...
		void set_length(uint16_t newlen = 0)
		{
			...
			set_data_end(ip_header_length() + tcp_header_length() +
				     newlen);
		}
