...
namespace tcp
{
    ...
    */
	class Connection {
	    ...
	    public:
		/** Called with the connection itself when it's been established. May be a nullptr if the
	 * connection failed. */
	 ...
	 *             NOTE: The Connection_ptr will be a nullptr when an outgoing connection
	 * failed.
	 *
	 ...
	 * @return     This connection
	 */
		inline Connection &on_read(size_t recv_bufsz,
	 ...
	 */
		...
		using DisconnectCallback =
			delegate<void(Connection_ptr self, Disconnect)>;
		/**
	 ...
	 *             Remember to ::close() the connection inside this event (if that's what you
	 * want to do)
	 *
	 * @param[in]  callback  The callback
	 *
	 ...
	 */
		...
		using PacketDroppedCallback =
			delegate<void(const Packet &, Drop_reason)>;
	 ...
	 */
		struct Disconnect {
		    public:
			enum Reason { CLOSING, REFUSED, RESET };
			...
			operator Reason() const noexcept
			{
				return reason;
			}
			...
			bool operator==(const Disconnect &dc) const
			{
				return reason == dc.reason;
			}
	 ...
	 */
		class State {
			...
			enum Result {
				CLOSED =
					-1, // This inditactes that a Connection is done and should be closed.
			...
			virtual Result handle(Connection &,
					      Packet_view &in) = 0;
	    ...
	    */
			...
			virtual void
			unallowed_syn_reset_connection(Connection &,
						       const Packet_view &);

			virtual bool check_ack(Connection &,
					       const Packet_view &);

			virtual void process_fin(Connection &,
						 const Packet_view &);
	 ...
	 */
		struct TCB {
			...
			uint32_t get_ts_recent() const noexcept
			{
				return TS_recent;
			}
	 ...
	 */
		...
		using Sack_list =
			sack::List<sack::Fixed_list<default_sack_entries> >;
	...
	*/
		size_t receive(seq_t seq, const uint8_t *data, size_t n,
			       bool PUSH);
	...
	*/
		...
		{
			return fin_recv_ and
			       static_cast<int32_t>(fin_seq_ - cb.RCV.NXT) == 0;
		}
	...
	*/
		...
		{
			return writeq.has_remaining_requests() and
			       usable_window() >= SMSS();
		}
	...
	*/
		void drop(const Packet_view &packet,
			  Drop_reason reason = Drop_reason::NA);
	...
	*/
		...
		{
			const int64_t x =
				(int64_t)send_window() - (int64_t)flight_size();
	...
	*/
	 ...
	 */
		bool is_win_update(const Packet_view &in,
				   const uint32_t win) const
		{
			return cb.SND.WND != win and
			       (cb.SND.WL1 < in.seq() or
				(cb.SND.WL1 == in.seq() and
				 cb.SND.WL2 <= in.ack()));
		}
	 ...
	 */
		...
		{
			return in.ack() == cb.SND.UNA and flight_size() > 0 and
			       !in.has_tcp_data() and cb.SND.WND == win and
			       !in.isset(SYN) and !in.isset(FIN);
		}
	 ...
	 */
		size_t fill_packet(Packet_view &packet, const uint8_t *data,
				   size_t n)
		{
