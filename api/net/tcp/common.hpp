...
namespace tcp
{
	...
	static constexpr uint32_t default_ws_window_size{
		8192 << default_window_scaling
	};
	...
	template <typename... Args> buffer_t construct_buffer(Args &&... args)
	{
		return std::make_shared<os::mem::buffer>(
	...
	uint16_t calculate_checksum4(const View4 &packet)
	{
		...
		// Compute sum of pseudo-header
		uint32_t sum = (ip_src.whole >> 16) + (ip_src.whole & 0xffff) +
			       (ip_dst.whole >> 16) + (ip_dst.whole & 0xffff) +
			       (Proto_TCP << 8) + htons(length);
