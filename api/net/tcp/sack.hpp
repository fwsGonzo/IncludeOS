...
namespace tcp
{
	...
	{
		struct Block {
			Block(seq_t start, seq_t end) : start(start), end(end)
			{
			}
			...
			{
				return static_cast<int32_t>(seq - start) >=
					       0 and
			...
			bool precedes(const seq_t seq,
				      const Block &other) const noexcept
			{
		...
		template <typename List_impl> class List {
		    public:
			List() = default;

			List(List_impl &&args) : impl{ std::forward(args) }
			{
			}

			Ack_result recv_out_of_order(const seq_t seq,
						     size_t len)
			{
			...
			bool contains(const seq_t seq,
				      const uint32_t len) const noexcept
			{
		...
		template <typename Iterator> struct Connect_result {
			Iterator end;
			Iterator start;
		};

		template <typename Iterator, typename Connectable>
		Connect_result<Iterator> connects_to(Iterator first,
						     Iterator last,
						     const Connectable &value)
		{
		...
		template <typename Iterator> struct Partial_connect_result {
			Iterator end;
			Iterator start;
			uint32_t offs_start;
		};
		...
		partial_connects_to(Iterator first, Iterator last,
				    const Connectable &value)
		{
			Partial_connect_result<Iterator> connected{ last, last,
								    0 };
			for (; first != last; ++first) {
				if (first->connects_end(value)) {
					connected.end = first;
		...
		template <int N = 3> class Fixed_list {
		    public:
			static auto constexpr capacity = N;
			using List =
				std::list<Block, Fixed_list_alloc<Block, N> >;
			using List_iterator = typename List::iterator;

			static_assert(N <= 32 && N > 0,
				      "N wrong sized - optimized for small N");
						     ...
						     size_t len)
			{
				...
				auto connected = partial_connects_to(
					blocks.begin(), blocks.end(), blk);

				if (connected.end !=
				    blocks.end()) // Connectes to an end
				{
					...
					if (connected.start != blocks.end()) {
						connected.end->end =
							connected.start->end;
						blk.end -=
							connected.offs_start; // shrink end with offset (if partial)
						Ensures(connected.offs_start <=
								len &&
							"Offset cannot be bigger than length");
						blocks.erase(connected.start);
					}

					move_to_front(connected.end);
				} else if (connected.start !=
					   blocks.end()) // Connected only to an start
				{
					connected.start->start = blk.start;
					blk.end -=
						connected.offs_start; // shrink end with offset (if partial)
					Ensures(connected.offs_start <= len &&
						"Offset cannot be bigger than length");
			...
			Ack_result new_valid_ack(const seq_t seq, size_t len)
			{
				...
				for (auto it = blocks.begin();
					...
					if (it->contains(ack)) {
						bytes_freed = it->size();
						len -= (ack -
							it->start); // result in 0 if not partial
						blocks.erase(it);
						break;
					}
				}
			...
			{
				if (it != blocks.begin())
					blocks.splice(blocks.begin(), blocks,
						      it);
			}
		...
		template <typename Scoreboard_impl> class Scoreboard {
		    public:
			...
			void recv_sack(const seq_t current, seq_t start,
				       seq_t end)
			{
		...
		template <int N = 3> class Scoreboard_list {
		    public:
			...
			void recv_sack(const seq_t current, Block blk)
			{
				auto connected = connects_to(blocks.begin(),
							     blocks.end(), blk);
