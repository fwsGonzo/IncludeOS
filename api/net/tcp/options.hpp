    ...
    */
	 ...
	 */
		struct opt_mss {
			...
			opt_mss(const uint16_t mss) : mss(htons(mss))
			{
			}
	 ...
	 */
		struct opt_ws {
			...
			opt_ws(const uint8_t shift) : shift_cnt{ shift }
			{
			}
		...
		struct opt_sack {
			...
			opt_sack(const Collection &blocks)
				: length{ static_cast<uint8_t>(
					  sizeof(kind) + sizeof(length) +
					  (blocks.size() *
					   sizeof(typename Collection::
							  value_type))) }
			{
				using T = typename Collection::value_type;
				Expects(blocks.size() * sizeof(T) <= 4 * 8);
				std::memcpy(&val[0], blocks.data(),
					    blocks.size() * sizeof(T));
			}
