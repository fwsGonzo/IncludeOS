...
class WebSocket {
	...
	class Message {
		...
		Message(const uint8_t *data, size_t len)
		{
			...
			const size_t hdr_bytes =
				std::min((size_t)wsh->header_length(), len);
		...
		{
			if (header().is_masked())
				writable_header().masking_algorithm(
					this->data());
		}
     ...
     */
	static WebSocket_ptr upgrade(http::Request &req,
     ...
     */
	static void connect(http::Basic_client &client, uri::URI dest,
