...
class WS_server_connector : public WS_connector {
	...
	using AcceptCallback =
		delegate<bool(net::Socket peer, const std::string &origin)>;
	using Request_handler =
		delegate<void(http::Request_ptr, http::Response_writer_ptr)>;
     ...
     */
	WS_server_connector(ConnectCallback on_connect,
	...
	{
		if (on_accept_) {
			const bool accepted = on_accept_(
...
class WS_client_connector : public WS_connector {
    public:
	using Response_handler = delegate<void(http::Error, http::Response_ptr,
					       http::Connection &)>;
	...
	WS_client_connector(ConnectCallback on_connect)
		: WS_connector(std::move(on_connect)), key_{}
	{
	}
     ...
     */
	static Response_handler create_response_handler(ConnectCallback cb,
							std::string key)
	{
		...
		//};
		auto ptr = std::make_shared<WS_client_connector>(
			std::move(cb), std::move(key));

		return [ptr{ std::move(ptr) }](auto err, auto res, auto &conn) {
     ...
     */
	void on_response(http::Error err, http::Response_ptr res,
	...
	WS_client_connector(ConnectCallback on_connect, std::string key)
		: WS_connector(std::move(on_connect)), key_(std::move(key))
	{
	}
