...
class Inet {
     ...
     *             This is NOT a change in the Path MTU in response to receiving an ICMP Too Big
     * message and no retransmission of packets should take place
     *
	...
	IP4::IP_packet_ptr create_ip_packet(Protocol proto)
	{
		...
		auto ip_packet =
			static_unique_ptr_cast<IP4::IP_packet>(std::move(raw));
	...
	IP6::IP_packet_ptr create_ip6_packet(Protocol proto)
	{
		auto raw = nic_.create_packet(nic_.frame_offset_link());
		auto ip_packet =
			static_unique_ptr_cast<IP6::IP_packet>(std::move(raw));
     ...
     **/
	void resolve(const std::string &hostname, resolve_func func,
		     bool force = false);

	void resolve(const std::string &hostname, net::Addr server,
		     resolve_func func, bool force = false);
	...
	void network_config(ip4::Addr addr, ip4::Addr nmask, ip4::Addr gateway,
	...
	void add_addr(
		const ip6::Addr &addr, uint8_t prefix = 64,
	...
	{
		return a.is_loopback() or
		       std::find(vip4s_.begin(), vip4s_.end(), a) !=
			       vip4s_.end();
	}
	...
	{
		return a.is_loopback() or
		       std::find(vip6s_.begin(), vip6s_.end(), a) !=
			       vip6s_.end();
	}
	...
	void add_vip(ip4::Addr a)
	{
		if (not is_loopback(a)) {
			INFO("inet", "adding virtual ip address %s",
			     a.to_string().c_str());
	...
	{
		if (not is_loopback(a)) {
			INFO("inet", "adding virtual ip6 address %s",
			     a.to_string().c_str());
	...
	{
		return addr.is_v4() ? is_valid_source4(addr.v4()) :
				      is_valid_source6(addr.v6());
	}
