...
namespace dhcp
{
	 ...
	 */
		...
		{
			static_assert(std::is_base_of<option::base, Opt>::value,
				      "Option do not inherit option::base");
			static_assert(
				sizeof(Opt) == sizeof(option::base),
				"Option do not have the same storage as option::base");
			return *(new (reinterpret_cast<uint8_t *>(&options[0]) +
				      offset) Opt(args...));
		}
     ...
     */
	template <typename Storage> class Readable_message {
	    public:
	 ...
	 */
		template <typename Addr> const Addr &chaddr() const
		{
	 ...
	 */
		template <typename Opt> const Opt *find_option() const
		{
			const auto *opt = find_option(Opt::CODE);

			return (opt->code == Opt::CODE) ?
				       static_cast<const Opt *>(opt) :
				       nullptr;
		}
	 ...
	 *
	 * @return     A pointer to an "anonymous" option, most likely with code == END if not
	 * found.
	 */
		const option::base *find_option(option::Code code) const
		{
			auto *raw = reinterpret_cast<uint8_t *>(
				const_cast<option::base *>(
					&message_.options[0]));
	 ...
	 */
		...
		{
			return &message_.options[0] +
			       (Message::LIMIT / sizeof(option::base));
		}

	    protected:
		Storage &message_;

		Readable_message(Storage &msg) : message_{ msg }
		{
		}
	 ...
	 */
		explicit Message_writer(Message *msg, const op_code op,
					const message_type type) noexcept
			: Readable_message{ *msg }
		{
			Expects(msg != nullptr);
			// null the buffer
	 ...
	 * @param[in]  type     The DHCP message type
	 */
		explicit Message_writer(uint8_t *buffer, const op_code op,
					const message_type type) noexcept
			: Message_writer{ reinterpret_cast<Message *>(buffer),
					  op, type }
		{
		}
	 ...
	 * @tparam     Addr    The type of the hardware address.
	 */
		template <typename Addr> void set_chaddr(const Addr *hwaddr)
		{
			Expects(message_.htype != 0 &&
				"hardware address type must be set");
			Expects(sizeof(Addr) == message_.hlen &&
				"size of Addr is not equal the set length of the hw address");
			// clean just to be sure
			std::memset(message_.chaddr.begin(), 0,
				    Message::CHADDR_LEN);
			std::memcpy(message_.chaddr.begin(), hwaddr,
				    sizeof(Addr));
		}
	 ...
	 */
		...
		{
			Expects((opt_offset + sizeof(option::base)) <
					Message::LIMIT &&
				"Adding option past artifical limit set on message (review the limit in "
				"dhcp::Message::size())");

			auto &opt = message_.add_option<Opt>(
				opt_offset, std::forward<Args>(args)...);
