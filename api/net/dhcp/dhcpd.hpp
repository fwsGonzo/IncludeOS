...
namespace dhcp
{
	...
	class DHCPD {
		static const uint32_t DEFAULT_LEASE =
			86400; // seconds. 1 day = 86 400 seconds.
		static const uint32_t DEFAULT_MAX_LEASE =
			345600; // seconds. 4 days = 345 600 seconds.
		static const uint8_t DEFAULT_PENDING = 30;
		static const uint8_t MAX_NUM_OPTIONS =
			30; // max number of options in a message from a client

	    public:
		DHCPD(UDP &udp, ip4::Addr pool_start, ip4::Addr pool_end,
		      uint32_t lease = DEFAULT_LEASE,
		      uint32_t max_lease = DEFAULT_MAX_LEASE,
		      uint8_t pending = DEFAULT_PENDING);
		...
		bool
		record_exists(const Record::byte_seq &client_id) const noexcept;

		int get_record_idx(
			const Record::byte_seq &client_id) const noexcept;
		...
		ip4::Addr
		network_address(ip4::Addr ip) const noexcept // x.x.x.0
		{
			return ip & netmask_;
		}
		...
		std::vector<Record>
			records_; // Temp - Instead of persistent storage
