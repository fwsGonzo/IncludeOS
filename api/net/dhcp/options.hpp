...
namespace dhcp
{
	 ...
	 */
		struct addr_option : public base {
			...
			constexpr addr_option(const Code c,
					      const Addr *addr) noexcept
				: base{ c, sizeof(Addr) }
			{
	 ...
	 */
		struct end : public type<END>, public base {
			constexpr end() noexcept : base{ END, 0 }
			{
			}
		};
	 ...
	 * @brief      SUBNET_MASK (1)
	 */
		struct subnet_mask : public type<SUBNET_MASK>,
				     public addr_option {
			template <typename Addr>
	 ...
	 */
		struct time_offset : public type<TIME_OFFSET>, public base {
			constexpr time_offset(int32_t offset_secs) noexcept
				: base{ type::CODE, 4 }
			{
				*(reinterpret_cast<int32_t *>(&val[0])) =
					htonl(offset_secs);
			}
		};
	 ...
	 */
		struct domain_name_servers : public type<DOMAIN_NAME_SERVERS>,
					     public addr_option {
			template <typename Addr>
	 ...
	 */
		struct domain_name : public type<DOMAIN_NAME>, public base {
			domain_name(const std::string &dname) noexcept
				: base{ type::CODE,
					static_cast<uint8_t>(dname.size()) }
			{
				std::memcpy(&val[0], dname.data(),
					    dname.size());
			}
	 ...
	 */
		struct broadcast_address : public type<BROADCAST_ADDRESS>,
					   public addr_option {
			template <typename Addr>
	 ...
	 */
		struct req_addr : public type<DHCP_REQUESTED_ADDRESS>,
				  public addr_option {
			template <typename Addr>
	 ...
	 */
		struct message_type : public type<DHCP_MESSAGE_TYPE>,
				      public base {
			constexpr
			message_type(const dhcp::message_type m_type) noexcept
	 ...
	 */
		struct server_identifier : public type<DHCP_SERVER_IDENTIFIER>,
					   public addr_option {
			template <typename Addr>
	 ...
	 */
		struct param_req_list
			: public type<DHCP_PARAMETER_REQUEST_LIST>,
			  public base {
			param_req_list(const std::vector<Code> &codes) noexcept
				: base{ DHCP_PARAMETER_REQUEST_LIST,
					static_cast<uint8_t>(codes.size()) }
			{
				Expects(codes.size() < 50); // or something
				std::memcpy(&val[0], codes.data(),
					    codes.size());
			}
		};
	 ...
	 */
		struct message : public type<DHCP_MESSAGE>, public base {
			...
			{
				Expects(not msg.empty() and
					msg.size() <= 128); // or something
	 ...
	 */
		struct client_identifier : public type<DHCP_CLIENT_IDENTIFIER>,
					   public base {
			template <typename Addr>
			constexpr client_identifier(const htype type,
						    const Addr *addr) noexcept
				: base{ DHCP_CLIENT_IDENTIFIER,
					sizeof(type) + sizeof(Addr) }
			{
