...
{
template <typename T> class Stateful_addr {
    public:
	using Timestamp = typename T::timestamp_t;
	static constexpr uint32_t
		infinite_lifetime = 0xFFFF'FFFF; // ¯\_(∞)_/¯
		...
		valid_ts_{ valid_lifetime != infinite_lifetime ?
				   (T::time_since_boot() + valid_lifetime) :
				   0 },
		prefix_{ prefix }
	{
	}
	...
	bool preferred() const noexcept
	{
		if (not valid())
			return false;
		return preferred_ts_ ? T::time_since_boot() < preferred_ts_ :
				       true;
	}
	...
	uint32_t remaining_valid_time()
	{
		...
		if (valid_ts_ < T::time_since_boot())
			return 0;
	...
	void update_preferred_lifetime(uint32_t preferred_lifetime)
	{
		preferred_ts_ =
			preferred_lifetime != infinite_lifetime ?
				(T::time_since_boot() + preferred_lifetime) :
				0;
	}
