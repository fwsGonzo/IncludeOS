...
class PacketIP6 : public Packet {
	...
	{
		return ((ver_tc_fl() & 0xF0000) >> 12) + (ver_tc_fl() & 0xF);
	}
	...
	{
		return ip6::parse_upper_layer_proto(ext_hdr_start(), data_end(),
						    next_protocol());
	}
	...
	{
		Expects(size() and
			static_cast<size_t>(size()) >= sizeof(ip6::Header));
	...
	{
		ip6_header().payload_length =
			htons(size() - sizeof(ip6::Header));
	}
	...
	const Byte *ip_data_ptr() const noexcept
		__attribute__((assume_aligned(4)))
	{
		return payload();
	}
