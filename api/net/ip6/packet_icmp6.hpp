...
class Packet {
	...
	struct RaHeader {
		...
		uint8_t ma_config_flag : 1, mo_config_flag : 1, reserved : 6;
	...
	uint16_t id() const noexcept
	{
		return reinterpret_cast<const IdSe *>(header().payload)
			->identifier;
	}
	...
	{
		reinterpret_cast<IdSe *>(header().payload[0])->identifier =
			error;
	}
	...
	**/
	...
	template <typename T> const T &view_payload_as()
	{
	...
	const PacketIP6 &ip() const
	{
		return *pckt_;
	}

	/** Construct from existing packet **/
	Packet(std::unique_ptr<PacketIP6> pckt) : pckt_{ std::move(pckt) }
	{
	}

	using IP_packet_factory =
		delegate<std::unique_ptr<PacketIP6>(Protocol)>;
	/** Provision fresh packet from factory **/
	Packet(IP_packet_factory create) : pckt_{ create(Protocol::ICMPv6) }
	{
