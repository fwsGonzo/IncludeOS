...
namespace ip6
{
     ...
     */
	struct Addr {
		constexpr Addr() noexcept : i32{ { 0, 0, 0, 0 } }
		{
		}
		...
		Addr(const Addr &a) noexcept : i64{ a.i64 }
		{
		}

		Addr(Addr &&a) noexcept : i64{ a.i64 }
		{
		}
	 ...
	 */
	    ...
	    **/
		...
		{
			return i32[0] == htonl(0xFF020000) and i32[1] == 0 and
			       i32[2] == htonl(0x1) and
			       (i32[3] & htonl(0xFF000000)) ==
				       htonl(0xFF000000);
		}
		...
		{
			return Addr{ 0xFF020000, 0, 0x1,
				     (0xFF000000 | ntohl(other.i32[3])) };
		}
		...
		static Addr link_local(uint64_t eui) noexcept
		{
			return Addr
			{
				0xFE80 '0000' 0000'0000, ntohll(eui)};
			}
	 ...
	 **/
			...
			template <typename T> T get_part(const uint8_t n) const
			{
				static_assert(
				...
				if
					constexpr(std::is_same_v<T, uint8_t>)
					{
						Expects(n < 16);
						return i8[n];
					}
				else if
					constexpr(std::is_same_v<T, uint16_t>)
					{
						Expects(n < 8);
						return i16[n];
					}
				else if
					constexpr(std::is_same_v<T, uint32_t>)
					{
						Expects(n < 4);
						return i32[n];
					}
	 ...
	 */
			...
			Addr operator&(uint8_t prefix) const noexcept
			{
				...
				return Addr{ addr[0], addr[1], addr[2],
					     addr[3] };
			}
...
namespace std
{
	template <> struct hash<net::ip6::Addr> {
