...
class NdpOptions {
	...
	struct route_info {
		...
		uint8_t len;
		uint8_t prefix_len;
		uint8_t reserved_l : 3, route_pref : 2, reserved_h : 2;
		uint32_t lifetime;
		uint8_t prefix[0];
	};
	...
	struct prefix_info {
		...
		uint8_t len;
		uint8_t prefix_len;
		uint8_t onlink : 1, autoconf : 1, reserved : 6;
		uint32_t valid;
		uint32_t prefered;
		uint32_t reserved2;
		ip6::Addr prefix;
	...
	{
		if (opt->type == ND_OPT_RDNSS || opt->type == ND_OPT_DNSSL) {
			return true;
		}
		return false;
	}
	...
	NdpOptions()
		: header_{ nullptr }, nd_opts_ri{ nullptr },
	...
	bool parse_prefix(Pinfo_handler autoconf_cb, Pinfo_handler onlink_cb);
	...
	struct nd_options_header *option(uint8_t option)
	{
		if (option < ND_OPT_ARRAY_MAX) {
			...
			return nd_opts_ri;
		} else if (option == ND_OPT_RDNSS || option == ND_OPT_DNSSL) {
		}
		return nullptr;
	}
};
