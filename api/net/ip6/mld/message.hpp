...
struct Message {
	...
	Message(uint16_t delay, ip6::Addr mcast)
		: max_res_delay{ htons(delay) }, mcast_addr{ std::move(mcast) }
	{
	}
};

struct Query : public Message {
	Query(uint16_t delay) : Message{ delay, ip6::Addr::addr_any }
	{
	}
...
struct Report : public Message {
	Report(ip6::Addr mcast) : Message{ 0, std::move(mcast) }
	{
	}
};
...
namespace v2
{
	...
	struct Mcast_addr_record {
		...
		Mcast_addr_record(Record_type rec, ip6::Addr mcast)
			: rec_type{ rec }, multicast{ std::move(mcast) }
		{
		}

		size_t size() const noexcept
		{
			return sizeof(Mcast_addr_record) +
			       num_src * (sizeof(ip6::Addr) + data_len);
		}
	};
