...
class Ndp {
	...
	using Autoconf_handler =
		delegate<void(const ndp::option::Prefix_info &)>;
	...
	void add_addr_onlink(ip6::Addr ip, uint8_t prefix,
	...
	void transmit(Packet_ptr, ip6::Addr next_hop,
		      MAC::Addr mac = MAC::EMPTY);

	/** Cache IP resolution. */
	void cache(ip6::Addr ip, MAC::Addr mac, NeighbourStates state,
		   uint32_t flags, bool update = true);
	void cache(ip6::Addr ip, uint8_t *ll_addr, NeighbourStates state,
		   uint32_t flags, bool update = true);
	...
	struct Neighbour_Cache_entry {
		...
		Neighbour_Cache_entry(MAC::Addr mac, NeighbourStates state,
		...
		bool expired() const noexcept
		{
			return RTC::time_since_boot() >
			       timestamp_ + neighbour_cache_exp_sec_;
		}
	...
	struct Queue_entry {
		Queue_entry(Packet_ptr p) : pckt{ std::move(p) }
		{
		}
		Packet_ptr pckt;
	...
	using DestCache =
		std::unordered_map<ip6::Addr, Destination_Cache_entry>;
	...
	Timer neighbour_reachability_timer_{
		{ *this, &Ndp::check_neighbour_reachability }
	};
	Timer resolve_timer_{ { *this, &Ndp::resolve_waiting } };
	Timer flush_neighbour_timer_{ { *this,
