...
class Addr_list {
	...
	{
		return (not dest.is_linklocal()) ? get_first_unicast() :
						   get_first_linklocal();
	}

	ip6::Addr get_first() const noexcept
	{
		return (not list.empty()) ? list.front().addr() :
					    ip6::Addr::addr_any;
	}
	...
	int input(const ip6::Addr &addr, uint8_t prefix, uint32_t pref_lifetime,
