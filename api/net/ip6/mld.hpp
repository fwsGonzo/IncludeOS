...
class Mld {
	...
	static const int MULTICAST_LISTENER_INTERVAL =
		(ROBUSTNESS_VAR * QUERY_INTERVAL * 1000) +
		QUERY_RESPONSE_INTERVAL; // in milliseconds
	static const int OTHER_QUERIER_PRESENT_INTERVAL =
		(ROBUSTNESS_VAR * QUERY_INTERVAL * 1000) +
		(QUERY_RESPONSE_INTERVAL / 2); // in milliseconds
	static constexpr double STARTUP_QUERY_INTERVAL =
		QUERY_INTERVAL / 4; // in seconds
	...
	enum RouterStates { QUERIER, NON_QUERIER, MAX_ROUTER_STATE };
	...
	struct MulticastHostNode {
		...
		const ip6::Addr &addr() const
		{
			return mcast_addr_;
		}
		RTC::timestamp_t timestamp() const
		{
			return timestamp_;
		}
		...
		void handle(icmp6::Packet &pckt)
		{
			state_handlers_[state_](pckt);
		}

	    private:
		const HostStates &state() const
		{
			return state_;
		}
		void setState(const HostStates state)
		{
			state_ = state;
		}
	...
	struct MulticastRouterNode {
	    public:
		MulticastRouterNode();
		const RouterStates &state() const
		{
			return state_;
		}
		void setState(const RouterStates state)
		{
			state_ = state;
		}
...
class Mld2 {
    public:
	static const int ROBUSTNESS_VAR = 2;
	enum class FilterMode : uint8_t { INCLUDE, EXCLUDE };
	using Stack = IP6::Stack;
	using ICMP_type = ICMP6_error::ICMP_type;
	...
	void join(ip6::Addr addr, FilterMode filtermode,
	...
	struct Multicast_listening_record {
	    ...
	    public:
		Multicast_listening_record(FilterMode filter_mode,
					   SourceList source_list) noexcept
		...
		void exclude(SourceList source_list)
		{
			...
			for (auto src = source_list_.begin();
			     src != source_list_.end();) {
				if (std::find(source_list.begin(),
					      source_list.end(),
					      *src) == source_list.end()) {
		...
		void include(SourceList source_list)
		{
			if (source_list_.empty()) {
				...
				for (auto src : source_list) {
					if (std::find(source_list_.begin(),
						      source_list_.end(),
						      src) ==
					    source_list_.end()) {
	...
	using MldRec =
		std::unordered_map<ip6::Addr, Multicast_listening_record>;
	Stack &inet_;
	MldRec mld_records_;
};
} // namespace net
