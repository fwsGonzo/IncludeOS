...
namespace net
{
template <class IPV> struct Route {
	using Stack = Inet;
	...
	bool operator==(const Route &b) const noexcept
	{
		return net_ == b.net() and netmask_ == b.netmask() and
		       cost_ == b.cost() and iface_ == b.interface();
	}
	...
	Route(Addr net, Netmask mask, Addr nexthop, Stack &iface,
	      int cost = 100)
		: net_{ net }, netmask_{ mask }, nexthop_{ nexthop },
		  iface_{ &iface }, cost_{ cost }
	{
...
template <class IPV> struct Router {
     ...
     **/
	inline void forward(Packet_ptr pckt, Stack &stack,
	...
	{
		Routing_table t;
		std::copy_if(routing_table_.begin(), routing_table_.end(),
			     std::back_inserter(t),
			     [dest](const Route<IPV> &route) {
	...
	Router(Routing_table tbl = {})
		: routing_table_{ tbl },
		  packets_fwd{ Statman::get()
				       .get_or_create(Stat::UINT64,
						      "router.packets_fwd")
				       .get_uint64() },
		  packets_dropped{ Statman::get()
					   .get_or_create(
						   Stat::UINT64,
						   "router.packets_dropped")
					   .get_uint64() },
		  bytes_fwd{ Statman::get()
				     .get_or_create(Stat::UINT64,
						    "router.bytes_fwd")
				     .get_uint64() }
	{
...
{
template <>
inline void Route<IP4>::ship(Packet_ptr pckt, Addr nexthop,
			     Conntrack::Entry_ptr ct)
{
...
template <>
inline void Route<IP6>::ship(Packet_ptr pckt, Addr nexthop,
			     Conntrack::Entry_ptr ct)
{
	iface_->ip6_obj().ship(std::move(pckt), nexthop, ct);
}

template <> inline IP4::addr Route<IP4>::nexthop(IP4::addr ip) const noexcept
{
...
template <> inline IP6::addr Route<IP6>::nexthop(IP6::addr ip) const noexcept
{
	// No need to go via nexthop if IP is on the same net as interface
	if ((ip & iface_->netmask6()) ==
	    (iface_->ip6_addr() & iface_->netmask6()))
		return ip;
...
template <>
inline void Router<IP4>::forward(Packet_ptr pckt, Stack &stack,
				 Conntrack::Entry_ptr ct)
{
	Expects(pckt);
	...
	if (pckt->ip_ttl() == 0) {
		...
		if (this->send_time_exceeded == true and
		    not pckt->ip_dst().is_multicast())
			stack.icmp().time_exceeded(
				std::move(pckt),
				icmp4::code::Time_exceeded::TTL);
		packets_dropped++;
		return;
	}
	...
	const bool should_decrement_ttl =
		not stack.is_valid_source(pckt->ip_src());
...
template <>
inline void Router<IP6>::forward(Packet_ptr pckt, Stack &stack,
				 Conntrack::Entry_ptr ct)
{
	Expects(pckt);
	...
	if (pckt->hop_limit() == 0) {
		...
		if (this->send_time_exceeded == true and
		    not pckt->ip_dst().is_multicast())
			stack.icmp6().time_exceeded(
				std::move(pckt),
				icmp6::code::Time_exceeded::HOP_LIMIT);
		return;
	}

	// When the packet originates from our host, it still passes forward
	// We add this check to prevent decrementing TTL for "none routed" packets. Hmm.
	const bool should_decrement_hop_limit =
		not stack.is_valid_source(pckt->ip_src());
