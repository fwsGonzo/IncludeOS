...
class Botan_server : public http::Server {
	...
	inline Botan_server(const std::string &name, fs::Dirent &ca_key,
			    fs::Dirent &ca_cert, fs::Dirent &server_key,
			    net::TCP &tcp, Server_args &&... server_args);
     ...
     */
	template <typename... Server_args>
	inline Botan_server(Botan::Credentials_Manager *in_credman,
			    Botan::RandomNumberGenerator &in_rng, net::TCP &tcp,
     ...
     * @param      server_key  The server key
     */
	void load_credentials(const std::string &name, fs::Dirent &ca_key,
			      fs::Dirent &ca_cert, fs::Dirent &server_key);
...
template <typename... Server_args>
inline Botan_server::Botan_server(const std::string &name, fs::Dirent &ca_key,
				  fs::Dirent &ca_cert, fs::Dirent &server_key,
				  net::TCP &tcp, Server_args &&... server_args)
	: Server{ tcp, std::forward<Server>(server_args)... }, rng(get_rng())
{
	load_credentials(name, ca_key, ca_cert, server_key);
}

template <typename... Server_args>
inline Botan_server::Botan_server(Botan::Credentials_Manager *in_credman,
				  Botan::RandomNumberGenerator &in_rng,
				  net::TCP &tcp, Server_args &&... server_args)
	: Server{ tcp, std::forward(server_args)... }, rng(in_rng),
	  credman(in_credman)
{
