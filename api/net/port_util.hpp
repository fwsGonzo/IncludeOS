...
class Port_util {
	...
	Port_util()
		: ports(),
		  eph_view{ // set the ephemeral view to be between 49152-65535
			    ...
			    static_cast<MemBitmap::index_t>(
				    size() / sizeof(MemBitmap::word))
		  },
		  ephemeral_(net::new_ephemeral_port()), eph_count(0)
	{
		// all ports are free
		ports.set_all();
	}
	...
	{
		return port_ranges::DYNAMIC_END + 1 -
	...
	void bind(const uint16_t port) noexcept
	{
		...
		if (port_ranges::is_dynamic(port))
			++eph_count;
	}
	...
	void increment_ephemeral()
	{
		...
		if (UNLIKELY(is_bound(ephemeral_))) {
			...
			Ensures(i != -1 &&
				"Did not found a free ephemeral even tho has_free_ephemeral() == true...");
			ephemeral_ = port_ranges::DYNAMIC_START + i;
		}

		Expects(not is_bound(ephemeral_) &&
			"Generated ephemeral port is already bound. Please fix me!");
	}
}; // < class Port_util
static_assert((port_ranges::DYNAMIC_START / 8) % sizeof(MemBitmap::word) == 0,
	      "Must be word-sized multiple");
static_assert(Port_util::size() % sizeof(MemBitmap::word) == 0,
	      "Must be word-sized multiple");
