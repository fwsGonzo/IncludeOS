...
class StreamBuffer : public net::Stream {
	...
	StreamBuffer(Timers::duration_t timeout = std::chrono::microseconds(10))
		: timer({ this, &StreamBuffer::congested }),
		  congestion_timeout(timeout)
	{
	}
	...
	{
		return construct_buffer_with_flag(m_read_congested,
	...
	{
		return construct_buffer_with_flag(m_write_congested,
						  std::forward<Args>(args)...);
	}
	...
	{
		return std::move(this->m_on_close);
	}
	...
	{
		try {
			auto buffer = std::make_shared<os::mem::buffer>(
				std::forward<Args>(args)...);
			flag = false;
			return buffer;
		...
		if (m_on_data != nullptr) {
			...
			for (auto buf : m_send_buffers) {
				...
				if (m_on_read == nullptr) {
					break;
				} // if calling m_on_read reset the callbacks
				// exit
			}
		}
	}
