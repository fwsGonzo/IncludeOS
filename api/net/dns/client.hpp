...
class Client {
	...
	using Resolve_handler =
		delegate<void(dns::Response_ptr, const Error &err)>;
     ...
     */
	void resolve(Address dns_server, Hostname hostname,
		     Resolve_handler handler, Timer::duration_t timeout,
		     bool force = false);
     ...
     */
	void resolve(Address dns_server, Hostname hostname,
		     Resolve_handler handler, bool force = false)
	{
		resolve(dns_server, std::move(hostname), std::move(handler),
			DEFAULT_RESOLVE_TIMEOUT, force);
	}
     ...
     */
	void add_cache_entry(const Hostname &hostname, Address addr,
	...
	struct Request {
		...
		Request(Client &cli, udp::Socket &sock, dns::Query q,
			Resolve_handler cb);
	    ...
	    private:
		void parse_response(Addr, udp::port_t, const char *data,
				    size_t len);
