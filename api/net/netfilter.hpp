...
namespace net
{
enum class Filter_verdict_type : uint8_t { ACCEPT, DROP };
 ...
 */
template <typename IPV> struct Filter_verdict {
	using IP_packet_ptr = typename IPV::IP_packet_ptr;

	Filter_verdict(IP_packet_ptr pkt,
		       const Filter_verdict_type verd) noexcept
		: packet{ std::move(pkt) },
		  verdict{ verd }
	{
	}
...
using Packetfilter =
	delegate<Filter_verdict<IPV>(typename IPV::IP_packet_ptr, Inet &,
				     Conntrack::Entry_ptr)>;
 ...
 * @tparam     IPV   IP Version (4 or 6)
 */
template <typename IPV> struct Filter_chain {
	using IP_packet_ptr = typename IPV::IP_packet_ptr;
     ...
     *  Execute the chain
     */
	Filter_verdict<IPV> operator()(IP_packet_ptr pckt, Inet &stack,
				       Conntrack::Entry_ptr ct)
	{
		Filter_verdict<IPV> verdict{ std::move(pckt),
					     Filter_verdict_type::ACCEPT };
		int i = 0;
	...
	Filter_chain(const char *chain_name,
		     std::initializer_list<Packetfilter<IPV> > filters)
