...
extern "C" ssize_t s2n_conn_serialize_to(struct s2n_connection *, void *addr,
					 size_t);
extern "C" struct s2n_connection *
s2n_conn_deserialize_from(struct s2n_config *config, const void *addr,
			  const size_t);

namespace s2n
{
typedef int s2n_connection_send(void *io_context, const uint8_t *buf,
				uint32_t len);
	...
	: m_transport(std::move(t))
{
	...
	if (s2n_connection_set_config(this->m_conn, config) < 0) {
		...
		throw std::runtime_error(
			"Error setting s2n::TLS_stream config");
	}
...
inline void TLS_stream::initialize(bool outgoing)
{
	...
	if (outgoing) {
		...
		S2N_PRINT("s2n_negotiate: %d / %d, blocked = %x\n", r,
			  m_readq.size(), blocked);
		if (r < 0) {
			if (s2n_error_get_type(s2n_errno) !=
			    S2N_ERR_T_BLOCKED) {
				fprintf(stderr,
					"Failed to negotiate: '%s'. %s\n",
					s2n_strerror(s2n_errno, "EN"),
					s2n_strerror_debug(s2n_errno, "EN"));
...
inline TLS_stream::~TLS_stream()
{
	...
	assert(m_busy == false &&
	       "Cannot delete stream while in its call stack");
...
inline void TLS_stream::tls_read(buffer_t data_in)
{
	if (data_in != nullptr) {
		S2N_PRINT("s2n::tls_read(%p): %p, %zu bytes -> %p\n", this,
			  data_in->data(), data_in->size(), m_readq.data());
	...
	do {
		...
		if (handshake_completed()) {
			...
			if (buffer == nullptr)
				return; // what else is there to do?

			r = s2n_recv(this->m_conn, buffer->data(),
				     buffer->size(), &blocked);
...
inline size_t TLS_stream::serialize_to(void *addr, size_t size) const
{
	...
	*hdr = {};
	...
	hdr->conn_size =
		s2n_conn_serialize_to(this->m_conn, hdr->conn_addr(), size);
	if (hdr->conn_size < 0) {
...
inline TLS_stream_ptr TLS_stream::deserialize_from(s2n_config *config,
						   Stream_ptr transport,
						   const bool outgoing,
						   const void *data,
						   const size_t size)
{
	...
	// restore connection
	auto *conn = s2n_conn_deserialize_from(config, hdr->conn_addr(),
					       hdr->conn_size);
	if (conn != nullptr) {
		// restore stream
		return std::make_unique<TLS_stream>(conn, std::move(transport),
						    outgoing);
	}
	return nullptr;
}
