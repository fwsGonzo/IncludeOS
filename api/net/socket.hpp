...
class Socket {
     ...
     */
	constexpr Socket() noexcept : Socket{ 0 }
	{
	}

	explicit constexpr Socket(const port_t port) noexcept : addr_{},
								port_{ port }
	{
	}
	...
	Socket(Socket &&other) noexcept : addr_{ std::move(other.addr_) },
					  port_{ other.port_ }
	{
	}
	...
	{
		return (addr_.v6() == ip6::Addr::link_unspecified) and
		       (port() == 0);
	}
	...
	bool operator<(const Socket &other) const noexcept
	{
		return addr_ < other.addr_ or
		       (addr_ == other.addr_ and port_ < other.port_);
	}
...
struct Quadruple {
	...
	Quadruple() = default;
	Quadruple(Socket s, Socket d) : src(std::move(s)), dst(std::move(d))
	{
	}
	...
	bool operator<(const Quadruple &other) const noexcept
	{
		return src < other.src or
		       (src == other.src and dst < other.dst);
	}
...
template <> struct hash<net::Socket> {
	...
	{
		return s.address().v6().i64[0] ^ s.address().v6().i64[1] ^
		       s.port();
	}
};
...
template <> struct hash<net::Quadruple> {
    public:
