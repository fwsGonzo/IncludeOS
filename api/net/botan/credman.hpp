...
class Credman : public Botan::Credentials_Manager {
	...
	{
		std::unique_ptr<Botan::Certificate_Store> store(
			new Botan::Certificate_Store_In_Memory(m_ca_cert));
	...
	static Credman *create(const std::string &name,
 ...
 *
 **/
inline Credman *Credman::create(const std::string &server_name,
				Botan::RandomNumberGenerator &rng,
				std::unique_ptr<Botan::Private_Key> ca_key,
				Botan::X509_Certificate ca_cert,
				std::unique_ptr<Botan::Private_Key> server_key)
{
	...
	auto req = Botan::X509::create_cert_req(server_opts, *server_key,
						"SHA-512", rng);
