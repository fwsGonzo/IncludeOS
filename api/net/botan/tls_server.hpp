...
namespace botan
{
	class Server : public Botan::TLS::Callbacks, public net::Stream {
		...
		Server(net::Stream_ptr remote,
			...
			: m_creds{ credman },
			  m_session_manager{}, m_tls{ *this, m_session_manager,
			  ...
			  m_transport{ std::move(remote) }
		{
			...
			m_transport->on_close(
				{ this, &Server::close_callback_once });
		}
		~Server()
		{
			assert(m_busy == false &&
		...
		void on_data(DataCallback cb) override
		{
			...
			throw std::runtime_error(
				"on_data not implemented on botan::server");
		}
		size_t next_size() override
		{
			// FIXME
			throw std::runtime_error(
				"next_size not implemented on botan::server");
		}
		buffer_t read_next() override
		{
			// FIXME
			throw std::runtime_error(
				"read_next not implemented on botan::server");
		}
		...
		void tls_read(buffer_t buf)
		{
			this->m_busy = true;
			try {
				int rem = m_tls.received_data(buf->data(),
							      buf->size());
				(void)rem;
		...
		bool
		tls_session_established(const Botan::TLS::Session &) override
		{
		...
		void tls_record_received(uint64_t, const uint8_t buf[],
					 size_t buf_len) override
		{
			if (m_on_read) {
				m_on_read(Stream::construct_buffer(
					buf, buf + buf_len));
			}
		}
