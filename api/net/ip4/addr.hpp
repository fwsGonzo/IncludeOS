...
namespace ip4
{
	 ...
	 */
		constexpr Addr(const uint8_t p1, const uint8_t p2,
			       const uint8_t p3, const uint8_t p4) noexcept
	 ...
	 */
		Addr(const std::string &str)
		{
			...
			if (length_error) {
				throw Invalid_address(
					"Missing byte value at end of " + str);
			}
			this->whole = value[0] | (value[1] << 8) |
	 ...
	 */
		constexpr bool
		operator==(const uint32_t raw_addr) const noexcept
		{
	 ...
	 */
		std::string str() const
		{
			...
			snprintf(ipv4_addr, sizeof(ipv4_addr),
				 "%1i.%1i.%1i.%1i", (whole >> 0) & 0xFF,
				 (whole >> 8) & 0xFF, (whole >> 16) & 0xFF,
				 (whole >> 24) & 0xFF);
