...
class IP4 {
	...
	using drop_handler =
		delegate<void(IP_packet_ptr, Direction, Drop_reason)>;
	...
	/**
     *  Path MTU Discovery increases a path's PMTU if it hasn't been decreased in the last 10
     * minutes to avoid stale PMTU values
     */
    ...
    */
	...
	void ship(Packet_ptr, ip4::Addr next_hop = 0,
	...
	/**
     * @brief      Updates the Path MTU for the specified path (represented by the destination
     * address and port (Socket)) If the path doesn't exist, a new entry/path with these input
     * values is created This method also starts the pmtu_timer_ if it's not running
     *
     * @param[in]  dest              The destination (address and port)
     * @param[in]  new_pmtu          The new pmtu
     * @param[in]  received_too_big  Indicates that an ICMP Too Big message has been received and
     * the method is called as a result of this
     * @param[in]  total_length      The IP header's total length, given if the ICMP Too Big message
     * contains a next hop MTU value of zero. This is used to make an estimate of the new Path MTU
     * value
     * @param[in]  header_length     The IP header's header length, given if the ICMP Too Big
     * message contains a next hop MTU value of zero. This is used to make an estimate of the new
     * Path MTU value
     */
     ...
     *
     * @param[in]  dest  The destination (represented by the destination Socket (IP address and
     * port))
     */
	...
	/**
     * @brief      Get the timestamp (time since boot) for when the PMTU entry's PMTU value was last
     * decreased
     *
     *
     ...
     *             Returns 0 if the entry wasn't found or the PMTU for the entry has never been
     * decreased
     */
     ...
     */
	PMTU pmtu_from_total_length(uint16_t total_length,
				    uint8_t header_length, PMTU current);
     ...
     *             requires sending packets larger than the current estimated PMTU, and because the
     * likelihood is that the PMTU will not have increased, this MUST be done at infrequent
     * intervals. An attempt to detect an increase (by sending a packet larger than the current
     * estimate) MUST NOT be done less than 5 minutes after a Packet Too Big message has been
     * received for the given path. The recommended setting for this timer is twice its minimum
     * value (10 minutes).
     */
	...
	class PMTU_entry {
	    public:
		PMTU_entry(PMTU pmtu, PMTU reset_pmtu,
			   bool received_too_big) noexcept
			: pmtu_{ pmtu },
			  reset_pmtu_{ reset_pmtu }
		{
	 ...
	 *
	 *             RFC 1191: Whenever a PMTU is decreased in response to a Datagram Too Big
	 * message, the timestamp is set to the current time
	 *
	 * @param[in]  pmtu             The new PMTU
	 * @param[in]  received_too_big The new PMTU value is set in a response to a ICMP Datagram
	 * Too Big message
	 */
		void set_pmtu(PMTU pmtu, bool received_too_big) noexcept
		{
			...
			if (received_too_big)
				timestamp_ = RTC::
					time_since_boot(); // timestamp set to the current "time"
		}
	 ...
	 * the first-hop data-link MTU, and the packetization layers should be notified of the
	 * change. This will cause the complete PMTU Discovery process to take place again. Can
	 * cause dropping of datagrams every 10 minutes.
	 */
		PMTU reset_pmtu_;
     ...
     *
     *  Key: Destination address (chosen as the local representation of a path after reviewing RFC
     * 1191, 1981 and 4821) Value: The Path MTU (the minimum link MTU of all the links in a path
     ...
     * The entry's timestamp is renewed/updated every time its PMTU value is decreased in response
     * to a ICMP Datagram Too Big message
     */
	...
	IP_packet_ptr drop(IP_packet_ptr ptr, Direction direction,
			   Drop_reason reason);
