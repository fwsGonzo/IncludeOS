...
class ICMP_view {
	...
	ICMP_view(icmp4::Packet &pckt)
		: id_{ pckt.id() }, seq_{ pckt.sequence() },
		  src_{ pckt.ip().ip_src() }, dst_{ pckt.ip().ip_dst() },
		  type_{ pckt.type() }, code_{ pckt.code() },
		  checksum_{ pckt.checksum() }, payload_{
			  (const char *)pckt.payload().data(),
			  (size_t)pckt.payload().size()
		  }
	{
	}
...
class ICMPv4 {
	...
	using Tuple =
		std::pair<uint16_t, uint16_t>; // identifier and sequence number
     ...
     */
	void destination_unreachable(Packet_ptr pckt,
				     icmp4::code::Dest_unreachable code);
     ...
     *  Sending a Time Exceeded message from a gateway when time to live exceeded in transit (code
     * 0)
     */
	void time_exceeded(Packet_ptr pckt, icmp4::code::Time_exceeded code);

	/**
     *  Sending a Parameter Problem message if the gateway or host processing a datagram finds a
     * problem with the header parameters such that it cannot complete processing the datagram. The
     * message is only sent if the error caused the datagram to be discarded Code 0 means Pointer
     * (uint8_t after checksum) indicates the error/identifies the octet where an error was detected
     *  in the IP header
	...
	void ping(ip4::Addr ip, icmp_func callback,
		  int sec_wait = SEC_WAIT_FOR_REPLY);

	void ping(const std::string &hostname);
	void ping(const std::string &hostname, icmp_func callback,
		  int sec_wait = SEC_WAIT_FOR_REPLY);

    private:
	static int
		request_id_; // message identifier for messages originating from IncludeOS
	Stack &inet_;
	downstream network_layer_out_ = nullptr;
	uint8_t includeos_payload_[48] = {
		'I', 'N', 'C', 'L', 'U', 'D', 'E', 'O', 'S', '1', '2', '3',
	...
	{
		return (pckt_size >=
			sizeof(IP4::header) + icmp4::Packet::header_size());
	}
	...
	struct ICMP_callback {
		...
		ICMP_callback(ICMPv4 &icmp, Tuple t, icmp_func cb, int sec_wait)
			: tuple{ t }, callback{ cb }
		{
			timer_id = Timers::oneshot(
				std::chrono::seconds(sec_wait),
				[&icmp, t](Timers::id_t) {
	...
	/**
     *  If a Destination Unreachable: Fragmentation Needed error mesage is received, the Path MTUs
     * in IP should be updated and the packetization/transportation layer should be notified
     * (through Inet) RFC 1191 (Path MTU Discovery IP4), 1981 (Path MTU Discovery IP6) and 4821
     * (Packetization Layer Path MTU Discovery) A Fragmentation Needed error message is sent as a
     * response to a packet with an MTU that is too big for a node in the path to its destination
     * and with the Don't Fragment bit set as well
     */
	...
	inline void execute_ping_callback(icmp4::Packet &ping_response)
	{
		...
		auto it = ping_callbacks_.find(std::make_pair(
			ping_response.id(), ping_response.sequence()));
	...
	void send_request(ip4::Addr dest_ip, ICMP_type type, ICMP_code code,
			  icmp_func callback = nullptr,
			  int sec_wait = SEC_WAIT_FOR_REPLY,
			  uint16_t sequence = 0);

	/** Send response without id and sequence number */
	void send_response(
		icmp4::Packet &req, ICMP_type type, ICMP_code code,
