...
namespace ip4
{
	 ...
	 */
		constexpr Cidr(const Addr address, const uint8_t mask) noexcept
			: from_{ calc_from(address, mask) },
			  to_{ calc_to(address, mask) }
		{
			Expects(mask <= 32);
		}
	 ...
	 *                    24 represents the netmask 255.255.255.0
	 */
		constexpr Cidr(const uint8_t p1, const uint8_t p2,
	 ...
	 */
	    ...
	    private:
		constexpr Addr calc_from(const Addr address,
					 const uint8_t mask) const noexcept
		{
			const uint32_t ip_mask = ntohl(
				(0xFFFFFFFFUL << (32 - mask)) & 0xFFFFFFFFUL);
			return { address.whole & ip_mask };
		}

		constexpr Addr calc_to(const Addr address,
				       const uint8_t mask) const noexcept
		{
			const uint32_t ip_mask = ntohl(
				(0xFFFFFFFFUL << (32 - mask)) & 0xFFFFFFFFUL);
		...
		uint8_t get_mask(const std::string &cidr) const
		{
			...
			if (UNLIKELY(found == std::string::npos)) {
				throw Invalid_cidr{ "Missing slash in " +
						    cidr };
			}

			if (UNLIKELY(found >= (cidr.length() - 1))) {
				throw Invalid_cidr{
					"Missing mask after slash in " + cidr
				};
			}
