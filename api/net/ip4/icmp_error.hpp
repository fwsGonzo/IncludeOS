...
class ICMP_error : public Error {
	...
	using ICMP_code =
		uint8_t; // Codes in icmp4_common.hpp in namespace icmp4::code
	...
	bool is_too_big() const noexcept
	{
		return icmp_type_ == ICMP_type::DEST_UNREACHABLE and
		       icmp_code_ == (ICMP_code)icmp4::code::Dest_unreachable::
	...
	uint16_t pmtu_{
		0
	}; // Is set if packet sent received an ICMP too big message
