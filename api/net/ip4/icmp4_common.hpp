...
namespace icmp4
{
	...
	enum class Type : uint8_t {
		...
		TIMESTAMP_REPLY = 14,
		NO_REPLY =
			100, // Custom: Type in ICMP_view if no ping reply received
		NO_ERROR = 200
	};
	...
	namespace code
	{
		...
		enum class Redirect : uint8_t { NET, HOST, TOS_NET, TOS_HOST };

		enum class Time_exceeded : uint8_t { TTL, FRAGMENT_REASSEMBLY };
	...
	get_code_string(Type type, uint8_t code)
	{
		switch (type) {
