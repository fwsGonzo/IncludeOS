...
namespace icmp4
{
	class Packet {
		...
		Header &header()
		{
			return *reinterpret_cast<Header *>(
				pckt_->layer_begin() + sizeof(IP4::header));
		}
	 ...
	 *  The payload of an ICMP error message packet contains the original packet sent that
	 * caused an ICMP error to occur (the original IP header and 8 bytes of the original
	 * packet's data)
	 */
		int payload_index()
		{
		...
		Span payload()
		{
			return { &(header().payload[0]),
				 pckt_->data_end() - &(header().payload[0]) };
		}
	 ...
	 */
		...
		{
			return { pckt_->layer_begin(),
				 pckt_->ip_header_length() + 8 };
		}
		...
		/**
	 * RFC 792 Parameter problem f.ex.: error (Pointer) is placed in the first byte after
	 * checksum (identifier and sequence is not used when pointer is used)
	 */
		...
		{
			return net::checksum(
				reinterpret_cast<const uint16_t *>(&header()),
				pckt_->data_end() -
		...
		{
			pckt_->set_data_end(pckt_->ip_header_length() +
					    header_size() + new_load.size());
			memcpy(payload().data(), new_load.data(),
			       payload().size());
		}
		...
		Packet(IP4::IP_packet_ptr pckt) : pckt_{ std::move(pckt) }
		{
		}
