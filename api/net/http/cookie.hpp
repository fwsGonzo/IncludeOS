...
class Cookie {
     ...
     *             is not required to retain the cookie until the specified date has passed. In
     * fact, user agents often evict cookies due to memory pressure or privacy concerns.
     *
     * @return     expires_
     */
	inline const std::string &get_expires() const noexcept
	{
		return expires_;
	}
     ...
     *             has precedence and controls the expiration date of the cookie. If a cookie has
     * neither the Max-Age nor the Expires attribute, the user agent will retain the cookie until
     *             "the current session is over" (as defined by the user agent).
     *
     * @return     max_age_
     */
	inline int get_max_age() const noexcept
	{
		return max_age_;
	}
	...
	/**
     * @brief      [RFC 6265] The Domain attribute specifies those hosts to which the cookie will be
     * sent. For example, if the value of the Domain attribute is "example.com", the user agent will
     * include the cookie in the Cookie header when making HTTP requests to example.com,
     *             www.example.com, and www.corp.example.com.
     *
     *             WARNING: Some existing user agents treat an absent Domain attribute as if the
     * Domain attribute were present and contained the current host name. For example, if
     * example.com returns a Set-Cookie header without a Domain attribute, these user agents will
     * erroneously send the cookie to www.example.com as well.
     *
     *             The user agent will reject cookies unless the Domain attribute specifies a scope
     * for the cookie that would include the origin server. For example, the user agent will accept
     * a cookie with a Domain attribute of "example.com" or of "foo.example.com" from
     * foo.example.com, but the user agent will not accept a cookie with a Domain attribute of
     * "bar.example.com" or of "baz.foo.example.com".
     *
     *             NOTE: For security reasons, many user agents are configured to reject Domain
     * attributes that correspond to "public suffixes". For example, some user agents will reject
     * Domain attributes of "com" or "co.uk".
     *
     * @return     domain_
     */
	inline const std::string &get_domain() const noexcept
	{
		return domain_;
	}
	...
	/**
     * @brief      [RFC 6265] The scope of each cookie is limited to a set of paths, controlled by
     * the Path attribute. If the server omits the Path attribute, the user agent will use the
     ...
     *
     *             Although seemingly useful for isolating cookies between different paths within a
     * given host, the Path attribute cannot be relied upon for security.
     *
     * @return     path_
     */
     ...
     *
     *             [RFC 6265] The Secure attribute limits the scope of the cookie to "secure"
     * channels (where "secure" is defined by the user agent). When a cookie has the Secure
     * attribute, the user agent will include the cookie in an HTTP request only if the request is
     ...
     * network attacker can overwrite Secure cookies from an insecure channel, disrupting their
     *             integrity.
     *
     * @return     true if Secure, false otherwise.
     */
	inline bool is_secure() const noexcept
	{
		return secure_;
	}

	inline void set_secure(bool secure) noexcept
	{
		secure_ = secure;
	}
     ...
     *
     *             [RFC 6265] The HttpOnly attribute limits the scope of the cookie to HTTP
     * requests. In particular, the attribute instructs the user agent to omit the cookie when
     * providing access to cookies via "non-HTTP" APIs (such as a web browser API that exposes
     * cookies to scripts).
     *
     ...
     */
	inline bool is_http_only() const noexcept
	{
		return http_only_;
	}

	inline void set_http_only(bool http_only) noexcept
	{
