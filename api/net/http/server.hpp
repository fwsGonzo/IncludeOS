...
class Server {
     ...
     * @param[in]  handler  The handler to be invoked when a request is received (optional in
     * constructor)
     * @param[in]  timeout  The duration for how long a connection can be idle (0 = no timeout)
     */
	explicit Server(TCP &tcp, Request_handler handler = nullptr,
			idle_duration timeout = DEFAULT_IDLE_TIMEOUT);
...
inline Server_ptr
make_server(Server::TCP &tcp, Server::Request_handler handler = nullptr,
	    Server::idle_duration timeout = Server::DEFAULT_IDLE_TIMEOUT)
{
