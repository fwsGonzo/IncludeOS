...
class Basic_client {
	...
	using Request_handler =
		delegate<void(Request &, Options &, const Host)>;
     ...
     * @param[in]  cb    Callback to be invoked when a response is received (or error)
     */
	void send(Request_ptr req, URI url, Response_handler cb,
		  Options options = {});
     ...
     * @param[in]  cb       Response handler
     */
	void request(Method method, Host host, std::string path,
	...
	void request(Method method, std::string url, Header_set hfields,
		     ...
		     Options options = {})
	{
		request(method, URI{ url }, std::move(hfields), std::move(data),
			std::move(cb), std::move(options));
	}
	...
	void request(Method method, Host host, std::string path,
		     ...
		     Response_handler cb, const bool secure = false,
		     Options options = {});
	...
	explicit Basic_client(TCP &tcp, Request_handler on_send,
	...
	void validate_secure(const bool secure) const
	{
		if (secure and not this->supports_https)
			throw Client_error{
				"Secured connections not supported with Basic_client (use the HTTPS Client)."
			};
	}
...
inline void Basic_client::get(URI url, Header_set hfields, Response_handler cb,
			      Options options)
{
	request(GET, std::move(url), std::move(hfields), std::move(cb),
		std::move(options));
}
...
inline void Basic_client::get(Host host, std::string path, Header_set hfields,
			      ...
			      Options options)
{
	request(GET, std::move(host), std::move(path), std::move(hfields),
		std::move(cb), secure, std::move(options));
}
...
inline void Basic_client::post(URI url, Header_set hfields, std::string data,
			       Response_handler cb, Options options)
{
	request(POST, std::move(url), std::move(hfields), std::move(data),
		std::move(cb), std::move(options));
}
...
{
	request(POST, std::move(host), std::move(path), std::move(hfields),
		std::move(data), std::move(cb), secure, std::move(options));
}
