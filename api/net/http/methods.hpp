...
namespace method
{
	...
	///
	template <typename = void> util::sview str(const Method m) noexcept
	{
		const static std::array<util::sview, 10> views{
			{ "GET", "POST", "PUT", "DELETE", "OPTIONS", "HEAD",
			  "TRACE", "CONNECT", "PATCH", "INVALID" }
		};
	...
	///
	template <typename = void> Method code(util::csview method) noexcept
	{
...
operator<<(std::basic_ostream<Char, Char_traits> &output_device, const Method m)
{
