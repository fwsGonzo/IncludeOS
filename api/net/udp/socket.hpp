...
class Socket {
	...
	using recvfrom_handler =
		delegate<void(addr_t, port_t, const char *, size_t)>;
	...
	void sendto(addr_t destIP, port_t port, const void *buffer,
		    size_t length, sendto_handler cb = nullptr,
		    error_handler ecb = nullptr);

	void bcast(addr_t srcIP, port_t port, const void *buffer, size_t length,
		   sendto_handler cb = nullptr, error_handler ecb = nullptr);
	...
	UDP &udp_;
	net::Socket socket_;
	recvfrom_handler on_read_handler = [](addr_t, port_t, const char *,
					      size_t) {};
