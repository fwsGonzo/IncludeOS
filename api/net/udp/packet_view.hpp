...
template <typename Ptr_type> class Packet_v {
	...
	{
		udp_header().sport = htons(p);
		return *this;
	}

	Packet_v &set_dst_port(port_t p) noexcept
	{
		udp_header().dport = htons(p);
		return *this;
	}
	...
	uint16_t udp_data_length() const noexcept
	{
		...
		uint16_t final_length =
			std::min(real_length, ntohs(udp_header().length));
	...
	Packet_v(Ptr_type ptr) : pkt{ std::move(ptr) }
	{
		Expects(pkt != nullptr);
	}
	...
	{
		Expects(hdr != nullptr);
		header = reinterpret_cast<Header *>(hdr);
	}
	...
	void set_length(uint16_t newlen = 0)
	{
		...
		pkt->set_data_end(ip_header_length() + udp_header_length() +
				  newlen);
	}
