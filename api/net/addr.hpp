...
union Addr {
	...
	constexpr Addr() noexcept : ip6_{}
	{
	}

	Addr(ip4::Addr addr) noexcept : ip4_{ 0, ip4_sign_be, std::move(addr) }
	{
	}

	Addr(ip6::Addr addr) noexcept : ip6_{ std::move(addr) }
	{
	}
	...
	// IP6 variant
	Addr(uint16_t a, uint16_t b, uint16_t c, uint16_t d, uint16_t e,
	...
	{
		return ip6_ == ip6::Addr::addr_any or
		       (is_v4() and ip4_.addr == 0);
	}

	Addr any_addr() const noexcept
	{
		return is_v4() ? Addr{ ip4::Addr::addr_any } :
				 Addr{ ip6::Addr::addr_any };
	}
	...
	Addr(const Addr &other) noexcept : ip6_{ other.ip6_ }
	{
	}

	Addr(Addr &&other) noexcept : ip6_{ std::move(other.ip6_) }
	{
	}
