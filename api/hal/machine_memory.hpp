 ...
 **/
template <typename T> struct Machine::Allocator {
	using value_type = T;

	Allocator() : resource{ os::machine().memory() }
	{
	}

	Allocator(os::Machine::Memory &mem) : resource{ mem }
	{
	}
	...
	T *allocate(std::size_t size)
	{
		auto res = reinterpret_cast<T *>(
			resource.allocate(size * sizeof(T)));
		if (res == nullptr)
			throw std::bad_alloc();
		return res;
	}
	...
	std::unique_ptr<U> make_unique(Args &&... args)
	{
		...
		auto deleter = [this](auto *ptr) {
			deallocate(ptr, sizeof(U));
		};
		return std::unique_ptr<U>(
			new (addr) U(std::forward<Args>(args)...), deleter);
	};
