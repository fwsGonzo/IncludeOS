...
class Machine {
	...
	template <typename T> using Allocator = os::Machine::Allocator<T>;
	...
	struct Part {
		enum class Storage : uint8_t { machine, heap };
	...
	using Device_types =
		std::set<std::type_index, std::less<std::type_index>,
	...
	template <typename T> Parts_vec &get_vector()
	{
		const std::type_index t_idx = std::type_index(typeid(T));
		auto vec_it = parts_.find(t_idx);
		if (vec_it == parts_.end()) {
			throw Machine_access_error(
				"Requested machine parts not found");
		}
	...
	template <typename T> os::Machine::Vector<T> get()
	{
		auto &vec = get_vector<T>();
	...
	template <typename T> ssize_t add(T *part, Part::Storage storage)
	{
		const std::type_index t_idx = std::type_index(typeid(T));
		auto vec_it = parts_.find(t_idx);
		if (vec_it == parts_.end()) {
		...
		if
			constexpr(std::is_base_of<hw::Device, T>::value)
				add_device_type(t_idx);

		return vec.size() - 1;
	}

	template <typename T, typename... Args> ssize_t add_new(Args &&... args)
	{
	...
	template <typename T> ssize_t count()
	{
	...
	template <typename T> void remove(int i)
	{
		...
		if (UNLIKELY(vec.size() < i))
			throw Machine_access_error{
				"Requested machine part not found: " +
				std::to_string(i)
			};
...
namespace os
{
template <typename T> ssize_t Machine::add(std::unique_ptr<T> part) noexcept
{
	try {
		return impl->add<T>(part.release(),
				    detail::Machine::Part::Storage::heap);
...
template <typename T> T &Machine::get(int id)
{
	return impl->get<T>(id);
}

template <typename T> Machine::Vector<T> Machine::get()
{
	return impl->get<T>();
}

template <typename T> void Machine::remove(int i)
{
	impl->remove<T>(i);
};

template <typename T> ssize_t Machine::count()
{
	return impl->count<T>();
}
...
inline void Machine::print_devices() const
{
	...
	for (const auto idx : device_types_) {
		...
		for (const auto &part : parts_.at(idx)) {
			const auto &dev =
				*reinterpret_cast<const hw::Device *>(part.ptr);
