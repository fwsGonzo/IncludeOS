...
class Fiber {
	...
	Fiber(int stack_size, R (*func)(P), void *arg)
		: id_{ next_id_++ }, stack_size_{ stack_size },
		  stack_{ Stack_ptr(new char[16 + stack_size_],
				    std::default_delete<char[]>()) },
		  stack_loc_{ (void *)(uintptr_t(stack_.get() + stack_size_) &
				       ~(uintptr_t)0xf) },
		  type_return_{ typeid(R) }, type_param_{ typeid(P) },
		  func_{ reinterpret_cast<init_func>(func) }, param_{ arg }
	{
	}

	template <typename R, typename P>
	Fiber(R (*func)(P), void *arg) : Fiber(default_stack_size, func, arg)
	{
	}

	template <typename R, typename P>
	Fiber(R (*func)(P)) : Fiber(default_stack_size, func, nullptr)
	{
	}

	Fiber() : Fiber(init_func{ nullptr })
	{
	}
	...
	Fiber(int stack_size, void (*func)())
		: id_{ next_id_++ }, stack_size_{ stack_size },
		  ...
		  stack_loc_{ (void *)(uintptr_t(stack_.get() + stack_size_) &
				       ~(uintptr_t)0xf) },
		  type_return_{ typeid(void) }, type_param_{ typeid(void) },
		  func_{ reinterpret_cast<init_func>(func) }
	{
	}

	Fiber(void (*func)()) : Fiber(default_stack_size, func)
	{
	}
	...
	Fiber(int stack_size, void (*func)(P), P par)
		: id_{ next_id_++ }, stack_size_{ stack_size },
		  ...
		  stack_loc_{ (void *)(uintptr_t(stack_.get() + stack_size_) &
				       ~(uintptr_t)0xf) },
		  type_return_{ typeid(void) }, type_param_{ typeid(P) },
		  func_{ reinterpret_cast<init_func>(func) }, param_{
	...
	template <typename P>
	Fiber(void (*func)(P), P par) : Fiber(default_stack_size, func, par)
	{
	}
	...
	Fiber(int stack_size, R (*func)())
		: id_{ next_id_++ }, stack_size_{ stack_size },
		  ...
		  stack_loc_{ (void *)(uintptr_t(stack_.get() + stack_size_) &
				       ~(uintptr_t)0xf) },
		  type_return_{ typeid(R) }, type_param_{ typeid(void) }, func_{
			  reinterpret_cast<init_func>(func)
		  }
	{
		static_assert(sizeof(R) <= sizeof(void *),
			      "Invalid return value size");
	}

	template <typename R>
	Fiber(R (*func)()) : Fiber(default_stack_size, func)
	{
	}
	...
	template <typename R> R ret()
	{
		...
		if (typeid(R) != type_return_)
			throw Err_bad_cast(
				"Invalid return type for this funcion");
