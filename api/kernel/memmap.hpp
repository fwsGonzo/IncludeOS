...
class Fixed_memory_range {
     ...
     */
	Fixed_memory_range(const uintptr_t begin, const uintptr_t end,
			   const char *name, In_use_delg in_use_operation);
     ...
     */
	static constexpr bool is_valid_range(const uintptr_t begin,
					     const uintptr_t end) noexcept
	{
		return (begin < end) or ((end - begin + 1) <=
					 static_cast<uintptr_t>(max_size()));
	}
...
class Memory_map {
     ...
     */
	Fixed_memory_range &
	assign_range(const Fixed_memory_range::size_type size);
