...
class PCI_Device { // public Device //Why not? A PCI device is too general to be accessible?
     ...
     */
	explicit PCI_Device(const uint16_t pci_addr, const uint32_t,
			    const uint32_t);
	...
	static uint32_t read_dword(const uint16_t pci_addr,
...
static const char *PCI::vendor_str(uint16_t code)
{
	const std::unordered_map<uint16_t, const char *> classcodes{
		{ VENDOR_AMD, "AMD" },	       { VENDOR_INTEL, "Intel" },
		{ VENDOR_CIRRUS, "Cirrus" },   { VENDOR_VIRTIO, "VirtIO" },
		{ VENDOR_REALTEK, "REALTEK" }, { VENDOR_VMWARE, "VMWare" },
		{ VENDOR_QEMU, "QEMU" }
	};
