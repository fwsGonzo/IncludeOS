...
union Addr {
	...
	constexpr Addr(const uint8_t a, const uint8_t b, const uint8_t c,
		       ...
		       const uint8_t f) noexcept : part{ a, b, c, d, e, f }
	{
	}
	...
	Addr(const std::string &smac) noexcept : Addr(smac.c_str())
	{
	}

	Addr(const char *smac) noexcept
	{
		uint8_t macaddr[PARTS_LEN] = { 0xff, 0xff, 0xff,
					       0xff, 0xff, 0xff };
	...
	std::string hex_str() const
	{
		...
		snprintf(hex_addr, sizeof(hex_addr),
			 "%02x:%02x:%02x:%02x:%02x:%02x", part[0], part[1],
	...
	{
		return (minor == other.minor) and (major == other.major);
	}
	...
	{
		std::array<uint8_t, 8> eui{ addr.part[0], addr.part[1],
					    addr.part[2], 0xFF,
					    0xFE,	  addr.part[3],
					    addr.part[4], addr.part[5] };
		eui[0] ^= (1UL << 1);
     ...
     */
	template <typename IPv6> static Addr ipv6_mcast(const IPv6 &v6)
	{
		return { 0x33,
			 0x33,
	...
	static constexpr const size_t PARTS_LEN{
		6
	}; //< Number of parts in a MAC address
...
constexpr const Addr BROADCAST{
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff
}; //< uint16_t(0xFFFF), uint32_t(0xFFFFFFFF)
constexpr const Addr MULTICAST{
	0x00, 0x00, 0x01, 0x00, 0x00, 0x00
}; //< uint16_t(0x0000), uint32_t(0x01000000)
constexpr const Addr IPv6mcast_01{
	0x33, 0x33, 0x01, 0x00, 0x00, 0x00
}; //< uint16_t(0x3333), uint32_t(0x01000000)
constexpr const Addr IPv6mcast_02{
	0x33, 0x33, 0x02, 0x00, 0x00, 0x00
