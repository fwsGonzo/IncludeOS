...
class RingBuffer {
	...
	{
		return (used <= cap and cap > 0) and
		       (start < cap and start >= 0) and
		       (end < cap and end >= 0);
	}
    ...
    protected:
	RingBuffer(void *rbuffer, int size) : cap(size), buffer((char *)rbuffer)
	{
		assert(size > 0);
	}
...
class HeapRingBuffer : public RingBuffer {
    public:
	HeapRingBuffer(int size) : RingBuffer(new char[size], size)
	{
	}
	~HeapRingBuffer()
	{
...
class MemoryRingBuffer : public RingBuffer {
    public:
	MemoryRingBuffer(void *location, int size) : RingBuffer(location, size)
	{
	}

	MemoryRingBuffer(void *loc, const int cap, int start, int end, int used)
		: RingBuffer(loc, cap)
	{
		this->start = start;
		this->end = end;
...
template <int N> class FixedRingBuffer : public RingBuffer {
    public:
	FixedRingBuffer() : RingBuffer(m_buffer, N)
	{
	}
	~FixedRingBuffer()
	{
