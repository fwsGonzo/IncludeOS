...
namespace alloc
{
	...
	enum class Lstack_opt : uint8_t { merge, no_merge };

	template <uintptr_t Min = 4096> struct Node {
		Node *next = nullptr;
		size_t size = 0;

		Node(Node *n, size_t s) : next(n), size(s)
		{
			Expects(s >= Min);
		}
	...
	class Lstack {
		...
		static constexpr bool merge_on_dealloc =
			Opt == Lstack_opt::merge;
		...
		static_assert(Min >= sizeof(Node),
			      "Requires Min. size >= node size");
		...
		Allocation allocate_front(size_t size) noexcept
		{
			return impl.allocate_front(size);
		}

		/** Allocate size from as high an address as possible */
		Allocation allocate_back(size_t size) noexcept
		{
			return impl.allocate_back(size);
		}

		/** Allocate the largest contiguous chunk of memory */
		Allocation allocate_largest() noexcept
		{
			return impl.allocate_largest();
		}

		/** Deallocate **/
		void deallocate(void *ptr, size_t size)
			noexcept(os::hard_noexcept)
		{
			deallocate({ ptr, size });
		}
	 ...
	 **/
		...
		auto make_unique(Args &&... args)
		{
			...
			auto deleter = [this](auto *ptr) {
				this->deallocate(ptr, sizeof(U));
			};
			return std::unique_ptr<U, decltype(deleter)>(
	...
	namespace detail
	{
		...
		template <Lstack_opt Opt, size_t Min> struct Lstack {
			// Avoid fragmentation at the cost of speed. Implies sort by addr.
			static constexpr bool merge_on_dealloc =
				Opt == Lstack_opt::merge;
			static constexpr bool is_sorted = merge_on_dealloc;
			...
			Allocation allocate_front(size_t size)
			{
				...
				if (a.ptr != nullptr) {
					...
					Ensures(bits::is_aligned(
						align, (uintptr_t)a.ptr));
					Ensures(bits::is_aligned(align,
								 a.size));
			...
			Allocation allocate_back(size_t size)
			{
				...
				Allocation chunk{
					(void *)((uintptr_t)hi_fit->end() -
						 size),
					size
				};

				Expects((char *)chunk.ptr >=
					(char *)hi_fit + size);
				hi_fit->size -= size;
				bytes_allocated_ += size;
				...
				noexcept(os::hard_noexcept)
			{
				...
				Expects(bits::is_aligned(Min,
							 (uintptr_t)a.ptr));
				Expects((uintptr_t)a.ptr >= donations_begin_);
				Expects((uintptr_t)a.ptr <=
					donations_end_ - min_alloc);
			...
			void donate(void *ptr, size_t size)
			{
				Expects(bits::is_aligned(align,
							 (uintptr_t)ptr));
				Expects(bits::is_aligned(align, size));

				push(ptr, size);
				bytes_total_ += size;
				if ((uintptr_t)ptr < donations_begin_ or
				    donations_begin_ == 0)
				...
				Ensures(pool_end() - pool_begin() >=
					pool_size());
			}

			Lstack(void *ptr, size_t size)
				: front_{ nullptr }, bytes_allocated_{ 0 },
				  bytes_total_{ 0 }, donations_begin_{ 0 },
				  donations_end_{ 0 }
			{
				donate(ptr, size);
			}
			...
			bool empty() const noexcept
			{
				return front_ == nullptr || front_->size == 0;
			}
		...
		*/
				auto *highest_free =
					find_prior((void *)(donations_end_));
				Expects((uintptr_t)highest_free->end() <=
					donations_end_);
				if ((uintptr_t)highest_free->end() ==
				    donations_end_)
			...
			Node *new_node(void *addr_begin, Node *next, size_t sz)
			{
				...
				Expects(((uintptr_t)addr_begin & (align - 1)) ==
					0);
			...
			void push_front(void *ptr, size_t size)
			{
				...
				Expects(size > 0);
				if
					constexpr(merge_on_dealloc)
					{
						if (front_ != nullptr) {
							Expects(ptr < front_);
							Expects((uintptr_t)ptr +
									size <=
								(uintptr_t)
									front_);
							if ((std::byte *)ptr +
			...
			{
				static_assert(merge_on_dealloc,
					      "Merge not enabled");
				Expects(prior);
				...
				Expects((uintptr_t)ptr + size <=
						(uintptr_t)next or
					next == nullptr);

				if ((uintptr_t)ptr ==
				    (uintptr_t)prior + prior->size) {
					// New node starts exactly at prior end, so merge
					// [prior] => [prior + size ]
					if (next != nullptr)
						Expects((char *)prior + size <=
							(char *)prior->next);
					prior->size += size;
				} else {
				...
				Expects((uintptr_t)prior + prior->size <=
					(uintptr_t)prior->next);

				if ((uintptr_t)prior + prior->size ==
				    (uintptr_t)prior->next) {
