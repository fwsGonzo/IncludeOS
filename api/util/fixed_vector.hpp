...
enum class Fixedvector_Init { UNINIT };

template <typename T, int N> class Fixed_vector {
    public:
	using value_type = T;
	using iterator = T *;
	...
	Fixed_vector(std::initializer_list<T> l) : count(l.size())
	{
	...
	{
		assert(begin() <= pos and pos <= end() &&
		       "pos do not belong to this vector");
		auto len = std::distance(first, last);
		assert((pos + len) <= (end() + remaining()) &&
	...
	template <typename R> bool operator==(const R &rhs) const noexcept
	{
		return size() == rhs.size() and
		       std::memcmp(data(), rhs.data(), size() * sizeof(T)) == 0;
	}
