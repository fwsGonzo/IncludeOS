...
namespace util
{
template <> struct enable_bitmask_ops<uint8_t> {
	using type = uint8_t;
	static constexpr bool enable = true;
};

template <> struct enable_bitmask_ops<os::mem::buddy::Flags> {
	using type = uint8_t;
	static constexpr bool enable = true;
};
} // namespace util
...
struct Alloc : public std::pmr::memory_resource {
	...
	static constexpr Size_t overhead(Size_t pool_size)
	{
		...
		auto overhead_ =
			bits::roundto(min_size, sizeof(Alloc) + nodes_size);
		return overhead_;
	}
     ...
     **/
	enum class Policy { underbook, overbook };
     ...
     **/
	template <Policy P> static Size_t pool_size(Size_t bufsize)
	{
		using namespace util;
		...
		pool_sz =
			bits::keeplast(bufsize - 1); // -1 starts the recursion
	...
	{
		using namespace util;
		return bits::roundto(min_size, pool_size) + overhead(pool_size);
	}

	/** Minimum total bufsize to manage pool_size memory **/
	template <Policy P> static Size_t min_bufsize(Size_t pool_size)
	{
		using namespace util;
	...
	static_assert(util::bits::is_pow2(min_size),
		      "Page size must be power of 2");

	Alloc(void *start, Size_t bufsize, Size_t pool_size)
		: nodes_{ (Node_t *)start, node_count(pool_size) },
		  start_addr_{ util::bits::roundto(
			  min_size, (Addr_t)start + node_count(pool_size)) },
		  addr_limit_{ reinterpret_cast<uintptr_t>(start) + bufsize },
...

	{
		...
		// Initialize nodes
		memset(nodes_.data(), 0,
		       nodes_.size() * sizeof(Node_arr::element_type));
	}
	...
	static Alloc *create(void *addr, Size_t bufsize)
	{
		...
		auto *alloc = new (addr)
			Alloc(alloc_begin, bufsize - sizeof(Alloc), pool_size_);
		return alloc;
	}
	...
	enum class Track { get = 0, inc = 1, start = 2 };
	...
	class Node_view {
		...
		{
			auto fl = data();
			return fl & Flags::taken or (fl & Flags::left_full and
						     fl & Flags::right_full);
		}
		...
		bool is_full_r() const noexcept
		{
			if (is_taken())
				return true;
			return is_parent() and left().is_full_r() and
			       is_parent() and right().is_full_r();
		}
		...
		Addr_t deallocate(Addr_t ptr, Size_t sz = 0)
		{
			...
			Expects(not(is_taken() and
				    (data() & Flags::left_used or
				     data() & Flags::right_used)));
	...
	std::string summary() const
	{
		...
		out << "Bytes used: " << util::Byte_r(bytes_used())
		    ...
		    << "Address pool: 0x" << std::hex << root().addr() << " - "
		    << root().addr() + pool_size_ << std::dec << " ( "
		    << util::Byte_r(pool_size()) << " ) \n";
		auto track = alloc_tracker();
		out << "Allocations:  " << track.allocs
		    << " Steps: Last: " << track.last << " Min:  " << track.min
		    << " Max:  " << track.max << " Total: " << track.total
	...
	std::string draw_tree(bool index = false) const
	{
		...
		for (int h = 0; h < tree_height(); h++) {
			...
			while (cnt--) {
				printf("[%s] ",
				       Node_view(i, this).to_string().c_str());
				i++;
			}
			printf("\n");
		}
