...
struct Tar_header {
	...
	char checksum[LENGTH_CHECKSUM]; // Checksum for header record (6 digit octal number with leading
		// zeroes)
...
class Element {
    public:
	Element(Tar_header &header) : header_{ header }
	{
	}
	...
	const Tar_header &header() const noexcept
	{
		return header_;
	}
	void set_header(const Tar_header &header)
	{
		header_ = header;
	}
	...
	void set_content_start(const uint8_t *content_start)
	{
	...
	{
		return std::string{ header_.mode };
	}
	std::string uid() const
	{
		return std::string{ header_.uid };
	}
	std::string gid() const
	{
		return std::string{ header_.gid };
	}
	long int size() const noexcept;
	std::string mod_time() const
	{
		return std::string{ header_.mod_time, LENGTH_MTIME };
	}
	std::string checksum() const
	{
		return std::string{ header_.checksum };
	}
	char typeflag() const noexcept
	{
		return header_.typeflag;
	}
	std::string linkname() const
	{
		return std::string{ header_.linkname };
	}
	std::string magic() const
	{
		return std::string{ header_.magic };
	}
	std::string version() const
	{
		return std::string{ header_.version, LENGTH_VERSION };
	}
	std::string uname() const
	{
		return std::string{ header_.uname };
	}
	std::string gname() const
	{
		return std::string{ header_.gname };
	}
	std::string devmajor() const
	{
		return std::string{ header_.devmajor };
	}
	std::string devminor() const
	{
		return std::string{ header_.devminor };
	}
	std::string prefix() const
	{
		return std::string{ header_.prefix };
	}
	std::string pad() const
	{
		return std::string{ header_.pad };
	}

	bool is_ustar() const noexcept
	{
		return (strncmp(header_.magic, TMAGIC, TMAGLEN) == 0);
	}
	bool is_dir() const noexcept
	{
		return header_.typeflag == DIRTYPE;
	}
	bool typeflag_is_set() const noexcept
	{
		return header_.typeflag not_eq ' ';
	}
...
class Reader {
    ...
    */
	static Tar_data decompress(const uint8_t *data, const size_t size,
				   unsigned int dlen = 0);
	...
	static Tar_data decompress(const Element &element)
	{
		return decompress(element.content(), element.size(),
				  decompressed_length(element.content(),
						      element.size()));
	}
	...
	static unsigned int decompressed_length(const uint8_t *data,
						const size_t size);
