...
{
class Pmr_pool : public std::enable_shared_from_this<Pmr_pool>,
		 public std::pmr::memory_resource {
    public:
	using Resource = os::mem::Pmr_pool::Resource;
	using Resource_ptr = os::mem::Pmr_pool::Resource_ptr;
	Pmr_pool(std::size_t total_max, std::size_t suballoc_max,
		 std::size_t max_allocs)
		: cap_total_{ total_max }, cap_suballoc_{ suballoc_max },
		  max_resources_{ max_allocs }
	{
	}
	...
	Resource_ptr get_resource()
	{
		if (!free_resources_.empty()) {
			...
			if (UNLIKELY(!res->empty() and
				     resource_count() < max_resources_)) {
				return new_resource();
			}
	...
	std::size_t resource_capacity()
	{
		if (cap_suballoc_ == 0) {
			auto div =
				cap_total_ /
				(used_resources_ +
				 os::mem::Pmr_pool::resource_division_offset);
...
std::size_t Pmr_pool::resource_capacity()
{
	return impl->resource_capacity();
}
std::size_t Pmr_pool::allocatable()
{
	return impl->allocatable();
}
std::size_t Pmr_pool::allocated()
{
	return impl->allocated();
}

void Pmr_pool::set_resource_capacity(std::size_t s)
{
	impl->set_resource_capacity(s);
}
void Pmr_pool::set_total_capacity(std::size_t s)
{
	impl->set_total_capacity(s);
};
...
Pmr_pool::Resource_ptr Pmr_pool::get_resource()
{
	return impl->get_resource();
}
std::size_t Pmr_pool::resource_count()
{
	return impl->resource_count();
}
std::size_t Pmr_pool::alloc_count()
{
	return impl->alloc_count();
}
std::size_t Pmr_pool::dealloc_count()
{
	return impl->dealloc_count();
}
void Pmr_pool::return_resource(Resource *res)
{
	impl->return_resource(res);
}
bool Pmr_pool::full()
{
	return impl->full();
...
void Pmr_resource::do_deallocate(void *ptr, std::size_t s, std::size_t a)
{
	...
	bool trigger_avail_thresh = UNLIKELY(
		allocatable() < avail_thresh and
		allocatable() + s >= avail_thresh and avail != nullptr);
...
bool Pmr_resource::do_is_equal(
	const std::pmr::memory_resource &other) const noexcept
{
	if (const auto *other_ptr =
		    dynamic_cast<const Pmr_resource *>(&other)) {
