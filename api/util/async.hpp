...
class Async {
	...
	static void upload_file(Disk, const Dirent &, Stream *, on_after_func,
				size_t = PAYLOAD_SIZE);

	static void disk_transfer(Disk, const Dirent &, on_write_func,
				  on_after_func, size_t = PAYLOAD_SIZE);
};
