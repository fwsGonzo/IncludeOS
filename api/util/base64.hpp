...
R encode(const char *data, const size_t length,
	 const url_alphabet url_alphabet_switch = url_alphabet{ false })
{
	if (data == nullptr)
		return R{};

	const char *const alphabet =
		url_alphabet_switch ? base64url_alphabet() : base64_alphabet();
	...
	for (size_t i = 0; i < length; i += 3) {
		...
		if ((i + 1) < length) {
			buffer.push_back(encode_unit(
				unit | ((data[i + 1] & 0xF0) >> 4)));
			unit = (data[i + 1] & 0x0F) << 2;
			if ((i + 2) < length) {
				buffer.push_back(encode_unit(
					unit | ((data[i + 2] & 0xC0) >> 6)));
				buffer.push_back(
					encode_unit(data[i + 2] & 0x3F));
			} else {
 ...
 */
template <typename R = std::string>
inline R encode(const char *data,
		const url_alphabet url_alphabet_switch = url_alphabet{ false })
{
...
R decode(const char *data, size_t length,
	 const url_alphabet url_alphabet_switch = url_alphabet{ false })
{
	if (data == nullptr)
		return R{};
	...
	const char *const alphabet =
		url_alphabet_switch ? base64url_alphabet() : base64_alphabet();
	auto fetch_unit = [data, alphabet](const size_t i)
				  noexcept -> std::array<char, 4> {
		std::array<char, 4> unit;

		unit[0] = std::distance(alphabet,
					std::find(alphabet, (alphabet + 64),
						  data[i]));
		unit[1] = std::distance(alphabet,
					std::find(alphabet, (alphabet + 64),
						  data[i + 1]));
		unit[2] = std::distance(alphabet,
					std::find(alphabet, (alphabet + 64),
						  data[i + 2]));
		unit[3] = std::distance(alphabet,
					std::find(alphabet, (alphabet + 64),
						  data[i + 3]));
 ...
 */
template <typename R = std::vector<char> >
inline R decode(const char *data,
		const url_alphabet url_alphabet_switch = url_alphabet{ false })
{
	const auto length = std::strlen(data);
