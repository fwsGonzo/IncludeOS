...
class Fixed_list_alloc {
    ...
    public:
	Fixed_list_alloc() : store_{ nullptr }
	{
	}

	template <typename U> Fixed_list_alloc(const U &) : store_{ nullptr }
	{
	...
	Fixed_list_alloc(const Fixed_list_alloc &) : store_{ nullptr }
	{
	...
	static Fixed_list_alloc
	select_on_container_copy_construction(const Fixed_list_alloc &)
	{
		return {};
	}

	template <typename U> using other = Fixed_list_alloc<U, N, alignment>;
	...
	T *allocate(std::size_t n)
	{
		...
		return reinterpret_cast<T *>(
			store_->template allocate<alignof(T)>(n * sizeof(T)));
	}
	...
	template <class T1, std::size_t N1, std::size_t A1, class U,
		  std::size_t M, std::size_t A2>
