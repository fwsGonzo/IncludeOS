...
class Stat {
	...
	enum Stat_type : uint8_t { FLOAT, UINT32, UINT64 };
	...
	{
		return m_bits & PERSIST_BIT;
	}
	void make_persistent() noexcept
	{
		m_bits |= PERSIST_BIT;
	}
	...
	{
		return (m_bits & GAUGE_BIT) == 0;
	}
	bool is_gauge() const noexcept
	{
		return (m_bits & GAUGE_BIT) == GAUGE_BIT;
	}

	void make_counter() noexcept
	{
		m_bits &= ~GAUGE_BIT;
	}
	void make_gauge() noexcept
	{
		m_bits |= GAUGE_BIT;
	}
...
class Statman {
	...
	Stat &get_or_create(const Stat::Stat_type type,
...
{
	if (UNLIKELY(type() != FLOAT))
		throw Stats_exception{ "Stat type is not a float" };
	return f;
}
inline uint32_t &Stat::get_uint32()
{
	if (UNLIKELY(type() != UINT32))
		throw Stats_exception{ "Stat type is not an uint32" };
	return ui32;
}
inline uint64_t &Stat::get_uint64()
{
	if (UNLIKELY(type() != UINT64))
		throw Stats_exception{ "Stat type is not an uint64" };
	return ui64;
}
