...
inline namespace bitops
{
	...
	template <typename E> struct enable_bitmask_ops {
		...
		operator|(E lhs, F rhs)
	{
		...
		return static_cast<E>(static_cast<type>(lhs) |
	...
	{
		using type = typename enable_bitmask_ops<F>::type;
		lhs = static_cast<E>(static_cast<type>(lhs) |
				     static_cast<type>(rhs));
		return lhs;
	}
	...
	template <typename E>
	constexpr
		typename std::enable_if<enable_bitmask_ops<E>::enable, bool>::type
		has_flag(E flag)
	{
	...
	template <> struct enable_bitmask_ops<uintptr_t> {
...
namespace bits
{
	...
	template <uintptr_t M> inline uintptr_t multip(uintptr_t x)
	{
		return (M - 1 + x) / M;
	}
	...
	template <uintptr_t M> inline uintptr_t roundto(uintptr_t x)
	{
		return multip<M>(x) * M;
	}
	...
	template <uintptr_t A> bool is_aligned(uintptr_t ptr) noexcept
	{
		return (ptr & (A - 1)) == 0;
	}

	template <uintptr_t A> bool is_aligned(void *ptr) noexcept
	{
