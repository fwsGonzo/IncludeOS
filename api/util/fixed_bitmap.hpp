 ...
 */
template <size_t N> class Fixed_bitmap : public MemBitmap {
    public:
	using Storage = MemBitmap::word;
	static_assert(N >= sizeof(Storage),
		      "Number of bits need to be atleast sizeof(Storage)");
	static_assert(N % sizeof(Storage) == 0,
		      "Number of bits need to be divisable by sizeof(Storage)");

    public:
	Fixed_bitmap() : MemBitmap{}, storage{}
	{
