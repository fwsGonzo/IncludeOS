...
class Syslog_facility {
	...
	Syslog_facility(const char *ident, int facility)
	...
	inline int calculate_pri()
	{
		return (facility_ * MUL_VAL) + priority_;
	}
	inline bool ident_is_set() const noexcept
	{
		return (ident_ not_eq nullptr);
	}
	inline void set_ident(const char *ident)
	{
		ident_ = ident;
	}
	inline const char *ident() const noexcept
	{
		return ident_;
	}
	inline void set_priority(int priority) noexcept
	{
		priority_ = priority;
	}
	...
	inline void set_logopt(int logopt) noexcept
	{
		logopt_ = logopt;
	}
	inline int logopt() const noexcept
	{
		return logopt_;
	}
	inline void set_facility(int facility) noexcept
	{
		facility_ = facility;
	}
	inline int facility() const noexcept
	{
		return facility_;
	}
...
class Syslog_udp : public Syslog_facility {
	...
	Syslog_udp(const char *ident, int facility)
		: Syslog_facility(ident, facility)
	{
	}
...
class Syslog_print : public Syslog_facility {
	...
	const net::Addr &ip() const noexcept override
	{
		return net::Addr::addr_any;
	}
	...
	std::string
	build_message_prefix(const std::string &binary_name) override;

	Syslog_print() : Syslog_facility()
	{
	}
	Syslog_print(const char *ident, int facility)
		: Syslog_facility(ident, facility)
	{
	}
	~Syslog_print()
