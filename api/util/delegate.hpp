...
using closure_decay =
	std::conditional<std::is_convertible<T, R (*)(Args...)>::value,
...
template <typename... Ts> using pack_first_t = typename pack_first<Ts...>::type;
...
template <size_t, size_t, typename R, typename... Args> class pure {
	...
	{
		static_assert(
			std::is_convertible<T, invoke_ptr_t>::value,
...
class inplace_triv {
			  ...
			  }) }
	{
		...
		static_assert(
			std::alignment_of<C>::value <= align,
			"inplace_triv delegate closure alignment too large");

		static_assert(
			std::is_trivially_copyable<C>::value,
...
class inplace {
	...
	inplace(const inplace &other)
		: invoke_ptr_{ other.invoke_ptr_ }, copy_ptr_{ other.copy_ptr_ },
...
class delegate<R(Args...), Spec, size, align> {
	...
	delegate(C *const object_ptr,
		 R (C::*const method_ptr)(Args...)) noexcept
		: delegate([object_ptr, method_ptr](Args &&... args) -> R {
			return (object_ptr->*method_ptr)(
				std::forward<Args>(args)...);
		})
	{
	}
	...
	template <typename C>
	delegate(C *const object_ptr,
		 R (C::*const method_ptr)(Args...) const) noexcept
		: delegate([object_ptr, method_ptr](Args &&... args) -> R {
			return (object_ptr->*method_ptr)(
				std::forward<Args>(args)...);
		})
	...
	delegate(C &object_ref, R (C::*const method_ptr)(Args...)) noexcept
		: delegate([&object_ref, method_ptr](Args &&... args) -> R {
			return (object_ref.*
				method_ptr)(std::forward<Args>(args)...);
		})
	{
	}

	template <typename C>
	delegate(C &object_ref,
		 R (C::*const method_ptr)(Args...) const) noexcept
		: delegate([&object_ref, method_ptr](Args &&... args) -> R {
			return (object_ref.*
				method_ptr)(std::forward<Args>(args)...);
	...
	template <typename C, typename... MemArgs,
		  typename std::enable_if_t<
			  std::is_same<detail::pack_first_t<Args...>, C *>::value,
			  int> = 0>
	delegate(R (C::*const method_ptr)(MemArgs...)) noexcept
		: delegate([method_ptr](C *object_ptr, MemArgs... args) -> R {
			return (object_ptr->*method_ptr)(
				std::forward<MemArgs>(args)...);
		})
	{
	}
	...
	delegate(R (C::*const method_ptr)(MemArgs...)) noexcept
		: delegate([method_ptr](C &object, MemArgs... args) -> R {
			return (object.*
				method_ptr)(std::forward<MemArgs>(args)...);
		})
	{
	}
