...
class MemBitmap {
	...
	MemBitmap() = default;
	MemBitmap(void *location, index_t chunks) : _chunks(chunks)
	{
		_data = (word *)location;
	}
    ...
    private:
	index_t windex(index_t b) const
	{
		return b / CHUNK_SIZE;
	}
	index_t woffset(index_t b) const
	{
		return b & (CHUNK_SIZE - 1);
	}
	index_t bit(index_t b) const
	{
		return 1 << woffset(b);
	}
