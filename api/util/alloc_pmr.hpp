...
class Pmr_pool {
	...
	using Resource_ptr =
		std::unique_ptr<Pmr_resource, delegate<void(Resource *)> >;

	inline Pmr_pool(size_t capacity, size_t cap_suballoc = 0,
...
class Pmr_resource : public std::pmr::memory_resource {
	...
	void on_non_full(Event e)
	{
		non_full = e;
	}

	/** Fires on transition from < N bytes to >= N bytes allocatable **/
	void on_avail(std::size_t N, Event e)
	{
		avail_thresh = N;
		avail = e;
	}
