...
namespace util
{
template <uint64_t POLY> struct crc64 {
	...
	static constexpr uint64_t checksum(uint64_t crc_accumulator,
					   const char *data,
					   size_t data_len) noexcept
	{
		...
		while (data_len > 8) {
			...
			crc_accumulator =
				crc64_table[7][crc_accumulator & 0xff] ^
		...
		for (size_t i = 0; i < data_len; ++i) {
			crc_accumulator =
				crc64_table[0][static_cast<uint8_t>(
						       crc_accumulator) ^
					       data[i]] ^
				(crc_accumulator >> 8);
		}
	...
	static constexpr const CRC64_table_t get_table() noexcept
	{
		...
		for (uint64_t i = 0; i < 256; ++i) {
			...
			for (uint64_t j = 1; j < 8; ++j) {
				crc_accumulator =
					crc64_itable[crc_accumulator & 0xff] ^
					(crc_accumulator >> 8);
	...
	static constexpr const std::array<uint64_t, 256>
	get_init_table() noexcept
	{
