...
struct Token {
	...
	bool is_string{
		false
	}; // If it is a string we only put/have a string in the name-attribute (path in
		// parse-method) So if this is true, we can ignore all attributes except name
 ...
 *
 *  Calls parse-method and then tokens_to_regex-method based on the tokens returned from
 * parse-method Puts the tokens that are keys (not string-tokens) into keys-vector
 *
 ...
 *      end = true
 */
std::regex path_to_regex(const std::string &path,
			 const Options &options = Options{});
 ...
 */
std::regex tokens_to_regex(const Tokens &tokens,
			   const Options &options = Options{});
