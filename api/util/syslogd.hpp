...
class Syslog {
	...
	syslog(int priority, const char *buf, ...);
	...
	{
		return (logopt & LOG_PID or logopt & LOG_CONS or
			logopt & LOG_NDELAY or logopt & LOG_ODELAY or
			logopt & LOG_NOWAIT or logopt & LOG_PERROR);
	}

	static bool valid_facility(int facility) noexcept
	{
		return ((facility >= LOG_KERN and facility <= LOG_CRON) or
			(facility >= LOG_LOCAL0 and facility <= LOG_LOCAL7));
	}
