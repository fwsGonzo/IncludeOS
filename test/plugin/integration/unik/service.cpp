...
void Service::start(const std::string &)
{
	...
	inet.negotiate_dhcp(5.0, [](auto timeout) {
		CHECK(true,
		      "A service can subscribe to the DHCP event even if Unik did so first");
		if (timeout) {
			INFO("Unik test", "DHCP timed out");
			CHECKSERT(not inet.udp().is_bound(unik::default_port),
				  "Unik UDP port is free as expected");

			INFO("Unik test", "Manual netwok config");
			inet.network_config({ 10, 0, 0, 56 },
					    { 255, 255, 255, 0 },
					    { 10, 0, 0, 1 }, { 8, 8, 8, 8 });
			unik::Client::register_instance(inet);
		} else {
			INFO("Unik test",
			     "DHCP OK. We can now use the IP stack");
			CHECK(inet.udp().is_bound(unik::default_port),
		...
		try {
			...
			CHECK(true,
			      "Trying to bound to the Unik port now fails");
			INFO("Unik test", "SUCCESS");
		}
	});
}
