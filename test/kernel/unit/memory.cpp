...
CASE("os::mem using protect_range and flags")
{
	...
	EXPECT_THROWS(mem::protect_range(unmapped, mem::Access::write |
							   mem::Access::read));
	EXPECT(mem::flags(unmapped) == mem::Access::none);
	...
	for (auto p = req.lin; p < req.lin + req.size; p += 4_KiB) {
		...
		EXPECT(mem::flags(p) ==
		       (mem::Access::execute | mem::Access::read));
	}
...
CASE("os::mem using protect")
{
	SETUP("Assuming a default page table setup")
	{
		...
		SECTION("os::mem protect basics")
		{
			...
			std::array<uintptr_t, 4> sizes_pre{
				mem::active_page_size(5_GiB),
			...
			EXPECT(mem::virt_to_phys(prot_begin) ==
			       req.phys + prot_offs);
			...
			for (uintptr_t v = prot_begin;
			     v < prot_begin + prot_size;
			     v += 4_KiB, p += 4_KiB) {
			...
			std::array<uintptr_t, 4> sizes_post{
				mem::active_page_size(5_GiB),
				mem::active_page_size(6_GiB),
			...
			EXPECT(sizes_pre[0] ==
			       sizes_post[0]); // Can't have changed, 1 GiB below
			EXPECT(sizes_pre[1] >=
			       sizes_post[1]); // Is now likely 2_MiB or 4_KiB
			EXPECT(sizes_pre[2] >
			       sizes_post[2]); // Must be the new size
			EXPECT(sizes_pre[3] ==
			       sizes_post[3]); // Can't have changed, 1 GiB above
		}
	}
}
...
CASE("os::mem::protect try to break stuff")
{
	...
	for (auto r : test::random) {
		auto lin =
			3 % 2 ? (uintptr_t)r % (1_GiB) : (uintptr_t)r % 2_MiB;
		...
		MYINFO("Allocated bytes after map: %zi == %zi tables\n",
		       bytes_after_map,
		       bytes_after_map / sizeof(decltype(*__pml4)));
		...
		MYINFO("Allocated bytes after unmap: %zi == %zi tables\n",
		       bytes_after_unmap,
		       bytes_after_unmap / sizeof(decltype(*__pml4)));
		...
		MYINFO("Allocated bytes after purge: %zi == %zi tables\n",
		       bytes_after_purge,
		       bytes_after_purge / sizeof(decltype(*__pml4)));
...
CASE("os::mem::protect verify consistency")
{
	...
	mem::Map req{ 6_GiB, 3_GiB, mem::Access::read | mem::Access::write,
		      300_MiB };
	auto res = mem::map(req);
	EXPECT(res);
	...
	// Verify each page
	for (auto i = prot_begin; i < prot_begin + prot_size;
	     i += mem::min_psize()) {
	...
	new_flags =
		mem::Access::read | mem::Access::write | mem::Access::execute;
