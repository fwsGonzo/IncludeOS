...
struct Pool {
	...
	void *end()
	{
		return (void *)((uintptr_t)data + size);
	}
...
CASE("os::Machine basics")
{
	...
	EXPECT((pool_begin >= (uintptr_t)pool.data and
		pool_begin <= (uintptr_t)pool.data + pool.size));
	...
	for (Pool &p : vec) {
		EXPECT(p.size == i++ * 100);
		EXPECT((std::addressof(p) > pool.begin() and
			std::addressof(p) < pool.end()));
	}
	...
	for (Pool &p : vec) {
		if (i++ < 3)
			EXPECT((std::addressof(p) < pool.begin() or
				std::addressof(p) > pool.end()));
		else
			EXPECT((std::addressof(p) > pool.begin() and
				std::addressof(p) < pool.end()));
