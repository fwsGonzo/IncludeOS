...
CASE("x86::paging: PML4 Page_dir entry helpers")
{
	...
	EXPECT(Pml4::is_page_dir(512_GiB |
				 (uintptr_t)(Flags::pdir | Flags::present)));
	for (auto rnd : test::random) {
	...
	EXPECT(Pml4::flags_of(test::random[1] | fl2) ==
...
CASE("x86::paging: PML1 Page_dir entry helpers")
{
	...
	EXPECT(Pml1::flags_of(test::random[1] | fl2) ==
	       (Flags::all & ~(Flags::pdir | Flags::huge)));
}
...
CASE("x86::paging 4-level x86_64 paging")
{
	SETUP("Initializing page tables")
	{
		...
		SECTION("Failing calls to  map_r")
		{
			...
			EXPECT_THROWS(
				__pml4->map_r({ 0, 1, Pflag::present, 0_KiB }));
		...
		SECTION("Recursively map pages within a single 4k page dir")
		{
			...
			auto map = __pml4->map_r(
				{ 0, 4_KiB, Pflag::present, 1_MiB });
...
void init_default_paging(uintptr_t exec_beg = 0xa00000,
			 uintptr_t exec_end = 0xb0000b)
{
...
CASE("x86::paging Verify execute protection")
{
	...
	EXPECT(os::mem::flags(__exec_begin) ==
	       (Access::execute | Access::read));
	EXPECT(os::mem::flags(__exec_end - 1) ==
	       (Access::execute | Access::read));
	EXPECT(os::mem::flags(__exec_end + 4_KiB) ==
...
CASE("x86::paging controlling page sizes")
{
	...
	paging::Map req{ 42_MiB, 10_MiB, paging::Flags::present, 42_MiB,
			 4_KiB };
	...
	req = { 42_MiB + 42_MiB, 10_MiB, paging::Flags::present, 42_MiB,
		2_MiB };
	res = __pml4->map_r(req);
	...
	req = { 42_GiB, 10_GiB, paging::Flags::present, 4_GiB,
		os::mem::max_psize() };
	res = __pml4->map_r(req);
	EXPECT(res);
...
CASE("x86::paging Verify default paging setup")
{
	SETUP("Initialize paging")
	{
		...
		SECTION("Verify page access")
		{
			...
			// .text segment has execute + read access up to next 4kb page
			EXPECT(os::mem::active_page_size(__exec_begin) ==
			       4_KiB);
			EXPECT(os::mem::flags(__exec_begin) ==
			       (Access::execute | Access::read));
			EXPECT(os::mem::flags(__exec_end) ==
			       (Access::execute | Access::read));
			EXPECT(os::mem::flags(__exec_end + 4_KiB) ==
			       (Access::read | Access::write));

			// Remaining address space is either read + write or not present
			EXPECT(os::mem::flags(100_MiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(1_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(2_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(4_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(8_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(16_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(32_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(64_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(128_GiB) ==
			       (Access::read | Access::write));
			EXPECT(os::mem::flags(256_GiB) ==
			       (Access::read | Access::write));
		...
		SECTION("Try to break stuff")
		{
			...
			EXPECT(not __pml4->within_range(
			...
			EXPECT(__pml4->entry_r(4_KiB - 1_b) ==
			       __pml4->entry_r(0));
			EXPECT(__pml4->entry_r(5_KiB) ==
			       __pml4->entry_r(4_KiB));
			EXPECT(__pml4->entry_r(255_TiB) != nullptr);
			EXPECT(__pml4->entry_r(255_TiB) ==
			       __pml4->entry(255_TiB));
			EXPECT(*__pml4->entry_r(255_TiB) == 0);
			EXPECT(__pml4->entry_r(512_GiB * 512) == nullptr);
			EXPECT(__pml4->entry_r(1000_TiB) == nullptr);
			EXPECT(__pml4->entry_r(
				       std::numeric_limits<uintptr_t>::max()) ==
			       nullptr);
			...
			auto map = __pml4->map_r(
				{ addr, addr, Flags::present, increment });
			...
			EXPECT(full_map.size ==
			       bits::roundto(increment, 1_GiB));
						...
						if (ent3 & Flags::pdir) {
							...
							for (auto &ent2 :
								...
								if (ent2 &
								    Flags::pdir) {
									page_dirs_found++;
									auto *sub1 = sub2->page_dir(
										&ent2);
									EXPECT(sub1);
									EXPECT_THROWS(sub1->at(
										512));
									Tbl &tbl1 =
										*(Tbl *)sub1
											 ->data();
			...
			auto diff_4k =
				summary_post.pages_4k - summary_pre.pages_4k;
...
CASE("Map various ranges")
{
	...
	req = { 790_GiB + 3_MiB + 4_KiB, 10_GiB + 2_MiB + 16_KiB,
		Flags::present, 100_MiB + 42_KiB };
	res = __pml4->map_r(req);
	EXPECT(res.size == bits::roundto(4_KiB, req.size));
	EXPECT(res.lin == req.lin);
	EXPECT(res.phys == req.phys);
	...
	req = { 120_TiB + 17_MiB + 4_KiB, 10_GiB + 2_MiB + 16_KiB,
		Flags::present, 11_MiB + 42_KiB };
	res = __pml4->map_r(req);
	EXPECT(res.size == bits::roundto(4_KiB, req.size));
	EXPECT(res.lin == req.lin);
	EXPECT(res.phys == req.phys);
