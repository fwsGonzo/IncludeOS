...
static struct testable_t test_struct {
	.value = 4
};
...
static void restore_something(Restore &thing)
{
	...
	assert(thing.is_int());
	stored.integer = thing.as_int();
	thing.go_next();
	assert(thing.get_id() == 1);
	assert(thing.is_string());
	stored.string = thing.as_string();
	thing.go_next();
	assert(thing.get_id() == 2);
	assert(thing.is_buffer()); // internally they are buffers
	stored.boolean = thing.as_type<bool>();
	thing.go_next();
	assert(thing.get_id() == 3);
	assert(thing.is_buffer()); // internally they are buffers
	stored.testable = thing.as_type<struct testable_t>();
	thing.go_next();
	assert(thing.get_id() == 4);
	assert(thing.is_buffer());
	stored.buffer = thing.as_buffer();
	thing.go_next();
	assert(thing.get_id() == 5);
	assert(thing.is_vector());
	stored.intvec1 = thing.as_vector<int>();
	thing.go_next();
	stored.intvec2 = thing.as_vector<int>();
	thing.go_next();
	assert(thing.get_id() == 6);
	assert(thing.is_string_vector());
	stored.strvec1 = thing.as_vector<std::string>();
	thing.go_next();
	stored.strvec2 = thing.as_vector<std::string>();
	thing.go_next();
	// TCP connection
	assert(thing.get_id() == 7);
	auto conn = thing.as_tcp_connection(inet->tcp());
	thing.go_next();
	// Statman stats
...
CASE("Setup LiveUpdate and perform no-op update")
{
	...
	EXPECT_THROWS_AS(LiveUpdate::exec(not_a_kernel, storage_area),
			 liveupdate_exec_success);
}
...
CASE("Store some data and restore it")
{
	...
	EXPECT_THROWS_AS(LiveUpdate::resume_from_heap(storage_area, "",
						      nullptr),
			 std::length_error);
