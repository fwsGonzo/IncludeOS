...
CASE("Using the fixed memory range class")
{
	GIVEN("A fixed memory range over raw sequence of bytes")
	{
		...
		char *seq = const_cast<char *>(
			"Spans provide a safer way to access memory");
		Fixed_memory_range range(reinterpret_cast<uintptr_t>(seq + 16),
					 reinterpret_cast<uintptr_t>(seq + 20),
					 "Demo range");
		...
		AND_THEN(
			"You can resize that range (but you should let the memory map do that)")
		{
			auto sz = range.size();
...
{
	EXPECT_THROWS(
		(Fixed_memory_range{ (uintptr_t)1000, (uintptr_t)10,
				     "Negative - A range going backwards" }));
	EXPECT_THROWS((Fixed_memory_range{
		(uintptr_t)1, (uintptr_t)Fixed_memory_range::max_size() + 100,
		"Too big - A range > max of ptrdiff_t" }));
}
...
CASE("Using the kernel memory map")
{
	GIVEN("An instance of the memory map with a named range")
	{
		...
		AND_THEN(
			"You can ask the memory map if an address is in that range")
		{
			EXPECT(map.in_range(0xff1) == init_key);
			EXPECT(map.at(map.in_range(0xfff)).addr_start() ==
			       init_key);
			EXPECT(map.at(map.in_range(0xfffa)).addr_start() ==
			       init_key);
		...
		AND_THEN(
			"You can increase the size of that range since nothing is above")
		{
			EXPECT(map.resize(init_key, init_size + 1) ==
			       init_size + 1);
			EXPECT(map.resize(init_key, init_size + 100) ==
			       init_size + 100);
		}

		AND_THEN(
			"You can't downsize that range, since bytes_in_use is all of it by default")
		{
		...
		AND_WHEN(
			"You provide a custom bytes_in_use delegate you can downsize")
		{
			...
			THEN("You still can't downsize to less than what it currently uses")
			{
				...
				AND_THEN(
					"If you assign a range above it, you can't increase it further")
				{
					auto &last_range = map.at(key);
					auto key2 = last_range.addr_end() + 1;
					map.assign_range({ key2, key2 + 1000,
							   "Above Resizable" });

					EXPECT_THROWS(map.resize(
						key, last_range.size() + 1));
					EXPECT_THROWS(map.resize(
						key, last_range.size() + 100));
				}
			}
		}
		...
		{
			EXPECT_THROWS(map.assign_range(
				{ 0x1, 0xfff,
				  "Range 2 - Overlapping beginning of Range 1" }));
			EXPECT_THROWS(map.assign_range(
				{ 0xfff, 0xfffff,
				  "Range 2 - Overlapping end of Range 1" }));

			uintptr_t rng2_key = 0x30000;
			uintptr_t rng2_end = 0xfffff;
			EXPECT(map.assign_range(
					  { rng2_key, rng2_end,
					    "Range 2 - OK range above initial" })
				       .addr_start() == rng2_key);

			// A range where end-address only is with the second range
			EXPECT_THROWS(map.assign_range(
				{ 0x20000, 0x40000,
				  "Range 3 - Between Range 1 and Range 2" }));
	    ...
	    **/
		...
		AND_THEN("You can't insert an embedded  range")
		{
			EXPECT_THROWS(map.assign_range(
				{ 0xfff, 0xfffa,
				  "Range 4 - Embedded in range 1" }));
		}

		AND_WHEN(
			"You've inserted several ranges you can iterate over them")
		{
			uintptr_t start = 0x10000;
			const size_t size = 100;
			const size_t count = 20;

			for (auto i = start; i < start + count * size;
			     i += size) {
				Fixed_memory_range rng = { i, i + size - 1,
							   "test" };
			...
			AND_THEN("Verify that the ranges are occupied")
			{
				...
				for (uintptr_t i = 0x10000;
				     i < 0x10000 + count * size; i += 1) {
