...
template <typename Q> class Sync_queue {
    public:
	...
	Sync_queue(std::initializer_list<Fiber> fibers) : queue{ fibers }
	{
	}
...
void worker(int i)
{
	...
	for (auto &c : src[i]) {
		...
		CPULOG("[ F%i::worker %i] @ pos %i yielding to F%i\n",
		       Fiber::current()->id(), i, pos,
		       Fiber::current()->parent()->id());
	...
	CPULOG("[ F%i::worker %i ] DONE returning to F%i, main F%i\n", i,
	       Fiber::current()->id(), Fiber::current()->parent()->id(),
	       Fiber::main()->id());
}
...
void threadrunner(int i)
{
	...
	while (jobs_remaining) {
		...
		Expects(Fiber::current());
		Expects(Fiber::current() == Fiber::main());
		Expects(Fiber::current()->id() ==
			runners_per_cpu[SMP::cpu_id()]);
		...
		CPULOG("[ R%i ] F%i yield. %i jobs left, %li fibers in queue \n",
		       Fiber::main()->id(), fiber->id(), jobs_remaining.load(),
		       smp_fibers.size());
		...
		if (not fiber->done()) {
			...
			CPULOG("[ R%i ] checked back F%i. Jobs left: %i, Fibers left: %li, _current: F%i \n",
			       Fiber::main()->id(), last_id,
			       jobs_remaining.load(), smp_fibers.size(),
			       Fiber::current()->id());
		} else {
...
void many_fibers()
{
	...
	for (auto &str : src) {
		...
		smp_fibers.sync_insert(
			std::make_unique<Fiber>(worker, string_idx++));
		jobs_remaining++;
