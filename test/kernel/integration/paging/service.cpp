...
{
	template <> struct enable_bitmask_ops<Pfault> {
		using type = std::underlying_type<Pfault>::type;
		static constexpr bool enable = true;
	};
	template <> struct enable_bitmask_ops<uint32_t> {
		using type = uint32_t;
		static constexpr bool enable = true;
	};
...
template <typename E> struct enable_log {
	static constexpr int level = 0;
};

// Logging ON
template <typename E>
typename std::enable_if<enable_log<E>::level != 0>::type log(const char *msg)
{
	printf("LOG: %s\n", msg);
}
...
// Logging OFF
template <typename E>
typename std::enable_if<enable_log<E>::level == 0>::type log(const char *)
{
	printf("NOTHING\n");
}

// Class TCP - which logging knows nothing about
template <int idx> class TCP {
};

// Template specialization of enable_log for TCP
template <> struct enable_log<TCP<1> > {
...
template <> void C<std::string>::func()
{
	s = "hello";
	printf("I am enabled and have a string %s \n", s.c_str());
}

template <> void C<int>::func()
{
	s = 10;
...
{
	std::vector<uintptr_t> test_entries{ 0_b,     4_KiB,	8_KiB,
					     1_MiB,   2_MiB,	100_MiB,
					     1_GiB,   1002_MiB, 500_GiB,
					     513_GiB, magic_loc };
...
void verify_integrity()
{
	...
	std::cout << "* Populating near memory with " << util::Byte_r(res.size)
		  << " random data\n";
...
void verify_magic()
{
	...
	auto m = __pml4->map_r({ magic_loc, (uintptr_t)magic_phys,
				 Pflag::writable | Pflag::present | Pflag::huge,
				 4_KiB });
	Expects(m);
...
void map_non_aligned()
{
	...
	std::cout << "* Allowed page sizes: "
		  << mem::page_sizes_str(mem::supported_page_sizes()) << "\n";
	...
	auto res = mem::map({ far_addr1, near_addr1,
			      mem::Access::read | mem::Access::write, psize });
	Expects(res);
	...
	std::cout << "* Mapping a " << util::Byte_r(psize) << " page to "
		  << Byte_r(near_addr2) << ", requiring page size "
		  << Byte_r(psize) << "\n";
	try {
		mem::map({ far_addr2, near_addr2,
			   mem::Access::read | mem::Access::write, psize,
			   psize });
	} catch (mem::Memory_exception &e) {
		Expects(std::string(e.what()).find(std::string(
...
int main()
{
	...
	Expects(Byte_r{ std::numeric_limits<int>::max() }.to_string() ==
		"2.000_GiB");
	Expects(Byte_r{ std::numeric_limits<uintptr_t>::max() }.to_string() ==
		"16777216.000_TiB");
	...
	if (magic->reboots < expected_reboots) {
		...
		mem::protect_range((uint64_t)protected_page,
				   mem::Access::read | mem::Access::write);
	...
	if (magic->reboots == 0) {
		...
		std::cout
			<< "* Writing to write-protected page, expecting page write fail\n\n";
		protected_page[magic->i] = 'b';
	}

	if (magic->reboots == 1) {
		Expects(magic->last_error = Page_fault);
		Expects(magic->last_code ==
			(Pfault::present | Pfault::write_failed));
		Expects(protected_page[magic->i] == 'a');
		protected_page[magic->i] = 'b';
		...
		std::cout
			<< "* Reading non-present page, expecting page read fail\n\n";
	...
	if (magic->reboots == 2) {
		...
		std::cout
			<< "* Executing code from execute-protected page, expecting instruction fetch fail\n\n";
		mem::protect_range((uint64_t)protected_page, mem::Access::read);
	...
	if (magic->reboots == 4) {
		...
		auto aligned_last =
			(uintptr_t)magic->last_access & ~(4_KiB - 1);
