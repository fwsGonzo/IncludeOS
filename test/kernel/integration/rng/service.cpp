...
void Service::start()
{
	...
	CHECKSERT(read(rand_fd, a, sizeof(a)) == sizeof(a),
		  "read returns the correct length (%u)", BUFLEN);

	read(rand_fd, b, sizeof(b));
	CHECKSERT(memcmp(a, b, BUFLEN) != 0, "reading returns new data");

	auto now = time(0);
	CHECKSERT(write(rand_fd, &now, sizeof(now)) == sizeof(now),
		  "write returns the correct length");
