...
static inline void server(net::Inet &inet, const uint16_t port,
			  delegate<void(liu::buffer_t &)> callback)
{
	auto &server = inet.tcp().listen(port);
	server.on_connect(net::tcp::Connection::ConnectCallback::make_packed(
		[callback, port](auto conn) {
			...
			conn->on_read(9000,
				...
				.on_close(net::tcp::Connection::CloseCallback::make_packed(
					[buffer, callback]() {
