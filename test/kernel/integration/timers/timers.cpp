...
void test_timers()
{
	...
	Timers::oneshot(30s, [](auto) {
		...
		CHECKSERT(repeat1 == 25 and repeat2 == 10,
			  "1s. timer fired 25 times, 2s. timer fired 10 times");
		CHECKSERT(Timers::active() == BASE_TIMERS + 0,
	...
	Timers::oneshot(500ms, [](auto) {
		CHECKSERT(one_shots == 0,
			  "After 0.5 sec, no other one-shot-timers have fired");
		one_shots++;
	});

	// 1 sec.
	Timers::oneshot(1s, [](auto) {
		CHECKSERT(one_shots == 1,
			  "After 1 sec, 1 other one-shot-timers has fired");
		one_shots++;
	});
	...
	auto in_a_second = [integers](Timers::id_t) {
		for (auto i : integers)
			CHECKSERT(i == integers[i - 1],
				  "%i == integers[%i - 1]", i, integers[i - 1]);
		one_shots++;
	};

	Timers::oneshot(1s, in_a_second);

	auto timer1s = Timers::periodic(1s, 1s, [](auto) {
		repeat1++;
		printf("1s. PULSE #%i \n", repeat1);
	});

	Timers::periodic(2s, 2s, [](auto id) {
		repeat2++;
		printf("2s. PULSE #%i \n", repeat2);

		// CHECKSERT(repeat1 == (repeat2 * 2), "2s timer fired %i, 1s fired %i x 2 == %i times",
		// repeat2, repeat2, repeat1);
		if (repeat2 >= 10) {
	...
	Timers::oneshot(25s + 20ms, [timer1s](auto) {
		one_shots++;
		printf("repeat1: %u,  repeat2: %u\n", repeat1, repeat2);
		CHECKSERT(
			repeat1 == 25 and repeat2 == 10,
		...
		CHECKSERT(Timers::active() == BASE_TIMERS + 1,
			  "Only the last finish timer is left");
