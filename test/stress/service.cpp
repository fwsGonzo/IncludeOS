...
std::string html()
{
	...
	stream << "<html><head>"
		  ...
		  "rel='stylesheet' type='text/css'>"
	       ...
	       << "<h1 style= \"color: "
	       << "#" << std::hex << (color >> 8) << "\">"
	       << "<span style=\"" + ubuntu_medium +
			  "\">Include</span><span style=\"" + ubuntu_light +
			  "\">OS</span> </h1>"
...
void Service::start(const std::string &)
{
	...
	for (int i = 0; i < 1000; i++)
		Timers::oneshot(std::chrono::microseconds(i + 200),
				[](auto) {});
	...
	Timers::periodic(1s, 10s, [](Timers::id_t) {
		auto memuse = os::total_memuse();
		printf("Current memory usage: %i b, (%f MB) \n", memuse,
		       float(memuse) / 1000000);
		...
		printf("eth0.sendq_max: %zu, eth0.sendq_now: %zu"
		       ...
		       Statman::get()
			       .get_by_name("eth0.stat_rx_total_packets")
			       .get_uint64(),
		       Statman::get()
			       .get_by_name("eth0.stat_tx_total_packets")
			       .get_uint64(),
		       Statman::get()
			       .get_by_name("eth0.stat_rx_total_bytes")
			       .get_uint64(),
		       Statman::get()
			       .get_by_name("eth0.stat_tx_total_bytes")
			       .get_uint64(),
		       Statman::get()
			       .get_by_name("eth0.sendq_dropped")
			       .get_uint64(),
		       Statman::get()
			       .get_by_name("eth0.rx_refill_dropped")
			       .get_uint64());
	});
	...
	server_mem.on_connect([](net::tcp::Connection_ptr conn) {
		conn->on_read(1024, [conn](net::tcp::buffer_t buf) {
			...
			std::string received{ (char *)buf->data(),
					      buf->size() };
			...
			printf("TCP Mem: Reporting memory size as %s bytes\n",
			       reply.c_str());
			conn->on_write([](size_t n) { TCP_BYTES_SENT += n; });
			conn->write(reply);

			conn->on_disconnect([](net::tcp::Connection_ptr c,
					       auto) { c->close(); });
		});
	});
	...
	conn.on_read([&](net::UDP::addr_t addr, net::UDP::port_t port,
	...
	conn_mem.on_read([&](net::UDP::addr_t addr, net::UDP::port_t port,
			     const char *data, int len) {
		std::string received = std::string(data, len);
