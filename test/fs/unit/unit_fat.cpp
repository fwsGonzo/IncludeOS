...
CASE("Initialize FAT fs")
{
	...
	disk->init_fs([&lest_env](auto err, File_system &fs) { EXPECT(!err); });
}
...
CASE("Validate /folder")
{
	...
	fs.ls("/folder", [&lest_env](auto error, Dirvec_ptr dirvec) {
		EXPECT(!error);
		auto &ents = *dirvec;
		EXPECT(ents.at(0).name() == ".");
		EXPECT(ents.at(1).name() == "..");
...
CASE("Validate /folder using dirent")
{
	...
	fs.ls(ent, [&lest_env](auto error, Dirvec_ptr dirvec) {
		EXPECT(!error);
		auto &ents = *dirvec;
		EXPECT(ents.at(0).name() == ".");
		EXPECT(ents.at(1).name() == "..");
