...
void Service::start(const std::string &)
{
	...
	CHECKSERT(disk->dev().size() == SIZE,
		  "Disk size is correct (%llu sectors)", SIZE);
	...
	CHECKSERT(mbr[0x1FE] == 0x55,
		  "MBR has correct signature (0x1FE == 0x55)");
	CHECKSERT(mbr[0x1FF] == 0xAA,
		  "MBR has correct signature (0x1FF == 0xAA)");
