...
void Service::start(const std::string &)
{
	...
	try {
		...
		CHECKSERT(
			true,
			"Exception thrown when fetching Thing 1 as Thing 2: %s",
			e.what());
	}
	...
	try {
		...
		CHECKSERT(true,
			  "Exception thrown when fetching Thing 1 as Base: %s",
			  e.what());
	}
	...
	CHECKSERT(
		true,
		"obj1 successfully converted to correct type Thing1 using cast operator");

	auto &&two_as_two = obj2.obj<Thing2>();
	CHECKSERT(
		true,
		"obj2 successfully converted to correct type Thing2 using getter");
	...
	delegate<void()> func1 = [&lambda_called] { lambda_called = true; };
	...
	CHECKSERT(fs::VFS::root().child_count() == 1,
		  "Mount added one element to root");
	...
	CHECKSERT(tau == 6.28f,
		  "Retreiving const-mounted object as const works");
	...
	try {
		...
		CHECKSERT(
			true,
			"Trying to mount to the same mount point again throws");
	}

	// Non-creating mount
	try {
		std::string plan("Make America Great Again");
		fs::mount<false>("/users/donald/master_plan.txt", plan,
				 "Donalds plan");
		FAIL("Mounting to a non-existing mount point with no create flag should throw");
	} catch (const fs::Err_mountpoint_invalid &e) {
		CHECKSERT(
			e.what(),
			"Trying to mount to non-existing path with no create flag throws");
	}
	...
	try {
		...
		CHECKSERT(
			true,
			"Trying to retreive a const-mounted object as non-const throws");
	}

	// Retreiving non-mounted objects
	try {
		auto &&string =
			fs::get<std::string>("/users/donald/master_plan.txt");
		FAIL("Retreiving a non mounted object should throw");
		assert(string.empty());

	} catch (const fs::Err_not_found &e) {
		CHECKSERT(true,
			  "Trying to retreive a non existing object throws");
	}
	...
	INFO("VFS_test",
	     "stat and read relative path '%s' from mounted dirent ",
	     forward_dir);
	...
	for (auto &drv : os::machine().get<hw::Block_device>()) {
		...
		fs::mount({ "dev", drv.get().device_name() }, drv.get(),
			  drv.get().driver_name());
	}
	...
	CHECKSERT(std::string(disk0->name()) == "vblk0",
		  "First drive mounted successfully");
	CHECKSERT(std::string(disk1->name()) == "vblk1",
		  "Second drive mounted successfully");

	// Mount a directory on a given disk, on a VFS path
	try {
		fs::VFS::mount(
			{ "/overlord/pictures/" }, disk0->device_id(),
			{ "/pictures/" }, "Image of our lord commander",
			[](auto) {
				FAIL("Mounting a directory from an uninitialized disk should not call "
				     "user callback");
			});
		...
		CHECKSERT(
			true,
			"Mounting a directory on an uninitialized disk throws");
	}

	auto my_disk = disk1;

	// initializing a file system
	my_disk->init_fs([my_disk](auto err, auto &fs) {
		if (err) {
		...
		fs.ls("/", [my_disk](auto err, auto dirvec) {
			if (err) {
				INFO("VFS_test", "ls on disk %s failed",
				     my_disk->name().c_str());
				return;
			}

			INFO("VFS_test",
			     "ls on disk %s:", my_disk->name().c_str());
			for (auto ent : *dirvec) {
				std::cout << "\t|-" << ent.name() << "\n";
			}
		});

		INFO("VFS_test", "Filesystem mounted on %s",
		     my_disk->name().c_str());
		fs::VFS::mount(
			{ "/overlord/pictures/" }, my_disk->device_id(),
			{ "/pictures/" }, "Images of our lord commander",
			[my_disk](auto err) {
				if (err)
					os::panic(
						"Error mounting dirent from disk on VFS path");

				INFO("VFS_test",
				     "Reading content of newly mounted folder");

				// Actual user interface for VFS
				fs::stat("/overlord/pictures/profile.txt", [](auto err,
									      auto dir) {
					if (err)
						os::panic(
							"Error stating file \n");
