...
void test2()
{
	...
	disk->init_fs(disk->MBR, [](fs::error_t err, fs::File_system &fs) {
		...
		fs.stat(shallow_banana, [](fs::error_t err,
			...
			CHECKSERT(ent.name() == "banana.txt",
				  "Name is 'banana.txt'");
			is_done();
		});

		fs.read_file(shallow_banana, [](fs::error_t err,
						fs::buffer_t buf) {
			INFO("FAT32", "Read file");
			CHECKSERT(not err, "read_file: Read %s asynchronously",
				  shallow_banana.c_str());
			printf("%s\n", internal_banana.c_str());
			std::string banana((const char *)buf->data(),
					   buf->size());
			CHECKSERT(banana == internal_banana,
				  "Correct shallow banana");
			is_done();
		});

		fs.stat(deep_banana, [](fs::error_t err,
					const fs::Dirent &ent) {
...
void Service::start()
{
	...
	CHECKSERT(disk->dev().size() == SIZE / 512, "Disk size is %llu sectors",
		  SIZE / 512);

	// auto-mount filesystem
	disk->init_fs([](fs::error_t err, fs::File_system &fs) {
		CHECKSERT(!err, "Filesystem auto-initializedd");

		std::string fat32_str{ "FAT32" };
		CHECKSERT(fs.name() == fat32_str,
			  "Filesystem recognized as FAT32");

		fs.ls("/", [](fs::error_t err, auto ents) {
			CHECKSERT(not err, "Listing root directory");
			CHECKSERT(ents->size() == 2,
				  "Exactly two ents in root dir");

			auto &e = ents->at(0);
			CHECKSERT(e.is_file(), "Ent is a file");
			CHECKSERT(e.name() == "banana.txt",
				  "Ents name is 'banana.txt'");
