...
void Service::start(const std::string &)
{
	...
	CHECKSERT(bufstore.available() == BUFFER_CNT - 1, "Bufcount is now %i",
		  BUFFER_CNT - 1);
	...
	for (int i = 0; i < chain_size - 1; i++) {
		...
		CHECKSERT(bufstore.available() == BUFFER_CNT - i - 2,
			  "Bufcount is now %i", BUFFER_CNT - i - 2);
	}

	// Release
	INFO("Test 1",
	     "Releasing packet-chain all at once: Expect bufcount restored");
	packet = nullptr;
	CHECKSERT(bufstore.available() == BUFFER_CNT, "Bufcount is now %i",
		  BUFFER_CNT);
	...
	CHECKSERT(bufstore.available() == BUFFER_CNT - 1, "Bufcount is now %u",
		  BUFFER_CNT - 1);
	...
	tail = 0;
	packet = 0;
	CHECK(bufstore.available() == TOTAL_BUFFERS, "Bufcount is now %u / %u",
	      bufstore.available(), TOTAL_BUFFERS);
