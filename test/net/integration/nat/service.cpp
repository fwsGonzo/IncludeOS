...
void Service::start()
{
	...
	eth0.network_config({ 10, 1, 0, 1 }, { 255, 255, 0, 0 },
			    { 10, 0, 0, 1 });

	static auto &eth1 = Interfaces::get(1);
	eth1.network_config({ 192, 1, 0, 1 }, { 255, 255, 255, 0 },
			    { 10, 0, 0, 1 });

	static auto &laptop1 = Interfaces::get(2);
	laptop1.network_config({ 10, 1, 0, 10 }, { 255, 255, 255, 0 },
			       eth0.ip_addr());

	static auto &internet_host = Interfaces::get(3);
	internet_host.network_config({ 192, 1, 0, 192 }, { 255, 255, 255, 0 },
				     eth1.ip_addr());

	static auto &server = Interfaces::get(4);
	server.network_config({ 10, 1, 10, 20 }, { 255, 255, 255, 0 },
			      eth0.ip_addr());
	...
	auto masq = [](IP4::IP_packet_ptr pkt, Inet &stack,
		       Conntrack::Entry_ptr entry) -> Filter_verdict<IP4> {
		natty->masquerade(*pkt, stack, entry);
		return { std::move(pkt), Filter_verdict_type::ACCEPT };
	};
	auto demasq = [](IP4::IP_packet_ptr pkt, Inet &stack,
			 Conntrack::Entry_ptr entry) -> Filter_verdict<IP4> {
	...
	internet_host.tcp().listen(80, [](auto conn) {
		INFO("TCP MASQ", "Internet host receiving connection - %s",
		     conn->to_string().c_str());
		CHECKSERT(
			conn->remote().address() == eth1.ip_addr(),
			"Received connection from (what appears to be) my gateway - %s",
			conn->remote().to_string().c_str());

		conn->on_read(1024, [](auto buf) {
			const auto str = std::string{ (const char *)buf->data(),
						      buf->size() };
			CHECKSERT(str == "Testing MASQ",
				  "Data from laptop is received - \"%s\"",
				  str.c_str());
	...
	static const auto website_socket =
		Socket{ internet_host.ip_addr(), 80 };
	laptop1.tcp().connect(website_socket, [](auto conn) {
		assert(conn);
		INFO("TCP MASQ", "Laptop connected out - %s",
		     conn->to_string().c_str());
		CHECKSERT(conn->remote() == website_socket,
			  "Laptop1 connected to internet - %s",
			  conn->remote().to_string().c_str());
	...
	auto dnat_rule = [](IP4::IP_packet_ptr pkt, Inet &stack,
			    Conntrack::Entry_ptr entry) -> Filter_verdict<IP4> {
		if (not entry)
	...
	auto snat_translate =
		[](IP4::IP_packet_ptr pkt, Inet &stack,
		   Conntrack::Entry_ptr entry) -> Filter_verdict<IP4> {
	...
	server.tcp().listen(DNAT_PORT, [](auto conn) {
		INFO("TCP DNAT", "Server received connection - %s",
		     conn->to_string().c_str());
		CHECKSERT(conn->remote().address().v4() != eth1.ip_addr(),
			  "Received non SNAT connection - %s",
			  conn->remote().to_string().c_str());

		conn->on_read(1024, [conn](auto buf) {
			const auto str = std::string{ (const char *)buf->data(),
						      buf->size() };
			CHECKSERT(str == "Testing DNAT",
				  "Data from laptop is received - \"%s\"",
				  str.c_str());
	...
	laptop1.tcp().connect(server_socket, [](auto conn) {
		assert(conn);
		INFO("TCP DNAT", "Laptop1 connected to %s",
		     conn->to_string().c_str());
		CHECKSERT(conn->remote() == server_socket,
			  "Laptop1 connected to server - %s",
			  conn->remote().to_string().c_str());
	...
	laptop1.icmp().ping(
		eth0.ip_addr(),
		[](auto reply) {
			CHECKSERT(reply, "Got ping reply from %s",
	...
	laptop1.icmp().ping(
		internet_host.ip_addr(),
		[](auto reply) {
			CHECKSERT(reply, "Got ping reply from %s",
				  internet_host.ip_addr().to_string().c_str());
			test_finished();
		},
		3);
}
