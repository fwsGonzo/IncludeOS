		...
		[&tcp](net::WebSocket_ptr ws) {
			...
			if (socket->is_alive()) {
				...
				for (int i = 0; i < 1000; i++)
					socket->write(PER_CPU(httpd).buffer,
						      net::op_code::BINARY);

				// socket->close();
				socket->on_close = [](uint16_t reason) {
					if (reason == 1000)
						printf("SUCCESS\n");
					else
						assert(0 && "FAILURE");
...
void Service::start()
{
	...
	inet.network_config({ 10, 0, 0, 54 }, // IP
...
void ws_client_test(net::TCP &tcp)
{
	...
	net::WebSocket::connect(
		client, "ws://10.0.0.1:8001/", [](net::WebSocket_ptr socket) {
			if (!socket) {
				printf("WS Connection failed!\n");
				return;
			}
			socket->on_error = [](std::string reason) {
