...
void print_error(const std::string &hostname, net::Addr server,
		 const Error &err)
{
	printf("Error occurred when resolving IP address of %s with DNS server %s: %s\n",
	       hostname.c_str(), server.to_string().c_str(), err.what());
...
void print_success(const std::string &hostname, net::Addr server,
		   dns::Response_ptr res)
{
	assert(res != nullptr);
	if (res->has_addr())
		printf("Resolved IP address of %s with DNS server %s: %s\n",
		       hostname.c_str(), server.to_string().c_str(),
		       res->get_first_ipv4().to_string().c_str());
	else
...
struct Name_request {
	...
	Name_request(std::string n) : name{ std::move(n) }
	{
	}
...
static void do_test(net::Inet &inet, std::vector<Name_request> &reqs)
{
	for (auto &req : reqs) {
		...
		inet.resolve(req.name, req.server,
			     [name = req.name,
			      server = req.server](auto res, const Error &err) {
				     if (err) {
