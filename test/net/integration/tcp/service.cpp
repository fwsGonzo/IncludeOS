...
/*
  TEST VARIABLES
*/
tcp::port_t TEST0{ 8080 }, TEST1{ 8081 }, TEST2{ 8082 }, TEST3{ 8083 },
	TEST4{ 8084 }, TEST5{ 8085 };

using HostAddress = std::pair<std::string, tcp::port_t>;
HostAddress TEST_ADDR_TIME{ "india.colorado.edu", 13 };

std::string small, big, huge;

int S{ 150 }, B{ 1500 }, H{ 150000 };

std::string TEST_STR{ "Kappa!" };
...
void FINISH_TEST()
{
	...
	Timers::oneshot(2 * MSL_TEST + 100ms, [](Timers::id_t) {
...
void OUTGOING_TEST_INTERNET(const HostAddress &address)
{
	...
	INFO("TEST", "Outgoing Internet Connection (%s:%u)",
	     address.first.c_str(), address.second);
	stack().resolve(address.first, [port](auto res, const Error &) {
		CHECK(res != nullptr, "Resolved host");

		if (res and res->has_addr()) {
			stack().tcp()
				.connect({ res->get_first_addr(), port })
				->on_connect([](tcp::Connection_ptr conn) {
					CHECKSERT(conn != nullptr, "Connected");
					conn->on_read(1024, [](tcp::buffer_t
								       buf) {
						CHECK(buf->size() > 0,
						      "Received a response");
					});
...
void OUTGOING_TEST(Socket outgoing)
{
	...
	stack().tcp().connect(outgoing, [](tcp::Connection_ptr conn) {
		CHECKSERT(conn != nullptr,
			  "Connection successfully established.");
		conn->on_read(small.size(), [](tcp::buffer_t buffer) {
			const std::string str((char *)buffer->data(),
					      buffer->size());
		...
		conn->on_disconnect([](tcp::Connection_ptr conn,
				       tcp::Connection::Disconnect) {
			    CHECK(true, "Connection closed by server");
			    CHECKSERT(conn->is_state({ "CLOSE-WAIT" }),
				      "State: CLOSE-WAIT");
			    conn->close();
...
void Service::start()
{
	...
	extern void create_network_device(int N, const char *route,
					  const char *ip);
	...
	auto &inet = stack();
	inet.network_config({ 10, 0, 0, 44 }, // IP
    ...
    */
	CHECK(tcp.listening_ports() == 0,
	      "No (0) open ports (listening connections)");
	...
	tcp.listen(TEST0).on_connect([](tcp::Connection_ptr conn) {
		...
		conn->on_read(424242, [conn](tcp::buffer_t buffer) {
			...
			if (chunks % 100 == 0) {
				std::string res =
					std::string("Received ") +
					util::Byte_r(recv).to_string() + "\n";
				printf("%s", res.c_str());
				auto new_buf = std::make_shared<
					std::pmr::vector<uint8_t> >(res.begin(),
								    res.end());
				conn->write(new_buf);
			}
			conn->write(buffer);
	...
	tcp.listen(TEST1).on_connect([](tcp::Connection_ptr conn) {
		...
		conn->on_read(small.size(), [conn](tcp::buffer_t buffer) {
			CHECKSERT(std::string((char *)buffer->data(),
					      buffer->size()) == small,
				  "Received SMALL");
    ...
    */
	tcp.listen(TEST2).on_connect([](tcp::Connection_ptr conn) {
		...
		conn->on_read(big.size(), [response,
					   conn](tcp::buffer_t buffer) {
			response->append(std::string{ (char *)buffer->data(),
						      buffer->size() });
    ...
    */
	tcp.listen(TEST3).on_connect([](tcp::Connection_ptr conn) {
		...
		conn->on_read(16384, [temp, conn](tcp::buffer_t buffer) {
			memcpy(temp->data + temp->written, buffer->data(),
			       buffer->size());
    ...
    */
	tcp.listen(TEST4).on_connect([](tcp::Connection_ptr conn) {
		...
		conn->on_disconnect([](auto conn, tcp::Connection::Disconnect) {
			...
			CHECKSERT(conn->is_state({ "FIN-WAIT-2" }),
				  "State: FIN-WAIT-2");
			Timers::oneshot(1s, [conn](auto) {
				CHECKSERT(conn->is_state({ "TIME-WAIT" }),
					  "State: TIME-WAIT");
