...
void Service::start()
{
	...
	INFO("Ping", "host1 => host2 (%s)",
	     host2.ip_addr().to_string().c_str());
	host1.icmp().ping(
		host2.ip_addr(),
		[](auto reply) {
			CHECKSERT(reply, "Got pong from %s",
				  host2.ip_addr().to_string().c_str());
			pongs++;
			verify();
		},
	...
	host1.icmp().ping(
		...
		[](auto reply) {
			CHECKSERT(reply, "Got pong from %s",
				  eth1.ip_addr().to_string().c_str());
			pongs++;
			verify();
		},
		1);

	INFO("Ping", "host2 => host1 (%s)",
	     host1.ip_addr().to_string().c_str());
	host2.icmp().ping(
		host1.ip_addr(),
		[](auto reply) {
			CHECKSERT(reply, "Got pong from %s",
	...
	INFO("TCP", "host1 => host2 (%s:%i)",
	     host2.ip_addr().to_string().c_str(), 5000);
	host1.tcp().connect({ host2.ip_addr(), 5000 }, [](auto conn) {
		CHECKSERT(conn, "Connection established with %s:%i",
			  host2.ip_addr().to_string().c_str(), 5000);
		verify();
	});

	INFO("TCP", "host1 => eth0 (%s:%i)", eth0.ip_addr().to_string().c_str(),
	     5001);
	host1.tcp().connect({ eth0.ip_addr(), 5001 }, [](auto conn) {
		CHECKSERT(conn, "Connection established with %s:%i",
			  eth0.ip_addr().to_string().c_str(), 5001);
		verify();
	});
	...
	host1.tcp().connect({ eth0.ip_addr(), 1337 }, [](auto conn) {
		CHECKSERT(conn, "Connection established with %s:%i",
			  eth0.ip_addr().to_string().c_str(), 1337);
		verify();
	});

	INFO("UDP", "eth0 => listen:4444");
	eth0.udp().bind(4444).on_read(
		[&](auto addr, auto, const char *, size_t) {
			CHECKSERT(addr == eth1.ip_addr(),
				  "Received UDP data from eth1");
			verify();
		});
	std::string udp_data{ "yolo" };
	eth1.udp().bind().sendto(eth0.ip_addr(), 3333, udp_data.data(),
				 udp_data.size());

	// some breathing room
	Timers::oneshot(std::chrono::seconds(1), [](auto) { test_vlan(); });
	Timers::oneshot(std::chrono::seconds(3),
			[](auto) { test_tcp_conntrack(); });
}
...
void test_tcp_conntrack()
{
	...
	static auto &stat =
		Statman::get().get_by_name("eth0.ip4.prerouting_dropped");
	...
	host2.tcp().listen(port, [](auto conn) {
		conn->on_read(1024, [conn](auto buf) {
			std::string str{ (const char *)buf->data(),
					 buf->size() };
			printf("Recv %s\n", str.c_str());
			if (str == "10")
				conn->close();
		...
		conn->on_close([] {
			...
			CHECKSERT(
				(drop_after - drop_before) > 5,
				"At least 6 packets has been denied(dropped) before=%u after=%u",
	...
	host1.tcp().connect({ host2.ip_addr(), port }, [](auto conn) {
		...
		while (i++ < 10) {
			auto n = i;
			Timers::oneshot(std::chrono::milliseconds(n * 500), [n,
									     conn](auto) {
...
void test_vlan()
{
	...
	// host1
	{
		...
		for (uint8_t id = id_start + 1; id < id_end; id++) {
			...
			inet.network_config(addr, netmask,
					    Interfaces::get(0, id).ip_addr());
		}
	}
	...
	INFO("VLAN", "TCP host1.N => host2.1337 (%s:%i)",
	     host2.ip_addr().to_string().c_str(), 4242);
	// establish a connection from every VLAN through the router
	for (uint8_t id = id_start; id < id_end; id++) {
		Timers::oneshot(
			std::chrono::milliseconds(10 * (id - 9)), [id](auto) {
				auto &host = Interfaces::get(2, id);
				host.tcp().connect({ host2.ip_addr(), 4242 },
						   [](auto conn) {
							   assert(conn);
						   });
			});
