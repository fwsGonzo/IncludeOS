...
static bool route_checker(IP6::addr addr)
{
	INFO("Route checker", "asked for route to IP %s",
...
static void ip_forward(IP6::IP_packet_ptr pckt, Inet &stack,
		       Conntrack::Entry_ptr)
{
	...
	if (route == &stack) {
		INFO("ip_fwd",
		     "* Oh, this packet was for me, so why was it forwarded here? \n");
		return;
	}

	debug("[ ip_fwd ] %s transmitting packet to %s", stack.ifname().c_str(),
	      route->ifname().c_str());
...
void Service::start(const std::string &)
{
	...
	Router<IP6>::Routing_table routing_table{
		{ { 0xfe80, 0, 0, 0, 0xabcd, 0xabcd, 0x1234, 0 },
		  112,
		  { 0xfe80, 0, 0, 0, 0xabcd, 0xabcd, 0x1234, 0x8367 },
		  inet2,
		  1 },
		{ { 0xfe80, 0, 0, 0, 0xe823, 0xfcff, 0xfef4, 0 },
		  112,
	...
	for (auto r : routing_table)
		INFO2("* %s/%i -> %s / %s, cost %i", r.net().str().c_str(),
		      ...
		      r.nexthop().to_string().c_str(), r.cost());
	printf("\n");
