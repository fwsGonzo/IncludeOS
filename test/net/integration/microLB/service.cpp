...
void print_nic_stats()
{
	...
	printf("eth1.sendq_max: %zu, eth1.sendq_now: %zu "
	       ...
	       Statman::get()
		       .get_by_name("eth1.stat_rx_total_packets")
		       .get_uint64(),
	       Statman::get()
		       .get_by_name("eth1.stat_tx_total_packets")
		       .get_uint64(),
	       Statman::get()
		       .get_by_name("eth1.stat_rx_total_bytes")
		       .get_uint64(),
	       Statman::get()
		       .get_by_name("eth1.stat_tx_total_bytes")
		       .get_uint64(),
	       Statman::get().get_by_name("eth1.sendq_dropped").get_uint64(),
	       Statman::get()
		       .get_by_name("eth1.rx_refill_dropped")
		       .get_uint64());
	printf("\n\n");
}
...
void print_mempool_stats()
{
	...
	printf("\n*** TCP0 ***\n%s\n pool: %zu / %zu allocs: %zu resources: %zu (used: %zu free: %zu)\n\n",
	       inet1.tcp().to_string().c_str(), pool1.allocated(),
	       pool1.total_capacity(), pool1.alloc_count(),
	       pool1.resource_count(), pool_ptr1->used_resources(),
	       pool_ptr1->free_resources());
	printf("*** TCP1 ***\n%s\npool: %zu / %zu allocs: %zu resources: %zu (used: %zu free: %zu)\n",
	       inet2.tcp().to_string().c_str(), pool2.allocated(),
	       pool2.total_capacity(), pool2.alloc_count(),
...
void print_lb_stats()
{
	...
	printf("Wait queue: %i nodes: %zu tot_sess: %zi open_sess: %i timeout_sess: %i pool_size: %i \n",
	       balancer->wait_queue(), nodes.size(), nodes.total_sessions(),
	       nodes.open_sessions(), nodes.timed_out_sessions(),
	       nodes.pool_size());
	printf("\n\n");
}
...
void Service::start()
{
	...
	Timers::oneshot(std::chrono::seconds(5),
			[](int) { printf("TCP MSL ended (4 seconds)\n"); });
	// StackSampler::begin();
