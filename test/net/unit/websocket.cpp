...
{
	dev1 = std::make_unique<hw::Async_device<UserNet> >(
		UserNet::create(1500));
	dev2 = std::make_unique<hw::Async_device<UserNet> >(
		UserNet::create(1500));
	dev1->connect(*dev2);
	dev2->connect(*dev1);
...
CASE("Open and close websocket")
{
	// FIXME
	return;
	websocket_do_thing([](net::WebSocket_ptr webs, bool &done) {
		auto *ws = webs.release();
		ws->on_close = [&](uint16_t reason) {
			printf("Reason: %s (%d)\n",
			       net::WebSocket::status_code(reason), reason);
			assert(std::string(net::WebSocket::status_code(
				       reason)) == "Closed");
			done = true;
