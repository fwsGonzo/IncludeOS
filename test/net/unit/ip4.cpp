...
std::map<net::Protocol, int> pass_count{ { net::Protocol::UDP, 0 },
					 { net::Protocol::TCP, 0 },
...
void ip_drop(net::Packet_ptr, net::IP4::Direction dir,
	     net::IP4::Drop_reason reason)
{
...
CASE("IP4 is still a thing")
{
	...
	SETUP("A pre-wired IP4 instance with custom upstream handlers")
	{
		...
		SECTION("Raw, uninitialized packets get dropped")
		{
			...
			EXPECT_DROP(std::move(raw_pckt), Direction::Upstream,
				    Drop_reason::Wrong_version);
		...
		SECTION("IP Packets with invalid checksum gets dropped")
		{
			...
			EXPECT_DROP(std::move(ip_pckt1), Direction::Upstream,
				    Drop_reason::Wrong_checksum);

			// Explicitly setting a wrong checksum for packet 1 cause drop as well
			ip_pckt2->set_ip_checksum(987);

			EXPECT_DROP(std::move(ip_pckt2), Direction::Upstream,
				    Drop_reason::Wrong_checksum);
		...
		SECTION("IP packets with bad destination address gets dropped")
		{
			...
			EXPECT_DROP(std::move(ip_pckt), Direction::Upstream,
		...
		SECTION("IP packets with bad source address gets dropped")
		{
...

			{
				...
				EXPECT(ip_packets_dropped == dropcount + 1);
				EXPECT(last_drop_direction ==
				       IP4::Direction::Downstream);
				EXPECT(last_drop_reason ==
				       IP4::Drop_reason::Bad_source);
			}
		...
		{
			auto ip_pckt =
				inet.create_ip_packet((Protocol)16); // CHAOS
