...
net::tcp::sack::Entries
expected(std::initializer_list<net::tcp::sack::Block> blocks)
{
	return { blocks };
}
...
CASE("SACK Fixed List implementation [RFC 2018]")
{
    ...
    */
	...
	res = sack_list.recv_out_of_order(7000, 500);
	EXPECT(sack_list.recent_entries() ==
	       expected({ { 7000, 7500 }, { 6000, 6500 } }));

	// 5500    8000   8500   7000   7500   6000   6500
	res = sack_list.recv_out_of_order(8000, 500);
	EXPECT(sack_list.recent_entries() ==
	       expected({ { 8000, 8500 }, { 7000, 7500 }, { 6000, 6500 } }));
...
CASE("SACK block list is full")
{
	...
	EXPECT(sack_list.recent_entries() ==
	       expected({ { seq - incr, (seq - incr) + blksz },
	...
	EXPECT(sack_list.recent_entries() ==
	       expected({ { seq - incr, (seq - incr) + blksz + blksz },
			  { seq - incr * 2, (seq - incr * 2) + blksz },
			  { seq - incr * 3, (seq - incr * 3) + blksz } }));
