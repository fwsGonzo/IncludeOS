...
CASE("Creating a WriteQueue and operate it"){
	GIVEN("An empty WriteQueue"){ Write_queue wq;
...
WHEN("A WriteRequest is inserted")
{
	...
	THEN("The size is increased, it has remaining requests, nxt() & una() points to the same "
	     "buffer, but current stays the same")
	{
		EXPECT(not wq.empty());
		EXPECT(wq.size() == 1u);
		...
		WHEN("The queue is advanced by 1000 bytes")
		{
			...
			THEN("The queue has no remaining requests, user callback should be complete, and "
			     "current should be 1")
			{
				...
				WHEN("The queue is acknowledged with 1000 bytes")
				{
					...
					THEN("The queue is empty and current is 0")
					{
						...
						EXPECT(wq.bytes_total() == 0u);
						EXPECT(wq.bytes_remaining() ==
						       0u);
						EXPECT(wq.bytes_unacknowledged() ==
						       0u);
					}
				}
			}
...
WHEN("Several WriteRequests are inserted")
{
	...
	int i = 0;
	wq.on_write([&written, &i](size_t n) mutable { written[i++] = n; });
		...
		WHEN("Advanced with 2500 bytes")
		{
			...
			THEN("The nxt() is the third element and una() is still the first element, current is 2")
			{
				...
				WHEN("Acknowledged with 2500 bytes")
				{
					...
					THEN("nxt() and una() is the third element, size has shrunk to 3, and current "
					     "is 0")
					{
						EXPECT(wq.nxt() == reqs[2]);
						...
						WHEN("The queue is reset")
						{
							...
							THEN("The queue is empty and on write has returned correct values on "
							     "the fully/half completed ones [1000, 1000, 500]")
							{
								EXPECT(wq.empty());
								EXPECT(wq.size() ==
								       0u);
