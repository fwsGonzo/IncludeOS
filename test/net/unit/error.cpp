...
CASE("Creating an ICMP_error of type Too Big")
{
	...
	EXPECT(err.to_string() ==
	       "ICMP DESTINATION UNREACHABLE (3): FRAGMENTATION NEEDED (4)");
...
{
	ICMP_error err{ icmp4::Type::DEST_UNREACHABLE,
			(uint8_t)icmp4::code::Dest_unreachable::PORT };
	...
	err.set_icmp_code((
		uint8_t)icmp4::code::Parameter_problem::POINTER_INDICATES_ERROR);
	...
	EXPECT(err.to_string() ==
	       "ICMP PARAMETER PROBLEM (12): POINTER INDICATES ERROR (0)");
	EXPECT(not err.is_too_big());
	EXPECT(err.pmtu() == 0);
}
