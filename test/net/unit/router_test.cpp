...
{
	const Route<IP4> r1{ { 10, 42, 42, 0 },
			     { 255, 255, 255, 0 },
			     { 10, 42, 42, 2 },
			     *eth1,
			     2 };
	const Route<IP4> r2{ { 10, 42, 0, 0 },
			     { 255, 255, 0, 0 },
			     { 10, 42, 42, 3 },
			     *eth2,
			     1 };
...
{
	Router<IP4>::Routing_table tbl{ { { 10, 42, 42, 0 },
					  { 255, 255, 255, 0 },
					  { 10, 42, 42, 2 },
					  *eth1,
					  2 },
	...
	std::copy_if(tbl.begin(), tbl.end(), std::back_inserter(filtered),
		     [](const Route<IP4> &r) {
			     return r.match({ 10, 42, 42, 3 }) != nullptr;
		     });
	...
	EXPECT((routes.front().nexthop() == IP4::addr{ 10, 42, 42, 2 } or
		routes.front().nexthop() == IP4::addr{ 10, 42, 42, 3 }));

	// Get the cheapest route for an IP
	auto route = router.get_cheapest_route({ 10, 42, 42, 10 });
	EXPECT((route->nexthop() == IP4::addr{ 10, 42, 42, 2 } and
	...
	router.set_routing_table({ { { 10, 42, 43, 0 },
				     { 255, 255, 255, 0 },
				     { 10, 42, 42, 2 },
				     *eth1,
				     2 } });
...
CASE("net::router: Actual routing verifying TTL")
{
	...
	inet1.ip_obj().set_linklayer_out([&](auto pckt, auto ip) {
		auto packet =
			static_unique_ptr_cast<net::PacketIP4>(std::move(pckt));
		...
		EXPECT(err.icmp_code() ==
		       static_cast<uint8_t>(icmp4::code::Time_exceeded::TTL));
	...
	inet2.ip_obj().set_linklayer_out([&](auto pckt, auto ip) {
		auto packet =
			static_unique_ptr_cast<tcp::Packet>(std::move(pckt));
	...
	router.forward_chain.chain.push_back(
		[](IP4::IP_packet_ptr pkt, Inet &,
		   Conntrack::Entry_ptr) -> Filter_verdict<IP4> {
			return Filter_verdict<IP4>{
				std::move(pkt), Filter_verdict_type::ACCEPT
			};
		});
	...
	router.forward_chain.chain.push_back(
		[](IP4::IP_packet_ptr pkt, Inet &,
		   Conntrack::Entry_ptr) -> Filter_verdict<IP4> {
			return Filter_verdict<IP4>{ std::move(pkt),
						    Filter_verdict_type::DROP };
		});
...
CASE("net::router: Calculate Route nexthop")
{
	...
	EXPECT(r1.nexthop({ 10, 0, 2, 20 }) ==
	       ip4::Addr(10, 0, 2, 20)); // == ip
	...
	const Route<IP4> r2{
		{ 10, 0, 6, 0 }, { 255, 255, 255, 0 }, { 10, 0, 2, 10 }, inet1
	};
	// Matches routes net, but not the interface's net, need to go via nexthop
	EXPECT(r2.nexthop({ 10, 0, 6, 20 }) ==
	       ip4::Addr(10, 0, 2, 10)); // == nexthop
	// Ok, this is really weird because you will never ask for nexthop on this route
	// (but interface will match)..
	EXPECT(r2.nexthop({ 10, 0, 2, 20 }) ==
	       ip4::Addr(10, 0, 2, 20)); // == ip
	...
	// Default route
	const Route<IP4> r3{
		{ 0, 0, 0, 0 }, { 0, 0, 0, 0 }, { 10, 0, 1, 1 }, inet2
	};
	// Matches routes net (duh), but not the interface, go via nexthop
	EXPECT(r3.nexthop({ 10, 0, 2, 10 }) ==
	       ip4::Addr(10, 0, 1, 1)); // == nexthop
	// Matches routes net (duh), and can be sent directly
	EXPECT(r3.nexthop({ 10, 0, 1, 10 }) ==
	       ip4::Addr(10, 0, 1, 10)); // == ip
}
