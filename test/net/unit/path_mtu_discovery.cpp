...
CASE("An ICMP Too Big message with a next hop MTU value of 45 is invalid")
{
	...
	ICMP_error err{
		icmp4::Type::DEST_UNREACHABLE,
		(uint8_t)icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 45
	};
...
CASE("An ICMP Too Big message with a next hop MTU value of 68 is valid")
{
	...
	ICMP_error err{
		icmp4::Type::DEST_UNREACHABLE,
		(uint8_t)icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 68
	};
	bool too_big = err.is_too_big();
...
CASE("Turning Path MTU Discovery off clears the PMTU cache")
{
	...
	ICMP_error err{
		icmp4::Type::DEST_UNREACHABLE,
		(uint8_t)icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED,
		1400
	};
	bool too_big = err.is_too_big();
	...
	inet.set_path_mtu_discovery(
		false); // Clears the PMTU cache and stops the timer if is running
...
CASE("No PMTU entry exists for non-existing path")
{
	...
	EXPECT(inet.ip_obj().pmtu(Socket{ ip4::Addr{ 10, 0, 0, 45 }, 80 }) ==
	       0);
}
...
CASE("An ICMP Too Big message with a next hop MTU value of 45 is invalid")
{
  ...
  ICMP_error err{icmp4::Type::DEST_UNREACHABLE, (uint8_t)
icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 45}; bool too_big = err.is_too_big();
...
CASE("An ICMP Too Big message with a next hop MTU value of 68 is valid")
{
  ...
  ICMP_error err{icmp4::Type::DEST_UNREACHABLE, (uint8_t)
icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 68}; bool too_big = err.is_too_big();
...
CASE("Turning Path MTU Discovery off clears the PMTU cache")
{
  ...
  ICMP_error err{icmp4::Type::DEST_UNREACHABLE, (uint8_t)
icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 1400}; bool too_big = err.is_too_big(); Socket
dest{{10,0,0,48}, 443};
...
CASE("The PMTU cache/map can be flushed")
{
  ...
  ICMP_error err1{icmp4::Type::DEST_UNREACHABLE, (uint8_t)
icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 1000}; bool too_big1 = err1.is_too_big();
  Socket dest1{{10,0,0,5}, 80};

  ICMP_error err2{icmp4::Type::DEST_UNREACHABLE, (uint8_t)
icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED, 900}; bool too_big2 = err2.is_too_big(); Socket
dest2{{10,0,0,6}, 80};
