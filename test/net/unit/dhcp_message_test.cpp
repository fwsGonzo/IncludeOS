...
CASE("Reading Message and parsing options with Message_view")
{
	...
	EXPECT(anon_opt->val[0] ==
	       static_cast<uint8_t>(message_type::DISCOVER));
	...
	int i = 0;
	auto y = view.parse_options([&i](const option::base *opt) { ++i; });
	EXPECT(i == 3);
	EXPECT(y == i);
}
...
CASE("Creating Message and adding options with Message_view")
{
	...
	Message_writer view{ &buffer[0], op_code::BOOTREQUEST,
	...
	view.add_option<option::param_req_list>(
