...
static void setup_networks()
{
	...
	inet_client.network_config({ 10, 0, 0, 42 }, { 255, 255, 255, 0 },
				   { 10, 0, 0, 1 });
...
void Service::start()
{
	...
	for (uint16_t i = 6001; i <= 6004; i++) {
		...
		balancer->nodes.add_node(
			socket, microLB::Balancer::connect_with_tcp(inet_server,
								    socket));
	}

	balancer->nodes.on_session_close = [](int idx, int current, int total) {
