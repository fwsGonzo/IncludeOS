		       ...
		       const uint8_t *data, const size_t size)
{
	...
	if (config.layer == IP6 || config.layer == ICMP6) {
		// link layer -> IP6
		eth_end = add_eth_layer(p->layer_begin(), fuzzer,
					net::Ethertype::IP6);
	} else {
		// link layer -> IP4
		eth_end = add_eth_layer(p->layer_begin(), fuzzer,
	...
	case ETH:
		...
		break;
	case IP4: {
		auto *next_layer = add_ip4_layer(
			eth_end, fuzzer, { 10, 0, 0, 1 }, inet.ip_addr());
	...
	case IP6: {
		...
		auto *next_layer = add_ip6_layer(eth_end, fuzzer, src,
						 inet.ip6_addr(), proto);
		fuzzer.fill_remaining(next_layer);
		break;
	}
	case UDP: {
		...
		auto *ip_layer = add_ip4_layer(eth_end, fuzzer, { 10, 0, 0, 1 },
					       inet.ip_addr(),
					       (uint8_t)net::Protocol::UDP);
		auto *udp_layer = add_udp4_layer(ip_layer, fuzzer, udp_port);
	...
	case TCP: {
		...
		auto *tcp_layer =
			add_tcp4_layer(ip_layer, fuzzer, config.ip_src_port,
				       config.ip_port, config.tcp_seq,
				       config.tcp_ack);
	...
	case ICMP6: {
		...
		auto *ip_layer = add_ip6_layer(eth_end, fuzzer, src,
					       inet.ip6_addr(), proto);
	...
	case DNS: {
		...
		auto *udp_layer = add_udp4_layer(ip_layer, fuzzer, udp_port);
		auto *dns_layer =
			add_dns4_layer(udp_layer, fuzzer, net::dns::g_last_xid);
