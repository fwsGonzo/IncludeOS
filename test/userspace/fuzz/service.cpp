...
void Service::start()
{
	...
	inet_server.network_config({ 10, 0, 0, 42 }, { 255, 255, 255, 0 },
				   { 10, 0, 0, 1 });
	auto &inet_client = net::Interfaces::get(1);
	inet_client.network_config({ 10, 0, 0, 43 }, { 255, 255, 255, 0 },
				   { 10, 0, 0, 1 });
	...
	if (mode == "DNS" || mode == "UDP") {
		inet_server.resolve(
			"www.oh.no",
			[](net::dns::Response_ptr resp,
			   const net::Error &error) -> void {
				(void)resp;
				(void)error;
				printf("!!\n!! resolve() call ended\n!!\n");
			});
	} else if (mode == "TCP") {
		inet_server.tcp().listen(TCP_PORT, [](auto connection) {
			// connection->write("test");
		});

		static bool connected = false;
		auto conn = inet_client.tcp().connect(
			{ inet_server.ip_addr(), TCP_PORT });
		conn->on_connect([](auto connection) { connected = true; });
	...
	dev1->set_transmit([](net::Packet_ptr packet) {
		(void)packet;
		// fprintf(stderr, "."); // drop
	});
...
extern net::tcp::Connection_ptr deserialize_connection(void *addr,
						       net::TCP &tcp);
...
static void test_tcp(const uint8_t *data, size_t size)
{
	...
	// IP-stack fuzzing
	const fuzzy::stack_config config{ .layer = fuzzy::TCP,
					  .ip_port = TCP_PORT,
...
{
	const fuzzy::stack_config config{ .layer = fuzzy::UDP, .ip_port = 0 };
	insert_into_stack(dev1, config, data, size);
}
