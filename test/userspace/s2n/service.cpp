...
{
	return std::make_unique<fuzzy::Stream>(
...
struct Testing {
	...
	void onread_function(net::Stream::buffer_t buffer)
	{
		...
		if (read_buffer == "Hello!")
			this->test_stage_advance();
		else if (read_buffer == "Second write")
			this->test_stage_advance();
		else if (read_buffer == long_string)
			this->test_stage_advance();
...
void do_test_serializing_tls(int index)
{
	...
	// 2.3.1:
	auto dstream =
		s2n::TLS_stream::deserialize_from(s2n::serial_get_config(),
						  std::move(server_side), false,
						  sbuffer, sbytes);
	assert(dstream != nullptr && "Deserialization must return stream");
	server_test.stream = dstream.release();

	dstream = s2n::TLS_stream::deserialize_from(s2n::serial_get_config(),
						    std::move(client_side),
						    false, cbuffer, cbytes);
	assert(dstream != nullptr && "Deserialization must return stream");
...
void Service::start()
{
	...
	fs::memdisk().init_fs(
		[](fs::error_t err, fs::File_system &) { assert(!err); });
	...
	server_test.index = 0;
	server_test.stream = new s2n::TLS_stream(s2n::serial_get_config(),
						 std::move(server_side), false);
	client_test.index = 1;
	client_test.stream = new s2n::TLS_stream(s2n::serial_get_config(),
						 std::move(client_side), true);
