...
namespace s2n
{
void serial_test(const std::string &ca_cert, const std::string &ca_key)
{
#ifdef __includeos__
...
s2n_config *serial_create_config()
{
	...
	int res = s2n_config_add_cert_chain_and_key(
		config, stored_ca_cert.c_str(), stored_ca_key.c_str());
	if (res < 0) {
	...
	res = s2n_config_set_verify_host_callback(
		config, verify_host_passthrough, nullptr);
	if (res < 0) {
