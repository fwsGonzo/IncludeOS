...
{
	using Alloc =
		util::alloc::detail::Lstack<util::alloc::Lstack_opt::no_merge>;
	Alloc heap;
...
CASE("lstack::nodes: testing lstack<no_merge> node traversal")
{
	...
	EXPECT((uintptr_t)(*(heap.find_largest())) ==
	       (uintptr_t)((char *)first_begin + sz));
	print_summary(heap);
	...
	EXPECT((uintptr_t)pop5 ==
	       (uintptr_t)first_begin + sz + sz2 + sz3 + sz4);
	EXPECT(heap.node_count() == 1);

	auto sz6 = sz * 6;
	auto pop6 = heap.pop_off(sz6);
	EXPECT((uintptr_t)pop6 ==
	       (uintptr_t)first_begin + sz + sz2 + sz3 + sz4 + sz5);
	EXPECT(heap.node_count() == 1);
...
CASE("lstack::nodes: testing lstack<merge> node traversal")
{
	using namespace util::literals;
	using Alloc =
		util::alloc::detail::Lstack<util::alloc::Lstack_opt::merge>;
	test::pool<Alloc, 1_MiB> pool;
	Alloc heap = pool.stack;
	;
	EXPECT(heap.bytes_allocated() == 0);
