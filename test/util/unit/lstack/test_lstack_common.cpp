...
CASE("lstack::" STR(LSTACK_OPT) " basics")
{
	...
	while (heap.bytes_free() >= heap.min_alloc) {
		size_t size = std::max(bits::roundto(heap.align,
						     heap.bytes_free() / 10),
				       heap.min_alloc);
	...
	for (auto alloc : allocs) {
		...
		if
			constexpr(heap.is_sorted)
			{
				if ((uintptr_t)alloc.ptr + alloc.size <
					    heap.pool_end() and
				    !highest_returned) {
					EXPECT(heap.allocation_end() ==
					       heap.pool_end());
				}
			}
	...
	while (heap.bytes_free() >= heap.min_alloc) {
		...
		alloc::Allocation allocation{
			heap.allocate(size), bits::roundto(heap.min_alloc, size)
		};
		EXPECT(heap.bytes_free() == old_use - allocation.size);
		if ((uintptr_t)allocation.ptr + allocation.size > alloc_max) {
			alloc_max = (uintptr_t)allocation.ptr + allocation.size;
...
CASE("lstack::" STR(LSTACK_OPT) " fragmentation ")
{
	...
	if
		constexpr(heap.is_sorted)
		{
			...
			heap.deallocate(a3);
			heap.deallocate(a5);
			EXPECT(heap.bytes_free() ==
			       a1.size + a3.size + a5.size);
...
CASE("lstack::" STR(LSTACK_OPT) " random allocs")
{
	...
	for (auto rnd : rnds) {
		...
		EXPECT(((uintptr_t)a.ptr >= heap.pool_begin() and
			(uintptr_t) a.ptr < heap.pool_end()));
		allocs.push_back(a);
...
CASE("lstack::" STR(LSTACK_OPT) " make_unique")
{
	...
	EXPECT(((uintptr_t)uptr.get() >= pool.begin() and
		(uintptr_t) uptr.get() < pool.end()));
	EXPECT(heap.bytes_free() < 1_MiB);
	uptr.reset();
	EXPECT(heap.bytes_free() == 1_MiB);
}
