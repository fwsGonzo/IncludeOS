...
CASE("Creating path_to_regex with no options")
{
	GIVEN("An empty vector of Tokens (keys) and no options")
	{
		...
		WHEN("Calling path_to_regex with path '/:test'")
		{
			...
			// Tested when testing parse-method really, except keys don't contain Tokens where
			// is_string is true
			EXPECT(t.name == "test");
			EXPECT(t.prefix == "/");
			EXPECT(t.delimiter == "/");
			...
			THEN("Only paths with one parameter should match. This can contain any character")
			{
				...
				EXPECT_NOT(std::regex_match("/route/123", r));
				EXPECT_NOT(std::regex_match(
					"/route/something/somethingelse12", r));
				EXPECT_NOT(std::regex_match("route", r));
		...
		WHEN("Calling path_to_regex with path '/:test/:date'")
		{
			...
			THEN("Only paths with two parameters should match. This can contain any character")
			{
				...
				EXPECT(std::regex_match("/route!)#/route321'",
							r));
			}
		}
		...
		{
			std::regex r =
				path_to_regex("/users/:test/:date", keys);
			...
			THEN("Only paths with three elements should match, starting with /users")
			{
				...
				EXPECT_NOT(std::regex_match(
					"/users/something/123/else", r));
				EXPECT_NOT(std::regex_match(
					"/route/something/somethingelse12", r));

				EXPECT(std::regex_match("/users/123/something",
							r));
				EXPECT(std::regex_match("/users/some12p-?/523",
							r));
				EXPECT(std::regex_match(
					"/users/route!)#/route321'", r));
			}
		}
		...
		WHEN("Calling path_to_regex with path '/test'")
		{
			...
			THEN("Only the path '/test' should match")
			{
				...
				EXPECT_NOT(std::regex_match(
					"/test/something/somethingelse12", r));
		...
		WHEN("Calling path_to_regex with path '/test/users'")
		{
			...
			THEN("Only the path '/test/users' should match")
			{
				...
				EXPECT_NOT(
					std::regex_match("/test/users/123", r));
				EXPECT_NOT(std::regex_match(
					"/test/something/somethingelse12", r));
		...
		WHEN("Calling path_to_regex with path '/:test?/:date?'")
		{
			...
			{
				EXPECT_NOT(std::regex_match(
					"/matilda/123/2016-08-20", r));
				EXPECT_NOT(std::regex_match(
					"/users/matilda/2016-08-20/something",
					r));
				EXPECT_NOT(std::regex_match(
					"/route/something/somethingelse12", r));
				...
				EXPECT(std::regex_match("/2016-08-20/matilda",
							r));
				EXPECT(std::regex_match("/matilda/2016-08-20",
							r));
			}
		}
		...
		{
			std::regex r =
				path_to_regex("/:test?/users/:date?", keys);
			...
			THEN("Only paths with zero parameters (but containing /users), one parameter (but "
			     "containing /users), or two parameters (but containing /users) should match")
			{
				EXPECT_NOT(std::regex_match("/", r));
				EXPECT_NOT(std::regex_match(
					"/users/something/matilda123", r));
				EXPECT_NOT(std::regex_match(
					"/matilda/123/2016-08-20", r));
				EXPECT_NOT(std::regex_match(
					"/users/matilda/2016-08-20/something",
					r));
				EXPECT_NOT(std::regex_match(
					"/route/users/somethingelse12/123", r));
				EXPECT_NOT(std::regex_match(
					"/123/matilda/users", r));
				...
				EXPECT(std::regex_match(
					"/matilda/users/2016-08-20", r));
				EXPECT(std::regex_match("/users/2016-08-20",
							r));
			}
		}
		...
		WHEN("Calling path_to_regex with path '/:test/:date?'")
		{
			...
			THEN("Only paths with one or two parameters should match")
			{
				...
				EXPECT_NOT(std::regex_match(
					"/matilda/123/2016-08-20", r));
				EXPECT_NOT(std::regex_match(
					"/test/matilda/2016-08-20/something",
					r));
		...
		WHEN("Calling path_to_regex with path '/:test*'")
		{
			...
			THEN("Paths with zero or more parameters should match")
			{
				...
				EXPECT(std::regex_match(
					"/users/something/matilda123", r));
				EXPECT(std::regex_match(
					"/test/something/matilda123/2016-08-15/somethingelse/testing",
					r));
			}
		}
		...
		{
			std::regex r =
				path_to_regex("/:test([a-z])+/:id(\\d+)", keys);
			...
			THEN("Paths with two parameters (or more of the first parameter) where the first "
			     "contains one lower case letter and the second contains one or more integers "
			     "should match")
			{
				EXPECT_NOT(std::regex_match("/", r));
				...
				EXPECT_NOT(
					std::regex_match("/b/2016-08-20", r));
		...
		WHEN("Calling path_to_regex with path '/:test(\\d+)'")
		{
			...
			THEN("Paths with one parameter that contains one or more integers should match")
			{
				...
				EXPECT(std::regex_match("/1234563728192938",
							r));
			}
		}
	    ...
	    std::regex test_regex_2("[a-z]+", std::regex_constants::ECMAScript |
	    std::regex_constants::icase); EXPECT(std::regex_match("A", test_regex_2));  // OK
	    ...
	    // Basic example for testing icase end */
			...
			THEN("Paths with one parameter containing one or more lower case letters should match")
			{
				...
				EXPECT_NOT(std::regex_match("/", r));
				EXPECT_NOT(
					std::regex_match("/matilda/users", r));
				...
				EXPECT(std::regex_match(
					"/A",
					r)); // OK WHEN NOT CASE SENSITIVE (icase set)
				// EXPECT(std::regex_match("/B", r));        // FAILS WHEN NOT CASE SENSITIVE (icase
				// set) EXPECT(std::regex_match("/Z", r));        // FAILS WHEN NOT CASE SENSITIVE
				// (icase set) EXPECT(std::regex_match("/Matilda", r));  // FAILS WHEN NOT CASE
				// SENSITIVE (icase set)

				EXPECT(std::regex_match("/a", r));
				EXPECT(std::regex_match("/users", r));
				EXPECT(std::regex_match(
					"/somethingelseentirely", r));
			}
		}
		...
		WHEN("Calling path_to_regex with path '/:test/:id(\\d+)'")
		{
			...
			THEN("Paths with two parameters where the second contains one or more integers should "
			     "match")
			{
				EXPECT_NOT(std::regex_match("/", r));
				EXPECT_NOT(std::regex_match("/m", r));
		...
		WHEN("Calling path_to_regex with path '/(.*)'")
		{
			...
			THEN("Paths with one parameter containing zero or more characters should match")
			{
				...
				EXPECT(std::regex_match(
					"/matilda/users",
					r)); // Because of .*: Any character allowed, incl. /
				EXPECT(std::regex_match("/", r));
		...
		WHEN("Calling path_to_regex with path '/users/([a-z]+)/(.?)'")
		{
			...
			THEN("Paths with three elements, where the first parameter contains one or more lower "
			     "case letters and the second parameter contains zero or one character, should "
			     "match")
			{
				EXPECT_NOT(std::regex_match("/", r));
				EXPECT_NOT(std::regex_match("/users", r));
				...
				// Default (is the case here) is that the regex is not case sensitive (sensitive is
				// false)
		...
		WHEN("Calling path_to_regex with path '/:id(\\d+)/:date/*'")
		{
			...
			THEN("Paths with three parameters, where the first parameter contains one or more "
			     "integers and the third contains zero or more characters, should match")
			{
				EXPECT_NOT(std::regex_match("", r));
				EXPECT_NOT(std::regex_match(
					"/b/optional-text/optional", r));
				EXPECT_NOT(std::regex_match("/1/matilda", r));
				EXPECT_NOT(std::regex_match(
					"/172!optional-text/abcd2918230", r));

				EXPECT(std::regex_match("/0/optional-text/",
							r));
				EXPECT(std::regex_match("/0/optional-text/1",
							r));
				EXPECT(std::regex_match(
					"/123/optional-text132/45", r));
				EXPECT(std::regex_match(
					"/2918293/172!optional-text/abcd2918230",
					r));
				EXPECT(std::regex_match("/2/143/", r));
				EXPECT(std::regex_match("/2/143/something", r));
				EXPECT(std::regex_match(
					"/2/143/something/somethingelse/and/so/on",
					r)); // Because of *: Any character allowed, incl. /
			}
		}
	}
