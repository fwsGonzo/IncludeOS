...
#ifdef DEBUG_UNIT
	std::cout << "Allocator root @ 0x" << std::hex << alloc.root().addr()
		  << " size: " << std::dec << alloc.root().size() << "\n";
#endif
...
CASE("mem::buddy random ordered allocation then deallocation")
{
	...
	for (auto rnd : test::random_1k) {
		...
		const auto sz = alloc.chunksize(
			rnd % std::max(size_t(32_KiB),
				       pool.size / test::random_1k.size()));
	...
	uintptr_t hi_used =
		(uintptr_t)addresses.at(highest_i) + sizes.at(highest_i);
...
struct Allocation {
	...
	{
		return overlaps(other.addr_begin()) or
		       overlaps(other.addr_end() - 1);
	}
...
CASE("mem::buddy random chaos with data verification")
{
	...
	for (auto rnd : test::random_1k) {
		auto sz = std::max<size_t>(rnd % alloc.pool_size_ / 1024,
					   alloc.min_size);
		EXPECT(sz);
		...
		if (not alloc.full()) {
			...
			EXPECT(a.overlaps(a));
			auto overlap =
				std::find_if(allocs.begin(), allocs.end(),
					     [&a](Allocation &other) {
		...
		if (rnd % 3 or alloc.full()) {
			...
			EXPECT(alloc.bytes_used() ==
			       use_pre - alloc.chunksize(a->size));
			allocs.erase(a);
		}
	}
...
CASE("mem::buddy as std::allocator")
{
	...
	std::vector<int, os::mem::Allocator<int, Pool::Alloc> > numbers(
		*resource);
