...
{
	uri::URI uri{
		"http://www.digi.no/artikler/nytt-norsk-operativsystem-vekker-oppsikt/319971"
	};
	EXPECT(uri.path() ==
	       "/artikler/nytt-norsk-operativsystem-vekker-oppsikt/319971");
}
...
CASE("URI construction, assignment")
{
	...
	const uri::URI uri4{
		"http://example.com/test/?param1=test&param2=foo&param3=bar"
	};
	uri::URI uri5{ uri4 };
	EXPECT(uri4 == uri5);
	EXPECT_NOT(uri == uri5);
