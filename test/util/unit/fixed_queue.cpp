...
void test_basic(lest::env &lest_env, T val_a, T val_b)
{
	SETUP("fixed_queue test_basic")
	{
		...
		{
			size_t count_folds{};
			fq.fold([&count_folds](const auto &val) -> void {
				++count_folds;
			});
...
template <typename T, size_t N>
void test_range(lest::env &lest_env, T val_a, T val_b)
{
...
CASE("Test fixed_queue for pod types")
{
	...
	test_basic<float, 3>(lest_env, std::numeric_limits<float>::min(),
...
CASE("Test fixed_queue with range types")
{
	...
	test_range<std::string, 100>(le, "short",
				     "a bit longer, and even more");
}
