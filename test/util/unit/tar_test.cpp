...
CASE("Reading single entry tar file")
{
	...
	EXPECT_NOT(fd == -1);
	const uint8_t *mem =
		(const uint8_t *)mmap(0, size, PROT_READ, MAP_PRIVATE, fd, 0);
	...
	EXPECT_THROWS_AS(auto &missing_element = tar.element("not_there.txt"),
			 tar::Tar_exception);

	// try to decompress a non-tar.gz element
	EXPECT_THROWS_AS(tar::Tar inner_tar = r.decompress(e),
			 tar::Tar_exception);
	close(fd);
}
...
CASE("Reading invalid tar file")
{
	...
	const uint8_t *mem =
		(const uint8_t *)mmap(0, size, PROT_READ, MAP_PRIVATE, fd, 0);
	EXPECT_THROWS_AS(tar::Tar tar = r.read_uncompressed(mem, size),
			 tar::Tar_exception);
	close(fd);
}
...
CASE("Reading corrupt tar.gz file")
{
	...
	const uint8_t *mem =
		(const uint8_t *)mmap(0, size, PROT_READ, MAP_PRIVATE, fd, 0);
	EXPECT_THROWS_AS(tar::Tar tar = r.decompress(mem, size),
			 tar::Tar_exception);
	close(fd);
}
