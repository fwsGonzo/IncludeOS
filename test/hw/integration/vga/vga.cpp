...
void Service::start()
{
	...
	OS::set_rsprint(
		[](const char *data, size_t len) { vga.write(data, len); });
	...
	auto test1_1 = []() -> bool {
		if (c >= '4') {
			hw::PIT::instance().onRepeatedTimeout(
				100ms,
				[] {
					vga.newline();
					iterations++;
	...
	auto test3 = [test1, test1_1]() {
		...
		vga.setColor(vga.make_color(ConsoleVGA::COLOR_WHITE,
					    ConsoleVGA::COLOR_BLACK));
