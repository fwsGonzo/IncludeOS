...
int main()
{
	...
	Syslog::syslog(LOG_INFO,
		       "(Info) No open has been called prior to this");
	Syslog::syslog(LOG_NOTICE,
		       "(Notice) Program created with two arguments: %s and %s",
		       one.c_str(), two.c_str());
	...
	Syslog::syslog(
		LOG_ERR,
		"(Err) Log after prepended message with one argument: %d", 44);
	Syslog::syslog(
		LOG_WARNING,
		"(Warning) Log number two after openlog set prepended message");

	Syslog::closelog();

	Syslog::syslog(
		LOG_WARNING,
		"(Warning) Log after closelog with three arguments. One is %zu, another is %s, "
		"a third is %d",
		number, "this", 4011);

	Syslog::openlog("Second prepended message", LOG_PID | LOG_CONS,
			LOG_USER);

	Syslog::syslog(LOG_EMERG,
		       "Emergency log after openlog and new facility: user");
	...
	Syslog::openlog("Open after close prepended message", LOG_CONS,
			LOG_MAIL);
