...
int main()
{
	...
	CHECKSERT(res < 0 && errno == EINVAL,
		  "Fails when already bound (EINVAL)");

	res = bind(socket(AF_INET, SOCK_DGRAM, 0), (struct sockaddr *)&myaddr,
		   1ul);
	CHECKSERT(res < 0 && errno == EINVAL,
		  "Fails when address is invalid (EINVAL)");

	res = bind(socket(AF_INET, SOCK_DGRAM, 0), (struct sockaddr *)&myaddr,
		   sizeof(myaddr));
	CHECKSERT(res < 0 && errno == EADDRINUSE,
	...
	res = recvfrom(fd, recvbuf, BUFSIZE, MSG_PEEK,
		       (struct sockaddr *)&remaddr, &rem_addrlen);
	CHECKSERT(res > 0, "Received data w/ MSG_PEEK (%i bytes)", res);
	recvbuf[res] = 0;

	const char *rm_message = "POSIX is for hipsters";
	CHECKSERT(strcmp((char *)&recvbuf, rm_message) == 0,
		  "Message is \"%s\"", recvbuf);

	CHECKSERT(remaddr.sin_addr.s_addr == htonl(inet.gateway().whole),
		  "Received from address %s",
		  inet.gateway().to_string().c_str());

	memset(recvbuf, 0, BUFSIZE);
	res = recvfrom(fd, recvbuf, BUFSIZE, 0, (struct sockaddr *)&remaddr,
		       &rem_addrlen);
	recvbuf[res] = 0;
	CHECKSERT(res == (int)strlen(rm_message) &&
			  strcmp((char *)&recvbuf, rm_message) == 0,
	...
	res = sendto(socket(AF_INET, SOCK_DGRAM, 0), my_message,
		     strlen(my_message), 0, (struct sockaddr *)&destaddr,
		     sizeof(destaddr));
	CHECKSERT(
		res > 0,
		"Message was sent from NEW socket to %s:%u (verified by script)",
	...
	CHECKSERT(res < 0 && errno == EDESTADDRREQ,
		  "Fails when not connected (EDESTADDRREQ)");

	res = connect(fd_send_connect, (struct sockaddr *)&destaddr,
		      sizeof(destaddr));
	...
	CHECKSERT(not inet.udp().is_bound(PORT),
		  "Underlying OS socket is closed");
