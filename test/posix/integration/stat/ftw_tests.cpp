...
int display_info(const char *fpath, const struct stat *sb, int flag,
		 struct FTW *ftwbuf);
int add_filesize(const char *fpath, const struct stat *sb, int flag,
		 struct FTW *ftwbuf);
int add_items(const char *fpath, const struct stat *sb, int flag,
	      struct FTW *ftwbuf);
bool is_before(const std::vector<std::string> &c, const char *item1,
	       const char *item2);
...
void ftw_tests()
{
	...
	auto directory_first = is_before(items, "/mnt/disk/folder1",
					 "/mnt/disk/folder1/foldera");
	CHECKSERT(directory_first,
		  "nftw() visits a directory before the directory's files");
	...
	auto files_first = is_before(items, "/mnt/disk/folder1/foldera",
				     "/mnt/disk/folder1");
	CHECKSERT(
		files_first,
		"nftw() visits the directory's files before the directory when FTW_DEPTH is specified");
	      ...
	      struct FTW *ftwbuf)
{
	...
	assert(std::find(items.begin(), items.end(), fpath) == items.end() &&
	       ...
	       const char *item2)
{
	...
	if (i1 == std::cend(c) || i2 == std::cend(c)) {
		throw std::domain_error(
			"Alas, you cannot find what isn't there");
	}
