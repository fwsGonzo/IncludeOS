...
void stat_tests()
{
	...
	CHECKSERT(res == -1 && errno == EFAULT,
		  "stat() with nullptr buffer fails with EFAULT");
    ...
    **/
    ...
    CHECKSERT(nullcwd == nullptr && errno == EINVAL, "getcwd() with 0-size buffer should fail with
    EINVAL");
    ...
    **/
	...
	CHECKSERT(shortcwd == nullptr && errno == ERANGE,
		  "getcwd() with too small buffer should fail with ERANGE");
