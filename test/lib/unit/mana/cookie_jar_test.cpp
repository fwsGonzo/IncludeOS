...
const lest::test test_cookie_jar[] = { SCENARIO(
	"Cookies can be inserted into and erased from a CookieJar"){ GIVEN(
	"A CookieJar"){
...
WHEN("Inserting three Cookies into a CookieJar by inserting name and value")
{
	...
	THEN("The CookieJar contains three Cookies")
	{
		...
		EXPECT(jar.cookie_value("another_name") == "another_value");
		EXPECT(jar.cookie_value("a_cookie_name") == "a_cookie_value");
		EXPECT(jar.cookie_value("a_third_cookie_name") ==
		       "a_third_cookie_value");
		EXPECT(jar.exists("another_name"));
		EXPECT(jar.exists("a_cookie_name"));
	...
	AND_WHEN("Erasing one of the Cookies from the CookieJar by name")
	{
		...
		THEN("The CookieJar contains only the other two Cookies")
		{
			...
			EXPECT(jar.cookie_value("a_third_cookie_name") ==
			       "a_third_cookie_value");
			EXPECT_NOT(jar.cookie_value("a_cookie_name") ==
				   "a_cookie_value");
			EXPECT(jar.exists("another_name"));
			EXPECT(jar.exists("a_third_cookie_name"));
	...
	AND_WHEN("Getting all the cookies that the CookieJar contains")
	{
		...
		THEN("The existing Cookies are returned in a map")
		{
			...
			EXPECT(all_cookies.find("another_name") not_eq
			       all_cookies.end());
			EXPECT(all_cookies.find("a_cookie_name") not_eq
			       all_cookies.end());
			EXPECT(all_cookies.find("a_third_cookie_name") not_eq
			       all_cookies.end());
			EXPECT(all_cookies.find("not_existing_cookie") ==
			       all_cookies.end());
		}
	}
}
...
,
	...
	SCENARIO(
		"A CookieJar can return an iterator to its first and last element")
{
	GIVEN("A CookieJar")
	{
		CookieJar jar{};
