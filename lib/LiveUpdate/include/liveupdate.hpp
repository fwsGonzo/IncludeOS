 ...
 * resume() can then be called with this same location, and it will call handlers for each @id it
 *finds, unless no such handler is registered, in which case it just calls the default handler which
 *is passed to the call to resume(). The call to resume returns true if everything went well.
 **/
struct LiveUpdate {
	...
	static void exec(const buffer_t &blob, std::string key,
			 storage_func func);
	...
	static bool
	partition_exists(const std::string &key,
			 const void *storage_area = nullptr) noexcept;

	// When explicitly resuming from heap, heap overrun checks are disabled
	static void resume_from_heap(void *location, std::string key,
				     resume_func);
...
struct Storage {
	...
	template <typename T> inline void add(uid, const T &type);
...
struct Restore {
	...
	net::Stream_ptr as_tls_stream(void *ctx, bool outgoing,
				      net::Stream_ptr tr);

	template <typename S> inline const S &as_type() const;
...
template <typename S> inline const S &Restore::as_type() const
{
	if (sizeof(S) != length()) {
		throw std::runtime_error("Mismatching length for id " +
					 std::to_string(get_id()));
	}
	return *reinterpret_cast<const S *>(data());
}
template <typename T> inline std::vector<T> Restore::as_vector() const
{
	size_t count = 0;
	auto *first = (T *)get_segment(sizeof(T), count);
	return std::vector<T>(first, first + count);
}
template <> inline std::vector<std::string> Restore::as_vector() const
{
	return rebuild_string_vector();
}

template <typename T> inline void Storage::add(uid id, const T &thing)
{
