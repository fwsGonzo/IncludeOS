...
namespace liu
{
template <typename ElfSym> struct SymTab {
...
void *find_sym(SymTab<ElfSym> syms, StrTab strs, const std::string &name)
{
	for (unsigned i = 0; i < syms.entries; i++) {
		...
		int type;
		if
			constexpr(Bits == 32) type = ELF32_ST_TYPE(sym.st_info);
		else
...
void *find_kernel_start(const ElfEhdr *hdr_ptr)
{
	...
	for (auto i = 0; i < hdr_ptr->e_shnum; i++) {
		...
		case SHT_SYMTAB:
			...
			break;
		case SHT_STRTAB:
			strtabs.emplace_back(&file_ptr[shdr[i].sh_offset],
					     shdr[i].sh_size);
			break;
		default:
	...
	if (symtab.base == nullptr || strtabs.empty())
		return nullptr;
	StrTab strtab =
		*std::max_element(std::begin(strtabs), std::end(strtabs),
