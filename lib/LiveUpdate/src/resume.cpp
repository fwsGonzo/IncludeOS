...
namespace liu
{
static bool resume_begin(storage_header &, std::string,
			 LiveUpdate::resume_func);
...
static bool resume_helper(void *location, std::string key,
			  LiveUpdate::resume_func func)
{
...
bool LiveUpdate::resume(std::string key, resume_func func)
{
	...
	if (kernel::heap_end() >= (uintptr_t)location) {
		...
		throw std::runtime_error(
			"LiveUpdate::resume(): Storage area inside heap");
	}
	return resume_helper(location, std::move(key), func);
}
bool LiveUpdate::partition_exists(const std::string &key,
				  const void *area) noexcept
{
	if (area == nullptr)
		area = kernel::liveupdate_storage_area();
...
void LiveUpdate::resume_from_heap(void *location, std::string key,
				  LiveUpdate::resume_func func)
{
	resume_helper(location, std::move(key), func);
}

bool resume_begin(storage_header &storage, std::string key,
		  LiveUpdate::resume_func func)
{
	if (key.empty())
		throw std::length_error(
			"LiveUpdate partition key cannot be an empty string");

	int p = storage.find_partition(key.c_str());
	if (p == -1)
		return false;
	LPRINT("* Resuming from partition %d at %p from %p\n", p,
	       storage.begin(p), &storage);
...
int Restore::as_int() const
{
	...
	if (ent->type == TYPE_INTEGER)
		return ent->len;
	throw std::runtime_error(
		"LiveUpdate: Restore::as_int() encountered incorrect type " +
...
std::string Restore::as_string() const
{
	...
	throw std::runtime_error(
		"LiveUpdate: Restore::as_string() encountered incorrect type " +
		std::to_string(ent->type));
}
buffer_t Restore::as_buffer() const
{
	...
	throw std::runtime_error(
		"LiveUpdate: Restore::as_buffer() encountered incorrect type " +
		std::to_string(ent->type));
}
...
const void *Restore::get_segment(size_t size, size_t &count) const
{
	if (ent->type != TYPE_VECTOR)
		throw std::runtime_error(
			"LiveUpdate: Restore::as_vector() encountered incorrect type " +
			std::to_string(ent->type));

	auto &segs = ent->get_segs();
	if (size != segs.esize)
		throw std::runtime_error(
			"LiveUpdate: Restore::as_vector() encountered incorrect type size " +
			std::to_string(size) + " vs " +
			std::to_string(segs.esize));
...
std::vector<std::string> Restore::rebuild_string_vector() const
{
	if (ent->type != TYPE_STR_VECTOR)
		throw std::runtime_error(
			"LiveUpdate: Restore::as_vector<std::string>() encountered incorrect type " +
			std::to_string(ent->type));
...
void Restore::go_next()
{
	if (is_end())
		throw std::out_of_range(
			"Restore::go_next(): Already reached end of partition");
...
{
	uint16_t result = 0;
	while (is_marker() == false && is_end() == false)
		go_next();
	if (is_marker()) {
		result = get_id();
		go_next();
...
void Restore::pop_marker(uint16_t id)
{
	...
	if (is_marker()) {
		if (get_id() != id)
			throw std::out_of_range(
				"Restore::pop_marker(): Ran past marker with another id: " +
