...
Connection::State *serialized_tcp::to_state(int8_t state) const
{
	switch (state) {
	case 0:
		return &Connection::Closed::instance();
	case 1:
		return &Connection::Listen::instance();
	case 2:
		return &Connection::SynSent::instance();
	case 3:
		return &Connection::SynReceived::instance();
	case 4:
		return &Connection::Established::instance();
	case 5:
		return &Connection::FinWait1::instance();
	case 6:
		return &Connection::FinWait2::instance();
	case 7:
		return &Connection::CloseWait::instance();
	case 8:
		return &Connection::Closing::instance();
	case 9:
		return &Connection::LastAck::instance();
	case 10:
		return &Connection::TimeWait::instance();
	}
	throw std::runtime_error("to_state: Unknown TCP state");
}
int8_t serialized_tcp::to_state(Connection::State *state) const
{
	if (state == &Connection::Closed::instance())
		return 0;
	if (state == &Connection::Listen::instance())
		return 1;
	if (state == &Connection::SynSent::instance())
		return 2;
	if (state == &Connection::SynReceived::instance())
		return 3;
	if (state == &Connection::Established::instance())
		return 4;
	if (state == &Connection::FinWait1::instance())
		return 5;
	if (state == &Connection::FinWait2::instance())
		return 6;
	if (state == &Connection::CloseWait::instance())
		return 7;
	if (state == &Connection::Closing::instance())
		return 8;
	if (state == &Connection::LastAck::instance())
		return 9;
	if (state == &Connection::TimeWait::instance())
		return 10;
	throw std::runtime_error("to_state: Unknown TCP state");
}
...
int Write_queue::deserialize_from(void *addr)
{
	...
	while (total--) {
		...
		std::copy(source, source + current->length,
...
int Read_buffer::deserialize_from(void *addr)
{
	...
	if (readq.size() > 0) {
		std::copy(readq.vla, readq.vla + readq.size(),
...
void Connection::deserialize_from(void *addr)
{
	...
	if (readq->capacity) {
		read_request = std::make_unique<Read_request>(
			readq->seq, readq->capacity, host_.max_bufsize(),
			bufalloc.get());
...
Connection_ptr deserialize_connection(void *addr, net::TCP &tcp)
{
	...
	auto conn =
		std::make_shared<Connection>(tcp, area->local, area->remote);
...
int Connection::serialize_to(void *addr) const
{
	...
	auto *readq = (read_buffer *)&area->vla[writeq_len];
	int readq_len = (read_request) ?
				read_request->front().serialize_to(readq) :
				sizeof(read_buffer);
...
{
	hdr.add_struct(TYPE_TCP, id, [&conn](char *location) -> int {
