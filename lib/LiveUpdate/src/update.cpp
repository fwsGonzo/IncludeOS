...
static std::unordered_map<std::string, LiveUpdate::storage_func>
	storage_callbacks;
...
void LiveUpdate::register_partition(std::string key, storage_func callback)
{
	...
	if (it == storage_callbacks.end()) {
		storage_callbacks.emplace(
			std::piecewise_construct,
			std::forward_as_tuple(std::move(key)),
			std::forward_as_tuple(std::move(callback)));
	} else {
		throw std::runtime_error("Storage key '" + key +
					 "' already used");
	}
#endif
}

template <typename Class> inline bool validate_header(const Class *hdr)
{
...
{
	if (func != nullptr)
		LiveUpdate::register_partition(key, func);
...
{
	if (location == nullptr)
		location = kernel::liveupdate_storage_area();
	...
	if (storage_area < (char *)0x200) {
		throw std::runtime_error(
			"LiveUpdate storage area is (probably) a null pointer");
	}
#if !defined(PLATFORM_UNITTEST) && !defined(USERSPACE_KERNEL)
	const uintptr_t storage_area_phys =
		os::mem::virt_to_phys((uintptr_t)storage_area);
	...
	if (storage_area >= &_ELF_START_ && storage_area < &_end) {
		throw std::runtime_error(
			"LiveUpdate storage area is inside kernel area");
	}
	if (storage_area >= (char *)kernel::heap_begin() &&
	    storage_area < (char *)kernel::heap_end()) {
		throw std::runtime_error(
			"LiveUpdate storage area is inside the heap area");
	}
#endif
	...
	if (!validate_header<Elf32_Ehdr>(hdr)) {
		...
		if (!validate_header<Elf32_Ehdr>(hdr)) {
			...
			/// simply return
			throw std::runtime_error(
				"Could not find any ELF header in blob");
		}
	}
	...
	if (hdr->e_ident[EI_CLASS] == ELFCLASS32) {
		...
		expected_total = hdr->e_shnum * hdr->e_shentsize + hdr->e_shoff;
		/// program entry point
	...
	// get offsets for the new service from program header
	if (bin_data == nullptr || phys_base == nullptr || bin_len <= 64) {
	...
	hotswap(phys_base, bin_data, bin_len, (void *)(uintptr_t)start_offset,
		sr_data);
	throw liveupdate_exec_success();
	...
	if (found_kernel_start == false) {
		...
		if (LIVEUPDATE_ZERO_OLD_MEMORY) {
			((decltype(&hotswap64))HOTSWAP_AREA)(
				phys_base, bin_data, bin_len,
				start_offset, /* binary entry point */
				sr_data, /* softreset location */
				(void *)kernel::
					heap_end() /* zero memory until this location */);
		} else {
			((decltype(&hotswap64))HOTSWAP_AREA)(phys_base,
							     bin_data, bin_len,
							     start_offset,
							     sr_data, nullptr);
		}
...
#endif
	// copy hotswapping function to sweet spot
	memcpy(HOTSWAP_AREA, (void *)&hotswap,
	       &__hotswap_length - (char *)&hotswap);
	/// the end
	((decltype(&hotswap))HOTSWAP_AREA)(phys_base, bin_data, bin_len,
					   (void *)(uintptr_t)start_offset,
					   sr_data);
}
...
size_t LiveUpdate::stored_data_length(const void *location)
{
	...
	if (storage->validate() == false)
		throw std::runtime_error(
			"Failed sanity check on LiveUpdate storage area");
...
void Storage::add_stream(net::Stream &stream)
{
	...
	hdr.add_struct(TYPE_STREAM, subid, [stream_ptr](char *location) -> int {
