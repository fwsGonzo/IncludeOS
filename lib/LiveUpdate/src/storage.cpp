...
storage_header::storage_header() : magic(LIVEUPD_MAGIC)
{
	// printf("%p --> %#llx\n", this, value);
}
...
storage_entry &storage_header::add_struct(int16_t type, uint16_t id,
					  construct_func func)
{
	return var_entry(type, id, func);
}
void storage_header::add_vector(uint16_t id, const void *buf, size_t cnt,
				size_t esize)
{
	auto &entry = create_entry(TYPE_VECTOR, id,
				   sizeof(segmented_entry) + cnt * esize);
...
void storage_header::add_string_vector(uint16_t id,
				       const std::vector<std::string> &vec)
{
	var_entry(TYPE_STR_VECTOR, id, [&vec](char *dest) -> int {
	...
	*((volatile int *)&ent.len) = END_CANARY;
	if (ent.len != END_CANARY)
		throw std::runtime_error(
			"Failed to write canary to end of storage");
...
void storage_header::finalize()
{
	if (this->magic != LIVEUPD_MAGIC)
		throw std::runtime_error(
			"Magic field invalidated during store process");
	// add end if missing
	if (this->length == 0)
		this->add_end();
...
bool storage_header::validate() const noexcept
{
	if (this->magic != LIVEUPD_MAGIC)
		return false;
	if (LIVEUPDATE_USE_CHEKSUMS == false)
		return true;
...
void storage_header::try_zero() noexcept
{
	for (uint32_t p = 0; p < partitions; p++) {
		...
		if (part.length != 0 && part.name[0] != 0)
			return;
	}
	// zero everything
	this->zero();
