...
template <> const size_t x86::paging::Map::any_size{ supported_page_sizes() };

template <> const size_t os::mem::Map::any_size{ supported_page_sizes() };
...
__attribute__((weak)) void __arch_init_paging()
{
	INFO("x86_64", "Initializing paging");
	auto default_fl =
		Flags::present | Flags::writable | Flags::huge | Flags::no_exec;
	__pml4 = new Pml4(0);
	Expects(__pml4 != nullptr);
	Expects(!__pml4->has_flag(0, Flags::present));

	INFO2("* Supported page sizes: %s",
	      os::mem::page_sizes_str(os::mem::Map::any_size).c_str());
    ...
    */
	...
	Expects(__pml4->has_flag(8_KiB, Flags::present | Flags::writable |
						Flags::no_exec));
	allow_executable();
...
namespace paging
{
	...
	__attribute__((weak)) void invalidate(void *pageaddr)
	{
...
#define mem_fail_fast(reason)                                                  \
	throw Memory_exception(std::string(__FILE__) + ":" +                   \
			       std::to_string(__LINE__) + ": " + reason)
...
{
	return { map.lin, map.phys, to_memflags(map.flags), map.size,
		 map.page_sizes };
}

Map_x86 to_x86(Map map)
{
	return { map.lin, map.phys, x86::paging::to_x86(map.flags), map.size,
		 map.page_sizes };
}
...
Map mem::protect(uintptr_t linear, size_t len, Access flags)
{
	...
	x86::paging::Map m{ linear, x86::paging::any_addr, xflags,
...
__attribute__((weak)) Map mem::map(Map m, const char *name)
{
	...
	if (UNLIKELY(not bits::is_aligned(min_psize(), m.lin) or
		     not bits::is_aligned(min_psize(), m.phys)))
		mem_fail_fast(
			"linear and physical must be aligned to supported page size");

	if (UNLIKELY(not bits::is_aligned(m.min_psize(), m.lin) or
		     not bits::is_aligned(m.min_psize(), m.phys)))
		mem_fail_fast(
			"linear and physical must be aligned to requested page size");
...
Map mem::unmap(uintptr_t lin)
{
	...
	if (key) {
		...
		m = __pml4->map_r({ key, 0, x86::paging::to_x86(Access::none),
				    (size_t)map_ent.size() });
...
{
	INFO2("* Allowing execute on %p -> %p", (void *)__exec_begin,
	      (void *)__exec_end);
...
void protect_pagetables_once()
{
	...
	__pml4->traverse([&table_entries](void *ptr, size_t len) {
		table_entries.push_back({ ptr, len });
	});

	for (auto it = table_entries.rbegin(); it != table_entries.rend();
	     ++it) {
		const auto &entry = *it;
		const x86::paging::Map m{ (uintptr_t)entry.addr,
					  x86::paging::any_addr,
					  x86::paging::Flags::present,
					  static_cast<size_t>(entry.len) };
		assert(m);
