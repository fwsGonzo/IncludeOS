...
extern "C" int syscall_SYS_utimensat(int dirfd, const char *pathname,
				     const struct timespec times[2], int flags)
{
	return strace(sys_utimensat, "utimensat", dirfd, pathname, times,
		      flags);
}

extern "C" int syscall_SYS_futimesat(int dirfd, const char *pathname,
				     const struct timespec times[2])
{
	return strace(sys_futimesat, "futimesat", dirfd, pathname, times);
}

extern "C" int syscall_SYS_utimes(const char *pathname,
