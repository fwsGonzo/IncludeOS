...
long sys_stat(const char *path, struct stat *buf)
{
	...
	try {
		...
		if (ent.is_valid()) {
			...
			buf->st_blocks =
				buf->st_size > 0 ?
					util::bits::roundto<512>(buf->st_size) :
					0;
...
extern "C" long syscall_SYS_stat(const char *path, struct stat *buf)
{
	return strace(sys_stat, "stat", path, buf);
}

extern "C" long syscall_SYS_lstat(const char *path, struct stat *buf)
{
