...
{
	auto aligned_begin =
		(addr_begin + Alloc::align - 1) & ~(Alloc::align - 1);
...
extern "C" __attribute__((weak)) void *kalloc(size_t size)
{
	Expects(kernel::heap_ready());
	return alloc->allocate(size);
}

extern "C" __attribute__((weak)) void kfree(void *ptr, size_t size)
{
		      ...
		      int fd, off_t /*offset*/)
{
	...
	if (fd > 0) {
		assert(false &&
		       "Mapping to file descriptor not yet implemented");
	}
...
extern "C" void *syscall_SYS_mmap(void *addr, size_t length, int prot,
...
extern "C" void *syscall_SYS_mmap2(void *addr, size_t length, int prot,
				   int flags, int fd, off_t offset)
{
	return strace(sys_mmap, "mmap2", addr, length, prot, flags, fd,
		      offset * 4096);
}
