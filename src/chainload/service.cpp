...
#define MYINFO(X, ...)                                                         \
	if (os_enable_boot_logging)                                            \
		printf("[ chainloader ] " X "\n", ##__VA_ARGS__);
...
void promote_mod_to_kernel()
{
	...
	if (bootinfo->mods_count)
		bootinfo->mods_addr = (uintptr_t)(
			(multiboot_module_t *)bootinfo->mods_addr + 1);
}

void Service::start()
{
	auto mods = os::modules();
	MYINFO("%u-bit chainloader found %u modules", sizeof(void *) * 8,
	       mods.size());
	...
	memcpy(hotswap_addr, (void *)&hotswap,
	       &__hotswap_end - (char *)&hotswap);
	...
	((decltype(&hotswap))hotswap_addr)(base, len, dest, start,
