...
namespace fs
{
Disk::Disk(hw::Block_device &dev) : device{ dev }
{
}
		...
		hw::Block_device::on_read_func::make_packed(
			[this, func](hw::Block_device::buffer_t data) {
				...
				if (bpb->bytes_per_sector >= 512 &&
				    bpb->fa_tables != 0 &&
				    (bpb->signature !=
					     0 // check MBR signature too
				     || bpb->large_sectors !=
						0)) // but its not set for FAT32
				{
					// detected FAT on MBR
				...
				for (int i = 0; i < 4; i++) {
					if (mbr->part[i].type !=
						    0 // 0 is unused partition
					    && mbr->part[i].lba_begin !=
						       0 // 0 is MBR anyways
					    &&
					    mbr->part[i].sectors !=
						    0) // 0 means no size, so...
					{
				...
				func({ error_t::E_MNT,
				       "No FAT partition auto-detected" },
				     fs());
			}));
}
			...
			hw::Block_device::on_read_func::make_packed(
				[this, part,
				 func](hw::Block_device::buffer_t data) {
					...
					auto lba_base =
						mbr->part[pint].lba_begin;
					auto lba_size = mbr->part[pint].sectors;
					assert(lba_size &&
					       "No such partition (length was zero)");
