...
namespace fs
{
void FAT::int_ls(uint32_t sector, Dirvec_ptr dirents,
		 on_internal_ls_func callback) const
{
	...
	*next = next_func_t::make_packed([this, callback, dirents,
					  weak_next](uint32_t sector) {
		FS_PRINT("int_ls: sec=%u\n", sector);
		auto next = weak_next.lock();
		device.read(
			sector,
			hw::Block_device::on_read_func::make_packed(
				[this, sector, callback, dirents,
				 next](buffer_t data) {
					if (data == nullptr) {
					...
					bool done = int_dirent(
						sector, data->data(), *dirents);
					if (done)
						// execute callback
...
void FAT::traverse(std::shared_ptr<Path> path, cluster_func callback,
		   const Dirent *const start) const
{
	...
	auto next = std::make_shared<next_func_t>();
	auto weak_next = std::weak_ptr<next_func_t>(next);
	*next = next_func_t::make_packed([this, path, weak_next,
					  callback](uint32_t cluster) {
		if (path->empty()) {
		...
		int_ls(S, dirents,
		       on_internal_ls_func::make_packed([name, dirents, next,
							 callback](
								error_t err,
								Dirvec_ptr ents) {
			       if (UNLIKELY(err)) {
...
void FAT::read(const Dirent &ent, uint64_t pos, uint64_t n,
	       on_read_func callback) const
{
		...
		hw::Block_device::on_read_func::make_packed(
			[n, callback, internal_ofs](buffer_t data) {
				...
				if (internal_ofs != 0) {
					...
					data = construct_buffer(
						data->begin() + internal_ofs,
						data->begin() + internal_ofs +
							n);
				} else {
	       ...
	       const Dirent *const start) const
{
	...
	traverse(path,
		 cluster_func::make_packed([this, filename,
					    func](error_t error,
						  Dirvec_ptr dirents) {
			 if (UNLIKELY(error)) {
				 // no path, no file!
				 func(error,
				      Dirent(this, INVALID_ENTITY, filename));
				 return;
			 }
...
void FAT::cstat(const std::string &strpath, on_stat_func func)
{
	...
	File_system::stat(strpath,
			  fs::on_stat_func::make_packed(
				  [this, strpath, func](error_t error,
							const Dirent &ent) {
