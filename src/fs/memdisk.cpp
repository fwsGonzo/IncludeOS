...
namespace fs
{
MemDisk::MemDisk() noexcept : MemDisk(&_DISK_START_, &_DISK_END_)
{
	INFO("Memdisk", "Initializing start=%p end=%p", image_start_,
	     image_end_);
}
...
MemDisk::MemDisk(const char *start, const char *end) noexcept
	: Block_device(),
	  image_start_{ start },
	  image_end_{ end },
	  stat_read(Statman::get()
			    .create(Stat::UINT64, device_name() + ".reads")
			    .get_uint64())
{
