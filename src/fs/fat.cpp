...
namespace fs
{
FAT::FAT(hw::Block_device &dev) : device(dev)
{
	//
}
...
void FAT::init(const void *base_sector)
{
	...
	this->data_index = bpb->reserved_sectors +
			   (bpb->fa_tables * this->sectors_per_fat) +
			   this->root_dir_sectors;
...
void FAT::init(uint64_t base, uint64_t size, on_init_func on_init)
{
	...
	device.read(
		base,
		hw::Block_device::on_read_func::make_packed([this, on_init](
								    buffer_t
									    data) {
			auto *mbr = (MBR::mbr *)data->data();
			if (mbr == nullptr) {
				on_init({ error_t::E_IO, "Could not read MBR" },
					*this);
				return;
			}
			...
			if (UNLIKELY(mbr->magic != 0xAA55)) {
				on_init({ error_t::E_MNT,
					  "Missing or invalid MBR signature" },
					*this);
				return;
			}
			...
			INFO2("[ofs=%u  size=%u (%u bytes)]\n", this->lba_base,
			      this->lba_size, this->lba_size * 512);
...
bool FAT::int_dirent(uint32_t sector, const void *data,
		     dirvector &dirents) const
{
			...
			if (LIKELY(root[i].is_longname())) {
				...
				if (L->is_last()) {
					...
					for (int idx = total; idx > 0; idx--) {
						...
						memcpy(longname + 0, L->first,
						       10);
						memcpy(longname + 5, L->second,
						       12);
						memcpy(longname + 11, L->third,
						       4);
						...
						if (UNLIKELY(final_count >
							     240)) {
							FS_PRINT(
								"Suspicious long name length, breaking...\n");
							break;
						}
					}
