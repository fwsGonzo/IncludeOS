...
Buffer FAT::read(const Dirent &ent, uint64_t pos, uint64_t n) const
{
	...
	buffer_t data = device.read_sync(
	...
	if (internal_ofs != 0) {
		data = construct_buffer(data->begin() + internal_ofs,
					data->begin() + internal_ofs + n);
	} else {
	...
	while (!path.empty()) {
		...
		for (auto &e : ents)
			if (UNLIKELY(e.name() == name)) {
				...
				FS_PRINT("traverse_sync: Found match for %s",
					 name.c_str());
