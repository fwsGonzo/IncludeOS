...
uint64_t fdt_load_addr(const struct fdt_property *prop, int *offset,
		       int addr_cells)
{
	uint64_t addr = 0;
	for (int j = 0; j < addr_cells; ++j) {
		addr |= (uint64_t)fdt32_ld(
				(const fdt32_t *)((char *)prop->data + *offset))
			<< ((addr_cells - j - 1) * 32);
...
uint64_t fdt_load_size(const struct fdt_property *prop, int *offset,
		       int size_cells)
{
	uint64_t size = 0;
	for (int j = 0; j < size_cells; ++j) {
		size |= (uint64_t)fdt32_ld(
				(const fdt32_t *)((char *)prop->data + *offset))
			<< ((size_cells - j - 1) * 32);
		*offset += sizeof(uint32_t);
	}
...
void gic_init_fdt(const char *fdt, uint32_t fdt_offset)
{
	...
	addr_cells = fdt_address_cells(
		fdt, fdt_offset); // fdt32_ld((const fdt32_t *)prop->data);
	// how to get ?
...
struct gic_params {
	...
	{
		gicc = static_cast<struct gic_v3_cpu_interface_gicc *>(
			(void *)addr);
	}
	void set_gicd(uint64_t addr)
	{
		gicd = static_cast<struct gic_v3_distributor_map *>(
			(void *)addr);
	}
};
...
void init_gicd()
{
	...
	// rounded up
	int regs_count = (irq_lines + GIC_V3_GICD_INT_PER_REG - 1) /
			 GIC_V3_GICD_INT_PER_REG;
	...
	// rounded up
	int priority_regs = (irq_lines + GIC_V3_GICD_PRIORITY_PER_REG - 1) /
			    GIC_V3_GICD_PRIORITY_PER_REG;
	/* Set priority to lowest on all interrupts*/
	for (auto reg = 0; reg < priority_regs; reg++) {
		GIC_DEBUG("REG %08x : %08x\r\n", &gic.gicd->priority[reg],
			  0xffffffff);
		gic.gicd->priority[reg] = 0xffffffff;
	}
	// rounded up
	int target_regs = (irq_lines + GIC_V3_GICD_TARGETS_PER_REG - 1) /
			  GIC_V3_GICD_TARGETS_PER_REG;
	// set all interrupts to target cpu0
	// from SPIO QEMU= 16
	// TODO Revisit
	for (auto reg = 16 / GIC_V3_GICD_TARGETS_PER_REG; reg < target_regs;
	     reg++) {
		GIC_DEBUG("REG %08x : %08x\r\n", &gic.gicd->targets[reg],
			  GIC_V3_GICD_TARGETS_CORE0_BMAP);
		gic.gicd->targets[reg] = GIC_V3_GICD_TARGETS_CORE0_BMAP;
	}

	int config_regs = (irq_lines + GIC_V3_GICD_CFGR_PER_REG - 1) /
			  GIC_V3_GICD_CFGR_PER_REG;
	// from PPIO QEMU 32
	// TODO revisit
	for (auto reg = 32 / GIC_V3_GICD_CFGR_PER_REG; reg < target_regs;
	     reg++) {
		GIC_DEBUG("REG %08x : %08x\r\n", &gic.gicd->config[reg],
			  GICD_ICFGR_LEVEL);
...
{
	int shift = (irq % GIC_V3_GICD_TARGETS_PER_REG) *
		    GIC_V3_GICD_TARGETS_SIZE_PER_REG;
	int reg = (irq / GIC_V3_GICD_TARGETS_PER_REG);
	gic.gicd->targets[reg] &= ~(0xff << shift); // clear
	gic.gicd->targets[reg] |=
		((processor & 0xff) << shift); // set new processor target
}

void gicd_set_priority(uint32_t irq, uint8_t priority)
{
	int shift = (irq % GIC_V3_GICD_PRIORITY_PER_REG) *
		    GIC_V3_GICD_PRIORITY_SIZE_PER_REG;
	int reg = (irq / GIC_V3_GICD_PRIORITY_PER_REG);
	gic.gicd->priority[reg] &= ~(0xff << shift); // clear
	gic.gicd->priority[reg] |=
		((priority & 0xff) << shift); // set new processor target
}

void gicd_set_config(uint32_t irq, uint8_t config)
{
	int shift = (irq % GIC_V3_GICD_CFGR_PER_REG) *
		    GIC_V3_GICD_CFGR_SIZE_PER_REG;
...
{
	int reg = irq / GIC_V3_GICD_INT_PER_REG;
	int pending = (gic.gicd->pending_set[reg] &
		       (1 << (irq % GIC_V3_GICD_INT_PER_REG)));
	return pending;
}
