...
__attribute__((constructor, used)) static void self_test_gconstr()
{
	gconstr_value = 1;
}
...
extern "C" void __init_sanity_checks() noexcept
{
#ifdef ENABLE_CRC_RO
...
extern "C" void kernel_sanity_checks()
{
	...
	if (!symbols_verified)
		os::panic(
			"Sanity checks: Consistency of Elf symbols and string areas");

	// global constructor self-test
	if (gconstr_value != 1) {
		kprintf("Sanity checks: Global constructors not working (or modified during run-time)!\n");
		os::panic(
			"Sanity checks: Global constructors verification failed");
	}
}
