...
extern "C" uint8_t nibble2hex(uint8_t nibble)
{
	nibble = nibble & 0xF;
	switch (nibble) {
	case 0x00:
...
extern "C" void print_memory(const char *name, const char *mem, int size)
{
	kprint(name);
	kprint(":\n");
	int i;
...
extern "C"
	//__attribute__((no_sanitize("all")))
	void
	kernel_start(uintptr_t magic, uintptr_t addrin)
{
	...
	// OK so these get overidden in the for loop which should return a map of memory and not just a
	// single one
	uint64_t addr = 0;
	uint64_t size = 0;
	...
	addr_cells = fdt_address_cells(
		fdt, 0); // fdt32_ld((const fdt32_t *)prop->data);
