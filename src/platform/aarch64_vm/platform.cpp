...
void __platform_init(uint64_t fdt_addr)
{
	...
	if (intc <
	    0) // interrupt controller not found in fdt.. should never happen..
	{
...
#define TIMER_IRQ 27
	...
	Timers::init(
		[](Timers::duration_t nanos) {
			...
			// prevent overflow
			uint64_t ticks =
				(nanos.count() / 1000) * ticks_per_micro;
			if (ticks > 0xFFFFFFFF)
				ticks = 0xFFFFFFFF;
			...
			timer_set_virtual_compare(timer_get_virtual_countval() +
						  ticks);
			timer_virtual_start();
		},
		[]() { timer_virtual_stop(); });
...
{
	// in nanoseconds..
	return ((timer_get_virtual_countval() * 10) /
		(timer_get_frequency() / 100000)) *
	       1000;
	// return 0;
}
