...
void APIC_Timer::init()
{
	...
	Timers::init(oneshot, // timer start function
		     stop); // timer stop function

	// set interrupt handler
	GET_TIMER().intr = Events::get().subscribe(Timers::timers_handler);
...
void APIC_Timer::calibrate()
{
	...
	uint32_t overhead;
	[&overhead] { overhead = APIC::get().timer_diff(); }();
	// restart counter
	lapic.timer_begin(0xFFFFFFFF);

	/// use PIT to measure <time> in one-shot ///
	PIT::oneshot(milliseconds(CALIBRATION_MS), [overhead] {
