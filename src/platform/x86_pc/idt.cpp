...
static void set_intr_entry(IDTDescr *idt_entry, intr_handler_t func,
			   uint8_t ist, uint16_t segment_sel, char attributes)
{
	addr_union addr;
...
{
	set_intr_entry(&entry[vec], (intr_handler_t)func, 2, RING0_CODE_SEG,
		       0x8e);
}
...
#define RIP_REG 16
	...
	fprintf(stderr, "  CR2:  %016lx  GDT:  %016lx (%u)\n", regs[20],
		gdt.location, gdt.limit);
	fprintf(stderr, "  CR3:  %016lx  IDT:  %016lx (%u)\n", regs[21],
		idt.location, idt.limit);
...
__cpu_exception(uintptr_t *regs, int error, uint32_t code)
{
	...
	// error message:
	char buffer[64];
	snprintf(buffer, sizeof(buffer), "%s (%d)", exception_names[error],
		 error);
