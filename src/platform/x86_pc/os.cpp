...
__attribute__((noinline)) void os::halt() noexcept
{
	...
	asm volatile(".global _irq_cb_return_location;\n"
		     "_irq_cb_return_location:");

	// Count sleep cycles
	PER_CPU(os_per_cpu).cycles_hlt +=
		os::Arch::cpu_cycles() - cycles_before;
}
...
void kernel::start(uint32_t boot_magic, uint32_t boot_addr)
{
	...
	MYINFO("Total memory detected as %s ",
	       util::Byte_r(kernel::memory_end()).to_string().c_str());
	...
	os::mem::map({ 0x1000, 0x1000, os::mem::Access::read, 0x7000 },
		     "Page tables");
	...
	INFO2("* Assigning heap 0x%zx -> 0x%zx", kernel::heap_begin(),
	      heap_range_max_);
...
void kernel::legacy_boot()
{
	...
	memmap.assign_range({ 0x0009FC00, 0x0009FFFF, "EBDA" });
	memmap.assign_range({ 0x000A0000, 0x000FFFFF, "VGA/ROM" });
}
