...
class CMOS {
	...
	static bool mode_24_hour()
	{
		return reg_b_value & b_24_hr_clock;
	}

	/** CMOS RTC uses binary representation - otherwise BCD **/
	static bool mode_binary()
	{
		return reg_b_value & b_binary_mode;
	}
	 ...
	 **/
		uint32_t to_epoch()
		{
			...
			uint32_t tm_yday =
				day_of_year() - 1; // Days since jan. 1st.
			return f.second + f.minute * 60 + f.hour * 3600 +
