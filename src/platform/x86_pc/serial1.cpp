...
__attribute__((no_sanitize("all"))) static inline void init_if_needed()
{
	if (initialized == true)
		return;
	initialized = true;
...
extern "C" __attribute__((no_sanitize("all"))) void
__serial_print1(const char *cstr)
{
	init_if_needed();
	while (*cstr) {
...
extern "C" void __serial_print(const char *str, size_t len)
{
	init_if_needed();
...
extern "C" void kprint(const char *c)
{
	__serial_print1(c);
}
