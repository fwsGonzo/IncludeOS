...
{
	now_called =
		&Statman::get().create(Stat::UINT32, "cmos.now").get_uint32();
	...
	INFO("CMOS", "RTC is %i hour format, %s mode", mode_24_hour() ? 24 : 12,
	     mode_binary() ? "binary" : "BCD");
}
...
std::string CMOS::Time::to_string()
{
	...
	int len = snprintf(buffer.data(), buffer.size(),
			   "%.2i-%.2i-%iT%.2i:%.2i:%.2iZ", f.year, f.month,
			   f.day_of_month, f.hour, f.minute, f.second);
