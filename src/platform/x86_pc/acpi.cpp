...
void ACPI::begin(const void *addr)
{
	...
	INFO2("OEM: %.*s Rev. %u", 6, rdsp->rdsp10.OEMID,
	      rdsp->rdsp10.Revision);
...
void ACPI::walk_madt(const char *addr)
{
	...
	INFO2("LAPIC base: 0x%x  (flags: 0x%x)", hdr->lapic_addr, hdr->flags);
	...
	while (len) {
		...
		case 1: {
			...
			INFO2("I/O APIC %u   ADDR 0x%x  INTR 0x%x", ioapic.id,
			      ioapic.addr_base, ioapic.intr_base);
		} break;
		case 2: // interrupt source override
		{
			...
			INFO2("IRQ redirect for bus %u from IRQ %u to VEC %u",
			      redirect.bus_source, redirect.irq_source,
			      redirect.global_intr);
		} break;
...
__attribute__((noreturn)) void ACPI::shutdown()
{
	asm("cli");
