...
struct PhysMemArray : public Header {
	...
	{
		const auto &inf = info();
		return (inf.capacity32 == 0x80000000) ? inf.capacity64 :
							inf.capacity32 * 1024;
	}
} __attribute__((packed));
...
void SMBIOS::parse(const char *mem)
{
	...
	for (int i = 0; i < structs; i++) {
		...
		case 0: // BIOS
			...
			INFO2("BIOS version: %s",
			      hdr->get_string(hdr->data[1]));
			break;
		case 1:
			INFO2("Manufacturer: %s",
			      hdr->get_string(hdr->data[0]));
			INFO2("Product name: %s",
			      hdr->get_string(hdr->data[1]));
			{
				// UUID starts at offset 0x4 after header
				char uuid[33];
				for (int i = 0; i < 16; i++) {
					sprintf(&uuid[i * 2], "%02hhx",
						hdr->data[0x4 + i]);
				}
