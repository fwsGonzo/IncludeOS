...
struct xapic : public IApic {
	...
	{
		return *(volatile uint32_t *)uintptr_t(0xfee00000 + xAPIC_ISR +
						       0x10 * index);
	}
	...
	void ap_init(int id) noexcept override
	{
		...
		write(xAPIC_ICRL, ICR_INIT | ICR_PHYSICAL | ICR_ASSERT |
	...
	{
		write(xAPIC_ICRH, (id & 0xff) << ICR_DEST_BITS);
		write(xAPIC_ICRL, vector | ICR_STARTUP | ICR_PHYSICAL |
					  ICR_ASSERT | ICR_EDGE |
					  ICR_NO_SHORTHAND);
		while (read(xAPIC_ICRL) & ICR_SEND_PENDING)
			;
