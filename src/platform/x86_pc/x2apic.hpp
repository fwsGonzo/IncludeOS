...
struct x2apic : public IApic {
	...
	void interrupt_control(uint32_t bits,
			       uint8_t spurious) noexcept override
	{
	...
	static int static_get_isr() noexcept
	{
		for (int i = 5; i >= 1; i--) {
			...
			if (reg)
				return 32 * i + __builtin_ffs(reg) - 1;
		}
		return -1;
	}
	...
	void ap_init(int id) noexcept override
	{
		writel(x2APIC_ICR, id,
		       ICR_INIT | ICR_PHYSICAL | ICR_ASSERT | ICR_EDGE |
			       ICR_NO_SHORTHAND);
	...
	{
		writel(x2APIC_ICR, id,
		       vector | ICR_STARTUP | ICR_PHYSICAL | ICR_ASSERT |
	...
	void send_ipi(int id, uint8_t vector) noexcept override
	{
		...
		// select APIC ID
		writel(x2APIC_ICR, id, ICR_ASSERT | ICR_FIXED | vector);
	}
	...
	void timer_init(const uint8_t timer_intr) noexcept override
	{
		...
		write(x2APIC_LVT_TMR,
		      TIMER_ONESHOT | (32 + timer_intr) | INTR_MASK);
	}
