...
namespace os
{
__attribute__((weak)) uintptr_t liveupdate_memory_size_mb = 0;
...
Machine::Machine(void *mem, size_t size) noexcept : impl{ nullptr }
{
	Expects(mem != nullptr);
	...
	// Placement new impl
	impl = { new (mem)
			 detail::Machine{ (char *)mem + sizeof(detail::Machine),
...
Machine::Machine(void *raw_mem, size_t size)
	: mem_{ (void *)bits::align(Memory::align, (uintptr_t)raw_mem),
		size - (bits::align(Memory::align, (uintptr_t)raw_mem) -
			(uintptr_t)raw_mem) },
...
void Machine::init()
{
	...
	kernel::state().liveupdate_phys =
		(uintptr_t)liu_mem.ptr + SYSTEMLOG_SIZE;
	...
	kernel::init_heap((uintptr_t)main_mem.ptr,
			  (uintptr_t)main_mem.ptr + main_mem.size);
}

const char *Machine::arch()
{
	return Arch::name;
}
} // namespace os::detail
