	...
	: _pcidev(dev), _virtio_device_id(dev.product_id() + 0x1040)
{
    ...
    */
	...
	bool _STD_ID =
		_virtio_device_id >= 0x1040 and _virtio_device_id < 0x107f;
	bool _LEGACY_ID =
		dev.product_id() >= 0x1000 and dev.product_id() <= 0x103f;
	...
	if (dev.msix_cap()) {
		...
		if (msix_vectors) {
			...
			for (int i = 0; i < msix_vectors; i++) {
				...
				dev.setup_msix_vector(current_cpu,
						      IRQ_BASE + irq);
				// store IRQ for later
...
{
	hw::outpw(iobase() + VIRTIO_PCI_QUEUE_SEL, index);
	hw::outpd(iobase() + VIRTIO_PCI_QUEUE_PFN,
		  kernel::addr_to_page((uintptr_t)queue_desc));
	...
	return hw::inpd(iobase() + VIRTIO_PCI_QUEUE_PFN) ==
	       kernel::addr_to_page((uintptr_t)queue_desc);
}
...
void Virtio::move_to_this_cpu()
{
	if (has_msix()) {
		...
		for (size_t i = 0; i < irqs.size(); i++) {
			...
			_pcidev.rebalance_msix_vector(i, current_cpu,
						      IRQ_BASE + this->irqs[i]);
		}
	}
}
