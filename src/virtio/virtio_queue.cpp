...
void Virtio::Queue::init_queue(int size, char *buf)
{
	...
	_queue.used = (virtq_used *)(((uintptr_t)&_queue.avail->ring[size] +
				      sizeof(uint16_t) + PAGE_SIZE - 1) &
				     ~(PAGE_SIZE - 1));
	debug("\t * Queue used  @ 0x%lx \n ", (long)_queue.used);
}

/** Constructor */
Virtio::Queue::Queue(const std::string &name, uint16_t size, uint16_t q_index,
		     uint16_t iobase)
	: qname(name), _size(size), _iobase(iobase), _free_head(0),
	  _num_added(0), _last_used_idx(0), _pci_index(q_index)
{
...
{
	debug("<%s> Dequeueing  last_used index %i ", qname.c_str(),
	      _last_used_idx);
