...
extern "C" void _enable_heap_debugging(int enabled)
{
	enable_debugging = enabled;
}
extern "C" void _enable_heap_debugging_verbose(int enabled)
{
	enable_debugging_verbose = enabled;
}
extern "C" void _enable_heap_debugging_buffer_protection(int enabled)
{
...
void *operator new(std::size_t len)
{
	...
	if (enable_buffer_protection) {
		...
		memcpy(temp, &buffer_protection_checksum,
	...
	if (enable_debugging) {
		...
		if (x == nullptr) {
			...
			if (enable_buffer_protection) {
				...
				auto underflow = memcmp(
					temp, &buffer_protection_checksum,
					sizeof(buffer_protection_checksum));
				auto overflow = memcmp(
					temp + sizeof(buffer_protection_checksum) +
						x->len,
					&buffer_protection_checksum,
					sizeof(buffer_protection_checksum));
...
static void safe_print_symbol(int N, void *addr)
{
	...
	auto symb = Elf::safe_resolve_symbol(addr, _symbol_buffer,
