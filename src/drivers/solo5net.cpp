...
const char *Solo5Net::driver_name() const
{
	return "Solo5Net";
}
...
Solo5Net::Solo5Net()
	: Link(Link_protocol{ { this, &Solo5Net::transmit }, mac() }),
	  packets_rx_{ Statman::get()
			       .create(Stat::UINT64,
				       device_name() + ".packets_rx")
			       .get_uint64() },
	  packets_tx_{ Statman::get()
			       .create(Stat::UINT64,
				       device_name() + ".packets_tx")
			       .get_uint64() },
	  bufstore_{ NUM_BUFFERS, 2048u } // don't change this
{
	...
	solo5_net_info(&ni);
	mac_addr = MAC::Addr(ni.mac_address[0], ni.mac_address[1],
			     ni.mac_address[2], ni.mac_address[3],
			     ni.mac_address[4], ni.mac_address[5]);
}
