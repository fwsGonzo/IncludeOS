...
vmxnet3::vmxnet3(hw::PCI_Device &d, const uint16_t mtu)
	: Link(Link_protocol{ { this, &vmxnet3::transmit }, mac() }),
	  m_pcidev(d),
	  m_mtu(mtu), stat_sendq_cur{ Statman::get()
					      .create(Stat::UINT32,
						      device_name() +
							      ".sendq_now")
					      .get_uint32() },
	  stat_sendq_max{ Statman::get()
				  .create(Stat::UINT32,
					  device_name() + ".sendq_max")
				  .get_uint32() },

	  // TODO make some of these stats generic and put them into LINK object so that they can be seen
	  // on all networking drivers
	  stat_tx_total_packets{
		  Statman::get()
			  .create(Stat::UINT64,
				  device_name() + ".stat_tx_total_packets")
			  .get_uint64()
	  },
	  stat_tx_total_bytes{ Statman::get()
				       .create(Stat::UINT64,
					       device_name() +
						       ".stat_tx_total_bytes")
				       .get_uint64() },
	  stat_rx_total_packets{
		  Statman::get()
			  .create(Stat::UINT64,
				  device_name() + ".stat_rx_total_packets")
			  .get_uint64()
	  },
	  stat_rx_total_bytes{ Statman::get()
				       .create(Stat::UINT64,
					       device_name() +
						       ".stat_rx_total_bytes")
				       .get_uint64() },
	  ...
	  stat_rx_refill_dropped{ Statman::get()
					  .create(Stat::UINT64,
						  device_name() +
							  ".rx_refill_dropped")
					  .get_uint64() },
	  stat_sendq_dropped{ Statman::get()
				      .create(Stat::UINT64,
					      device_name() + ".sendq_dropped")
				      .get_uint64() },
	  bufstore_{ 1024, buffer_size_for_mtu(mtu) }
{
	...
	Statman::get()
		.create(Stat::UINT32, device_name() + ".buffer_size")
	...
	if (d.msix_cap()) {
		...
		if (msix_vectors > 2 + NUM_RX_QUEUES)
			msix_vectors = 2 + NUM_RX_QUEUES;
		...
		Events::get().subscribe(irqs[0],
					{ this, &vmxnet3::msix_evt_handler });
		Events::get().subscribe(irqs[1],
					{ this, &vmxnet3::msix_xmit_handler });
		for (int q = 0; q < NUM_RX_QUEUES; q++)
			Events::get().subscribe(
				irqs[2 + q],
				{ this, &vmxnet3::msix_recv_handler });
	} else {
	...
	this->dma =
		(vmxnet3_dma *)memalign(VMXNET3_DMA_ALIGN, sizeof(vmxnet3_dma));
	...
	for (int q = 0; q < NUM_RX_QUEUES; q++) {
		...
		queue.cfg.driver_data_len =
			sizeof(vmxnet3_rx_desc) + 2 * sizeof(vmxnet3_rx_desc);
	...
	memset(shared.interrupt.moderation_level, UPT1_IML_ADAPTIVE,
	       VMXNET3_MAX_INTRS);
...
void vmxnet3::refill(rxring_state &rxq)
{
	...
	while (rxq.prod_count < VMXNET3_RX_FILL) {
		...
		if (rxq.prod_count > 0 /* prevent full stop? */
		    && not Nic::buffers_still_available(
			       bufstore().buffers_in_use())) {
			stat_rx_refill_dropped +=
				VMXNET3_RX_FILL - rxq.prod_count;
			break;
		}
	...
	if (added_buffers) {
		// send count to NIC
		mmio_write32(this->ptbase + VMXNET3_PT_RXPROD1 +
				     0x200 * rxq.index,
			     rxq.producers % vmxnet3::NUM_RX_DESC);
	}
}

net::Packet_ptr vmxnet3::recv_packet(uint8_t *data, uint16_t size)
{
	auto *ptr = (net::Packet *)(data - DRIVER_OFFSET - sizeof(net::Packet));
	new (ptr) net::Packet(DRIVER_OFFSET, size, DRIVER_OFFSET + size,
			      &bufstore());
	return net::Packet_ptr(ptr);
}
net::Packet_ptr vmxnet3::create_packet(int link_offset)
{
	auto *ptr = (net::Packet *)bufstore().get_buffer();
	new (ptr) net::Packet(DRIVER_OFFSET + link_offset, 0,
...
bool vmxnet3::transmit_handler()
{
	...
	while (true) {
		...
		uint32_t gen = (tx.consumers & VMXNET3_NUM_TX_COMP) ?
				       0 :
				       VMXNET3_TXCF_GEN;

		auto &comp = dma->tx_comp[idx];
		if (gen != (comp.flags & VMXNET3_TXCF_GEN))
			break;
		...
		auto *packet =
			(net::Packet *)(tx.buffers[desc] - DRIVER_OFFSET -
					sizeof(net::Packet));
...
bool vmxnet3::receive_handler(const int Q)
{
	...
	while (true) {
		...
		uint32_t gen = (rx[Q].consumers & VMXNET3_NUM_RX_COMP) ?
				       0 :
				       VMXNET3_RXCF_GEN;

		auto &comp = dma->rx[Q].comp[idx];
		// break when exiting this generation
		if (gen != (comp.flags & VMXNET3_RXCF_GEN))
			break;
		...
		if (UNLIKELY((comp.len & (VMXNET3_MAX_BUFFER_LEN - 1)) == 0)) {
			...
			auto *packet = (net::Packet *)(tx.buffers[desc] -
						       DRIVER_OFFSET -
						       sizeof(net::Packet));
			delete packet; // call deleter on Packet to release it
...
void vmxnet3::poll()
{
	if (tqa_events_.empty())
		return;
	if (this->already_polling)
		return;
...
{
	this->dma->shared.rx_filter.vlan_filter[((id >> 3) & 0x1FF)] |=
		(1 << (id & 7));
	// TODO in smp/mt protect this with IRQ disable / enable for the entire driver
	mmio_write32(this->iobase + VMXNET3_VD_CMD,
		     VMXNET3_CMD_UPDATE_VLAN_FILTERS);
}
...
void vmxnet3::move_to_this_cpu()
{
	...
	if (m_pcidev.has_msix()) {
		for (size_t i = 0; i < irqs.size(); i++) {
			...
			m_pcidev.rebalance_msix_vector(
				i, SMP::cpu_id(), IRQ_BASE + this->irqs[i]);
		}
	}
}

#include <hw/pci_manager.hpp>
__attribute__((constructor)) static void register_func()
{
	hw::PCI_manager::register_nic(PCI::VENDOR_VMWARE, PRODUCT_ID,
				      &vmxnet3::new_instance);
}
