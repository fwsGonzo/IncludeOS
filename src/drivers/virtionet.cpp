...
VirtioNet::VirtioNet(hw::PCI_Device &d, const uint16_t /*mtu*/)
	: Virtio(d),
	  ...
	  m_pcidev(d), bufstore_{ VNET_TOT_BUFFERS(),
				  2048 /* half-page buffers */ },

	  stat_sendq_max_{ Statman::get()
				   .create(Stat::UINT64,
					   device_name() + ".sendq_max")
				   .get_uint64() },
	  stat_sendq_now_{ Statman::get()
				   .create(Stat::UINT64,
					   device_name() + ".sendq_now")
				   .get_uint64() },
	  stat_sendq_limit_dropped_{ Statman::get()
					     .create(Stat::UINT64,
						     device_name() +
							     ".sendq_dropped")
					     .get_uint64() },
	  stat_rx_refill_dropped_{ Statman::get()
					   .create(Stat::UINT64,
						   device_name() +
							   ".rx_refill_dropped")
					   .get_uint64() },
	  stat_bytes_rx_total_{ Statman::get()
					.create(Stat::UINT64,
						device_name() +
							".stat_rx_total_bytes")
					.get_uint64() },
...
#undef VNET_TOT_BUFFERS
	...
	uint32_t needed_features =
		...
		(1
		 << VIRTIO_NET_F_STATUS); //| (1 << VIRTIO_NET_F_MRG_RXBUF); //Merge RX Buffers
		//(Everything i 1 buffer)
	...
	CHECK(features() & (1 << VIRTIO_NET_F_MRG_RXBUF), "Merge RX buffers");

	/** RX que is 0, TX Queue is 1 - Virtio Std. §5.1.2  */
	new (&rx_q) Virtio::Queue(device_name() + ".rx_q", queue_size(0), 0,
				  iobase());
	new (&tx_q) Virtio::Queue(device_name() + ".tx_q", queue_size(1), 1,
				  iobase());
	new (&ctrl_q) Virtio::Queue(device_name() + ".ctl_q", queue_size(2), 2,
				    iobase());

	// Step 1 - Initialize RX/TX queues
	auto success = assign_queue(0, rx_q.queue_desc());
	CHECKSERT(success, "RX queue (%u) assigned (%p) to device", rx_q.size(),
		  rx_q.queue_desc());

	success = assign_queue(1, tx_q.queue_desc());
	CHECKSERT(success, "TX queue (%u) assigned (%p) to device", tx_q.size(),
		  tx_q.queue_desc());
	...
	if (has_msix()) {
		...
		Events::get().subscribe(
			irqs[0], { this, &VirtioNet::msix_recv_handler });
		Events::get().subscribe(
			irqs[1], { this, &VirtioNet::msix_xmit_handler });
		Events::get().subscribe(
			irqs[2], { this, &VirtioNet::msix_conf_handler });
	} else {
		auto irq = Virtio::get_legacy_irq();
		Events::get().subscribe(irq,
					{ this, &VirtioNet::legacy_handler });
	}
...
void VirtioNet::msix_recv_handler()
{
	...
	while (rx_q.new_incoming() && max-- > 0) {
		...
		if (not Nic::buffers_still_available(
...
void VirtioNet::add_receive_buffer(uint8_t *pkt)
{
	...
	Token token2{ { vnet + sizeof(virtio_net_hdr), max_packet_len() },
		      Token::IN };
...
net::Packet_ptr VirtioNet::recv_packet(uint8_t *data, uint16_t size)
{
	auto *ptr = (net::Packet *)(data - sizeof(net::Packet));

	new (ptr) net::Packet(sizeof(virtio_net_hdr),
			      size - sizeof(virtio_net_hdr), size, &bufstore());

	return net::Packet_ptr(ptr);
}

net::Packet_ptr VirtioNet::create_packet(int link_offset)
{
	auto *ptr = (net::Packet *)bufstore().get_buffer();

	new (ptr) net::Packet(sizeof(virtio_net_hdr) + link_offset, 0,
			      sizeof(virtio_net_hdr) + frame_offset_link() +
				      MTU(),
			      &bufstore());
...
void VirtioNet::move_to_this_cpu()
{
	...
	Events::get().subscribe(irqs[0],
				{ this, &VirtioNet::msix_recv_handler });
	Events::get().subscribe(irqs[1],
				{ this, &VirtioNet::msix_xmit_handler });
	Events::get().subscribe(irqs[2],
				{ this, &VirtioNet::msix_conf_handler });
#ifndef NO_DEFERRED_KICK
...
__attribute__((constructor)) void autoreg_virtionet()
{
	hw::PCI_manager::register_nic(PCI::VENDOR_VIRTIO, 0x1000,
