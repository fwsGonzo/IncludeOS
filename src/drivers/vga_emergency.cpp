...
static inline const char *find_rev_nth(const char *ptr, const char *begin,
				       int counter)
{
	int lc = 0;
...
static inline const char *find_nth(const char *ptr, const char *end,
				   int counter)
{
	int lc = 0;
...
extern "C" void keyboard_emergency_handler()
{
	render_vga_text();
	using namespace hw;
	/*
...
extern "C" void panic_perform_inspection_procedure()
{
	if (procedure_entered)
		return;
...
static __attribute__((constructor)) void hest()
{
