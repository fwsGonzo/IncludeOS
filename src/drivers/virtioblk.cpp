...
VirtioBlk::VirtioBlk(hw::PCI_Device &d)
	: Virtio(d), hw::Block_device(),
	  req(device_name() + ".req0", queue_size(0), 0, iobase()), inflight(0)
{
	INFO("VirtioBlk", "Initializing");
	{
		auto &reqs = Statman::get().create(Stat::UINT32,
						   device_name() + ".requests");
		this->requests = &reqs.get_uint32();
		*this->requests = 0;

		auto &err = Statman::get().create(Stat::UINT32,
						  device_name() + ".errors");
	...
	CHECK(features() & FEAT(VIRTIO_BLK_F_BARRIER), "Barrier is enabled");
	CHECK(features() & FEAT(VIRTIO_BLK_F_SIZE_MAX), "Size-max is known");
	CHECK(features() & FEAT(VIRTIO_BLK_F_SEG_MAX), "Seg-max is known");
	CHECK(features() & FEAT(VIRTIO_BLK_F_GEOMETRY),
	      "Geometry structure is used");
	CHECK(features() & FEAT(VIRTIO_BLK_F_RO), "Device is read-only");
	CHECK(features() & FEAT(VIRTIO_BLK_F_BLK_SIZE), "Block-size is known");
	CHECK(features() & FEAT(VIRTIO_BLK_F_SCSI), "SCSI is enabled :(");
	CHECK(features() & FEAT(VIRTIO_BLK_F_FLUSH), "Flush enabled");
	...
	if (has_msix()) {
		...
		Events::get().subscribe(irqs[0],
					{ this, &VirtioBlk::service_RX });
		Events::get().subscribe(
			irqs[1], { this, &VirtioBlk::msix_conf_handler });
	} else {
		auto &irqs = this->get_irqs();
		Events::get().subscribe(irqs[0],
					{ this, &VirtioBlk::irq_handler });
	}

	// Done
	INFO("VirtioBlk", "Block device with %zu sectors capacity",
	     config.capacity);
}
...
{
	Token token1{ { (uint8_t *)&vbr->hdr, sizeof(scsi_header_t) },
		      Token::OUT };
	Token token2{ { (uint8_t *)&vbr->io, sizeof(blk_io_t) }, Token::IN };
	Token token3{ { (uint8_t *)&vbr->resp, 1 },
		      Token::IN }; // 1 status byte
...
void VirtioBlk::read(block_t blk, size_t cnt, on_read_func func)
{
	...
	for (size_t i = 0; i < cnt; i++) {
		...
		auto *vbr = new request_t(
			blk + i,
			request_handler_t::make_packed([this, i, func, results,
							bigbuf](uint8_t *data) {
...
struct Autoreg_virtioblk {
	Autoreg_virtioblk()
	{
		hw::PCI_manager::register_blk(PCI::VENDOR_VIRTIO, 0x1001,
					      &VirtioBlk::new_instance);
	}
} autoreg_virtioblk;
