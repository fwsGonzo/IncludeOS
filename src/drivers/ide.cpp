...
struct workq_item {
	...
	workq_item(uint8_t id, block_t blk, uint32_t cnt, on_read_func call)
		: drive_id(id), read(true), sector(blk), total(cnt),
		  readcall(std::move(call))
	{
		buffer = std::make_shared<os::mem::buffer>(total *
							   IDE::SECTOR_SIZE);
	}
#endif
	...
	workq_item(uint8_t id, block_t blk, buffer_t buf, on_write_func call)
		: drive_id(id), read(false), sector(blk), buffer(buf),
		  writecall(std::move(call))
	{
...
IDE::IDE(hw::PCI_Device &pcidev, selector_t sel) : drive_id{ (uint8_t)sel }
{
	INFO("IDE", "VENDOR_ID : 0x%x, PRODUCT_ID : 0x%x", pcidev.vendor_id(),
	     pcidev.product_id());
	INFO("IDE", "Attaching to  PCI addr 0x%x", pcidev.pci_addr());

	/** PCI device checking */
	CHECKSERT(pcidev.vendor_id() == IDE_VENDOR_ID, "Vendor ID is INTEL");
	CHECKSERT(pcidev.product_id() == IDE_PRODUCT_ID,
		  "Product ID is IDE Controller");
	...
	if (command_sets & (1 << 26)) { // 48-bits MAX_LBA_EXT
		this->num_blocks = (read_array[101] << 16) | read_array[100];
	} else { // 28-bits CHS (MAX_LBA)
		this->num_blocks = (read_array[61] << 16) | read_array[60];
	}
	INFO("IDE", "%" PRIu64 " sectors (%" PRIu64 " bytes)", num_blocks,
	     num_blocks * IDE::SECTOR_SIZE);
...
{
	uint8_t ret;
	while (((ret = hw::inb(IDE_STATUS)) & IDE_BUSY) == IDE_BUSY)
		asm("pause");
}
...
__attribute__((constructor)) static void autoreg()
{
	PCI_manager::register_blk(PCI::VENDOR_INTEL, IDE_PRODUCT_ID,
				  &IDE::new_instance);
};
