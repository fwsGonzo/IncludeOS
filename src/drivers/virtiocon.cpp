...
VirtioCon::VirtioCon(hw::PCI_Device &d) : Virtio(d), m_index(index_counter)
{
	index_counter++;
	INFO("VirtioCon", "Driver initializing");

	new (&rx) Virtio::Queue(device_name() + ".rx_q", queue_size(0), 0,
				iobase());
	new (&tx) Virtio::Queue(device_name() + ".tx_q", queue_size(1), 1,
				iobase());
	new (&ctl_rx) Virtio::Queue(device_name() + ".ctl_rx_q", queue_size(2),
				    2, iobase());
	new (&ctl_tx) Virtio::Queue(device_name() + ".ctl_tx_q", queue_size(3),
				    3, iobase());

	const uint32_t needed_features = FEAT(VIRTIO_CONSOLE_F_MULTIPORT);
	...
	if (has_msix()) {
		...
		Events::get().subscribe(
			irqs[0], { this, &VirtioCon::msix_recv_handler });
		Events::get().subscribe(
			irqs[1], { this, &VirtioCon::msix_xmit_handler });
		Events::get().subscribe(irqs[2],
					{ this, &VirtioCon::event_handler });
	} else {
	...
	// Done
	INFO("VirtioCon", "Console with size (%u, %u), %u ports", config.cols,
	     config.rows, config.max_nr_ports);
	rx.kick();
}
