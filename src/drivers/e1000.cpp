...
static inline uint16_t buffer_size_for_mtu(const uint16_t mtu)
{
	...
	const uint16_t total =
		header + sizeof(net::ethernet::VLAN_header) + mtu;
	if (total <= 2048)
		return 2048;
...
e1000::e1000(hw::PCI_Device &d, uint16_t mtu)
	: Link(Link_protocol{ { this, &e1000::transmit }, mac() }), m_pcidev(d),
	  m_mtu(mtu), bufstore_{ NUM_PACKET_BUFFERS, buffer_size_for_mtu(mtu) }
{
	static_assert((NUM_RX_DESC * sizeof(rx_desc)) % 128 == 0,
		      "Ring length must be 128-byte aligned");
	static_assert((NUM_TX_DESC * sizeof(tx_desc)) % 128 == 0,
		      "Ring length must be 128-byte aligned");
	...
	if (this->use_msix == false) {
		...
		if (variant == 0x153A || variant == 0x1539) {
			...
			Events::get().subscribe(I217_IRQ, [this, real_irq]() {
	...
	for (unsigned i = 8; i < 24+32; i++)
	{
	  ...
	  [this, i] () {
		...
		Events::get().subscribe(real_irq,
					{ this, &e1000::event_handler });
		this->irqs.push_back(real_irq);
	}

	if (deferred_event == 0) {
		deferred_event =
			Events::get().subscribe(&e1000::do_deferred_xmit);
	}
	...
	Timers::periodic(std::chrono::seconds(2), [this](int) {
	...
	Timers::periodic(std::chrono::milliseconds(1),
			 [this](int) { this->event_handler(); });
#endif
#ifdef E1000E_FAKE_EVENT_HANDLER
	Timers::periodic(std::chrono::milliseconds(1), [this](int) {
...
#define IVAR_INT_ALLOC_VALID 0x8 // 10.2.4.9 p.328
	...
	// rx queue 0 2:0
	uint8_t vec0 =
		Events::get().subscribe({ this, &e1000::receive_handler });
	int m0 = m_pcidev.setup_msix_vector(SMP::cpu_id(), IRQ_BASE + vec0);
	ivar |= (IVAR_INT_ALLOC_VALID | m0);

	// tx queue 0 10:8
	uint8_t vec1 =
		Events::get().subscribe({ this, &e1000::transmit_handler });
...
void e1000::wait_millis(int millis)
{
	...
	Timers::oneshot(std::chrono::milliseconds(millis),
			[&done_waiting](int) { done_waiting = true; });
...
void e1000::detect_eeprom()
{
	...
	INFO2("NVM present: %d  type: %s", NVM_present,
	      (use_eeprom) ? "EEPROM" : "Flash");
...
void e1000::link_up()
{
	...
	if (this->link_state_up == false)
		...
		const char *duplex =
			(status & 1) ? "Full Duplex" : "Half Duplex";
...
net::Packet_ptr e1000::recv_packet(uint8_t *data, uint16_t size)
{
...
net::Packet_ptr e1000::create_packet(int link_offset)
{
...
void e1000::transmit_handler()
{
	...
	if (can_transmit() || sendq_size < NUM_TX_QUEUE) {
		const uint32_t free_tokens =
			free_transmit_descr() + (NUM_TX_QUEUE - sendq_size);
...
__attribute__((constructor)) static void register_func()
{
	using namespace hw;
	// e1000
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x100E,
				  &e1000::new_instance);
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x100F,
				  &e1000::new_instance);
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x109A,
				  &e1000::new_instance);
	// e1000e 82574L
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x10D3,
				  &e1000::new_instance);
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x10EA,
				  &e1000::new_instance);
	// I217
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x153A,
				  &e1000::new_instance);
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x1539,
				  &e1000::new_instance);
	// I219
	PCI_manager::register_nic(PCI::VENDOR_INTEL, 0x15B8,
				  &e1000::new_instance);
}
