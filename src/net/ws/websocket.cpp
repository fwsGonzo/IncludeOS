...
namespace net
{
static inline std::string encode_hash(const std::string &key)
{
...
WebSocket_ptr WebSocket::upgrade(http::Request &req,
				 http::Response_writer &writer)
{
	// validate handshake
...
WebSocket_ptr WebSocket::upgrade(http::Error err, http::Response &res,
...
WebSocket::create_request_handler(Connect_handler on_connect,
	...
	return http::Server::Request_handler::make_packed(
		[on_connect{ std::move(on_connect) },
		 on_accept{ std::move(on_accept) }](
			...
			if (on_accept) {
				const bool accepted = on_accept(
					writer->connection().peer(),
					std::string(
						req->header().value("Origin")));
				if (not accepted) {
					writer->write_header(
						http::Unauthorized);
					on_connect(nullptr);
...
http::Basic_client::Response_handler
WebSocket::create_response_handler(Connect_handler on_connect, std::string key)
{
	return http::Basic_client::Response_handler::make_packed(
		[on_connect{ std::move(on_connect) },
		 key{ std::move(key) }](http::Error err, http::Response_ptr res,
			...
			Connect_handler callback)
{
	...
	client.get(remote, ws_headers,
		   WS_client_connector::create_response_handler(
...
size_t WebSocket::Message::append(const uint8_t *data, size_t len)
{
	...
	if (UNLIKELY(this->header_complete() == false)) {
		auto hdr_bytes =
			std::min(header().header_length() - this->header_length,
				 (int)len);
	...
	if (this->header_complete()) {
		const size_t insert_size =
			std::min(data_.capacity() - data_.size(), len);
...
void WebSocket::finalize_message()
{
	...
	case op_code::BINARY:
		...
		if (this->on_read) {
			...
			if (this->m_deferred_close)
				this->close(this->m_deferred_close);
		}
		return;
	case op_code::CLOSE:
	...
	case op_code::PING:
		if (on_ping(hdr.data(),
			    hdr.data_length())) // if return true, pong back
			write_opcode(op_code::PONG, hdr.data(),
				     hdr.data_length());
		break;
	case op_code::PONG:
		ping_timer.stop();
...
static Stream::buffer_t create_wsmsg(size_t len, op_code code, bool client)
{
	...
	if (client) {
		hdr.set_masked(
			(os::cycles_since_boot() ^ (uintptr_t)buffer.get()) &
			0xffffffff);
	}
...
void WebSocket::close(const uint16_t reason)
{
	...
	if (this->stream->is_writable()) {
		...
		this->write_opcode(op_code::CLOSE, (const char *)&data,
				   sizeof(data));
	}
