...
namespace net
{
UDP::UDP(Stack &inet) : stack_(inet), ports_(inet.udp_ports())
{
...
void UDP::receive4(net::Packet_ptr ptr)
{
	...
	const bool is_bcast = (dst_ip == IP4::ADDR_BCAST or
			       dst_ip == stack_.broadcast_addr());
...
void UDP::receive(udp::Packet_view_ptr udp_packet, const bool is_bcast)
{
	...
	if (it != sockets_.end()) {
		PRINT("<%s> UDP found listener on %s\n",
		      stack_.ifname().c_str(),
		      udp_packet->destination().to_string().c_str());
...
void UDP::send_dest_unreachable(udp::Packet_view_ptr udp)
{
	if (udp->ipv() == Protocol::IPv4) {
		...
		stack_.icmp().destination_unreachable(
			std::move(ip4), icmp4::code::Dest_unreachable::PORT);
	}
}
...
void UDP::error_report(const Error &err, Socket dest)
{
	...
	// Report to application layer that got an ICMP error message of type and code (reason and
	// subreason)

	// Find callback with this destination (address and port), and call it with the incoming err
	// If the error f.ex. is an ICMP_error of type Destination Unreachable and code Fragmentation
	// Needed, the err object contains the Path MTU so the user can get a hold of it in the
	// application layer and choose to retransmit or not (the packet has been dropped by a node
	// somewhere along the path to the receiver because the Don't Fragment bit was set on the packet
	// and the packet was larger than the node's MTU value)
...
udp::Socket &UDP::bind(const net::Socket &socket)
{
	...
	if (UNLIKELY(not stack_.is_valid_source(addr)))
		throw UDP_error{ "Cannot bind to address: " +
				 addr.to_string() };
	...
	auto it = sockets_.emplace(std::piecewise_construct,
...
void UDP::flush_expired()
{
	...
	for (auto it = error_callbacks_.begin();
	...
	while (!sendq.empty() && num != 0) {
		...
		if (buffer.done()) {
			...
			if (buffer.error_callback != nullptr) {
				error_callbacks_.emplace(
					std::piecewise_construct,
					std::forward_as_tuple(buffer.dst),
					std::forward_as_tuple(Error_entry{
						buffer.error_callback }));
...
UDP::WriteBuffer::WriteBuffer(UDP &stack, net::Socket source, net::Socket dest,
	...
	: udp(stack), src{ std::move(source) }, dst{ std::move(dest) },
	  len(length), offset(0), send_callback(cb), error_callback(ecb)
{
	// create a copy of the data,
	auto *copy = new uint8_t[len];
	memcpy(copy, data, length);
	// make it shared
	this->buf = std::shared_ptr<uint8_t>(copy,
...
void UDP::WriteBuffer::write()
{
	...
	PRINT("<%s> UDP: %i bytes to write, need %i packets \n",
	      udp.stack().ifname().c_str(), remaining(),
	      remaining() / udp.max_datagram_size() +
		      (remaining() % udp.max_datagram_size() ? 1 : 0));
	...
	while (remaining()) {
		...
		total = (total > udp.max_datagram_size()) ?
					...
					const net::Socket &dst)
{
	if (src.address().is_v6()) {
		...
		auto pkt = std::make_unique<udp::Packet6_view>(
			stack_.create_ip6_packet(Protocol::UDP));
		pkt->init(src, dst);
		return pkt;
	} else {
		Expects(dst.address().is_v4());
		auto pkt = std::make_unique<udp::Packet4_view>(
			stack_.create_ip_packet(Protocol::UDP));
		pkt->init(src, dst);
		return pkt;
	}
}
