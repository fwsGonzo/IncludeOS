...
inline std::unique_ptr<Botan::Private_Key> read_pkey(fs::Dirent &key_file)
{
	...
	return std::unique_ptr<Botan::Private_Key>(
		Botan::PKCS8::load_key(data, get_rng()));
}
...
void Botan_server::load_credentials(const std::string &server_name,
				    ...
				    fs::Dirent &file_server_key)
{
	...
	auto *credman = net::Credman::create(
		server_name, get_rng(), std::move(ca_key),
...
{
	connect(std::make_unique<net::botan::Server>(
		std::make_unique<net::tcp::Stream>(std::move(conn)), rng,
		*credman));
}
