...
namespace http
{
void S2N_server::initialize(const std::string &ca_cert,
			    const std::string &ca_key)
{
	...
	int res = s2n_config_add_cert_chain_and_key(config, ca_cert.c_str(),
						    ca_key.c_str());
	if (res < 0) {
...
{
	connect(std::make_unique<s2n::TLS_stream>(
