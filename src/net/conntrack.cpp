...
{
	switch (state) {
	case Conntrack::State::NEW:
		return "NEW";
	case Conntrack::State::ESTABLISHED:
		return "EST";
	case Conntrack::State::RELATED:
		return "RELATED";
	case Conntrack::State::UNCONFIRMED:
		return "UNCONFIRMED";
	default:
		return "???";
	}
...
Conntrack::Entry *Conntrack::simple_track_in(Quadruple q, const Protocol proto)
{
	...
	CTDBG("<Conntrack> Track in S: %s - D: %s\n", q.src.to_string().c_str(),
	      q.dst.to_string().c_str());
	...
	update_timeout(*entry, (entry->state == State::ESTABLISHED) ?
				       timeout.established :
				       timeout.confirmed);
...
Conntrack::Conntrack() : Conntrack(0)
{
}

Conntrack::Conntrack(size_t max_entries)
	: maximum_entries{ max_entries }, tcp_in{ &dumb_in },
	  tcp6_in{ &dumb6_in }, flush_timer({ this, &Conntrack::on_timeout })
{
}
...
Conntrack::Entry *Conntrack::get(const Quadruple &quad,
...
Conntrack::Entry *Conntrack::add_entry(const Quadruple &quad,
...
Conntrack::Entry *Conntrack::update_entry(const Protocol proto,
					  const Quadruple &oldq,
					  const Quadruple &newq)
{
	...
	auto &quad = (entry->first == oldq) ? entry->first : entry->second;
...
int Conntrack::deserialize_from(void *addr)
{
	...
	for (auto i = size; i > 0; i--) {
		...
		bool insert = false;
		insert = entries.insert_or_assign(
					{ entry->first, entry->proto }, entry)
				 .second;
		if (not insert)
			dupes++;
		insert = entries.insert_or_assign(
					{ entry->second, entry->proto }, entry)
				 .second;
		if (not insert)
			dupes++;
	}
...
void Conntrack::serialize_to(std::vector<char> &buf) const
{
	...
	const auto expected_buf_size =
		sizeof(size) + (size * (sizeof(Entry) - sizeof(Entry_handler)));
