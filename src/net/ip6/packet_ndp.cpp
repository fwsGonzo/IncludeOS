...
bool NdpPacket::parse_prefix(Pinfo_handler autoconf_cb, Pinfo_handler onlink_cb)
{
			      ...
			      Pinfo_handler onlink_cb)
{
	...
	for (pinfo = reinterpret_cast<struct prefix_info *>(opt);
	     pinfo != nullptr; pinfo = pinfo_next(pinfo)) {
		...
		if (pinfo->onlink) {
			...
			if (pinfo->prefix_len == 64) {
				confaddr.set_part<uint64_t>(
					1, pinfo->prefix.get_part<uint64_t>(1));
			} else {
				PRINT("NDP: Prefix option: autoconf: "
				      " prefix with wrong len: %d",
				      pinfo->prefix_len);
				return false;
			}
...
{
	return *reinterpret_cast<RouterRedirect *>(
		&(icmp6_.header().payload[0]));
}
...
void NdpPacket::set_ndp_options_header(uint8_t type, uint8_t len)
{
	...
	icmp6_.add_payload(reinterpret_cast<uint8_t *>(&header), sizeof header);
}
} // namespace net::ndp
