...
IP6::IP6(Stack &inet) noexcept
	: stack_{ inet },
	  packets_rx_{ Statman::get()
			       .create(Stat::UINT64,
				       inet.ifname() + ".ip6.packets_rx")
			       .get_uint64() },
	  packets_tx_{ Statman::get()
			       .create(Stat::UINT64,
				       inet.ifname() + ".ip6.packets_tx")
			       .get_uint64() },
...
IP6::IP_packet_ptr IP6::drop(IP_packet_ptr ptr, Direction direction,
			     Drop_reason reason)
{
	packets_dropped_++;
...
{
	return stack_.is_valid_source6(dst) or local_ip() == ADDR_ANY;
}
...
void PacketIP6::calculate_payload_offset()
{
	...
	while (next_proto != Protocol::IPv6_NONXT) {
		...
		// bounds check
		if (reader + sizeof(ip6::Extension_header) >=
		    this->data_end()) {
			break;
		}
...
void IP6::receive(Packet_ptr pckt, const bool link_bcast)
{
	...
	PRINT("<IP6 Receive> Source IP: %s, Dest.IP: %s, Next header: %d,"
	      ...
	      (int)packet->next_header(), packet->payload_length(),
	      packet->hop_limit(), packet->ip6_version(),
	      packet->traffic_class(), packet->flow_label());
	...
	default:
		...
		// stack_.icmp().destination_unreachable(std::move(packet),
		// icmp6::code::Dest_unreachable::PROTOCOL);
...
void IP6::ship(Packet_ptr pckt, ip6::Addr next_hop, Conntrack::Entry_ptr ct)
{
	...
	if (next_hop == ip6::Addr::addr_any) {
		...
		next_hop =
			dst.is_linklocal() ? dst : stack_.ndp().next_hop(dst);
