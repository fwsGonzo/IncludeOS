...
constexpr int ip6_max_stringlength =
	ip6_delimiters + (ip6_max_entry_length * 8);
...
Addr::Addr(const std::string &addr)
{
	...
	for (auto &character : addr) {
		if (!isLegalCharacter(character)) {
			throw Invalid_Address("Illegal character " +
	...
	while (std::getline(tokenStream, token, ':')) {
		...
		if (token.size() > 4) {
			throw Invalid_Address(
				"To many entries in hexadectet/quibble/quad-nibble " +
				token + " in " + addr);
		}
