...
Ndp::Ndp(Stack &inet) noexcept
	: requests_rx_{ Statman::get()
				.create(Stat::UINT32,
					inet.ifname() + ".ndp.requests_rx")
				.get_uint32() },
	  requests_tx_{ Statman::get()
				.create(Stat::UINT32,
					inet.ifname() + ".ndp.requests_tx")
				.get_uint32() },
	  replies_rx_{ Statman::get()
			       .create(Stat::UINT32,
				       inet.ifname() + ".ndp.replies_rx")
			       .get_uint32() },
	  replies_tx_{ Statman::get()
			       .create(Stat::UINT32,
				       inet.ifname() + ".ndp.replies_tx")
			       .get_uint32() },
	  inet_{ inet },
	  host_params_{},
	  router_params_{},
...
void Ndp::send_neighbour_advertisement(icmp6::Packet &req)
{
	...
	if (res.ip().capacity() <
	    IP6_HEADER_LEN + (int)res.header_size() + req.payload().size()) {
	...
	// we do that for now.
	using Target_ll_addr =
		ndp::option::Target_link_layer_address<MAC::Addr>;
...
{
	const auto &adv =
		req.view_payload_as<const ndp::View<ndp::Neighbor_adv> >();
	...
	adv.parse_options(data + payload.size(), [&](const auto *opt) {
		...
		case TARGET_LL_ADDR: {
			const auto lladdr =
				reinterpret_cast<const Target_link_layer_address<
					MAC::Addr> *>(opt)
					->addr;

			// For now, just create a cache entry, if one doesn't exist
			cache(target, lladdr,
			      adv.solicited() ? NeighbourStates::REACHABLE :
						NeighbourStates::STALE,
			      NEIGH_UPDATE_WEAK_OVERRIDE |
				      (adv.override() ? NEIGH_UPDATE_OVERRIDE :
							0) |
...
void Ndp::send_neighbour_solicitation(ip6::Addr target)
{
    ...
    */
	if (req.ip().ip_src() != IP6::ADDR_ANY) {
		using Source_ll_addr =
			ndp::option::Source_link_layer_address<MAC::Addr>;
	...
	PRINT("NDP: Sending Neighbour solicit size: %i payload size: %i,"
	      ...
	      req.ip().ip_src().str().c_str(), req.ip().ip_dst().str().c_str(),
	      dest_mac.str().c_str());
...
void Ndp::receive_neighbour_solicitation(icmp6::Packet &req)
{
	...
	auto *data = payload.data();
	const auto &sol =
		*reinterpret_cast<const ndp::View<ndp::Neighbor_sol> *>(data);
	...
	sol.parse_options(data + payload.size(), [&](const auto *opt) {
		...
		case SOURCE_LL_ADDR: {
			using Source_ll_addr =
				Source_link_layer_address<MAC::Addr>;
			lladdr = reinterpret_cast<const Source_ll_addr *>(opt)
					 ->addr;
			break;
		}
		case NONCE: {
...
void Ndp::receive_router_solicitation(icmp6::Packet &req)
{
	...
	auto *data = payload.data();
	const auto &sol =
		*reinterpret_cast<const ndp::View<ndp::Router_sol> *>(data);
	...
	sol.parse_options(data + payload.size(), [&](const auto *opt) {
		...
		case SOURCE_LL_ADDR: {
			using Source_ll_addr =
				Source_link_layer_address<MAC::Addr>;
			const auto lladdr =
				reinterpret_cast<const Source_ll_addr *>(opt)
					->addr;

			cache(req.ip().ip_src(), lladdr, NeighbourStates::STALE,
			      NEIGH_UPDATE_WEAK_OVERRIDE |
				      NEIGH_UPDATE_OVERRIDE |
				      NEIGH_UPDATE_OVERRIDE_ISROUTER);
...
void Ndp::receive_router_advertisement(icmp6::Packet &req)
{
	...
	auto *data = payload.data();
	const auto &adv =
		*reinterpret_cast<const ndp::View<ndp::Router_adv> *>(data);
	...
	adv.parse_options(data + payload.size(), [&](const auto *opt) {
		...
		case PREFIX_INFO: {
			const auto *pinfo =
				reinterpret_cast<const Prefix_info *>(opt);
			...
			if (pinfo->onlink()) {
				add_addr_onlink(pinfo->prefix,
						pinfo->prefix_len,
						pinfo->valid_lifetime());
			}
		...
		case SOURCE_LL_ADDR: {
			const auto lladdr =
				reinterpret_cast<const Source_link_layer_address<
					MAC::Addr> *>(opt)
					->addr;
		...
		case MTU: {
			const auto mtu =
				reinterpret_cast<const Mtu *>(opt)->mtu;
...
ip6::Addr Ndp::next_hop(const ip6::Addr &dst) const
{
	...
	for (const auto &entry : prefix_list_) {
		if (entry.match(dst)) {
			if (match)
				match = (entry.prefix() > match->prefix()) ?
						&entry :
						match;
			else
				match = &entry;
...
void Ndp::cache(ip6::Addr ip, uint8_t *ll_addr, NeighbourStates state,
		uint32_t flags, bool update)
{
	if (ll_addr) {
...
void Ndp::cache(ip6::Addr ip, MAC::Addr mac, NeighbourStates state,
		uint32_t flags, bool update)
{
	...
	if (entry != neighbour_cache_.end()) {
		PRINT("Cached entry found: %s recorded @ %zu. Updating timestamp\n",
		      entry->second.mac().str().c_str(),
		      entry->second.timestamp());
		if (entry->second.mac() != mac) {
			neighbour_cache_.erase(entry);
			neighbour_cache_.emplace(std::make_pair(
				ip, Neighbour_Cache_entry{ mac, state,
							   flags })); // Insert
		} else if (update) {
		...
		neighbour_cache_.emplace(std::make_pair(
			ip,
			Neighbour_Cache_entry{ mac, state, flags })); // Insert
...
void Ndp::dest_cache(ip6::Addr dest_ip, ip6::Addr next_hop)
{
	...
	if (entry != dest_cache_.end()) {
		...
		dest_cache_.emplace(std::make_pair(
			dest_ip, Destination_Cache_entry{ next_hop }));
	}
}
...
void Ndp::resolve_waiting()
{
	...
	for (auto it = waiting_packets_.begin();
	     it != waiting_packets_.end();) {
...
void Ndp::await_resolution(Packet_ptr pckt, ip6::Addr next_hop)
{
	...
	if (queue != waiting_packets_.end()) {
		...
		waiting_packets_.emplace(std::make_pair(
			next_hop, Queue_entry{ std::move(pckt) }));
...
void Ndp::perform_dad(ip6::Addr tentative_addr, Dad_handler delg)
{
...
void Ndp::add_addr_onlink(ip6::Addr ip, uint8_t prefix, uint32_t valid_lifetime)
{
	...
	if (entry == prefix_list_.end()) {
		if (valid_lifetime) {
			prefix_list_.emplace_back(ip, prefix, 0,
						  valid_lifetime);
		}
	} else {
		if (valid_lifetime) {
			    ...
			    uint32_t valid_lifetime)
{
	PRINT("NDP: RA: Adding address %s with preferred lifetime: %u"
	      " and valid lifetime: %u\n",
	      ip.to_string().c_str(), preferred_lifetime, valid_lifetime);
	...
	if (entry == prefix_list_.end()) {
		prefix_list_.emplace_back(ip, prefix, preferred_lifetime,
					  valid_lifetime);
