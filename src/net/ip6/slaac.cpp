...
Slaac::Slaac(Stack &inet)
	: stack(inet), token_{ 0 }, use_token_{ false },
	  tentative_addr_({ IP6::ADDR_ANY, 64, 0, 0 }),
	  linklocal_completed(false),
	  dad_transmits_(LINKLOCAL_RETRIES), timeout_timer_{
		  { this, &Slaac::autoconf_trigger }
	  }
{
	// default timed out handler spams logs
	this->on_config([this](bool completed) {
		if (completed) {
...
void Slaac::autoconf_trigger()
{
	...
	if (!linklocal_completed) {
		...
		PRINT("Auto-configuring ip6-address %s for stack %s\n",
		      tentative_addr_.addr().str().c_str(),
		      stack.ifname().c_str());
...
void Slaac::autoconf_start(int retries, uint64_t token, bool use_token)
{
	...
	if (use_token) {
		...
		tentative_addr_ = { ip6::Addr::link_local(
					    stack.link_addr().eui64()),
				    64, 0, 0 };
	}
...
void Slaac::autoconf_global()
{
	...
	stack.ndp().send_router_solicitation(
		{ this, &Slaac::process_prefix_info });
...
void Slaac::perform_dad()
{
	dad_transmits_--;
	// Perform DAD
	stack.ndp().perform_dad(tentative_addr_.addr(),
				{ this, &Slaac::dad_handler });
...
void Slaac::process_prefix_info(const ndp::option::Prefix_info &pinfo)
{
	...
	auto eui64 =
		(use_token_) ? token_ : MAC::Addr::eui64(stack.link_addr());
	...
	if (not stack.addr6_config().has(addr)) {
		...
		tentative_addr_ = { addr, prefix_len, preferred_lifetime,
				    valid_lifetime };
		dad_transmits_ = GLOBAL_RETRIES;
		timeout_timer_.stop();
		...
		stack.add_addr_autoconf(addr, prefix_len, preferred_lifetime,
					valid_lifetime);
	}
}
