...
Mld::Mld(Stack &inet) noexcept : inet_{ inet }, host_{ *this }, router_{ *this }
{
}

Mld::MulticastHostNode::MulticastHostNode()
{
	state_handlers_[Mld::HostStates::NON_LISTENER] = State_handler{
		this, &Mld::MulticastHostNode::non_listener_state_handler
	};

	state_handlers_[Mld::HostStates::DELAYING_LISTENER] = State_handler{
		this, &Mld::MulticastHostNode::delay_listener_state_handler
	};

	state_handlers_[Mld::HostStates::IDLE_LISTENER] = State_handler{
		this, &Mld::MulticastHostNode::idle_listener_state_handler
	};
}
	...
	if (mcast_addr != IP6::ADDR_ANY) {
		if (addr() == mcast_addr) {
			if (auto diff = now_ms - timestamp();
			    now_ms < timestamp() && resp_delay < timestamp()) {
...
void Mld::send_report(const ip6::Addr &mcast)
{
	...
	MAC::Addr dest_mac(0x33, 0x33, dest.get_part<uint8_t>(12),
...
Mld2::Mld2(Stack &inet) noexcept : inet_{ inet }
{
}
...
void Mld2::join(ip6::Addr mcast, FilterMode filtermode, SourceList source_list)
{
	...
	if (rec != mld_records_.end()) {
		...
		mld_records_.emplace(std::make_pair(
			mcast,
