...
ICMPv6::ICMPv6(Stack &inet) : inet_{ inet }
{
}
...
void ICMPv6::execute_ping_callback(icmp6::Packet &ping_response)
{
	...
	const auto &id_se =
		ping_response.view_payload_as<icmp6::Packet::IdSe>();
...
void ICMPv6::destination_unreachable(Packet_ptr pckt,
...
void ICMPv6::parameter_problem(Packet_ptr pckt, uint8_t error_pointer)
{
	...
	auto pckt_ip6 = static_unique_ptr_cast<PacketIP6>(std::move(pckt));
	auto pckt_icmp6 = icmp6::Packet(std::move(pckt_ip6));
	send_response(pckt_icmp6, ICMP_type::PARAMETER_PROBLEM, 0,
		      error_pointer);
}
...
void ICMPv6::send_response(icmp6::Packet &req, ICMP_type type, ICMP_code code,
			   uint8_t error_pointer)
{
	...
	res.add_payload(req.header_and_data().data(),
			req.header_and_data().size());
