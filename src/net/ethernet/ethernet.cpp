...
Ethernet::Ethernet(downstream physical_downstream, const addr &mac) noexcept
	: mac_(mac),
	  ...
	  packets_rx_{ Statman::get()
			       .create(Stat::UINT64,
				       link_name() + ".ethernet.packets_rx")
			       .get_uint64() },
	  packets_tx_{ Statman::get()
			       .create(Stat::UINT64,
				       link_name() + ".ethernet.packets_tx")
			       .get_uint64() },
	  packets_dropped_{ Statman::get()
				    .create(Stat::UINT32,
					    link_name() +
						    ".ethernet.packets_dropped")
				    .get_uint32() },
	  trailer_packets_dropped_{
		  Statman::get()
			  .create(Stat::UINT32,
				  link_name() +
					  ".ethernet.trailer_packets_dropped")
			  .get_uint32()
	  },
...
void Ethernet::transmit(net::Packet_ptr pckt, addr dest, Ethertype type)
{
	...
	if (UNLIKELY(t == net::ntohs(static_cast<uint16_t>(
				  Ethertype::TRAILER_NEGO)) or
		     (t >= net::ntohs(static_cast<uint16_t>(
	...
	do {
		...
		PRINT(" \t <Eth unchain> Transmitting %i b, from %s -> %s. Type: 0x%hx\n",
		      next->size(), mac_.str().c_str(),
		      hdr.dest().str().c_str(), hdr.type());
...
void Ethernet::receive(Packet_ptr pckt)
{
	...
	default:
		...
		// Trailer negotiation and encapsulation RFC 893 and 1122
		if (UNLIKELY(type == net::ntohs(static_cast<uint16_t>(
					     Ethertype::TRAILER_NEGO)) or
			     (type >= net::ntohs(static_cast<uint16_t>(
					      Ethertype::TRAILER_FIRST)) and
			      type <= net::ntohs(static_cast<uint16_t>(
					      Ethertype::TRAILER_LAST))))) {
			trailer_packets_dropped_++;
