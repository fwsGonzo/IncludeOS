...
namespace net
{
Ethernet_8021Q::Ethernet_8021Q(downstream phys_down, const addr &mac,
...
{
	auto &vlan =
		*reinterpret_cast<ethernet::VLAN_header *>(pkt->layer_begin());
	...
	default:
		...
		if (UNLIKELY(type == ntohs(static_cast<uint16_t>(
					     Ethertype::TRAILER_NEGO)) or
			     (type >= ntohs(static_cast<uint16_t>(
					      Ethertype::TRAILER_FIRST)) and
			      type <= ntohs(static_cast<uint16_t>(
					      Ethertype::TRAILER_LAST))))) {
...
void Ethernet_8021Q::transmit(Packet_ptr pkt, addr dest, Ethertype type)
{
	...
	// Trailer negotiation and encapsulation RFC 893 and 1122
	if (UNLIKELY(t == ntohs(static_cast<uint16_t>(
				  Ethertype::TRAILER_NEGO)) or
		     (t >= ntohs(static_cast<uint16_t>(
				   Ethertype::TRAILER_FIRST)) and
		      t <= ntohs(static_cast<uint16_t>(
				   Ethertype::TRAILER_LAST))))) {
	...
	do {
		...
		next->increment_layer_begin(
			-(int)sizeof(ethernet::VLAN_header));

		auto &hdr = *reinterpret_cast<ethernet::VLAN_header *>(
			next->layer_begin());
		...
		PRINT(" \t <802.1 unchain> Transmitting %i b, from %s -> %s. Type: 0x%hx ID: %d\n",
		      next->size(), hdr.src.str().c_str(),
		      hdr.dest.str().c_str(), hdr.type, hdr.vid());
