...
DHCPD::DHCPD(UDP &udp, ip4::Addr pool_start, ip4::Addr pool_end, uint32_t lease,
	     uint32_t max_lease, uint8_t pending)
	: stack_{ udp.stack() }, socket_{ udp.bind(DHCP_SERVER_PORT) },
	  pool_start_{ pool_start }, pool_end_{ pool_end },
	  server_id_{ stack_.ip_addr() }, netmask_{ stack_.netmask() },
	  router_{ stack_.gateway() }, dns_{ stack_.dns_addr() },
...
bool DHCPD::valid_pool(ip4::Addr start, ip4::Addr end) const
{
	...
	if (network_address(start) != network_address(end) or
	    network_address(start) != network_address(server_id_) or
	    network_address(end) != network_address(server_id_))
		return false;
...
void DHCPD::init_pool()
{
	...
	while (start < pool_end_ and
	       network_address(start) == network_address(server_id_)) {
...
void DHCPD::listen()
{
	socket_.on_read([this](net::Addr, UDP::port_t port, const char *data,
			       size_t len) {
		if (port == DHCP_CLIENT_PORT) {
			...
			// Message
			const auto *msg =
				reinterpret_cast<const Message *>(data);
...
{
	if (UNLIKELY(msg->op not_eq
			     static_cast<uint8_t>(op_code::BOOTREQUEST) or
		     msg->hops not_eq 0)) {
		debug("Invalid op\n");
		return;
	}
	...
	if (UNLIKELY(
		    ...
		    msg->htype >
			    static_cast<uint8_t>(
				    htype::HFI))) { // http://www.iana.org/assignments/arp-parameters/arp-parameters.xhtml
		debug("Invalid htype\n");
		return;
	}

	if (UNLIKELY(msg->yiaddr not_eq 0 or msg->siaddr not_eq 0 or
		     msg->giaddr not_eq 0)) {
	...
	if (msg->magic[0] not_eq 99 or msg->magic[1] not_eq 130 or
	    msg->magic[2] not_eq 83 or msg->magic[3] not_eq 99) {
		debug("Invalid magic\n");
		return;
	}
	...
	case message_type::DECLINE:
		...
		// The server must mark the network address as not available and should notify the network
		// admin of a possible configuration problem - TODO
		ridx = get_record_idx(cid);
		// Erase record if exists because client hasn't got a valid IP
		if (ridx not_eq -1)
			records_.erase(records_.begin() + ridx);
		update_pool(get_requested_ip_in_opts(msg),
			    Status::IN_USE); // In use - possible config problem
		return;
	case message_type::RELEASE:
		...
		// Or: Keep config parameters (record) and set status to RELEASED f.ex. instead (but need to
		// handle this - when a new discover (DHCPDISCOVER after DHCPRELEASE) is received - look
		// through records and find record with client id) if (ridx not_eq -1)
...
void DHCPD::handle_request(const Message *msg)
{
	...
	if (opt != nullptr) { // Then this is a response to a DHCPOFFER message
		...
		if (sid not_eq
		    server_id()) { // The client has not chosen this server
			...
			if (ridx != -1) {
				...
				update_pool(records_.at(ridx).ip(),
					    Status::AVAILABLE);
				records_.erase(records_.begin() + ridx);
			}
			return;
		...
		if (ridx not_eq -1) { // A record for this client already exists
			...
			// If ciaddr is zero and requested IP address is filled in with the yiaddr value from
			// the chosen DHCPOFFER: Client state: SELECTING
			if (msg->ciaddr == ip4::Addr{ 0 } and
			    get_requested_ip_in_opts(msg) == record.ip()) {
				// RECORD
			...
			// Else ciaddr is not zero or record's ip is not the same as the requested ip in the
			// request
	...
	// but a request to verify or extend an existing lease
...
void DHCPD::verify_or_extend_lease(const Message *msg)
{
	...
	if (msg->ciaddr == ip4::Addr{ 0 }) {
		...
		// the local subnet mask or remote subnet mask (if giaddr is not zero) to requested IP
		// address value doesn't match reality), then the server should send a DHCPNAK message to
		// the client.
		// 2. (After or in the if statement) If client is on correct network: Check if the client's
		// notion of IP is correct
		if ((not on_correct_network(msg)) or
		    (get_requested_ip_in_opts(msg) not_eq
		     records_.at(ridx).ip())) {
	...
	if (get_requested_ip_in_opts(msg) == ip4::Addr{ 0 } and
	    msg->ciaddr not_eq ip4::Addr{ 0 }) {
		...
		// A client MAY choose to renew or extend its lease prior to T1. The server may choose not
		// to extend the lease (as a policy decision by the network administrator), but should
		// return a DHCPACK message regardless.

		// Same if the client is in REBINDING state so check the giaddr (gateway IP address) to find
		// out if message has been unicast to the server or not. If zero: Unicast. If non-zero:
		// Broadcast. REBINDING: The client is completely configured and is trying to extend its
		// lease. This message MUST be broadcast to the 0xffffffff IP broadcast address. The server
		// should check ciaddr for correctness before replying to the DHCPREQUEST
...
void DHCPD::offer(const Message *msg)
{
	...
	// Servers need not reserve the offered network address, although the protocol will work more
	// efficiently if the server avoids allocating the offered network address to another client.
	...
	// Servers SHOULD be implemented so that network admins MAY choose to disable probes of newly
	// allocated addresses.
	...
	// OFFER
	// NOTE: It may be possible with some changes to use the incoming "msg" as buffer to avoid some
	// extra copy due to some of the fields are sent in return to the client
	char buffer[Message::size()];
	Message_writer offer{ reinterpret_cast<Message *>(buffer),
			      op_code::BOOTREPLY, message_type::OFFER };
	...
	offer.set_chaddr(reinterpret_cast<const MAC::Addr *>(
		&msg->chaddr)); // assume ethernet
	...
	if (msg->giaddr != ip4::Addr{ 0 }) {
		socket_.sendto(msg->giaddr, DHCP_SERVER_PORT, buffer,
			       sizeof(buffer));
		return;
	}
	...
	if (msg->ciaddr != ip4::Addr{ 0 }) {
		socket_.sendto(msg->ciaddr, DHCP_CLIENT_PORT, buffer,
			       sizeof(buffer));
		return;
	}
	// If giaddr is zero and ciaddr is zero, and the broadcast bit (leftmost bit in flags field) is
	// set, then the server broadcasts DHCPOFFER and DHCPACK messsages to 0xffffffff
	if (ntohs(msg->flags) & static_cast<uint16_t>(flag::BOOTP_BROADCAST)) {
		socket_.bcast(server_id_, DHCP_CLIENT_PORT, buffer,
			      sizeof(buffer));
		return;
	}
...
{
	// The client has obtained a network address through some other means (e.g., manual
	// configuration) and may use a DHCPINFORM request message to obtain other local configuration
	// parameters. Servers receiving a DHCPINFORM message construct a DHCPACK message with any local
	...
	char buffer[Message::size()];
	Message_writer ack{ reinterpret_cast<Message *>(buffer),
			    op_code::BOOTREPLY, message_type::ACK };
	...
	ack.set_chaddr(reinterpret_cast<const MAC::Addr *>(
		&msg->chaddr)); // assume ethernet
...
void DHCPD::request_ack(const Message *msg)
{
	...
	// Any configuration parameters in the DHCPACK message SHOULD NOT conflict with those in the
	// earlier DHCPOFFER message to which the client is responding. DHCPACK: Server to client with
	// configuration parameters, including committed network address
	...
	ack.set_yiaddr(
		(ridx not_eq -1) ?
			records_.at(ridx).ip() :
			0); // TODO: else what?     // IP address assigned to client
	ack.set_ciaddr(msg->ciaddr); // or 0
	ack.set_giaddr(msg->giaddr);
...
void DHCPD::nak(const Message *msg)
{
	...
	// NAK
	char buffer[Message::size()];
	Message_writer nak{ reinterpret_cast<Message *>(buffer),
			    op_code::BOOTREPLY, message_type::NAK };
	...
	nak.set_chaddr(reinterpret_cast<const MAC::Addr *>(
		&msg->chaddr)); // assume ethernet
...
bool DHCPD::valid_options(const Message *msg) const
{
	const Message_reader reader{ msg };
	int num_bytes =
		0; // this may be unecessary due to the limit in message reader
	int num_options = reader.parse_options(
		[&num_bytes](const auto *opt) { num_bytes += opt->size(); });
...
{
	// The client is on the wrong network if the result of applying the local subnet mask or remote
	// subnet mask (if giaddr is not zero) to requested IP address option value doesn't match
	// reality

	const auto subnet =
		(msg->giaddr == 0) ? netmask_ : get_remote_netmask(msg);
