...
DHClient::DHClient(Stack &inet)
	: stack(inet), domain_name{}, timeout_timer_{
		  { this, &DHClient::restart_negotation }
	  }
{
	// default timed out handler spams logs
	this->on_config([this](bool timed_out) {
		if (timed_out)
			MYINFO("Negotiation timed out (%s)",
			       this->stack.ifname().c_str());
		else
			MYINFO("Configuration complete (%s)",
			       this->stack.ifname().c_str());
	});
}
...
void DHClient::send_first()
{
	...
	Message_writer msg{ &buffer[0], op_code::BOOTREQUEST,
	...
	msg.add_option<option::param_req_list>(std::vector<option::Code>{
		option::ROUTERS, option::SUBNET_MASK,
		option::DOMAIN_NAME_SERVERS, option::DOMAIN_NAME });
	// END
	msg.end();
	...
	socket->on_read([this](net::Addr addr, UDP::port_t port,
...
void DHClient::offer(const char *data, size_t)
{
	...
	if (msg_opt != nullptr) {
		...
		if (UNLIKELY(msg_opt->type() != message_type::OFFER))
			return;
	...
	} else { // just try using ROUTER as DNS server
...
void DHClient::request(const option::server_identifier *server_id)
{
	...
	// DHCP Parameter Request Field
	msg.add_option<option::param_req_list>(
		std::vector<option::Code>{ option::ROUTERS, option::SUBNET_MASK,
...
void DHClient::acknowledge(const char *data, size_t)
{
	...
	if (msg_opt != nullptr) {
		...
		if (UNLIKELY(msg_opt->type() != message_type::ACK))
			return;
