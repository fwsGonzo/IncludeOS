...
Inet &Interfaces::get(int N)
{
	if (N < 0 || N >= os::machine().count<hw::Nic>())
		throw Stack_not_found{ "No IP4 stack found with index: " +
				       std::to_string(N) +
...
Inet &Interfaces::get(int N, int sub)
{
	...
	if (it != stacks.end()) {
		Expects(it->second != nullptr &&
			"Creating empty subinterfaces doesn't make sense");
		return *it->second;
	}

	throw Stack_not_found{ "IP4 Stack not found [" + std::to_string(N) +
			       "," + std::to_string(sub) + "]" };
}
...
hw::Nic &Interfaces::get_nic(int idx)
{
	try {
		...
		throw Interfaces_err{ "No NIC found with index " +
				      std::to_string(idx) };
	}
}
...
hw::Nic &Interfaces::get_nic(const MAC::Addr &mac)
{
	try {
		...
		throw Interfaces_err{ "No NICs found: " +
				      std::string{ err.what() } };
	}

	throw Interfaces_err{ "No NIC found with MAC address " +
			      mac.to_string() };
}
