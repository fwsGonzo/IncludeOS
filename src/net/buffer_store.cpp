...
BufferStore::BufferStore(uint32_t num, uint32_t bufsize)
	: poolsize_{ num * bufsize }, bufsize_{ bufsize }
{
	assert(num != 0);
	assert(bufsize != 0);
	...
	if (UNLIKELY(available_.empty())) {
		if (this->growth_enabled())
			...
			throw std::runtime_error(
				"This BufferStore has run out of buffers");
	}
...
void BufferStore::create_new_pool()
{
	...
	if (UNLIKELY(pool == nullptr)) {
		throw std::runtime_error(
			"Buffer store failed to allocate memory");
	}
...
__attribute__((weak)) bool BufferStore::growth_enabled() const
{
	return true;
}
