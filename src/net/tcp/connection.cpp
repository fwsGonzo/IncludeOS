...
Connection::Connection(TCP &host, Socket local, Socket remote,
	...
	: host_(host), local_{ std::move(local) }, remote_{ std::move(remote) },
	  is_ipv6_(local_.address().is_v6()),
	  state_(&Connection::Closed::instance()),
	  prev_state_(state_), cb{ (is_ipv6_) ? default_mss_v6 : default_mss,
				   host_.window_size() },
	  read_request(nullptr), writeq(), on_connect_{ std::move(callback) },
	  ...
	  queued_(false), dack_{ 0 }, last_ack_sent_{ cb.RCV.NXT }, smss_{
		  MSS()
	  }
{
...
void Connection::_on_read(size_t recv_bufsz, ReadCallback cb)
{
	...
	if (read_request == nullptr) {
		...
		read_request.reset(
			new Read_request(this->cb.RCV.NXT, host_.min_bufsize(),
					 host_.max_bufsize(), bufalloc.get()));
		read_request->on_read_callback = cb;
		const size_t avail_thres =
			host_.max_bufsize() * Read_request::buffer_limit;
		bufalloc->on_avail(avail_thres,
				   { this,
...
Connection::TCB::TCB(const uint16_t mss, const uint32_t recvwin)
	: SND{ 0, 0, default_window_size, 0, 0, 0, mss, 0, false },
	  ISS{ (seq_t)4815162342 }, RCV{ 0, recvwin, 0, 0, 0 }, IRS{ 0 },
	  ssthresh{ recvwin }, cwnd{ 0 }, recover{ 0 }, TS_recent{ 0 }
{
}
...
void Connection::offer(size_t &packets)
{
	...
	while (can_send() and packets) {
		...
		while (can_send() and
		       (x = fill_packet(*packet, writeq.nxt_data(),
					writeq.nxt_rem()))) {
			written += x;
			cb.SND.NXT += x;
			writeq.advance(x);
		...
		debug2("<Connection::offer> Wrote %u bytes (%u remaining) with [%u] packets left and a "
		       "usable window of %u.\n",
		...
		if (UNLIKELY(not writeq.has_remaining_requests() and
			     is_closing())) {
	...
	debug2("<Connection::offer> Finished working offer with [%u] packets left and a queue of (%u) "
	       "with a usable window of %i\n",
...
void Connection::limited_tx()
{
	...
	const auto written =
		fill_packet(*packet, writeq.nxt_data(), writeq.nxt_rem());
	cb.SND.NXT += written;
	packet->set_flag(ACK);
...
void Connection::segment_arrived(Packet_view &incoming)
{
	...
	//  (((uint32_t*)&incoming->tcp_header())[3]), FMASK, ((((uint32_t*)&incoming->tcp_header())[3])
	//  & FMASK), pred_flags);
	...
	case State::OK:
		return; // // Do nothing.
	case State::CLOSED:
		debug("<TCP::Connection::receive> (%s => %s) State handle finished with CLOSED. We're "
		      "done, ask host() to delete the connection.\n",
		      prev_state_->to_string().c_str(),
...
__attribute__((weak)) void Connection::deserialize_from(void *)
{
}
__attribute__((weak)) int Connection::serialize_to(void *) const
{
	return 0;
}
...
{
	update_rcv_wnd();
	auto packet = (is_ipv6_) ? host_.create_outgoing_packet6() :
				   host_.create_outgoing_packet();
	// Set Source (local == the current connection)
	packet->set_source(local_);
	// Set Destination (remote)
	packet->set_destination(remote_);

	packet->set_win(std::min((cb.RCV.WND >> cb.RCV.wind_shift),
				 (uint32_t)default_window_size));

	if (cb.SND.TS_OK)
		packet->add_tcp_option_aligned<Option::opt_ts_align>(
			host_.get_ts_value(), cb.get_ts_recent());
...
{
	if (!cb.SND.TS_OK and !rttm.active() and packet->end() == cb.SND.NXT) {
...
bool Connection::handle_ack(const Packet_view &in)
{
    ...
    */
	...
	if (cb.SND.TS_OK) {
		...
		if (ts !=
		    nullptr) // TODO: not sure the packet is valid if TS missing
	...
	(not fast_recovery_) ? congestion_control(in) : fast_recovery(in);
...
void Connection::congestion_control(const Packet_view &in)
{
	...
	// try to write
	if (can_send() and
	    (!in.has_tcp_data() or cb.RCV.WND < in.tcp_data_length())) {
...
void Connection::on_dup_ack(const Packet_view &in)
{
	...
	*/
		if (limited_tx_ and cb.SND.WND >= SMSS() and
		    (flight_size() <= cb.cwnd + 2 * SMSS()) and
		    writeq.has_remaining_requests()) {
	...
	// 3 dup acks
		...
		// 4.1.  ACK Heuristic
		else if (send_window() > SMSS() and
			 (highest_ack_ - prev_highest_ack_ <= 4 * SMSS())) {
			goto fast_rtx;
		}
		return;
...
void Connection::recv_data(const Packet_view &in)
{
	...
	// if we're inside the window. if packet is out of order tho we can change the RCV wnd (i
	// think).
	...
	if (cb.RCV.NXT == in.seq()) {
		...
		if (UNLIKELY(sack_list != nullptr)) {
			const auto res =
				sack_list->new_valid_ack(in.seq(), length);
		...
		if (read_request != nullptr) {
			const auto recv = read_request->insert(
				in.seq(), in.tcp_data(), length, in.isset(PSH));
			// this ensures that the data we ACK is actually put in our buffer.
			Ensures(recv == length);
		}
	}
	// Packet out of order
	else if (((in.seq() + in.tcp_data_length()) - cb.RCV.NXT) <
		 cb.RCV.WND) {
...
void Connection::recv_out_of_order(const Packet_view &in)
{
	...
	if (fits >= length) {
		...
		const auto inserted = read_request->insert(
			seq, in.tcp_data(), length, in.isset(PSH));
    ...
    while(rem != 0 and fits != 0)
    {
      ...
      auto inserted = read_request->insert(seq, in.tcp_data()+offset, std::min(rem, fits),
    in.isset(PSH));
...
void Connection::take_rtt_measure(const Packet_view &packet)
{
	if (cb.SND.TS_OK) {
		...
		if (ts) {
			rttm.rtt_measurement(RTTM::milliseconds{
				host_.get_ts_value() - ntohl(ts->ecr) });
			return;
		}
	}
...
void Connection::retransmit()
{
	...
	// If not, check if there is data and retransmit
		...
		// printf("<Connection::retransmit> With data (wq.sz=%zu) buf.size=%zu buf.unacked=%zu
		// SND.WND=%u CWND=%u\n",
		//       writeq.size(), buf->size(), buf->size() - writeq.acked(), cb.SND.WND, cb.cwnd);
		fill_packet(*packet, buf->data() + writeq.acked(),
			    buf->size() - writeq.acked());
		packet->set_flag(PSH);
	}
...
void Connection::rtx_timeout()
{
	...
	if (rto_limit_reached()) {
		debug("<TCP::Connection::rtx_timeout> RTX attempt limit reached, closing. rtx=%u "
		      "syn_rtx=%u\n",
    ...
    */
	...
	/*
			 NOTE: It's unclear which one comes first, or if finish_fast_recovery includes changing the
			 cwnd.
		       */
}
...
void Connection::timewait_start()
{
	...
	timewait_dack_timer.restart(timeout,
				    { this, &Connection::timewait_timeout });
}
...
void Connection::signal_connect(const bool success)
{
	...
	if (on_connect_)
		(success) ? on_connect_(retrieve_shared()) :
			    on_connect_(nullptr);
	...
	while ((Byte *)opt < packet.tcp_data()) {
		...
		case Option::MSS: {
			if (UNLIKELY(option->length != sizeof(Option::opt_mss)))
				throw TCPBadOptionException{ Option::MSS,
							     "length != 4" };
		...
		case Option::WS: {
			if (UNLIKELY(option->length != sizeof(Option::opt_ws)))
				throw TCPBadOptionException{ Option::WS,
							     "length != 3" };
		...
		case Option::TS: {
			if (host_.uses_timestamps()) {
				...
				} else if (ntohl(opt_ts.val) >= cb.TS_recent and
					   packet.seq() <= last_ack_sent_) {
					cb.TS_recent = ntohl(opt_ts.val);
				}
			}
			opt += option->length;
		...
		case Option::SACK_PERM: {
			if (UNLIKELY(option->length !=
				     sizeof(Option::opt_sack_perm)))
				throw TCPBadOptionException{ Option::SACK_PERM,
							     "length != 2" };
...
void Connection::add_option(Option::Kind kind, Packet_view &packet)
{
	...
	case Option::MSS: {
		...
		debug2("<TCP::Connection::add_option@Option::MSS> Packet: %s - MSS: %u\n",
		       packet.to_string().c_str(),
		       ntohs(*(uint16_t *)(packet.tcp_options() + 2)));
		break;
	}
	...
	case Option::TS: {
		...
		packet.add_tcp_option<Option::opt_ts>(host_.get_ts_value(),
						      ts_ecr);
		break;
	}
...
void Connection::add_synack_options(Packet_view &packet)
{
	...
	if (cb.SND.wind_shift > 0) {
		add_option(Option::WS, packet);
		packet.set_win(
			std::min((uint32_t)default_window_size, cb.RCV.WND));
	}
	// SACK permitted
	if (sack_perm == true) {
		add_option(Option::SACK_PERM, packet);
...
void Connection::drop(const Packet_view &packet,
		      [[maybe_unused]] Drop_reason reason)
{
	/*printf("Drop %s %#.x RCV.WND: %u RCV.NXT %u alloc free: %zu flight size: %u SND.WND: %u \n",
      packet.to_string().c_str(), reason, cb.RCV.WND, cb.RCV.NXT, bufalloc->allocatable(),
      flight_size(), cb.SND.WND);*/
	host_.drop(packet);
}
