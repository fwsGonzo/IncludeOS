...
[[maybe_unused]] static inline std::string state_str(const uint8_t state);
...
Conntrack::Entry *tcp4_conntrack(Conntrack &ct, Quadruple q,
				 const PacketIP4 &pkt)
{
	...
	if (UNLIKELY(state(entry) == Ct_state::SYN_SENT and
		     q == entry->second)) {
		// if SYN/ACK reply
	...
	if (UNLIKELY(state(entry) == Ct_state::SYN_RECV and
		     q == entry->first)) {
		if (tcp.isset(ACK)) {
	...
	if (UNLIKELY(state(entry) == Ct_state::LAST_ACK or
		     state(entry) == Ct_state::TIME_WAIT)) {
...
static inline void set_state(Conntrack::Entry *entry, const Ct_state state)
{
	CTDBG("<CT_TCP4> State change %s => %s on %s\n",
	      state_str(entry->other).c_str(),
	      state_str((uint8_t)state).c_str(), entry->to_string().c_str());
...
static inline void update_timeout(Conntrack::Entry *entry)
{
	const auto &dur = [&]() -> const auto &
	{
		...
		case Ct_state::ESTABLISHED:
			return timeout::ESTABLISHED;
		case Ct_state::SYN_SENT:
			return timeout::SYN_SENT;
		case Ct_state::SYN_RECV:
			return timeout::SYN_RECV;
		case Ct_state::FIN_WAIT:
			return timeout::FIN_WAIT;
		case Ct_state::TIME_WAIT:
			return timeout::TIME_WAIT;
		case Ct_state::CLOSE_WAIT:
			return timeout::CLOSE_WAIT;
		case Ct_state::LAST_ACK:
			return timeout::LAST_ACK;
		case Ct_state::CLOSE:
