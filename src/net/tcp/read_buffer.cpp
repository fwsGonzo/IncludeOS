...
namespace tcp
{
	Read_buffer::Read_buffer(const seq_t startv, const size_t min,
				 const size_t max)
		: buf(tcp::construct_buffer()), start{ startv }, cap{ max },
		  hole{ 0 }
	{
	...
	Read_buffer::Read_buffer(const seq_t startv, const size_t min,
				 const size_t max, const Alloc &alloc)
		: buf(tcp::construct_buffer(alloc)), start{ startv },
		  cap{ max }, hole{ 0 }
	{
		Expects(util::bits::is_pow2(cap));
		Expects(util::bits::is_pow2(min));
	...
	size_t Read_buffer::insert(const seq_t seq, const uint8_t *data,
				   size_t len, bool push)
	{
		assert(buf != nullptr &&
		       "Buffer seems to be stolen, make sure to renew()");

		// get the relative sequence number (the diff)
		size_t rel = (seq_t)(seq - start);
		assert(rel < capacity() &&
		       "No point trying to write at or above the end");
	...
	__attribute__((weak)) int Read_buffer::deserialize_from(void *)
	{
		return 0;
	}
	__attribute__((weak)) int Read_buffer::serialize_to(void *) const
	{
		return 0;
	}
