...
TCP::TCP(IPStack &inet, bool smp_enable)
	: inet_{ inet }, listeners_(),
	  connections_(), total_bufsize_{ default_total_bufsize },
	  mempool_{ total_bufsize_ }, min_bufsize_{ default_min_bufsize },
	  max_bufsize_{ default_max_bufsize }, ports_(inet.tcp_ports()),
	  writeq(), max_seg_lifetime_{ default_msl }, // 30s
	  ...
	  max_syn_backlog_{ default_max_syn_backlog } // 64
{
	...
	if (this->smp_enabled == false) {
		inet.on_transmit_queue_available(
			{ this, &TCP::process_writeq });
		stat_prefix = inet.ifname();
	} else {
		SMP::global_lock();
		inet.on_transmit_queue_available(
			{ this, &TCP::smp_process_writeq });
		SMP::global_unlock();
		stat_prefix =
			inet.ifname() + ".cpu" + std::to_string(this->cpu_id);
	}
	bytes_rx_ = &Statman::get()
			     .create(Stat::UINT64, stat_prefix + ".tcp.rx")
			     .get_uint64();
	bytes_tx_ = &Statman::get()
			     .create(Stat::UINT64, stat_prefix + ".tcp.tx")
			     .get_uint64();
	packets_rx_ =
		&Statman::get()
			 .create(Stat::UINT64, stat_prefix + ".tcp.packets_rx")
			 .get_uint64();
	packets_tx_ =
		&Statman::get()
			 .create(Stat::UINT64, stat_prefix + ".tcp.packets_tx")
			 .get_uint64();
	incoming_connections_ =
		&Statman::get()
			 .create(Stat::UINT64,
				 stat_prefix + ".tcp.conn_incoming")
			 .get_uint64();
	outgoing_connections_ =
		&Statman::get()
			 .create(Stat::UINT64,
				 stat_prefix + ".tcp.conn_outgoing")
			 .get_uint64();
	connection_attempts_ =
		&Statman::get()
			 .create(Stat::UINT64,
				 stat_prefix + ".tcp.conn_attempts")
			 .get_uint64();
	packets_dropped_ =
		&Statman::get()
			 .create(Stat::UINT32, stat_prefix + ".tcp.dropped")
			 .get_uint32();
}
...
void TCP::smp_process_writeq(size_t packets)
{
	...
	SMP::add_task([this, packets]() { this->process_writeq(packets); },
		      this->cpu_id);
...
Listener &TCP::listen(const Socket &socket, ConnectCallback cb)
{
	...
	auto &listener =
		listeners_
			.emplace(socket, std::make_unique<tcp::Listener>(
						 *this, socket, std::move(cb)))
			.first->second;
...
Listener &TCP::listen(const tcp::port_t port, ConnectCallback cb,
		      const bool ipv6_only)
{
	Socket socket{ ip6::Addr::addr_any, port };
	bind(socket);

	auto ptr = std::make_shared<tcp::Listener>(*this, socket, std::move(cb),
						   ipv6_only);
	...
	if (not ipv6_only) {
		...
		bind(ip4_sock);
		Ensures(listeners_.emplace(ip4_sock, ptr).second &&
			"Could not insert IPv4 listener");
	}
...
void TCP::insert_connection(Connection_ptr conn)
{
	connections_.emplace(std::piecewise_construct,
			     std::forward_as_tuple(conn->local(),
						   conn->remote()),
...
void TCP::receive4(net::Packet_ptr ptr)
{
	...
	PRINT("<TCP::receive> Recv TCP4 packet %s => %s\n",
	      pkt.source().to_string().c_str(),
	      pkt.destination().to_string().c_str());
...
string TCP::to_string() const
{
	...
	for (auto &listen_it : listeners_) {
		auto &l = listen_it;
		str += l.first.to_string() + "\t" +
		       std::to_string(l.second->syn_queue_size()) + "\n";
	}
	str += "\nCONNECTIONS:\nLocal\tRemote\tState\n";
...
void TCP::error_report(const Error &err, Socket dest)
{
	if (err.is_icmp()) {
		...
		if (network().path_mtu_discovery() and
		    icmp_err->is_too_big() and
		    icmp_err->pmtu() >= network().minimum_MTU()) {
			/*
	    Note RFC 1191 p. 14:
	    TCP performance can be reduced if the sender’s maximum window size is not an exact
	    multiple of the segment size in use (this is not the congestion window size, which is
	    always a multiple of the segment size).  In many system (such as those derived
	    from 4.2BSD), the segment size is often set to 1024 octets, and the maximum window size
	    (the "send space") is usually a multiple of 1024 octets, so the proper relationship
	    holds by default. If PMTU Discovery is used, however, the segment size may not be a
	    submultiple of the send space, and it may change during a connection; this means that
	    the TCP layer may need to change the transmission window size when PMTU Discovery
	    changes the PMTU value. The maximum window size should be set to the greatest multiple
	    of the segment size (PMTU - 40) that is less than or equal to the sender’s buffer space
	    size.
	    */

			// Find all connections sending to this destination
			// Notify the TCP Connection that the sent packet has been dropped and needs to be
			// retransmitted
			for (auto &conn_entry : connections_) {
				if (conn_entry.first.second == dest) {
					/*
		    Note: One MUST not retransmit in response to every Datagram Too Big message,
		    since a burst of several oversized segments will give rise to several such
		    messages and hence several retransmissions of the same data. If the new
		    estimated PMTU is still wrong, the process repeats, and there is an exponential
		    growth in the number of superfluous segments sent. This means that the TCP layer
		    must be able to recongnize when a Datagram Too Big message actually decreases
		    the PMTU that it has already used to send a datagram on the given connection,
		    and should ignore any other notifications.
		    */

					// PMTU is maximum transmission unit including the size of the IP header, while
					// SMSS is minus the size of the IP header and minus the size of the TCP header
					auto new_smss = icmp_err->pmtu() -
							sizeof(ip4::Header) -
							sizeof(tcp::Header);

					if (conn_entry.second->SMSS() >
					    new_smss) {
						conn_entry.second->set_SMSS(
							new_smss);
						...
						// (i.e., only one segment should be retransmitted until acknowledgements
...
{
	if (UNLIKELY(not network().path_mtu_discovery() or
		     pmtu < network().minimum_MTU()))
		return;
	...
	for (auto &conn_entry : connections_) {
		if (conn_entry.first.second == dest)
			conn_entry.second->set_SMSS(pmtu - sizeof(ip4::Header) -
						    sizeof(tcp::Header));
	}
}
...
{
	auto packet = std::make_unique<tcp::Packet4_view>(
		inet_.create_ip_packet(Protocol::TCP));
	packet->init();
	return packet;
}
...
{
	auto packet = std::make_unique<tcp::Packet6_view>(
		inet_.create_ip6_packet(Protocol::TCP));
	packet->init();
	return packet;
}
...
void TCP::send_reset(const tcp::Packet_view &in)
{
	...
	auto out = (in.ipv() == Protocol::IPv6) ? create_outgoing_packet6() :
...
void TCP::bind(const Socket &socket)
{
	if (UNLIKELY(is_valid_source(socket.address()) == false))
		throw TCP_error{ "Cannot bind to address: " +
				 socket.address().to_string() };

	if (UNLIKELY(is_bound(socket)))
		throw TCP_error{ "Socket is already in use: " +
				 socket.to_string() };
...
bool TCP::add_connection(tcp::Connection_ptr conn)
{
	...
	if (UNLIKELY(resource == nullptr or
		     resource->allocatable() < alloc_thres)) {
...
Connection_ptr TCP::create_connection(Socket local, Socket remote,
				      ConnectCallback cb)
{
	const size_t alloc_thres = max_bufsize() * Read_request::buffer_limit;
	...
	if (UNLIKELY(resource == nullptr or
		     resource->allocatable() < alloc_thres)) {
		throw TCP_error{
			"Unable to create new connection: Not enough allocatable memory"
		};
	}
...
void TCP::request_offer(Connection &conn)
{
	...
	debug2("<TCP::request_offer> %s requestin offer: uw=%u rem=%u\n",
	       conn.to_string().c_str(), conn.usable_window(),
	       conn.sendq_remaining());
...
{
	return ((ipv == Protocol::IPv6) ? inet_.ip6_obj().MDDS() :
					  network().MDDS()) -
	       sizeof(tcp::Header);
}
...
TCP::Listeners::iterator TCP::find_listener(const Socket &socket)
{
	...
	if (not socket.address().is_any())
		it = listeners_.find(
			{ socket.address().any_addr(), socket.port() });
