...
Listener::Listener(TCP &host, Socket local, ConnectCallback cb,
	...
	: host_(host), local_(local), syn_queue_(),
	  ...
	  _on_close_({ host_, &TCP::close_listener }), ipv6_only_{ ipv6_only }
{
}
...
void Listener::segment_arrived(Packet_view &packet)
{
	...
	if (it != syn_queue_.end()) {
		...
		TCPL_PRINT2(
			"<Listener::segment_arrived> Connection done handling segment\n");
		return;
	}
	// if it's a new attempt (SYN)
		...
		if (UNLIKELY(not packet.isset(SYN) or packet.has_tcp_data())) {
			TCPL_PRINT2(
				"<Listener::segment_arrived> Packet did not have SYN - dropping\n");
		...
		if (UNLIKELY(on_accept_(packet.source()) == false)) {
			TCPL_PRINT2(
				"<Listener::segment_arrived> on_accept says NO\n");
			return;
		}
		...
		if (syn_queue_.size() >= host_.max_syn_backlog()) {
			TCPL_PRINT2(
				"<Listener::segment_arrived> Queue is full\n");
			Expects(not syn_queue_.empty());
			debug("<Listener::segment_arrived> Connection %s dropped to make room for new "
			      "connection\n",
		...
		auto &conn = *(syn_queue_.emplace(
			...
			std::make_shared<Connection>(
				host_, packet.destination(), packet.source(),
				ConnectCallback{ this, &Listener::connected })));
...
{
	char buffer[256];
	int len = snprintf(buffer, sizeof(buffer), "[%s] SynQueue (%zu) ",
			   local_.to_string().c_str(), syn_queue_.size());
