...
namespace tcp
{
	...
	{
		buffers.push_back(
			std::make_unique<Read_buffer>(start, min, max, alloc));
	}

	size_t Read_request::insert(seq_t seq, const uint8_t *data, size_t n,
				    bool psh)
	{
		...
		while (n) {
			...
			while (buf->is_ready() and
			       buf == buffers.front().get()) {
				const auto rem = buf->capacity() - buf->size();
				const auto end_seq =
					buf->end_seq(); // store end_seq if reseted in callback
	...
	Read_buffer *Read_request::get_buffer(const seq_t seq)
	{
		...
		if (buffers.size() < buffer_limit) {
			...
			buffers.push_back(std::make_unique<Read_buffer>(
				cur_back->end_seq(), cur_back->capacity(),
				cur_back->capacity(), alloc));
	...
	void Read_request::set_start(seq_t seq)
	{
		for (auto &ptr : buffers) {
			Expects(ptr->size() == 0 &&
				"Cannot change start sequence when there already is data");
			ptr->set_start(seq);
			seq += ptr->capacity();
