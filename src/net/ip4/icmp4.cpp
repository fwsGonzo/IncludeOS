...
ICMPv4::ICMPv4(Stack &inet) : inet_{ inet }
{
}
...
void ICMPv4::receive(Packet_ptr pckt)
{
	if (not is_full_header(
		    (size_t)pckt->size())) // Drop if not a full header
		return;
	...
	case ICMP_type::DEST_UNREACHABLE:
		if (req.code() ==
		    (uint8_t)
			    icmp4::code::Dest_unreachable::FRAGMENTATION_NEEDED) {
...
void ICMPv4::handle_too_big(icmp4::Packet &req)
{
	...
	// Inet updates the corresponding Path MTU value in IP and notifies the transport/packetization
	// layer
	inet_.error_report(err, std::move(packet_ptr));
}

void ICMPv4::destination_unreachable(Packet_ptr pckt,
				     icmp4::code::Dest_unreachable code)
{
	...
	if (pckt_ip4->ip_dst() != inet_.broadcast_addr() and
	    pckt_ip4->ip_dst() != IP4::ADDR_BCAST and
	    not pckt_ip4->ip_dst().is_multicast()) {
		auto pckt_icmp4 = icmp4::Packet(std::move(pckt_ip4));
		send_response(pckt_icmp4, ICMP_type::DEST_UNREACHABLE,
			      (ICMP_code)code);
	}
}
...
void ICMPv4::parameter_problem(Packet_ptr pckt, uint8_t error_pointer)
{
	...
	auto pckt_ip4 = static_unique_ptr_cast<PacketIP4>(std::move(pckt));
	auto pckt_icmp4 = icmp4::Packet(std::move(pckt_ip4));
	send_response(pckt_icmp4, ICMP_type::PARAMETER_PROBLEM, 0,
		      error_pointer);
}
...
void ICMPv4::ping(const std::string &hostname, icmp_func callback, int sec_wait)
{
	inet_.resolve(hostname,
		      Inet::resolve_func::make_packed(
			      [this, callback, sec_wait](dns::Response_ptr res,
							 Error err) {
				      if (!err) {
					      auto addr = res->get_first_ipv4();
					      if (addr != 0)
			  ...
			  icmp_func callback, int sec_wait, uint16_t sequence)
{
	...
	if (callback) {
		ping_callbacks_.emplace(
			std::piecewise_construct,
			std::forward_as_tuple(
				std::make_pair(temp_id, sequence)),
			std::forward_as_tuple(ICMP_callback{
				*this, std::make_pair(temp_id, sequence),
				callback, sec_wait }));
	}
...
void ICMPv4::send_response(icmp4::Packet &req, ICMP_type type, ICMP_code code,
