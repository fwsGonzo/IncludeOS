...
IP4::IP4(Stack &inet) noexcept
	: addr_{ IP4::ADDR_ANY },
	  ...
	  packets_rx_{ Statman::get()
			       .create(Stat::UINT64,
				       inet.ifname() + ".ip4.packets_rx")
			       .get_uint64() },
	  packets_tx_{ Statman::get()
			       .create(Stat::UINT64,
				       inet.ifname() + ".ip4.packets_tx")
			       .get_uint64() },
	  packets_dropped_{ Statman::get()
				    .create(Stat::UINT32,
					    inet.ifname() +
						    ".ip4.packets_dropped")
				    .get_uint32() },
	  ...
	  prerouting_dropped_{
		  Statman::get()
			  .create(Stat::UINT32,
				  inet.ifname() + ".ip4.prerouting_dropped")
			  .get_uint32()
	  },
	  postrouting_dropped_{
		  Statman::get()
			  .create(Stat::UINT32,
				  inet.ifname() + ".ip4.postrouting_dropped")
			  .get_uint32()
	  },
	  input_dropped_{ Statman::get()
				  .create(Stat::UINT32,
					  inet.ifname() + ".ip4.input_dropped")
				  .get_uint32() },
	  output_dropped_{ Statman::get()
				   .create(Stat::UINT32,
					   inet.ifname() +
						   ".ip4.output_dropped")
				   .get_uint32() }
...
IP4::IP_packet_ptr IP4::drop(IP_packet_ptr ptr, Direction direction,
			     Drop_reason reason)
{
	packets_dropped_++;
...
IP4::IP_packet_ptr IP4::drop_invalid_in(IP4::IP_packet_ptr packet)
{
	...
	// Here dropping if the source ip address is a multicast address or is this interface's
	// broadcast address
	if (UNLIKELY(packet->ip_src().is_multicast() or
		     packet->ip_src() == IP4::ADDR_BCAST or
...
IP4::IP_packet_ptr IP4::drop_invalid_out(IP4::IP_packet_ptr packet)
{
	...
	if (packet->ip_ttl() == 0)
		return drop(std::move(packet), Direction::Downstream,
			    Drop_reason::TTL0);

	// RFC-1122 3.2.1.7, MUST NOT send packet addressed to 127.*
	if (packet->ip_dst().part(0) == 127)
		drop(std::move(packet), Direction::Downstream,
		     Drop_reason::Bad_destination);
...
{
	return stack_.is_valid_source(dst) or dst == stack_.broadcast_addr() or
	       dst == ADDR_BCAST;
}
...
void IP4::receive(Packet_ptr pckt, [[maybe_unused]] const bool link_bcast)
{
	...
	PRINT("<IP4 Receive> Source IP: %s Dest.IP: %s Type: 0x%x ",
	      packet->ip_src().str().c_str(), packet->ip_dst().str().c_str(),
	...
	if (not is_for_me(packet->ip_dst())) {
		...
		if (not forward_packet_) {
			...
			drop(std::move(packet), Direction::Upstream,
			     Drop_reason::Bad_destination);
		}
		// Forwarding enabled
		else {
	...
	if (conntrack) {
		ct = (ct != nullptr) ?
			     conntrack->confirm(ct->second, ct->proto) :
			     conntrack->confirm(*packet);
	}
	if (stack_.conntrack())
	...
	default:
		...
		stack_.icmp().destination_unreachable(
			std::move(packet),
			icmp4::code::Dest_unreachable::PROTOCOL);

		drop(std::move(packet), Direction::Upstream,
		     Drop_reason::Unknown_proto);
		break;
	}
}
...
void IP4::ship(Packet_ptr pckt, ip4::Addr next_hop, Conntrack::Entry_ptr ct)
{
	...
	if (conntrack) {
		ct = (ct != nullptr) ?
			     conntrack->confirm(ct->first, ct->proto) :
			     conntrack->confirm(*packet);
	}
	...
	if (next_hop == 0) {
		if (UNLIKELY(packet->ip_dst() == IP4::ADDR_BCAST)) {
			...
			next_hop = target == local ? packet->ip_dst() :
						     stack_.gateway();
	...
	PRINT("<IP4> Transmitting packet, layer begin: buf + %li ip.len=%u pkt.size=%zu\n",
	      packet->layer_begin() - packet->buf(), packet->ip_total_length(),
	      packet->size());
...
{
	if (UNLIKELY(not path_mtu_discovery_ or
		     dest.address() == IP4::ADDR_ANY or
		     (new_pmtu > 0 and new_pmtu < minimum_MTU())))
		return;
	...
	if (it != paths_.end()) {
		// If a router returns a next hop MTU value of zero: Try to discover the correct MTU value
		// from the Total Length field
		if (UNLIKELY(new_pmtu == 0 and total_length not_eq 0))
			new_pmtu = pmtu_from_total_length(
				total_length, header_length, it->second.pmtu());
		...
		if (UNLIKELY(new_pmtu == 0 and total_length not_eq 0))
			new_pmtu = pmtu_from_total_length(
				total_length, header_length, default_PMTU());

		// Add to paths_ if the entry doesn't exist
		// Initially, the PMTU value for a path is assumed to be the (known) MTU of the first-hop
		// link
		...
		if (UNLIKELY(not pmtu_timer_.is_running()))
			pmtu_timer_.start(
				pmtu_timer_interval_); // interval in seconds
	}
}
...
IP4::PMTU IP4::pmtu_from_total_length(uint16_t total_length,
				      uint8_t header_length, PMTU current)
{
    ...
    */
	...
	if (total_length >= current and
	    total_length >= (quad + (PMTU)PMTU_plateau::ONE))
		total_length -= quad;
...
void IP4::reset_stale_paths()
{
	...
	auto rtc_aged = (RTC::timestamp_t)
		pmtu_aged_; // cast from uint16_t to int64_t (RTC::timestamp_t)
	...
	*/
		...
		if (it->second.timestamp() != 0 and
		    RTC::time_since_boot() > rtc_aged and
		    it->second.timestamp() <
			    (RTC::time_since_boot() - rtc_aged)) {
			stack_.reset_pmtu(it->first, it->second.pmtu());
			paths_.erase(
				it); // Optional, if keep the entry in the map: it->second.reset_pmtu();
		}
	}
}
