...
struct Entry {
	...
	bool matches(RTC::timestamp_t now,
		     IP4::IP_packet &packet) const noexcept
	{
		return is_dead(now) == false && this->src == packet.ip_src() &&
...
IP4::IP_packet_ptr Entry::assemble_from(IP4::IP_packet &packet)
{
	...
	if (fragoff == 0) {
		...
		// first fragment
		try {
			// NOTE: we can run out of memory here
			auto raw = create_packet(MAX_DATAGRAM);
			this->buffer = static_unique_ptr_cast<IP4::IP_packet>(
				std::move(raw));
		} catch (std::exception &) {
			return nullptr;
		}
...
Entry::Entry(RTC::timestamp_t now, IP4::IP_packet &packet)
	: frag_id(packet.ip_id()), received(0), total_length(0), last_seen(now),
	  buffer(nullptr)
{
...
int Reassembly::get_entry(RTC::timestamp_t now, IP4::IP_packet &packet)
{
	...
	// find matching entry
	for (int i = 0; i < (int)entries.size(); i++) {
		if (entries[i].matches(now, packet))
			return i;
	}
...
IP4::IP_packet_ptr Reassembly::process(IP4::IP_packet_ptr packet)
{
	...
	if (UNLIKELY(packet->ip_data_length() == 0))
		return nullptr;
	if (UNLIKELY(packet->ip_src() == IP4::ADDR_ANY))
		return nullptr;
	// non-last fragments ...
	if (packet->ip_flags() == ip4::Flags::MF) {
		// must have length mult of 8
		if (UNLIKELY(packet->ip_data_length() % 8 != 0))
			return nullptr;
		// should be at least 400 octets long
		if (UNLIKELY(packet->ip_data_length() < 400))
			return nullptr;
	}
