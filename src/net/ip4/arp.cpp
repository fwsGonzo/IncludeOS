...
Arp::Arp(Stack &inet) noexcept
	: requests_rx_{ Statman::get()
				.create(Stat::UINT32,
					inet.ifname() + ".arp.requests_rx")
				.get_uint32() },
	  requests_tx_{ Statman::get()
				.create(Stat::UINT32,
					inet.ifname() + ".arp.requests_tx")
				.get_uint32() },
	  replies_rx_{ Statman::get()
			       .create(Stat::UINT32,
				       inet.ifname() + ".arp.replies_rx")
			       .get_uint32() },
	  replies_tx_{ Statman::get()
			       .create(Stat::UINT32,
				       inet.ifname() + ".arp.replies_tx")
			       .get_uint32() },
	  inet_{ inet },
	  mac_(inet.link_addr())
{
...
void Arp::arp_respond(header *hdr_in, ip4::Addr ack_ip)
{
	...
	PRINT("\t IP: %s is at My Mac: %s\n", res->source_ip().str().c_str(),
	      res->source_mac().str().c_str());
