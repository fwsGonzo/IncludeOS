...
namespace nat
{
	...
	inline void update_dnat(Conntrack &ct, Conntrack::Entry_ptr entry,
				Socket socket)
	{
				...
				ip4::Addr addr)
	{
		if (entry->second.src.address().v4() != addr) {
			ct.update_entry(
				entry->proto, entry->second,
				{ { addr,
				    entry->second.src
					    .port() }, // change return addr but keep port
				  entry->second.dst });
		}
	}
				...
				uint16_t port)
	{
		if (entry->second.src.port() != port) {
			ct.update_entry(
				entry->proto, entry->second,
				{ { entry->second.src.address(),
				    port }, // keep return address but change port
				  entry->second.dst });
		}
	}

	inline void update_snat(Conntrack &ct, Conntrack::Entry_ptr entry,
				Socket socket)
	{
		...
		if (entry->second.dst.address().v4() != addr) {
			ct.update_entry(
				entry->proto, entry->second,
				{
					...
					{ addr,
					  entry->second.dst
						  .port() } // change dst address but keep port
				});
		}
	}
				...
				uint16_t port)
	{
		if (entry->second.dst.port() != port) {
			ct.update_entry(
				...
				{
					entry->second.src,
					{ entry->second.dst.address(),
					  port } // keep dst address but change port
				});
		}
	}

	NAPT::NAPT(std::shared_ptr<Conntrack> ct) : conntrack(std::move(ct))
	{
		Expects(conntrack != nullptr);
	}

	void NAPT::masquerade(IP4::IP_packet &pkt, Stack &inet,
			      Conntrack::Entry_ptr entry)
	{
		...
		case Protocol::ICMPv4: {
			...
			if (not is_snat(entry)) {
				...
				auto masq_sock =
					Socket{ ip, entry->second.dst.port() };
				conntrack->update_entry(
					Protocol::ICMPv4, entry->second,
					{ entry->second.src, masq_sock });
			}
			NATDBG("<NAPT> MASQ: %s => %s\n",
			       entry->to_string().c_str(),
			       entry->second.dst.to_string().c_str());
			// static source nat
	...
	void NAPT::demasquerade(IP4::IP_packet &pkt, const Stack &,
				Conntrack::Entry_ptr entry)
	{
		...
		NATDBG("<NAPT> DEMASQ: %s => %s\n", entry->to_string().c_str(),
		       entry->first.src.to_string().c_str());
		switch (pkt.ip_protocol()) {
		case Protocol::TCP:
	...
	Socket NAPT::masq(Conntrack::Entry_ptr entry, const ip4::Addr addr,
			  Port_util &ports)
	{
		...
		if (not is_snat(entry)) {
			...
			auto on_close = [&ports, port](Conntrack::Entry_ptr) {
				ports.unbind(port);
			};
	...
	void NAPT::dnat(IP4::IP_packet &p, Conntrack::Entry_ptr entry)
	{
		...
		case Protocol::TCP: {
			if (Conntrack::get_quadruple(p).dst ==
			    entry->second.dst) // assume reply
			{
				...
				tcp_dnat(
					p,
					entry->first
						.src); // TODO: currently rewrites full socket
			}
			return;
		}
		case Protocol::UDP: {
		...
		case Protocol::ICMPv4: {
			if (Conntrack::get_quadruple_icmp(p).dst ==
			    entry->second.dst) // assume reply
			{
				NATDBG("<NAPT> Found DNAT target: %s => %s\n",
				       entry->to_string().c_str(),
				       entry->first.src.address()
					       .to_string()
					       .c_str());
	...
	void NAPT::snat(IP4::IP_packet &p, Conntrack::Entry_ptr entry)
	{
		...
		switch (p.ip_protocol()) {
		case Protocol::TCP: {
			if (Conntrack::get_quadruple(p).src ==
			    entry->second.src) // assume reply
			{
				NATDBG("<NAPT> Found SNAT target: %s => %s\n",
				       entry->to_string().c_str(),
				       entry->first.dst.to_string().c_str());
				tcp_snat(
					p,
					entry->first
						.dst); // TODO: currently rewrites full socket
			}
			return;
		}
		case Protocol::UDP: {
		...
		case Protocol::ICMPv4: {
			if (Conntrack::get_quadruple_icmp(p).src ==
			    entry->second.src) // assume reply
			{
				NATDBG("<NAPT> Found SNAT target: %s => %s\n",
				       entry->to_string().c_str(),
				       entry->first.dst.address()
					       .to_string()
					       .c_str());
