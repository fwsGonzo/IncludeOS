	...
	: m_transport(std::move(t))
{
	...
	if (outgoing == true) {
		if (this->tls_perform_handshake() < 0)
			return;
	}
}
...
void TLS_stream::write(buffer_t buffer)
{
	...
	if (this->m_deferred_close) {
		TLS_PRINT(
			"::write() close on m_deferred_close after tls_perform_stream_write\n");
		this->close();
		return;
	}
}
...
void TLS_stream::write(const std::string &str)
{
	...
	write(net::StreamBuffer::construct_write_buffer(
...
int TLS_stream::decrypt(const void *indata, int size)
{
	...
	if (UNLIKELY(!handshake_completed())) {
		...
		if (status == STATUS_WANT_IO) {
			...
			if (num < 0) {
				TLS_PRINT(
					"TLS_stream::SSL_do_handshake() returned %d\n",
					num);
		...
		if (this->m_deferred_close) {
			TLS_PRINT(
				"::read() close on m_deferred_close after tls_perform_stream_write\n");
			this->close();
			return -1;
		}
	}
...
bool TLS_stream::tls_read(buffer_t buffer)
{
	...
	while (len > 0) {
		...
		if (status == STATUS_WANT_IO) {
			...
			TLS_PRINT(
				"::read() close on STATUS_FAIL after tls_perform_stream_write\n");
			this->close();
			return true;
		}
...
int TLS_stream::tls_perform_stream_write()
{
	...
	if (pending > 0) {
		TLS_PRINT("::tls_perform_stream_write() pending=%d bytes\n",
			  pending);
		auto buffer =
			net::StreamBuffer::construct_write_buffer(pending);
		if (buffer == nullptr) {
			return 0;
		}
		...
		if (UNLIKELY((pending = BIO_ctrl_pending(this->m_bio_wr)) >
			     0)) {
			return pending;
		}
		return 0;
	...
	if (!BIO_should_retry(this->m_bio_wr)) {
		TLS_PRINT(
			"::tls_perform_stream_write() close on !BIO_should_retry\n");
		this->close();
		return -1;
	}
	return 0;
...
int TLS_stream::tls_perform_handshake()
{
	...
	if (n == STATUS_WANT_IO) {
		do {
			...
			if (n < 0) {
				TLS_PRINT(
					"TLS_stream::tls_perform_handshake() stream write failed\n");
			}
		} while (n > 0);
		return n;
	} else {
