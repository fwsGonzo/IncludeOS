...
static void tls_check_for_errors()
{
...
static void tls_load_from_memory(X509_STORE *store, fs::Buffer cert_buffer)
{
...
static void tls_private_key_for_ctx(SSL_CTX *ctx, int bits = 2048)
{
	BIGNUM *bne = BN_new();
...
static SSL_CTX *tls_init_client(fs::List ents)
{
	...
	if (preverify == 0) {
		if (current_cert) {
			X509_NAME_print_ex_fp(
				stdout, X509_get_subject_name(current_cert), 0,
				XN_FLAG_ONELINE);
			printf("\n");
		}
