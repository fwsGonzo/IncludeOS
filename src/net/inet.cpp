...
Inet::Inet(hw::Nic &nic)
	: dns_server_(IP4::ADDR_ANY), nic_(nic), arp_(*this), ndp_(*this),
	  mld_(*this), /*mld2_(*this),*/
	  ip4_(*this), ip6_(*this), icmp_(*this), icmp6_(*this), udp_(*this),
	  tcp_(*this), dns_(*this), domain_name_{}, MTU_(nic.MTU())
{
	static_assert(sizeof(ip4::Addr) == 4, "IPv4 addresses must be 32-bits");

	/** SMP related **/
	this->cpu_id = SMP::cpu_id();
	INFO("Inet", "Bringing up %s on CPU %d", ifname().c_str(),
	     this->get_cpu_id());
...
void Inet::error_report(Error &err, Packet_ptr orig_pckt)
{
	...
	if (err.is_icmp()) {
		...
		if (icmp_err->is_too_big()) {
			...
			// We have received a response to a packet with an MTU that is too big for a node in the
			// path, and the packet has been dropped (the original packet was too big and the Don't
			// Fragment bit was set)

			// Notify every protocol of the received MTU if any of the protocol's connections use
			// the given path (based on destination address)

			// Also need to notify the instance that sent the packet that the packet has been
			// dropped, so it can retransmit it
			...
			// If the received MTU value is zero, the method will use the original packet's Total
			// Length and Header Length values to estimate a new Path MTU value
			ip4_.update_path(dest, icmp_err->pmtu(), too_big,
					 pckt_ip4->ip_total_length(),
					 pckt_ip4->ip_header_length());
...
void Inet::autoconf_v6(int retries, slaac_timeout_func handler, uint64_t token,
		       bool use_token)
{
...
void Inet::network_config(ip4::Addr addr, ip4::Addr nmask, ip4::Addr gw,
			  ip4::Addr dns)
{
...
void Inet::network_config6(IP6::addr addr6, uint8_t prefix6, IP6::addr gateway6)
{
	ndp().set_static_addr(std::move(addr6));
	ndp().set_static_prefix(prefix6);
	ndp().set_static_gateway(std::move(gateway6));
	INFO("Inet6", "Network configured (%s)",
	     nic_.mac().to_string().c_str());
...
{
	Expects(prefix != 0);
	int r = this->ip6_.addr_list().input(addr, prefix, pref_lifetime,
					     valid_lifetime);
	if (r == 1) {
		INFO("Inet6", "Address configured %s/%u",
		     addr.to_string().c_str(), prefix);
	}
}
			     ...
			     uint32_t pref_lifetime, uint32_t valid_lifetime)
{
	...
	if (r == 1) {
		INFO("Inet6", "Address configured %s (autoconf)",
...
{
	tcp_.reset_pmtu(dest,
			pmtu); /* Maybe later: udp_.reset_pmtu(dest, pmtu);*/
}

void Inet::resolve(const std::string &hostname, resolve_func func, bool force)
{
...
void Inet::resolve(const std::string &hostname, net::Addr server,
		   resolve_func func, bool force)
{
