...
namespace http
{
const Basic_client::timeout_duration Basic_client::DEFAULT_TIMEOUT{
...
Basic_client::Basic_client(TCP &tcp, Request_handler on_send,
			   const bool https_supported)
	: tcp_(tcp), on_send_{ std::move(on_send) },
			...
			const bool secure, Options options)
{
	Expects(cb != nullptr);
	using namespace std;
	auto &conn = (not secure) ? get_connection(host) :
				    get_secure_connection(host);
	...
	conn.send(move(req), move(cb), options.follow_redirect,
		  options.timeout);
}

void Basic_client::send(Request_ptr req, URI url, Response_handler cb,
			Options options)
{
	...
	if (url.host_is_ip4()) {
		...
		tcp_.stack().resolve(
			...
			ResolveCallback::make_packed(
				[this, request = move(req), url{ move(url) },
				 cb{ move(cb) }, opt{ move(options) }, secure,
				 port](net::dns::Response_ptr res,
				       const net::Error &) mutable {
					auto addr = res->get_first_addr();
					if (UNLIKELY(addr ==
						     net::Addr::addr_any)) {
						cb({ Error::RESOLVE_HOST },
						   nullptr,
						   Connection::empty());
						return;
					}
					// Host resolved
					send(move(request), { addr, port },
					     move(cb), secure, move(opt));
				}));
	}
}
			   ...
			   Response_handler cb, Options options)
{
	...
	if (url.host_is_ip4()) {
		...
		*req << hfields;
		...
		tcp_.stack().resolve(
			std::string(url.host()),
			ResolveCallback::make_packed([this, method,
						      url{ move(url) },
						      hfields{ move(hfields) },
						      cb{ move(cb) },
						      opt{ move(options) },
						      secure](net::dns::Response_ptr
								      res,
							      const net::Error
								      &err) {
				// Host resolved
...
void Basic_client::request(Method method, Host host, std::string path,
			   ...
			   Response_handler cb, const bool secure,
			   Options options)
{
	validate_secure(secure);
...
void Basic_client::add_data(Request &req, const std::string &data)
{
	...
	req.header().set_field(header::Content_Length,
...
{
	throw Client_error{
		"Secured connections not supported (use the HTTPS Client)."
	};
}
...
void Basic_client::close(Client_connection &c)
{
	debug("<http::Basic_client> Closing %u:%s %p\n", c.local_port(),
	      c.peer().to_string().c_str(), &c);
	auto &cset = conns_.at(c.peer());

	cset.erase(std::remove_if(
		cset.begin(), cset.end(),
		[port = c.local_port()](
			const std::unique_ptr<Client_connection> &conn)
			-> bool { return conn->local_port() == port; }));
}
