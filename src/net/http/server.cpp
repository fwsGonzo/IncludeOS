...
namespace http
{
const Server::idle_duration Server::DEFAULT_IDLE_TIMEOUT{ std::chrono::seconds(
	60) };

Server::Server(TCP &tcp, Request_handler cb, idle_duration timeout)
	: tcp_(tcp), on_request_(std::move(cb)), keep_alive_(true),
	  timer_id_(Timers::UNUSED_ID),
	  idle_timeout_(timeout), stat_conns_{ Statman::get().create(
					  Stat::UINT32,
					  tcp.stack().ifname() +
						  ".http_server.connections") },
	  stat_req_rx_{ Statman::get().create(
		  Stat::UINT64,
		  tcp.stack().ifname() + ".http_server.requests_rx") },
	  stat_req_bad_{ Statman::get().create(
		  Stat::UINT32,
		  tcp.stack().ifname() + ".http_server.requests_bad") },
	  stat_timeouts_{ Statman::get().create(
		  Stat::UINT32,
		  tcp.stack().ifname() + ".http_server.timeouts") }
{
}

void Server::listen(uint16_t port)
{
	assert(on_request_ != nullptr &&
	       "You must set 'on_request' on the server to receive requests!");
	...
	if (idle_timeout_ != idle_duration::zero())
		timer_id_ = Timers::periodic(
			30s, 1min, { this, &Server::timeout_clients });
}
...
void Server::bind(const uint16_t port)
{
	...
	SMP::global_lock();
	INFO("HTTP Server", "Listening on port %u on CPU %d", port,
	     SMP::cpu_id());
	SMP::global_unlock();
}
...
void Server::connect(Connection::Stream_ptr stream)
{
	...
	if (free_idx_.size() > 0) {
		...
		connections_[idx] = std::make_unique<Server_connection>(
			*this, std::move(stream), idx);
		free_idx_.pop_back();
	}
	// if not, add a new shared ptr
	else {
		connections_.emplace_back(std::make_unique<Server_connection>(
			*this, std::move(stream), connections_.size()));
	}
	++stat_conns_;
}
...
void Server::receive(Request_ptr req, status_t code, Server_connection &conn)
{
	...
	if (code == OK) {
		on_request_(std::move(req),
			    std::make_unique<Response_writer>(
				    create_response(code), conn));
	} else {
		++stat_req_bad_;
