...
namespace http
{
Server_connection::Server_connection(Server &server, Stream_ptr stream,
				     size_t idx, const size_t bufsize)
	: Connection(std::move(stream)), server_(server), req_(nullptr),
	  idx_(idx), idle_since_{ 0 }
{
