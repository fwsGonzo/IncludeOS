...
{
	return caseInsCompare(option_name, C_EXPIRES) ||
	       caseInsCompare(option_name, C_MAX_AGE) ||
	       caseInsCompare(option_name, C_DOMAIN) ||
	       caseInsCompare(option_name, C_PATH) ||
	       caseInsCompare(option_name, C_SECURE) ||
	       caseInsCompare(option_name, C_HTTP_ONLY);
}
...
bool Cookie::valid_expires_time(const std::string &expires) const noexcept
{
	...
	if (strptime(expires.c_str(), "%a, %d %b %Y %H:%M:%S %Z", &tm) not_eq
	    nullptr)
		return true;
	...
	if (strptime(expires.c_str(), "%a %b %d %H:%M:%S %Y", &tm) not_eq
	    nullptr)
		return true;
...
{
	if (not valid(name))
		throw CookieException{ "Invalid name (" + name +
				       ") of cookie!" };

	// value can be empty
	if (not value.empty() and not valid(value))
		throw CookieException{ "Invalid value (" + value +
				       ") of cookie!" };
	...
	: Cookie{ name, value }
{
	...
	if (options.size() % 2 not_eq 0)
		throw CookieException{
			"Invalid number of elements in cookie's options vector!"
		};
	...
	for (size_t i = 0; i < options.size(); i += 2) {
		...
		if (not valid_option_name(nm))
			throw CookieException{ "Invalid name (" + nm +
					       ") of cookie option!" };
			...
			try {
				...
				if (age < 0)
					throw CookieException{
						...
						val +
						") of cookie! Negative number of seconds not allowed."
					};

				set_max_age(age);
			} catch (std::exception &e) {
				throw CookieException{
					"Invalid max-age attribute (" + val +
					") of cookie! Invalid integer value."
				};
			}
...
void Cookie::set_expires(const std::string &expires)
{
	if (not valid_expires_time(expires))
		throw CookieException{ "Invalid expires attribute (" + expires +
				       ") of cookie!" };
...
void Cookie::set_domain(const std::string &domain)
{
	...
	std::transform(domain_.begin(), domain_.end(), domain_.begin(),
		       ::tolower);
}
...
void Cookie::set_path(const std::string &path)
{
	...
	if (path.at(0) not_eq '/')
		throw CookieException{
			"Invalid path (" + path +
			") of cookie! It must start with a / character."
		};
...
std::string Cookie::to_string() const
{
	...
	if (max_age_ not_eq -1)
		cookie_string +=
			"; " + C_MAX_AGE + "=" + std::to_string(max_age_);
