...
Response_writer::Response_writer(Response_ptr res, Connection &conn)
	: response_(std::move(res)), connection_(conn)
{
...
void Response_writer::pre_write(size_t len)
{
	...
	if (not header_sent_) {
		...
		// content-length is already set
			...
			if (cl < len)
				throw Response_writer_error{
					"Trying to write more than Content-Length allows: " +
					std::to_string(cl)
				};
		}
		// write headers
...
void Response_writer::write_header(status_t code)
{
	if (LIKELY(not header_sent_)) {
		...
		if (response_->header().value(http::header::Connection) ==
		    "close")
