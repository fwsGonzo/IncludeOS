...
__attribute__((constructor)) static void _GFRGRGRgegerjiuo_()
{
	settings.on_url = [](http_parser *parser, const char *at,
			     size_t length) {
	...
	settings.on_header_value = [](http_parser *parser, const char *at,
				      size_t length) {
		auto req = reinterpret_cast<Request *>(parser->data);
		req->header().set_field(std::string(req->private_field()),
					{ at, length });
		return 0;
	};
	...
	settings.on_headers_complete = [](http_parser *parser) {
		...
		req->set_method(http::method::code(http_method_str(
...
Request::Request(std::string request, const std::size_t limit, const bool parse)
	: Message{ limit }, request_{ std::move(request) }
{
	if (parse)
		this->parse();
}
