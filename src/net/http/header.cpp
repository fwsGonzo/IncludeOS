...
bool Header::set_field(std::string field, std::string value)
{
	...
	//-----------------------------------
		...
		return add_field(std::move(field), std::move(value));
}
...
util::sview Header::value(util::csview field) const noexcept
{
	...
	return (it not_eq fields_.cend()) ? util::csview{ it->second } :
					    util::sview();
}
...
Header::Const_iterator Header::find(util::csview field) const noexcept
{
	...
	//-----------------------------------
	return std::find_if(
		fields_.cbegin(), fields_.cend(), [&field](const auto _) {
			return std::equal(_.first.cbegin(), _.first.cend(),
