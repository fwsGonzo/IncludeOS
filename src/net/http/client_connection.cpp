...
Client_connection::Client_connection(Basic_client &client, Stream_ptr stream)
	: Connection{ std::move(stream) }, client_(client), req_(nullptr),
	  res_(nullptr), on_response_{ nullptr },
	  timer_({ this, &Client_connection::timeout_request }),
	  timeout_dur_{ timeout_duration::zero() }, redirect_{
		  client.default_follow_redirect
	  }
{
	// setup close event
...
void Client_connection::send(Request_ptr req, Response_handler on_res,
			     int redirects, timeout_duration timeout)
{
	Expects(available());
	req_ = std::move(req);
	...
	if (not stream_->is_connected()) {
		stream_->on_connect([this](auto &) { this->send_request(); });
	} else {
		send_request();
	}
}
...
void Client_connection::recv_response(buffer_t buf)
{
	...
	if (!header.is_empty()) {
		...
		if (header.has_field(header::Content_Length)) {
			try {
				const unsigned conlen = std::stoul(std::string(
					header.value(header::Content_Length)));
		...
		// is always terminated by the first empty line after the header fields
			 ...
			 is_informational(code) or code == No_Content or
			 code == Not_Modified or req_->method() == HEAD) {
			end_response();
		}
...
bool Client_connection::can_redirect(const Response_ptr &res) const
{
	...
	if (res == nullptr)
		return false;
	if (not is_redirection(res->status_code()))
		return false;
	if (not res->header().has_field("Location"))
		return false;
