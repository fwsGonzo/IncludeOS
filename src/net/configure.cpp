...
inline void parse_addr(v4_config &cfg4, v6_config &cfg6, std::string str)
{
	if (is_v6(str)) {
		...
		PRINT("v6 Addr: %s Prefix: %u\n",
		      cfg6.addr.back().addr.to_string().c_str(),
		      cfg6.addr.back().prefix);
	} else {
		if (cfg4.addr != 0) {
			MYINFO("WARN: Multiple v4 addresses not supported: skipping %s",
			       str.c_str());
			return;
		}
		...
		PRINT("v4 Addr: %s Netmask: %s\n",
		      cfg4.addr.to_string().c_str(),
		      cfg4.netmask.to_string().c_str());
	}
}
...
inline void parse_gateway(v4_config &cfg4, v6_config &cfg6, std::string str)
{
	if (is_v6(str)) {
		...
		if (cfg4.gateway != 0) {
			MYINFO("WARN: Multiple v4 gateways not supported: skipping %s",
			       str.c_str());
			return;
		}
...
inline void parse_dns(v4_config &cfg4, v6_config &cfg6, std::string str)
{
	if (is_v6(str)) {
		...
		if (cfg4.dns != 0) {
			MYINFO("WARN: Multiple v4 DNS not supported: skipping %s",
			       str.c_str());
			return;
		}
...
inline void config6(Inet &stack, const v6_config &cfg)
{
	...
	for (const auto &addr : cfg.gateway)
		stack.ndp().add_router(addr,
				       0xFFFF); // waiting for API to change
	...
	for (auto &val : configs) {
		...
		for (const auto &method : methods) {
			const double timeout =
				(val.HasMember("timeout")) ?
					val["timeout"].GetDouble() :
					10.0;
			...
			{
				auto static_cfg = [v4cfg,
						   &stack](bool timedout) {
					if (timedout) {
						MYINFO("DHCP timeout (%s) - falling back to static configuration",
						       stack.ifname().c_str());
						config4(stack, v4cfg);
					}
				};
				...
				MYINFO("WARN: Unrecognized config method \"%s\"",
				       method.c_str());
			}
		}
	}
