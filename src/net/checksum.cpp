...
uint16_t checksum(uint32_t tsum, const void *data, size_t length) noexcept
{
	...
	alignas(16) static const uint8_t shift_tab[48] = {
		...
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x01, 0x02, 0x03,
		0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
		0x0e, 0x0f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff
	};
	/* we could extend this to fast convert 4 words to LE from BE */
	alignas(16) static const uint8_t swap32[16]{ 0x03, 0x02, 0x01, 0x00,
						     0x80, 0x80, 0x80, 0x80,
						     0x80, 0x80, 0x80, 0x80,
						     0x80, 0x80, 0x80, 0x80 };
	...
	if (LIKELY(rest != 16)) {
		...
		dblock = _mm_shuffle_epi8(
			dblock,
			_mm_loadu_si128((__m128i *)(&shift_tab[16 - rest])));
		dblock = _mm_shuffle_epi8(
			dblock,
			_mm_loadu_si128((__m128i *)(&shift_tab[16 + rest])));
		suma = _mm_add_epi32(suma, _mm_shuffle_epi8(dblock, swap16a));
	...
	oldsum = _mm_shuffle_epi8(_mm_cvtsi32_si128(tsum),
				  _mm_loadu_si128((__m128i *)&swap32[0]));
...
void checksum_adjust(uint8_t *chksum, const void *odata, int olen,
		     const void *ndata, int nlen)
{
