...
int Record::parse_name(const char *reader, const char *buffer, size_t tot_len,
		       std::string &output) const
{
	...
	while (*ureader) {
		if (*ureader >= 192) {
			...
			jumped =
				true; // we have jumped to another location so counting wont go up!
		} else {
			output[namelen++] = *ureader++;

			// maximum label size
			if (UNLIKELY(namelen > 63))
				break;
		}
