...
Client::Client(Stack &stack)
	: stack_{ stack }, cache_ttl_{ DEFAULT_CACHE_TTL }, flush_timer_{
		  { this, &Client::flush_expired }
	  }
{
}

void Client::resolve(Address dns_server, Hostname hostname,
		     Resolve_handler func, Timer::duration_t timeout,
		     bool force)
{
	...
	auto &socket = (dns_server.is_v6()) ? stack_.udp().bind6() :
					      stack_.udp().bind();

	// Create our query
	Query query{ std::move(hostname),
		     (dns_server.is_v6() ? Record_type::AAAA :
					   Record_type::A) };
#ifdef LIBFUZZER_ENABLED
	g_last_xid = query.id;
#endif
	...
	auto emp = requests_.emplace(
		std::piecewise_construct, std::forward_as_tuple(query.id),
		std::forward_as_tuple(*this, socket, std::move(query),
				      std::move(func)));
...
Client::Request::Request(Client &cli, udp::Socket &sock, dns::Query q,
			 Resolve_handler cb)
	: client{ cli }, query{ std::move(q) }, response{ nullptr },
	  socket{ sock }, callback{ std::move(cb) },
...
void Client::Request::parse_response(Addr, UDP::port_t, const char *data,
				     size_t len)
{
...
void Client::add_cache_entry(const Hostname &hostname, Address addr,
