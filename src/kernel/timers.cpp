...
void Timers::init(const start_func_t &start, const stop_func_t &stop)
{
	...
	system.oneshot_started =
		(int64_t *)&Statman::get()
			.create(Stat::UINT64, CPU + ".timers.oneshot_started")
			.get_uint64();
	system.oneshot_stopped =
		(int64_t *)&Statman::get()
			.create(Stat::UINT64, CPU + ".timers.oneshot_stopped")
			.get_uint64();
	system.periodic_started =
		&Statman::get()
			 .create(Stat::UINT32, CPU + ".timers.periodic_started")
			 .get_uint32();
	system.periodic_stopped =
		&Statman::get()
			 .create(Stat::UINT32, CPU + ".timers.periodic_stopped")
			 .get_uint32();
}
...
Timers::id_t Timers::periodic(duration_t when, duration_t period,
			      handler_t handler)
{
	...
	if (UNLIKELY(system.free_timers.empty())) {
		...
		// occupy free slot
		new (&system.timers[id])
			SystemTimer(real_time, period, handler);
	}

	// Stat increment timer started
	if (system.timers[id].is_oneshot()) {
		if (system.oneshot_started)
			(*system.oneshot_started)++;
	} else {
		if (system.periodic_started)
			(*system.periodic_started)++;
	}
...
{
	auto &system = get();
	if (UNLIKELY(system.timers.at(id).already_dead))
		return;
