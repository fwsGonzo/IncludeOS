...
static inline uint16_t make_vgaentry(const char c, const uint8_t color) noexcept
{
	uint16_t c16 = c;
...
TextmodeVGA::TextmodeVGA() noexcept : row{ 0 }, column{ 0 }
{
...
void TextmodeVGA::put(const char c, uint8_t color, uint8_t x,
		      uint8_t y) noexcept
{
...
void TextmodeVGA::newline() noexcept
{
	...
	if (++this->row == VGA_HEIGHT) {
		...
		for (size_t n{ 0 }; n < total; n += 8) {
			scan = _mm_load_si128(reinterpret_cast<__m128i *>(
				&buffer[n + VGA_WIDTH]));
			_mm_store_si128(reinterpret_cast<__m128i *>(&buffer[n]),
					scan);
		}
		...
		for (size_t n{ 0 }; n < VGA_WIDTH; n += 8) {
			_mm_store_si128(
				reinterpret_cast<__m128i *>(&buffer[total + n]),
				scan);
		}
#else
		// Copy rows upwards
