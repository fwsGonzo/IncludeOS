...
uintptr_t _multiboot_free_begin(uintptr_t boot_addr)
{
	...
	if (info->flags & MULTIBOOT_INFO_CMDLINE and
	    info->cmdline > multi_end) {
		debug("* Multiboot cmdline @ 0x%x: %s \n", info->cmdline,
		      (char *)info->cmdline);
		// We can't use a cmdline that's either insde our ELF or pre-ELF area
		Expects(info->cmdline > multi_end or info->cmdline < 0x100000);
		...
		if (info->cmdline > multi_end) {
			...
			const uintptr_t cmdline_end =
				info->cmdline + strlen(cmdline_ptr) + 1;
			if (cmdline_end > multi_end)
				multi_end = cmdline_end;
		}
	}
...
void kernel::multiboot(uint32_t boot_addr)
{
	...
	if (info->flags & MULTIBOOT_INFO_MEMORY) {
		...
		uint32_t mem_high_end =
			mem_high_start + (info->mem_upper * 1024) - 1;
		uint32_t mem_high_kb = info->mem_upper;

		INFO2("* Valid memory (%i Kib):", mem_low_kb + mem_high_kb);
		INFO2("  0x%08x - 0x%08x (%i Kib)", mem_low_start, mem_low_end,
		      mem_low_kb);
		INFO2("  0x%08x - 0x%08x (%i Kib)", mem_high_start,
		      mem_high_end, mem_high_kb);
		INFO2("");
	} else {
	...
	if (info->flags & MULTIBOOT_INFO_MEM_MAP) {
		...
		for (auto map : mmap) {
			...
			INFO2("  0x%010zx - 0x%010zx %s (%zu Kb.)", addr,
			      addr + size - 1, str_type, size / 1024);
			...
			if (not(map.type & MULTIBOOT_MEMORY_AVAILABLE)) {
				...
				// Map as free memory
				// os::mem::map_avail({map.addr, map.addr, {os::mem::Access::read |
				// os::mem::Access::write}, map.len}, "Reserved (Multiboot)");
			}
		}
		printf("\n");
	}
