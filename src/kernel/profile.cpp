	...
	void add(void *current)
	{
		...
		if (samplerq->free_capacity()) {
			if (mode == StackSampler::MODE_CURRENT)
				...
				samplerq->push_back(
					(uintptr_t)__builtin_return_address(2));
		}
...
void gather_stack_sampling()
{
	...
	if (get().lockless == 1) {
		for (auto *addr = get().transferq->begin();
		     addr < get().transferq->end(); addr++) {
...
std::vector<Sample> StackSampler::results(int N)
{
	...
	std::sort(vec.begin(), vec.end(),
		  [](const sample_pair &sample1, const sample_pair &sample2)
			  -> int { return sample1.second > sample2.second; });
	...
	for (auto &sa : vec) {
		// resolve the addr
		auto func = Elf::safe_resolve_symbol((void *)sa.first, buffer,
						     sizeof(buffer));
		if (func.name) {
			res.push_back(Sample{ sa.second, (void *)func.addr,
					      func.name });
		} else {
			int len = snprintf(buffer, sizeof(buffer), "0x%08zx",
					   func.addr);
			res.push_back(Sample{ sa.second, (void *)func.addr,
...
void StackSampler::print(const int N)
{
	...
	for (auto &sa : samp) {
		...
		printf("%5.2f%%  %*u: %.*s\n", perc, 8, sa.samp,
		       (int)sa.name.size(), sa.name.c_str());
	}
}
...
std::string HeapDiag::to_string()
{
	...
	int len = snprintf(buffer, sizeof(buffer),
			   ...
			   "Heap usage  %lu kB\n",
			   heap_begin, heap_size / 1024, heap_end, last,
			   last / 1024, heap_usage / 1024);
