...
#define frp(N, ra)                                                             \
	(__builtin_frame_address(N) != nullptr) &&                             \
		(ra = __builtin_return_address(N)) != nullptr &&ra !=          \
			(void *)-1

extern "C" char *__cxa_demangle(const char *name, char *buf, size_t *n,
				int *status);

template <typename N> static std::string to_hex_string(N n)
{
	char buffer[16];
...
class ElfTables {
	...
	void set(const ElfSym *syms, uint32_t entries, const char *strs,
		 uint32_t strsize, uint32_t csum_syms, uint32_t csum_strs)
	{
		/*
	...
	*/
	...
	safe_func_offset getsym_safe(ElfAddr addr, char *buffer, size_t length)
	{
		...
		if (LIKELY(addr > 0x1000)) {
			...
			if (LIKELY(sym)) {
				...
				return { demangle_safe(sym_name(sym), buffer,
						       length),
					 static_cast<uintptr_t>(base), offset };
			}
		} else if (addr == 0x0) {
			return { "0x0 (null)", static_cast<uintptr_t>(addr),
				 0 };
		}
	...
	const ElfSym *getaddr(ElfAddr addr)
	{
		...
		for (int i = 0; i < (int)symtab.entries; i++) {
			if (addr >= symtab.base[i].st_value &&
			    (addr < symtab.base[i].st_value +
					    symtab.base[i].st_size)) {
	...
	bool verify_symbols() const
	{
		...
		uint32_t csum = crc32_fast(symtab.base,
					   symtab.entries * sizeof(ElfSym));
	...
	const char *demangle_safe(const char *name, char *buffer,
				  size_t buflen) const
	{
		int status;
...
safe_func_offset Elf::safe_resolve_symbol(const void *addr, char *buffer,
					  size_t length)
{
...
void os::print_backtrace(void (*stdout_function)(const char *, size_t)) noexcept
{
	...
	if (Elf::get_strtab() == NULL) {
		int len = snprintf(
			_btrace_buffer, sizeof(_btrace_buffer),
...
#define PRINT_TRACE(N, ra)                                                     \
	auto symb = Elf::safe_resolve_symbol(ra, _symbol_buffer,               \
					     sizeof(_symbol_buffer));          \
...
extern "C" void _print_elf_symbols()
{
	...
	for (size_t i = 0; i < symtab.entries; i++) {
		kprintf("%16p: %s\n", (void *)symtab.base[i].st_value,
			&strtab[symtab.base[i].st_name]);
	}
	kprintf("*** %u entries\n", symtab.entries);
}
void Elf::print_info()
...
extern "C" int _get_elf_section_datasize(const void *location)
{
	auto &hdr = *(elfsyms_header *)location;
	// special case for missing call to elf_syms
	if (hdr.symtab_entries == 0)
		return 0;
	return hdr.symtab_entries * sizeof(ElfSym) + hdr.strtab_size;
}

extern "C" void _move_elf_syms_location(const void *location,
					void *new_location)
{
	...
	uint32_t csum_strs =
		crc32c(&hdr->syms[hdr->symtab_entries], hdr->strtab_size);
...
extern "C" void elf_check_symbols_ok()
{
...
void elf_protect_symbol_areas()
{
	...
	if (size % os::mem::min_psize())
		size += os::mem::min_psize() -
			(size & (os::mem::min_psize() - 1));
	...
	os::mem::vmmap().assign_range({ (uintptr_t)src,
					(uintptr_t)src + size - 1,
					"Symbols & strings" });
