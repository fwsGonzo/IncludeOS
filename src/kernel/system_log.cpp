...
#define MRB_LOG_SIZE                                                           \
	(MRB_AREA_SIZE - sizeof(MemoryRingBuffer) - sizeof(Log_buffer))
#define VIRTUAL_MOVE
...
{
	return get_system_log_loc() + sizeof(Log_buffer) +
	       sizeof(MemoryRingBuffer);
}
...
void SystemLog::initialize()
{
	...
	os::mem::vmmap().assign_range(
		{ lu_phys - MRB_AREA_SIZE, lu_phys - 1, "SystemLog physical" });
	...
	if (buffer.magic != Log_buffer::MAGIC) {
		...
		INFO2("Created @ %p (%i kB)", get_system_log_loc(),
		      mrb->capacity() / 1024);
	...
	// Correct magic means (hopefully) existing system log
		...
		new (mrb) MemoryRingBuffer(get_ringbuffer_data(), state[0],
					   state[1], state[2], state[3]);

		INFO2("Restored @ %p (%i kB) Flags: 0x%x", mrb->data(),
		      mrb->capacity() / 1024, buffer.flags);
	}
	Ensures(mrb != nullptr);
...
__attribute__((constructor)) static void system_log_gconstr()
{
