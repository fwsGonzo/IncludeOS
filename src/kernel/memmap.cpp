...
///////////////////////////////////////////////////////////////////////////////
Fixed_memory_range::Fixed_memory_range(const uintptr_t begin,
				       const uintptr_t end, const char *name,
				       In_use_delg in_use_operation)
	: name_{ name }, in_use_op_{ in_use_operation }
{
	if (begin > end) {
	...
	if ((end - begin + 1) > static_cast<uintptr_t>(max_size())) {
		throw Memory_range_exception{ "Maximum range size is " +
					      std::to_string(max_size()) };
	}

	range_ = Memory_range(reinterpret_cast<uint8_t *>(begin),
			      (end - begin + 1));
}

///////////////////////////////////////////////////////////////////////////////
Fixed_memory_range::Fixed_memory_range(Memory_range &&range, const char *name)
	: range_{ std::move(range) }, name_{ name }
{
...
bool Fixed_memory_range::is_valid_range(const Memory_range &range) noexcept
{
	...
	const auto end =
		reinterpret_cast<uintptr_t>(range.data() + range.size() - 1);
	return (start < end) and
	       ((end - start + 1) <= static_cast<uintptr_t>(max_size()));
}
...
{
	return (in_range(other.addr_start()) or
		in_range(
			other.addr_end())) //< Other range overlaps with my range
	       or
	       (other.in_range(addr_start()) and
		other.in_range(addr_end())); //< My range is inside other range
}
...
std::string Fixed_memory_range::to_string() const
{
	...
	out << std::hex << std::setfill('0') << "0x" << std::setw(10)
	    << addr_start() << " - "
	    << "0x" << std::setw(10) << addr_end() << ", " << std::dec
	    << std::setfill(' ') << std::setw(11) << util::Byte_r(size())
	    << " (";
	...
	out << std::fixed << std::setw(6) << std::left << pct_used << "%) "
	    << std::setw(25) << name_;
...
Memory_map::Key Memory_map::in_range(const Key addr)
{
	...
	if ((closest_above == map_.end()) or
	    (closest_above not_eq map_.begin())) {
		closest_above--;
	}
...
Fixed_memory_range &Memory_map::assign_range(Fixed_memory_range &&rng)
{
	...
	if (UNLIKELY(map_.empty())) {
		...
		debug("* Range inserted (success: 0x%x, %i)",
		      new_entry.first->first, new_entry.second);
	...
	if (UNLIKELY(rng.overlaps(closest_match->second))) {
		throw Memory_range_exception{
			"Range '" + std::string(rng.name()) +
	...
	if (UNLIKELY((closest_match not_eq map_.begin()) and
		     (closest_match->second.addr_start() > key))) {
		closest_match--;
...
{
	return const_cast<Fixed_memory_range &>(
		static_cast<const Memory_map *>(this)->at(key));
}
...
ptrdiff_t Memory_map::resize(const Key key, const ptrdiff_t size)
{
	...
	debug("Resize range 0x%x using %lib to use %lib", key,
	      range.bytes_in_use(), size);

	if (range.bytes_in_use() >= size) {
		throw Memory_range_exception{
			"Can't resize. Range " + std::string(range.name()) +
			" uses " + std::to_string(range.bytes_in_use()) +
			"b, more than the requested " + std::to_string(size) +
			"b"
		};
	}
	...
	if (collision and (collision not_eq range.addr_start())) {
		throw Memory_range_exception{
			"Can't resize. Range would collide with " +
