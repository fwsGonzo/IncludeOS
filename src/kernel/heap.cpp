...
{
	return kernel::heap_usage() + kernel::state().liveupdate_size +
	       kernel::heap_begin();
}
...
bool kernel::heap_ready()
{
	return __heap_ready;
}
bool os::mem::heap_ready()
{
	return kernel::heap_ready();
}
...
void kernel::init_heap(uintptr_t free_mem_begin, uintptr_t memory_end) noexcept
{
	...
	kernel::state().heap_begin =
		util::bits::roundto<heap_alignment>(free_mem_begin);
