...
extern "C" void cpu_enable_panicking()
{
...
void os::panic(const char *why) noexcept
{
	...
	const int len =
		strnlen(get_crash_context_buffer(), get_crash_context_length());
	if (len > 0) {
		printf("\n\t**** CPU %d CONTEXT: ****\n %*s\n\n", current_cpu,
	...
	fprintf(stderr, "Heap is at: %p / %p  (diff=%lu)\n",
		(void *)kernel::heap_end(), (void *)kernel::heap_max(),
		(ulong)(kernel::heap_max() - kernel::heap_end()));
	fprintf(stderr,
		"Heap area: %lu / %lu Kb (allocated %zu kb)\n", // (%.2f%%)\n",
		(ulong)(kernel::heap_end() - kernel::heap_begin()) / 1024,
		(ulong)heap_total / 1024,
		kernel::heap_usage() / 1024); //, total * 100.0);
	fprintf(stderr, "Total memory use: ~%zu%% (%zu of %zu b)\n",
		util::bits::upercent(os::total_memuse(), kernel::memory_end()),
		os::total_memuse(), kernel::memory_end());

	// print plugins
	fprintf(stderr, "*** Found %u plugin constructors:\n",
		uint32_t(&__plugin_ctors_end - &__plugin_ctors_start));
	for (kernel::ctor_t *ptr = &__plugin_ctors_start;
	     ptr < &__plugin_ctors_end; ptr++) {
		char buffer[4096];
		auto res = Elf::safe_resolve_symbol((void *)*ptr, buffer,
						    sizeof(buffer));
