...
void thread_t::resume()
{
	THPRINT("Returning to tid=%ld tls=%p nexti=%p stack=%p\n", this->tid,
		this->my_tls, this->stored_nexti, this->stored_stack);
...
thread_t *thread_create(thread_t *parent, int flags, void *ctid,
			void *stack) noexcept
{
...
extern "C" void __thread_suspend_and_yield(void *next_instr, void *stack)
{
	// don't go through the ardous yielding process when alone
	if (kernel::suspended.empty())
		return;
...
extern "C" long
syscall_SYS_sched_setscheduler(pid_t /*pid*/, int /*policy*/,
