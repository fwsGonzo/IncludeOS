...
extern "C" {
void __fiber_jumpstart(volatile void *th_stack, volatile Fiber *f,
		       volatile void *parent_stack);
...
void Fiber::resume()
{
	...
	Expects(stack_loc_ > stack_.get() and
		stack_loc_ < stack_.get() + stack_size_);
	Expects(not done_);
