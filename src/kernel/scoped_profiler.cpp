...
void ScopedProfiler::record()
{
	...
	if (UNLIKELY(guard == Guard::NOT_SELECTED)) {
		if (CPUID::is_intel_cpu()) {
			...
			printf("[WARNING] ScopedProfiler only works with an Intel or AMD CPU that supports "
			       "SSE2!\n");
...
ScopedProfiler::~ScopedProfiler()
{
	...
	for (auto &entry : entries) {
		if (entry.function_address == function_address) {
			// Update the entry
			entry.cycles_average =
				((entry.cycles_average * entry.num_samples) +
				 cycles) /
				(entry.num_samples + 1);
			entry.num_samples += 1;
			return;
	...
	for (auto &entry : entries) {
		if (entry.function_address == 0) {
			...
			const auto symbols =
				Elf::safe_resolve_symbol(function_address,
							 symbol_buffer,
	...
	if (num_entries > 0) {
		if (sorted) {
			...
			std::sort(entries.begin(),
				  entries.begin() + num_entries,
				  [](const Entry &a, const Entry &b) {
					  return a.cycles_average >
						 b.cycles_average;
				  });
		}
		...
		for (auto i = 0u; i < num_entries; i++) {
			...
			double micros =
				entry.cycles_average / os::cpu_freq().count();
			ss.width(10);
			ss << micros / 1000.0 << " ms | ";
