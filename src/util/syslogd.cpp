...
std::unique_ptr<Syslog_facility> Syslog::fac_ =
	std::make_unique<Syslog_print>();
...
*/
	...
	if (not valid_priority(priority)) {
		syslog(LOG_ERR, "Syslog: Unknown priority %d. Message: %s",
		       priority, buf);
		return;
	}
	...
	std::string message =
		fac_->build_message_prefix(Service::binary_name());
	...
	/*
	    ...
	    specifications of the "%n$" form. If a complete conversion specification with the m
       conversion specifier character is not just %m, the behavior is undefined. A trailing
...
void Syslog::openlog(const char *ident, int logopt, int facility)
{
	...
	if (valid_logopt(logopt) or
	    logopt == 0) // Should be possible to clear the logopt
