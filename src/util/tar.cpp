...
const Element &Tar::element(const std::string &path) const
{
	...
	throw Tar_exception(std::string{
		"Path " + path +
		" doesn't exist. Folder names should have a trailing /" });
}
...
Tar Reader::read(const uint8_t *data, const size_t size)
{
	...
	for (size_t i = 0; i < size; i += SECTOR_SIZE) {
		...
		if (not element.typeflag_is_set() or not element.is_dir()) {
			...
			i += (SECTOR_SIZE *
			      (element.num_content_blocks() -
			       1)); // move to the end of the element
		}
...
Tar_data Reader::decompress(const uint8_t *data, const size_t size,
			    unsigned int dlen)
{
	if (!has_uzlib_init) {
		uzlib_init();
	...
	if (res != TINF_OK)
		throw Tar_exception(std::string{ "Error parsing header: " +
						 std::to_string(res) });
	...
	if (res not_eq TINF_DONE)
		throw Tar_exception(
			std::string{ "Error during decompression. Res: " +
				     std::to_string(res) });
