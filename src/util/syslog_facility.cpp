...
std::string Syslog_udp::build_message_prefix(const std::string &binary_name)
{
	...
	// Third: Hostname ( Preferably: 1. FQDN (RFC1034) 2. Static IP address 3. Hostname 4. Dynamic
	// IP address 5. NILVALUE (-) )
	message += std::string{ timebuf } + " " +
		   net::Interfaces::get(0).ip_addr().str() + " ";

	// Fourth: App-name, PROCID (LOG_PID) and MSGID
	message += std::string(binary_name) + " " + std::to_string(getpid()) +
		   " UDPOUT ";
