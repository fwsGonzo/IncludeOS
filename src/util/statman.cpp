...
Stat::Stat(const Stat_type type, const std::string &name) : ui64(0)
{
	// stats are persisted by default
	this->m_bits = uint8_t(type) | PERSIST_BIT;

	if (name.size() > MAX_NAME_LEN)
		throw Stats_exception("Stat name cannot be longer than " +
...
void Stat::operator++()
{
	...
	case FLOAT:
		...
		break;
	default:
		throw Stats_exception(
			"Invalid stat type encountered when incrementing");
	}
}
...
std::string Stat::to_string() const
{
	switch (this->type()) {
	case UINT32:
		return std::to_string(ui32);
	case UINT64:
		return std::to_string(ui64);
	case FLOAT:
...
Stat &Statman::get_or_create(const Stat::Stat_type type,
