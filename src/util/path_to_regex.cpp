...
std::regex path_to_regex(const std::string &path, Keys &keys,
...
std::vector<Token> parse(const std::string &str)
{
	...
	for (std::sregex_iterator i =
		     std::sregex_iterator{ str.begin(), str.end(),
					   PATH_REGEXP };
		...
		path += str.substr(
			index,
			(offset -
			 index)); // from index, number of chars: offset - index

		index = offset + m.size();

		if (not escaped.empty()) {
			path += escaped
				[1]; // if escaped == \a, escaped[1] == a (if str is "/\\a" f.ex.)
			continue;
		}

		std::string next = ((size_t)index < str.size()) ?
					   std::string{ str.at(index) } :
					   "";
		...
		bool partial = (not prefix.empty()) and (not next.empty()) and
			       (next not_eq prefix);
...
std::regex tokens_to_regex(const Tokens &tokens, const Options &options)
{
	...
	if (not options.empty()) {
		...
		strict = (it not_eq options.end()) ?
				 options.find("strict")->second :
				 false;

		it = options.find("sensitive");
		sensitive = (it not_eq options.end()) ?
				    options.find("sensitive")->second :
				    false;

		it = options.find("end");
		end = (it not_eq options.end()) ? options.find("end")->second :
						  true;
	}
	...
	std::regex re{ "(.*\\/$)" };
	bool endsWithSlash =
		lastToken.is_string and std::regex_match(lastToken.name, re);
	...
	return std::regex{ "^" + route, std::regex_constants::ECMAScript |
