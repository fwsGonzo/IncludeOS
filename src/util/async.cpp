...
void Async::upload_file(Disk disk, const Dirent &ent, Stream *stream,
			on_after_func callback, const size_t CHUNK_SIZE)
{
	disk_transfer(
		disk, ent,
		[stream](fs::buffer_t buffer, next_func next) mutable {
...
void Async::disk_transfer(Disk disk, const Dirent &ent,
			  on_write_func write_func, on_after_func callback,
			  const size_t CHUNK_SIZE)
{
	...
	*next = next_func_t::make_packed([weak_next, disk, ent, write_func,
					  callback, CHUNK_SIZE](size_t pos) {
		...
		disk->fs().read(
			ent, pos * CHUNK_SIZE, CHUNK_SIZE,
			fs::on_read_func::make_packed([next, pos, write_func,
						       callback](
							      fs::error_t err,
							      fs::buffer_t
								      buffer) {
				...
				write_func(
					buffer,
					next_func::make_packed([next, pos,
								callback](
								       bool good) {
