...
int utimensat(int filedes, const char *path, const struct timespec times[2],
	      int flag)
{
	try {
		auto &fd = FD_map::_get(filedes);
...
int stat(const char *path, struct stat *buf)
{
	...
	try {
		...
		if (ent.is_valid()) {
			...
			buf->st_blocks = buf->st_size > 0 ?
						 round_up(buf->st_size, 512) :
						 0;
...
extern "C" int __fxstat(int ver, int fd, struct stat *stat_buf)
{
