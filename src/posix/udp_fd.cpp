...
void UDP_FD::recv_to_buffer(net::udp::addr_t addr, net::udp::port_t port,
			    const char *buf, size_t len)
{
	...
	if (buffer_.size() < max_buffer_msgs()) {
		...
		buffer_.emplace_back(htonl(addr.v4().whole), htons(port),
				     std::move(buff));
	}
}
		       ...
		       const struct sockaddr *dest_addr, socklen_t dest_len)
{
	if (not is_connected()) {
		...
		// The specified address is not a valid address for the address family of the specified
		// socket.
	...
	const auto &dest =
		(not is_connected()) ? *((sockaddr_in *)dest_addr) : peer_;
	...
	// Sending
	bool written = false;
	this->sock->sendto(net::ip4::Addr{ ntohl(dest.sin_addr.s_addr) },
			   ntohs(dest.sin_port), message, len,
			 ...
			 socklen_t *__restrict__ address_len)
{
	...
	if (!buffer_.empty()) {
		return read_from_buffer(buffer, len, flags, address,
					address_len);
	}
	// Else make a blocking receive
		...
		this->sock->on_read(net::udp::Socket::recvfrom_handler::make_packed(
			[&bytes, &done, this, buffer, len, flags, address,
			 address_len](net::udp::addr_t addr,
				      net::udp::port_t port, const char *data,
				      size_t data_len) {
				// if this already been called once while blocking, buffer
				if (done) {
					recv_to_buffer(addr, port, data,
						       data_len);
					return;
				}
				...
				// TODO: If the actual length of the address is greater than the length of the
...
int UDP_FD::getsockopt(int level, int option_name, void *option_value,
...
int UDP_FD::setsockopt(int level, int option_name, const void *option_value,
		       socklen_t option_len)
{
