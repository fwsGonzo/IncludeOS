...
long TCP_FD::connect(const struct sockaddr *address, socklen_t address_len)
{
	...
	bool refused = false;
	outgoing->on_connect(
		[&refused](auto conn) { refused = (conn == nullptr); });
	...
	while (not(outgoing->is_connected() or outgoing->is_closing() or
		   outgoing->is_closed() or refused)) {
		os::block();
	}
...
ssize_t TCP_FD::recvfrom(void *dest, size_t len, int flags, struct sockaddr *,
			 socklen_t *)
{
	return recv(dest, len, flags);
}

long TCP_FD::accept(struct sockaddr *__restrict__ addr,
...
TCP_FD_Conn::TCP_FD_Conn(net::tcp::Connection_ptr c)
	: conn{ std::move(c) }, buffer{ nullptr }, buf_offset{ 0 }
{
	assert(conn != nullptr);
	set_default_read();
...
{
	listener.on_connect([this, backlog](net::tcp::Connection_ptr conn) {
...
long TCP_FD_Listen::accept(struct sockaddr *__restrict__ addr,
			   socklen_t *__restrict__ addr_len)
{
