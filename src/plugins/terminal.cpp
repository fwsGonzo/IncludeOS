...
{
	terms.emplace(std::piecewise_construct, std::forward_as_tuple(counter),
		      std::forward_as_tuple(conn));
	auto idx = counter++;
	conn->on_close([idx]() { terms.erase(idx); });
	return terms.at(idx);
}
...
static void spawn_terminal()
{
	...
	inet.tcp().listen(TERM_PORT,
			  [](auto conn) { create_connection_from(conn); });
#ifdef USE_LIVEUPDATE
	if (liu::LiveUpdate::is_resumable()) {
		liu::LiveUpdate::resume(
			"terminal", [TERM_NET](liu::Restore &restore) {
...
__attribute__((constructor)) static void feijfeifjeifjeijfei()
{
