			   ...
			   const net::ip4::Addr addr, const uint16_t port)
{
	...
	INFO2("%s:%u @ %s", addr.to_string().c_str(), port,
	      stack.ifname().c_str());
...
static void syslog_mount()
{
	...
	if (not cfg.HasMember("type") or
	    not(cfg["type"].GetString() == std::string{ "udp" })) {
	...
	const uint16_t port =
		cfg.HasMember("port") ? cfg["port"].GetUint() : default_port;

	mount_udp_sock(path, stack, addr, port);
}

#include <os>
__attribute__((constructor)) static void syslog_plugin()
{
