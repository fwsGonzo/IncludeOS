 ...
 **/
void unik::Client::register_instance(net::Inet &inet,
				     const net::UDP::port_t port)
{
	INFO("Unik client", "Initializing Unik registration service");
	INFO("Unik client", "Listening for UDP hearbeat on %s:%i",
	     inet.ip_addr().to_string().c_str(), port);
	INFO("Unik client", "IP is attached to interface %s ",
	     inet.link_addr().str().c_str());

	// Set up an UDP port for receiving UniK heartbeat
	auto &sock = inet.udp().bind(port);
	CHECK(net::Interfaces::get(0).udp().is_bound(sock.local()),
	      "Unik UDP port is bound as expected");
	sock.on_read([&inet](auto addr, auto port, const char *data,
			     size_t len) {
		...
		if (dotloc == std::string::npos) {
			INFO("Unik client",
			     "Unexpected UDP data format - no ':' in string.");
			return;
		}
		...
		attempts_left--;
		INFO("Unik client",
		     "Connecting to UniK instance listener %s:%i (attempt %i / %i) ",
		     ip.str().c_str(), 3000, max_attempts - attempts_left,
		     max_attempts);
		...
		http->on_connect([&http, &inet](auto unik) {
			...
			// Construct a HTTP request to the Unik instance listener, providing our mac-address in
			// the query string
			std::string http_request =
				"POST /register?mac_address=" +
				std::string(mac_str) + " HTTP/1.1\r\n\n";
			INFO("Unik client",
			     "Connected to UniK instance listener. Sending HTTP request: %s ",
			     http_request.c_str());
			...
			unik->on_read(1024, [&http](auto buf) {
				std::string response((char *)buf->data(),
						     buf->size());
...
void unik::Client::register_instance_dhcp()
{
	...
	inet.negotiate_dhcp(10.0, [](bool timeout) {
		if (timeout) {
			INFO("Unik client",
			     "DHCP request timed out. Nothing to do.");
			return;
		}
...
__attribute__((constructor)) void register_plugin_unik()
{
	os::register_plugin(unik::Client::register_instance_dhcp,
			    "Unik register instance");
}
