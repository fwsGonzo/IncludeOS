...
static void parse_config()
{
	...
	for (auto &val : mounts) {
		...
		Expects(disk->fs_ready() &&
			"Filesystem not ready (async disk?)");

		fs::VFS::mount(fs::Path{ val["mount"].GetString() }, dev.id(),
			       fs::Path{ val["root"].GetString() },
			       val["description"].GetString(), [](auto err) {
				       Expects(not err &&
...
{
	RNG::get().open_fd = []() -> FD & { return FD_map::_open<RNG_fd>(); };
...
#include <os>
__attribute__((constructor)) static void vfs_mount_plugin()
{
