...
{
	INFO("Madness", "Starting allocation timer every %lld sec.",
	     alloc_freq.count());
	Timers::periodic(alloc_freq, alloc_freq, [](uint32_t id) {
		...
		if (ptr == nullptr) {
			INFO("Madness",
			     "Allocation of %s failed. Available heap: %s",
			     util::Byte_r(sz).to_string().c_str(),
			     util::Byte_r(kernel::heap_avail())
				     .to_string()
				     .c_str());
			...
			free((void *)first);
			Timers::oneshot(dealloc_delay, [](int) {
...
{
	Timers::periodic(1s, alloc_freq, [](int) {
		static int i = 0;
		INFO("Madness",
		     "Runtime: %lld s. Available heap: %s. Occupied by me: %s",
		     i++ * alloc_freq.count(),
		     util::Byte_r(kernel::heap_avail()).to_string().c_str(),
...
__attribute__((constructor)) void madness_plugin()
{
