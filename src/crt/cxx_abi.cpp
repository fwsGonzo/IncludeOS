...
const char *type_kind_string[] = { "load of",
				   "store to",
...
void __ubsan_handle_divrem_overflow(struct overflow *data, unsigned long lhs,
				    unsigned long rhs)
{
	print_src_location(data->src);
...
void __ubsan_handle_type_mismatch_v1(struct mismatch *data, unsigned long ptr)
{
	...
	snprintf(buffer, sizeof(buffer),
		 ...
		 "ubsan: type name %s\n"
		 "ubsan: symbol    %s",
		 reason, (void *)ptr, alignment, data->type.type_name,
		 res.name);
	undefined_throw(buffer);
}
void __ubsan_handle_function_type_mismatch(struct function_type_mismatch *data,
					   unsigned long ptr)
{
	print_src_location(data->src);
	char sbuf[1024];
	...
	snprintf(buffer, sizeof(buffer),
		 ...
		 "ubsan: type name %s\n"
		 "ubsan: function  %s",
		 (void *)ptr, data->type.type_name, res.name);
	undefined_throw(buffer);
}
...
[[noreturn]] void __ubsan_handle_builtin_unreachable(struct unreachable *data)
{
	print_src_location(data->src);
