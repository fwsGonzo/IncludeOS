...
__attribute__((no_sanitize("all"))) uint32_t _move_symbols(void *sym_loc)
{
	...
	if (elfsym_size & 4095)
		elfsym_size += 4096 - (elfsym_size & 4095);
	return align_size + elfsym_size;
}

#define __assert(expr)                                                         \
	if (__builtin_expect(expr, 0)) {                                       \
		fprintf(stderr, "Expression %s failed %s:%d in %s", #expr,     \
...
__attribute__((format(printf, 2, 3))) int __printf_chk(int flag,
						       const char *format, ...)
{
	(void)flag;
	va_list ap;
...
__attribute__((noreturn, weak)) void __longjmp_chk(jmp_buf env, int val)
{
	longjmp(env, val);
