...
namespace hw
{
template <typename Driver> using Driver_entry = std::pair<uint32_t, Driver>;
...
void PCI_manager::scan_bus(const int bus)
{
	...
	for (uint16_t device = 0; device < 255; ++device) {
		...
		const uint32_t id = hw::PCI_Device::read_dword(
			pci_addr, PCI::CONFIG_VENDOR);

		if (id != PCI::WTF) {
			hw::PCI_Device::class_revision_t devclass;
			devclass.reg = hw::PCI_Device::read_dword(
				pci_addr, PCI::CONFIG_CLASS_REV);
			...
			if (devclass.classcode != PCI::BRIDGE) {
				...
				if (devclass.subclass == 0x4) {
					uint16_t buses =
						hw::PCI_Device::read_dword(
							pci_addr, 0x18);
					scan_bus(buses >>
						 8); // secondary is bits 8-15
				}
			}
		}
	}
...
void PCI_manager::init_devices(const uint8_t classcode)
{
	...
	for (const auto &[pci_addr, id, devclass] : devinfos_) {
		...
		auto &stored_dev =
			devices_.emplace_back(pci_addr, id, devclass.reg);
		// translate classcode to device and register
		switch (devclass.classcode) {
		case PCI::STORAGE: {
			register_device<BLK_driver, hw::Block_device>(
				stored_dev, blk_fact);
			break;
		}
		case PCI::NIC: {
			register_device<NIC_driver, hw::Nic>(stored_dev,
							     nic_fact);
			break;
		}
		default:
			INFO2("|--[ %s ] (Unsupported type)",
			      stored_dev.to_string().c_str());
			break;
		}
	}
	INFO2("o");
...
void PCI_manager::register_nic(uint16_t vendor, uint16_t prod,
			       NIC_driver factory)
{
	nic_fact.emplace_back(driver_id(vendor, prod), factory);
}
void PCI_manager::register_blk(uint16_t vendor, uint16_t prod,
			       BLK_driver factory)
{
