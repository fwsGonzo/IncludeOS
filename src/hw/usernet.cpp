...
void UserNet::receive(void *data, net::BufferStore *bufstore)
{
	...
	new (ptr) net::Packet(sizeof(driver_hdr), size,
			      sizeof(driver_hdr) + packet_len(), bufstore);
...
void UserNet::receive(const void *data, int len)
{
	...
	new (ptr) net::Packet(sizeof(driver_hdr), len,
			      sizeof(driver_hdr) + packet_len(), nullptr);
	// copy data over
...
net::Packet_ptr UserNet::create_packet(int link_offset)
{
	...
	new (ptr) net::Packet(sizeof(driver_hdr) + link_offset, 0,
			      sizeof(driver_hdr) + packet_len(), &buffer_store);
