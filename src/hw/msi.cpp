...
uintptr_t msix_t::get_bar_paddr(size_t offset)
{
	...
	PRINT("[MSI-X] offset %p -> bir %u => %p  res: %p\n", (void *)offset,
	      bar, (void *)pcibar.start, (void *)(pcibar.start + capbar_off));
	return pcibar.start + capbar_off;
}

msix_t::msix_t(PCI_Device &device, uint32_t cap) : dev(device)
{
