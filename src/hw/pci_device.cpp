...
namespace hw
{
static constexpr std::array<const char *, 3> bridge_subclasses{ "Host", "ISA",
								"Other" };

static constexpr std::array<const char *, 2> nic_subclasses{ "Ethernet",
							     "Other" };

static unsigned long pci_size(const unsigned long base,
...
void PCI_Device::probe_resources() noexcept
{
	...
	for (int bar = 0; bar < 6; ++bar) {
		...
		if (value & 1) {
			...
			pci__size = pci_size(len, PCI::BASE_ADDRESS_IO_MASK &
							  0xFFFF);
			this->m_iobase = bar;
		} else {
			// Resource type Mem
...
uint32_t PCI_Device::read_dword(const uint16_t pci_addr,
				const uint8_t reg) noexcept
{
	PCI::msg req;
...
PCI_Device::read16(const uint8_t reg) noexcept
{
	PCI::msg req;
	req.data = 0x80000000;
	req.addr = pci_addr_;
...
std::string PCI_Device::to_string() const
{
	...
	case PCI::classcode::BRIDGE:
		len = snprintf(buffer, sizeof(buffer), "%s, %s, %s (%#06x)",
			       ...
			       (devtype_.subclass < bridge_subclasses.size() ?
					bridge_subclasses[devtype_.subclass] :
					bridge_subclasses.back()),
			       devtype_.subclass);
		break;
	case PCI::classcode::NIC:
		len = snprintf(buffer, sizeof(buffer), "%s, %s, %s (%#x:%#06x)",
			       ...
			       (devtype_.subclass < nic_subclasses.size() ?
					nic_subclasses[devtype_.subclass] :
					nic_subclasses.back()),
	...
	default:
		len = snprintf(buffer, sizeof(buffer), "%s, %s (%#x:%#06x)",
			       PCI::classcode_str(devtype_.classcode),
			       PCI::vendor_str(vendor_id()), vendor_id(),
			       product_id());
